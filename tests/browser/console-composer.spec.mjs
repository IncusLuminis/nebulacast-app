...
test("Console Composer mounts declared platform roots and preserves Hero launcher behavior", async ({ page }) => {
  ...
  try {
    for (const selector of ["#console-hero", "#w-location", "#w-weather", "#w-weather-matrix", "#w-sun", "#w-sunmoon-panel", "#skyMount", "#db-sky-root", "#fs-sky", "#nrc-main", "#nrw-main", "#dbp-helio-body", "#db-map-root"]) {
    ...
    await expect(page.locator("#db-map-root")).toHaveCount(1);
