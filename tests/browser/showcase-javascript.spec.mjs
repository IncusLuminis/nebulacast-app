...
test("experimental showcase mirrors the card catalog and opens staging entry points", async ({ page }) => {
  ...
  await expect(page.locator('[data-showcase-group="observation"] [data-widget-title]')).toHaveCount(7);
  ...
  const hero = page.locator('[data-widget-title="Hero"]');
  await expect(hero.locator('[data-showcase-action="standalone"]')).toHaveAttribute("href", "https://staging.nebulacast.app/hero/?orientation=horizontal");
  await expect(hero.locator('[data-showcase-action="embed"]')).toHaveAttribute("href", "/embed/?src=%2Fhero%2F%3Forientation%3Dhorizontal&title=Hero");
});
