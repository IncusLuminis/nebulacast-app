...
| `/helio/` Space Weather | `.page-shell` · `sites/staging/helio/index.html` | Space Weather `#w-helio`; diagnostics `.page-diag` | `page-composer.mjs` mounts the registered `space-weather` Runtime adapter through a bounded route context; diagnostics remain host shell. The legacy IIFE remains compatibility-only. | Runtime-backed Space Weather route with preserved data URL and diagnostics. |
...
| `/calendar/` Events | `.page-wrap` · `sites/staging/calendar/index.html` | Events calendar `#nrc-root-page`; JSON/RSS links `.rss-link` | `page-composer.mjs` mounts the registered Events Runtime adapter; feed links remain host-owned compatibility links. | Thin Runtime Events host preserving JSON/RSS contracts. |
| `/sun/` Sun & Moon | `body` · `sites/staging/sun/index.html` | Sun & Moon `#w-sun` | `page-composer.mjs` adapts the existing weather state into shared Platform Context and mounts the registered Runtime widget. | Thin Runtime Sun & Moon host. |
