...
`,Pt=!1;function be(){if(Pt)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=fe,document.head.appendChild(t),Pt=!0}function ve(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,a=s/o,c=t.map((i,l)=>{let n=Math.max(2,Math.min(e,i.kp/9*e)),r=e-n,d=l*a,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${r.toFixed(1)}" width="${(a-1).toFixed(1)}" height="${n.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(yt(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${c}</svg>`}function Dt(t,s,e,o,a){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,i=Math.min(...t),l=Math.max(...t),n=l-i||1,r=g=>o-2-(g-i)/n*(o-4),d=t.map((g,w)=>`${(w/(t.length-1)*c).toFixed(1)},${r(g).toFixed(1)}`).join(" "),p="";if(a&&i<0&&l>0){let g=r(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${c}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((g,w)=>`<rect x="${(w/(t.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[w]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${p}
  ...
  </div>`}function ke(t){var r;let s=(r=t.metrics.wind_history_1h)!=null?r:[],e=s[s.length-1],o=t.metrics.solar_wind_kms,a=o!=null?`${Math.round(o)} km/s`:"\u2014",c=o!=null?o>=700?"#e05c5c":o>=500?"#e0a84a":o>=400?"#d4cc5c":"#5cce8c":"#607880",i=(e==null?void 0:e.density)!=null?`${e.density.toFixed(2)} cm\u207B\xB3`:"\u2014",l=(e==null?void 0:e.temp_kk)!=null?`${e.temp_kk.toFixed(0)} kK`:"\u2014",n=(e==null?void 0:e.pressure_npa)!=null?`${e.pressure_npa.toFixed(2)} nPa`:"\u2014";return`<div class="hw-kpi-popover">
    ...
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function Se(t){let n='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',r=[-10,-5,5,10].map(y=>{let k=100+y/20*100;return`<line x1="${k.toFixed(1)}" y1="16" x2="${k.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),d='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(t==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${n}${r}${d}</svg>`;let p=t<=-10?"#e05c5c":t<=-5?"#e0a84a":t<0?"#d4b84a":t>=5?"#5cce8c":"#7acca8",h=Math.max(-20,Math.min(20,t)),g=100+h/20*100,w=3,u=h<0?g-w:100-w,f=Math.max(2*w,Math.abs(g-100)+2*w),$=`<rect x="${u.toFixed(1)}" y="16" width="${f.toFixed(1)}" height="6" rx="${w}" fill="${p}" opacity="0.82"/>`,x=5,b=15,v=b-x*1.1,_=`<polygon points="${g.toFixed(1)},${b.toFixed(1)} ${(g-x).toFixed(1)},${v.toFixed(1)} ${(g+x).toFixed(1)},${v.toFixed(1)}" fill="${p}"/>`,H=`<line x1="${g.toFixed(1)}" y1="${b.toFixed(1)}" x2="${g.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${p}" stroke-width="1" opacity="0.6"/>`,C=`<text x="${g.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${p}" font-weight="600">${t>=0?"+":""}${t.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${n}${r}${d}${$}${_}${H}${C}
  </svg>`}function Me(t){var h;let s=t.metrics.imf_bz_nt,e=t.metrics.imf_bt_nt,o=t.metrics.solar_wind_kms,a=(h=t.metrics.pressure_npa)!=null?h:null,c=s!=null?s<=-10?"#e05c5c":s<=-5?"#e0a84a":s>=5?"#5cce8c":"#a0b4b8":"#607880",i=s!=null?(s>=0?"+":"")+s.toFixed(1)+" nT":"\u2014",l=e!=null?e.toFixed(1)+" nT":"\u2014",n=o!=null?`${Math.round(o)} km/s`:"\u2014",r=a!=null?`${a.toFixed(2)} nPa`:"\u2014",d=gt(t),p=s!=null&&s<-5?{msg:"Southward IMF \xB7 Aurora favorable",color:"#5cce8c"}:s!=null&&s<0?{msg:"Weakly southward \xB7 Conditions may improve",color:"#d4cc5c"}:{msg:"Northward IMF \xB7 Stable magnetosphere",color:"#96a8b8"};return`<div class="hw-kpi-popover">
  ...
  </g>`,E=`<g id="${c}-base-axis">
    <line x1="${l}" y1="130" x2="${u}" y2="130"
          stroke="rgba(255,255,255,0.10)" stroke-width="1.5" stroke-dasharray="8 5"/>
  </g>`,A="";if(t==="magnetosphere"){let I=(O=s.windKms)!=null?O:0,B=I>500,Y=I<350,P=(B?.5:Y?1.4:.9)*1.3,j=I>700?"#e05c5c":I>500?"#e0a84a":I>350?"#d4c840":"#5cce8c",Z=l+8,tt=u-14,L=8,R=6,D=(tt-Z)/(L-1),nt=260/(R+1),et=38,at=24,rt=2,wt=Array.from({length:L},(lt,pt)=>{let it=Z+pt*D,ct=Array.from({length:R},(_t,st)=>{let ot=nt*(st+1);return`<path d="M ${it.toFixed(1)},${ot.toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} M ${(it+at).toFixed(1)},${(ot-6).toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} L ${(it+at).toFixed(1)},${(ot+6).toFixed(1)}"
          stroke="${j}" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`}).join("");return Array.from({length:rt},(_t,st)=>{let ot=-((L-pt)/L*P)-st*P/rt;return`<g style="opacity:.12;animation:hw-arrow-chase ${P}s linear ${ot.toFixed(3)}s infinite">${ct}</g>`}).join("")}).join(""),T=(N=s.bz)!=null?N:null,dt=T==null?"":(()=>{let lt=T>0?"#5cce8c":"#e05c5c";return`${T>0?`<path d="M ${n},137 L ${n},123 M ${n-3},126 L ${n},123 L ${n+3},126"
        ...
        fill="${lt}" font-family="monospace">Bz${T>0?"\u2191":"\u2193"}</text>`})();A=`${wt}${dt}`}let U=`<g id="${c}-overlay-solar-wind">${A}</g>`,V="";if(t==="coronal_hole"&&s.hssState){let I=s.hssState,B=(q=I.speed)!=null?q:0,Y=B>=420,P=I.color,j=Y?"0.85":"0.25",Z=B>=500?"0.18":Y?"0.10":"0.04",tt=l+8,L=d-16,R=60,D=`${tt},130 ${L},${130-R} ${L},${130+R}`,nt=L+6,et=`${nt},120 ${nt+18},130 ${nt},140`;V=`
    ...
    <polygon points="${et}" fill="${P}" opacity="${Y?"0.9":"0.25"}"/>`}let G=`<g id="${c}-overlay-coronal-hole">${V}</g>`,F="";if(t==="cme_cone"&&s.cmeState){let I=s.cmeState,B=l,Y=d-10,P=Y-B,j=D=>Math.tan(D*Math.PI/180),Z=Math.round(j(9)*P),tt=Math.round(j(6)*P),L=Math.round(j(3)*P),R=D=>`${B},130 ${Y},${130-D} ${Y},${130+D}`;if(I.status!=="quiet"){let D=I.status==="impact"?"#e05c5c":"#d4cc5c";F=`
    <polygon points="${R(Z)}" fill="#253238" opacity="0.85"/>
    <polygon points="${R(tt)}"   fill="#d4cc5c" opacity="0.14"/>
    ...
    <line x1="${B+14}" y1="130" x2="${d-14}" y2="130"
          stroke="#1e2c30" stroke-dasharray="7 5" stroke-width="2"/>`}let K=`<g id="${c}-overlay-cme-cone">${F}</g>`,J=`<g id="${c}-overlay-labels">
    ...
    <div class="hw-kpi-hint" style="margin-bottom:0">${m(g)}</div>
  </div>`}function Wt(t,s,e){if(!t.length)return null;let o=(e%360+360)%360,a=-1,c=1/0,i=Math.cos(s*Math.PI/180);for(let l of t){let n=l.lat-s,r=(l.lon-o+180+360)%360-180,d=n*n+r*i*(r*i);d<c&&(c=d,a=l.prob)}return a>=0?a:null}function Ut(t){return`<svg viewBox="0 0 100 100" width="100%" height="100%"
    style="position:absolute;top:0;left:0;pointer-events:none">${['<text x="50" y="4"   font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">N</text>','<text x="96"  y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">W</text>','<text x="50" y="97"  font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">S</text>','<text x="4"   y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">E</text>'].join("")}</svg>`}function Ee(t,s){let e=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,o=null;s&&t.lat!=null&&t.lon!=null&&(o=Wt(s.entries,t.lat,t.lon));let a=t.lat!=null&&t.lon!=null,c=o!=null?o>=30?"#5cce8c":o>=10?"#d4cc5c":"#9ab4bc":"#607880",i=o!=null?`${o}%`:s?"n/a":"\u2026",l=a?`
  ...
  </div>`}function ze(t,s,e,o){switch(s){case"solar_wind":return ke(t);case"xray":return _e(t);case"imf_bz":return Me(t);case"aurora":return Ee(e,o);case"magnetosphere":return Le(t);default:return""}}function bt(t,s){if(t.length<2)return"\u2192";let e=t[t.length-1],o=Math.max(0,t.length-4),a=t[o];if(!isFinite(e)||!isFinite(a))return"\u2192";let c=e-a;return c>s?"\u2191":c<-s?"\u2193":"\u2192"}function Te(t,s,e){let o=mt[s],a=Qe,c=Xe(o.url,e);return`<div class="hw-solar-mini-wrap" style="cursor:default">
    <div class="hw-solar-mini-inner">
      <img class="hw-solar-mini-img" src="${W(c)}" alt="${W(o.label)}"
  ...
  </div>`}function Ae(t,s,e,o,a,c,i,l=0){var q,I,B,Y,P,j,Z,tt;let{summary:n,scales:r,metrics:d,aurora_hint:p}=t,h=(q=Rt[n.status])!=null?q:Rt.quiet,g=a!=null?a.kp.toFixed(1):d.kp_latest!=null?d.kp_latest.toFixed(1):"\u2014",u=[a?a.gScale:r.g_scale,r.r_scale,r.s_scale].map(L=>{let R=$e(L),D=R?`color:${h.accent};border-color:${h.accent}33`:"";return`<span class="hw-scale-chip${R?" hw-scale-active":""}" style="${D}">${m(L)}</span>`}).join(""),f=a?a.auroraLabel:p.aurora_label,$=f==="good"?"#5cce8c":f==="possible"?"#d4cc5c":"#607880",x=f.charAt(0).toUpperCase()+f.slice(1),b="#b4c6cc",v=d.solar_wind_kms!=null?`${Math.round(d.solar_wind_kms)} km/s`:"\u2014",_=d.imf_bz_nt,H=_!=null?_<=-10?"#e05c5c":_<=-5?"#e0a84a":_>=5?"#5cce8c":"#a0b4b8":"#607880",M=_!=null?(_>=0?"+":"")+_.toFixed(1)+" nT":"\u2014",C=d.xray_class,y=C?(I=Ht[C])!=null?I:"#a0b4b8":"#607880",k=C?`${C}-class`:"\u2014",E=bt(((B=d.kp_history_1h)!=null?B:[]).map(L=>L.kp),.5),A=bt(((Y=d.wind_history_1h)!=null?Y:[]).map(L=>L.kms),20),U=bt(((P=d.bz_history_1h)!=null?P:[]).map(L=>L.bz),1.5),V=bt(((j=d.xray_history_1h)!=null?j:[]).map(L=>Math.log10(L.flux+1e-9)),.15),G=(Z=d.kp_history_1h)!=null?Z:[],F=G.length?ht(G[G.length-1].t_utc):null,K=F?`Recent history \xB7 Last step ${F}`:"Recent history",J=gt(t),S=(tt=d.kp_latest)!=null?tt:0,z=S>=5,X=z?`linear-gradient(160deg, #0d2a1a 0%, ${h.bg}22 75%)`:`${h.bg}18`,O=(L,R,D,nt,et)=>{let at=et?`<span class="hw-trend">${et}</span>`:"";return`<div class="hw-kpi-item${o===L?" hw-kpi-active":""}" data-kpi="${L}">
    ...
    <div class="hw-hero" style="background:${X}">
      ...
      <div class="hw-quick-details">
        ${O("aurora","Aurora",m(x),$)}
        ${O("solar_wind","Solar wind",m(v),b,A)}
        ${O("imf_bz","IMF Bz",m(M),H,U)}
        ${O("xray","X-ray",m(k),y,V)}
      </div>
      ${o?ze(t,o,c,i):""}`:""}
      ${N}
    </div>`}function Fe(t){var d,p,h,g;let{metrics:s}=t,e=(d=s.kp_history_1h)!=null?d:[],o=(p=s.wind_history_1h)!=null?p:[],a=(h=s.bz_history_1h)!=null?h:[],c=(g=s.xray_history_1h)!=null?g:[],i=ve(e),l=Dt(o.map(w=>{var u;return(u=w.kms)!=null?u:0}).filter(w=>w>0),o.map(w=>yt(w.t_utc)),"#5cce8c",28,!1),n=Dt(a.map(w=>w.bz),a.map(w=>yt(w.t_utc)),"#d4cc5c",28,!0),r=ye(c);return`
    ...
    </div>`;let $=320,x=38,b=14,v=x+b,_=$/d,H=S=>x-Math.max(2,Math.min(x-2,S/9*(x-2))),M="",C=r.map(S=>S.kp),y=Ie(C);r.forEach((S,z)=>{let X=H(S.kp),O=x-X,N=z*_,q=N+_/2,I=S.kp>=6?"#e05c5c":S.kp>=5?"#e0a84a":S.kp>=4?"#d4cc5c":"#5cce8c",B=`Kp ${S.kp.toFixed(1)} \xB7 ${ht(S.t_utc)}`;if(M+=`<rect x="${N.toFixed(1)}" y="${X.toFixed(1)}" width="${(_-1.5).toFixed(1)}" height="${O.toFixed(1)}" fill="${I}" fill-opacity="0.85" rx="1.5"/>`,M+=`<rect x="${N.toFixed(1)}" y="0" width="${_.toFixed(1)}" height="${x}" fill="transparent"><title>${W(B)}</title></rect>`,d<=8||z%2===0){let P=new Date(S.t_utc).getHours();M+=`<text x="${q.toFixed(1)}" y="${(v-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${P.toString().padStart(2,"0")}</text>`}});let E=`<polyline points="${r.map((S,z)=>{let X=z*_+_/2,O=H(y[z]);return`${X.toFixed(1)},${O.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,A="";if(s>0&&d>0){let S=Math.min($-1,s/(d*3)*$);A=`
...
C: ${n.c_flare_probability}%  M: ${n.m_flare_probability}%  X: ${n.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    <div class="hw-radio-meta">X-ray: <b style="color:${(p=Ht[o])!=null?p:"#a0b4b8"}">${m(o)}-class</b> \xB7 ${m(c)}</div>
  </div>`}var Xt={g1:"#d4cc5c",g2:"#e0a84a",g3:"#e05c5c"};function qt(t){var l;let s=(l=t.metrics.kp_forecast_3h)!=null?l:[],e=Date.now(),o=e+24*60*60*1e3,a=s.filter(n=>{let r=new Date(n.t_utc).getTime();return r>=e-3*60*60*1e3&&r<=o});if(a.length===0)return{g1:0,g2:0,g3:0};let c=Math.max(...a.map(n=>n.kp)),i=n=>{if(c<n-.7)return 0;if(c>n+1)return 90;let r=(c-(n-.7))/1.7;return Math.round(Math.pow(Math.max(0,r),.7)*90)};return{g1:i(5),g2:i(6),g3:i(7)}}var Bt={rising:"#e0884a",peak:"#e05c5c",decline:"#d4cc5c"};function ts(t){var r,d,p;let s=(r=t.metrics.kp_latest)!=null?r:0,e=t.metrics.imf_bz_nt,o=t.metrics.solar_wind_kms,a=parseInt(((d=t.scales.g_scale)!=null?d:"G0").replace("G",""),10)||0,c=s>=5||a>=1,i=(p=t.metrics.kp_history_1h)!=null?p:[],l=0;if(i.length>=2&&(l=i[i.length-1].kp-i[i.length-2].kp),!c)return{active:!1,phase:"quiet",kp_current:s,kp_trend:l,bz_nt:e,solar_wind_kms:o};let n;return l>.3&&(e==null||e<-5)?n="rising":l<-.5?n="decline":n="peak",{active:!0,phase:n,kp_current:s,kp_trend:l,bz_nt:e,solar_wind_kms:o}}function es(t){if(!t.active)return"";let s=[{key:"rising",label:"Rising"},{key:"peak",label:"Peak"},{key:"decline",label:"Decline"}],e=s.findIndex(l=>l.key===t.phase),o=Bt[t.phase],a=s[e].label,c=s.map((l,n)=>{let r=n===e,d=n<e,p=Bt[l.key],h=r?`background:${p};border-color:${p};box-shadow:0 0 6px ${p}88`:d?`background:${p}44;border-color:${p}66`:"background:#111b1e;border-color:#1e2c30",g=r?" hw-spi-dot-active":"",w=r?`color:${p};font-weight:700`:d?`color:${p}66`:"color:#2e4248",u=n<s.length-1?`<div class="hw-spi-arr">${d?`<span style="color:${p}55">\u2192</span>`:"\u2192"}</div>`:"";return`<div class="hw-spi-node">
  ...
  </div>`}function ss(t,s){let e=qt(t),o=ts(t),a=(()=>{var g;let r=(g=t.metrics.kp_forecast_3h)!=null?g:[],d=Date.now(),p=d+24*60*60*1e3,h=r.filter(w=>new Date(w.t_utc).getTime()<=p);return h.length?Math.max(...h.map(w=>w.kp)):null})(),i=[{key:"g1",label:"G1"},{key:"g2",label:"G2"},{key:"g3",label:"G3"}].map(({key:r,label:d})=>{let p=e[r],h=Xt[r];return`<div class="hw-gstorm-row">
  ...
  </div>`}var vt={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Qt={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function os(t){var y;let s=vt,e=(y=Qt[s.phase])!=null?y:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),a=280,c=52,i=10,l=c-6,n=c-18,r=.5,d=.19,p=k=>Math.exp(-Math.pow((k-r)/d,2)/2),h=k=>i+k*(a-2*i),g=k=>l-p(k)*n,w=80,u=[];for(let k=0;k<=w;k++){let E=k/w;u.push(`${k===0?"M":"L"}${h(E).toFixed(1)},${g(E).toFixed(1)}`)}let f=Math.round(s.progress_0_1*w),$=[];for(let k=0;k<=f;k++){let E=k/w;$.push(`${k===0?"M":"L"}${h(E).toFixed(1)},${g(E).toFixed(1)}`)}let x=h(s.progress_0_1),b=[`M${i},${l}`,...$.slice(1),`L${x.toFixed(1)},${l} Z`],v=g(s.progress_0_1),_=5,H=`M${x.toFixed(1)},${v.toFixed(1)} L${(x-_).toFixed(1)},${(v-_*1.8).toFixed(1)} L${(x+_).toFixed(1)},${(v-_*1.8).toFixed(1)} Z`,M=l+11;return`<div class="hw-impact-tip${t?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function Vt(t){var n,r,d,p,h;let s=(r=(n=t.coronal_hole)==null?void 0:n.estimated_speed_kms)!=null?r:t.metrics.solar_wind_kms,e=(d=t.coronal_hole)==null?void 0:d.status,o=s!=null?s:0,a=e!=null?e:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",c={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},i={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},l={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:a,color:c[a],label:i[a],desc:(h=(p=t.coronal_hole)==null?void 0:p.note)!=null?h:l[a],speed:s}}function ns(t,s){var r;let e=Vt(t),o=(r=e.speed)!=null?r:0,a=e.speed!=null?`${Math.round(e.speed)} km/s`:"\u2014",c=s?" hw-impact-tip-open":"",i=o>=420,l=kt("coronal_hole",{hssState:e,uid:"hss"}),n=i?'<div class="hw-hss-meta" style="font-size:.72em">Elevated speed may indicate Earth-facing coronal hole stream</div>':'<div class="hw-hss-meta" style="font-size:.72em">Background solar wind \xB7 no HSS detected</div>';return`<div class="hw-impact-tip${c}">
    ${l}
    <div class="hw-hss-meta">Solar wind: <b style="color:${e.color}">${m(a)}</b> \xB7 ${m(e.desc)}</div>
    ${n}
  </div>`}function Zt(t){var a,c;let s=(a=t.scales.g_scale)!=null?a:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let i=(c=t.metrics.kp_forecast_3h)!=null?c:[],l=Date.now(),n=i.filter(r=>new Date(r.t_utc).getTime()<=l+3*60*60*1e3).sort((r,d)=>new Date(d.t_utc).getTime()-new Date(r.t_utc).getTime());n.length>0&&(o=n[0].kp)}return e>=2||o!=null&&o>=6?{level:2,color:"#e05c5c",label:"High",kp:o,gScale:s}:e>=1||o!=null&&o>=4?{level:1,color:"#d4cc5c",label:"Moderate",kp:o,gScale:s}:{level:0,color:"#5cce8c",label:"Low",kp:o,gScale:s}}function as(t,s){let e=Zt(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33,desc:"Normal density"},{l:1,label:"Moderate",color:"#d4cc5c",width:64,desc:"Elevated density"},{l:2,label:"High",color:"#e05c5c",width:100,desc:"Strong expansion"}].map(n=>{let r=n.l===e.level,d=r?n.color:"#566068",p=r?"0.88":"0.16";return`<div class="hw-satdrag-rung">
  ...
  </div>`}function Jt(t){var r,d,p;let s=(r=t.scales.g_scale)!=null?r:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let h=(d=t.metrics.kp_forecast_3h)!=null?d:[],g=Date.now(),w=h.filter(u=>new Date(u.t_utc).getTime()<=g+3*60*60*1e3).sort((u,f)=>new Date(f.t_utc).getTime()-new Date(u.t_utc).getTime());w.length>0&&(o=w[0].kp)}let a=0;e>=2||o!=null&&o>=6?a=2:(e>=1||o!=null&&o>=4)&&(a=1);let c=parseInt(((p=t.scales.r_scale)!=null?p:"R0").slice(1),10),i=c>=2&&a<2;c>=2&&(a=Math.min(2,a+1));let l={0:"#5cce8c",1:"#d4cc5c",2:"#e05c5c"},n={0:"Low",1:"Moderate",2:"High"};return{level:a,color:l[a],label:n[a],kp:o,gScale:s,boostedByFlare:i}}function is(t,s){var r;let e=Jt(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33},{l:1,label:"Moderate",color:"#d4cc5c",width:64},{l:2,label:"High",color:"#e05c5c",width:100}].map(d=>{let p=d.l===e.level,h=p?d.color:"#566068",g=p?"0.88":"0.16";return`<div class="hw-gnss-rung">
    ...
    ${n}
  </div>`}function te(t){var a;let s=t.metrics.pressure_npa,e=s!=null?s:null,o=(a=t.metrics.density)!=null?a:null;return e==null?{pressure:null,color:"#607880",label:"\u2014",density:o}:e>=6?{pressure:e,color:"#e05c5c",label:"Extreme",density:o}:e>=4?{pressure:e,color:"#e0a84a",label:"Strong",density:o}:e>=2?{pressure:e,color:"#d4cc5c",label:"Elevated",density:o}:e>=1?{pressure:e,color:"#5cce8c",label:"Typical",density:o}:{pressure:e,color:"#7a9298",label:"Weak",density:o}}function rs(t,s){let e=te(t),o=s?" hw-impact-tip-open":"",a=e.pressure,c=200,i=6,l=10,n=i+l,r=n+4,d=r+11,p=n+9,h=d+4,w=[{x:0,w:50,color:"#5cce8c"},{x:50,w:50,color:"#d4cc5c"},{x:100,w:50,color:"#e0a84a"},{x:150,w:50,color:"#e05c5c"}].map(y=>`<rect x="${y.x}" y="${i}" width="${y.w}" height="${l}" fill="${y.color}" opacity="0.55" rx="0"/>`).join(""),u=[{x:0,label:"0",anchor:"start"},{x:50,label:"2",anchor:"middle"},{x:100,label:"4",anchor:"middle"},{x:150,label:"6",anchor:"middle"},{x:200,label:"8+",anchor:"end"}],f=u.map(y=>`<line x1="${y.x}" y1="${n}" x2="${y.x}" y2="${r}" stroke="#3a5058" stroke-width="1"/>`).join(""),$=u.map(y=>`<text x="${y.x}" y="${d}" class="hw-swdp-axlabel" text-anchor="${y.anchor}">${y.label}</text>`).join(""),x="";if(a!=null){let k=Math.min(Math.max(a,0),8)/8*c;x=`<polygon points="${`${k-5},${p} ${k+5},${p} ${k},${n}`}" fill="${e.color}" opacity="0.95"/>
    <line x1="${k}" y1="${i}" x2="${k}" y2="${n}" stroke="${e.color}" stroke-width="1.5" opacity="0.7"/>`}let b=`<rect x="0" y="${i}" width="${c}" height="${l}" fill="none" stroke="#2a3c42" stroke-width="0.8" rx="0"/>`,v=`<svg class="hw-swdp-gauge" viewBox="0 0 ${c} ${h}" preserveAspectRatio="none" aria-hidden="true">
    ${w}${b}${x}${f}${$}
  ...
  </div>`}function ee(t){var r,d;let s=(r=t.alerts_all)!=null?r:[],e=s.find(p=>p.kind==="cme_impact"),o=s.find(p=>p.kind==="cme_watch"),a=e!=null?e:o;if(!a)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let c=((d=a.raw_body)!=null?d:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),i=c?parseInt(c[1],10):null,l=null;if(i&&a.t_utc){let p=1496e5/i*1e3;l=new Date(new Date(a.t_utc).getTime()+p).toISOString().replace(".000Z","Z")}let n=e?"impact":"watch";return{status:n,color:n==="impact"?"#e05c5c":"#d4cc5c",label:n==="impact"?"Active":"Watch",speed_kms:i,issued_utc:a.t_utc,arrival_utc:l}}function ls(t,s){let e=ee(t),o=s?" hw-impact-tip-open":"",a="\u2014";if(e.arrival_utc){let n=new Date(e.arrival_utc),r=n.toLocaleString("en-US",{month:"short",timeZone:"UTC"}),d=n.getUTCDate(),p=String(n.getUTCHours()).padStart(2,"0"),h=String(n.getUTCMinutes()).padStart(2,"0");a=`~${r}\xA0${d}\xA0${p}:${h}\u202FUTC`}let c=e.speed_kms?`${e.speed_kms}\u202Fkm/s`:"\u2014",i=e.status!=="quiet"?`Velocity: <b style="color:#b4c6cc">${m(c)}</b>&ensp;Arrival: <b style="color:#b4c6cc">${m(a)}</b>`:"No Earth-directed CME in forecast window",l=kt("cme_cone",{cmeState:e,uid:"cme"});return`<div class="hw-impact-tip${o}">
    ${l}
    <div class="hw-cme-footer">${i}</div>
  </div>`}function cs(t,s,e,o,a,c,i,l,n){var at,rt,wt;let r=(rt=(at=s==null?void 0:s.impacts)!=null?at:t.observer_impacts)!=null?rt:[],d=r.map(T=>{var Et,zt;let dt=(Et=he[T.level])!=null?Et:"#666",lt=T.level==="none"?"None":T.level.charAt(0).toUpperCase()+T.level.slice(1),pt=(zt=Ue[T.kind])!=null?zt:Ge,it=T.level==="none"?"#606870":dt,ct=T.kind==="solar_activity"?a:i.has(T.kind),_t=ct?" hw-impact-open":"",st;if(T.kind==="solar_activity"){let xt=a?" hw-solar-open":"",St=Kt.map(Q=>{let $t=c.has(Q.id),Mt=$t?Q.color+"22":"transparent",Ct=$t?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${Q.id}" style="color:${Q.color};border-color:${Q.color};background:${Mt};opacity:${Ct}">${Q.label}</button>`}).join("");st=`<div class="hw-solar-tip${xt}">
        ...
        </div>`}else if(T.kind==="aurora"){let xt=ct?" hw-aurora-tip-open":"",St=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,Q=null;l&&n.lat!=null&&n.lon!=null&&(Q=Wt(l.entries,n.lat,n.lon));let $t=n.lat!=null&&n.lon!=null,Mt=Q!=null?Q>=30?"#5cce8c":Q>=10?"#d4cc5c":"#9ab4bc":"#607880",Ct=Q!=null?`${Q}%`:l?"n/a":"\u2026",oe=$t?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
          ...
          ${oe}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else T.kind==="radio"?st=Je(t,ct):st=`<div class="hw-impact-tip${ct?" hw-impact-tip-open":""}">${m(T.summary)}</div>`;let ot=T.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${W(T.kind)}"`;return`<div class="hw-impact-row${_t}"${ot}>
      <span class="hw-impact-caret">\u25B6</span>
      <span class="hw-impact-kind" style="color:${it}">${pt}<span style="color:#b4c6cc">${m(T.label)}</span></span>
    ...
    <div class="hw-section-row" data-impacts-toggle>
      ...
      ${os(M)}
    </div>`,A=Vt(t),U=i.has("hss"),G=`<div class="hw-impact-row${U?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${as(t,K)}
    </div>`,z=Jt(t),X=i.has("gnss"),N=`<div class="hw-impact-row${X?" hw-impact-open":""}" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${is(t,X)}
    </div>`,q=te(t),I=i.has("sw_pressure"),B=q.pressure!=null?`${q.pressure.toFixed(2)} nPa`:"\u2014",P=`<div class="hw-impact-row${I?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${ls(t,Z)}
    </div>`,R=gt(t),D=i.has("magnetosphere"),et=`<div class="hw-impact-row${D?" hw-impact-open":""}" data-impact-row="magnetosphere">
      <span class="hw-impact-caret">\u25B6</span>
...
${e.join(" \xB7 ")}`}function gs(t,s,e){var n;let o=t.is_active?hs:(n=ps[t.level])!=null?n:"#445c64",a=t.event_time===e,c=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",l=a?`<div class="hw-tl-detail">${m(t.description)}${m(us(t))}</div>`:"";return`
    ...
    </div>`}function ws(t,s,e,o){var $,x;let a=($=t.timeline)!=null?$:[],c=Date.now(),i=new Date(c).toISOString().slice(0,10),l=new Date(c-864e5).toISOString().slice(0,10),n=new Date(c-1728e5).toISOString().slice(0,10),r=new Set([i,l,n]),d=a.filter(b=>{var v;return r.has(((v=b.event_time)!=null?v:"").slice(0,10))}).slice().reverse(),p=d.length,h=e?"\u25BC":"\u25B6",g=p>0?`Solar Activity Timeline (${p})`:"Solar Activity Timeline",w=`
    <div class="hw-section-row" data-tl-section>
      ...
      <div class="hw-tl-day-row" data-tl-day="${W(b)}">
        ...
        ${y}
      </div>`,E=M?"":v.map((A,U)=>gs(A,U<v.length-1,s)).join("");return`<div class="hw-tl-group">${k}${E}</div>`}).join("");return`
    <div class="hw-timeline">
      ${w}
      ${f}
    </div>`}function xs(t,s,e){var r;let o=(r=t.alerts_all)!=null?r:[],a=o.length,c=s?"\u25BC":"\u25B6",i=a>0?`SWPC Alerts (${a})`:"SWPC Alerts",l=`
    ...
    </div>`;if(!s||a===0)return`<div class="hw-alerts">${l}${s&&a===0?'<div class="hw-empty-alerts">No significant recent SWPC alerts</div>':""}</div>`;let n=o.map(d=>ds(d,d.dedupe_key===e)).join("");return`
    <div class="hw-alerts">
      ${l}
      ${n}
    </div>`}function $s(t,s){var V,G,F;let e=t.cme_tracker;if(!e)return"";let o=(V=ue[e.impact_level])!=null?V:"#96a8b8",a=(G=ge[e.status])!=null?G:e.status,c=300,i=44,l=18,n=i/2,r=10,d=c-18,p=7,h=`<line x1="${l+r}" y1="${n}" x2="${d-p}" y2="${n}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,g=`<circle cx="${l}" cy="${n}" r="${r}" fill="#f0c040" opacity="0.92"/>`,w=`
    <circle cx="${d}" cy="${n}" r="${p}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${d}" cy="${n}" r="2.5" fill="#fff" opacity="0.7"/>`,u=`<text x="${l}" y="${n+r+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,f=`<text x="${d}" y="${n+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,$="";if(e.progress!=null){let K=l+r+4,J=d-p-4,S=K+e.progress*(J-K),z=5;e.status==="arrival_window"?$=`
        ...
        </g>`:$=`<circle cx="${S.toFixed(1)}" cy="${n}" r="${z}" fill="${o}" opacity="0.85"/>`}let x=`<svg class="hw-cme-svg" viewBox="0 0 ${c} ${i}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${h}
    ${g}${u}
    ${w}${f}
    ${$}
    ...
    </div>`,E=s?"\u25BC":"\u25B6",A=e.impact_level==="unknown"?"Unrated":e.impact_level.charAt(0).toUpperCase()+e.impact_level.slice(1),U=s?`${x}${k}`:"";return`
    <div class="hw-cme">
