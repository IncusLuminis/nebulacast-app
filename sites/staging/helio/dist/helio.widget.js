...
`,Ye=!1;function Ft(){if(Ye)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Lt,document.head.appendChild(e),Ye=!0}function Et(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.length,n=s/o,i=e.map((r,l)=>{let a=Math.max(2,Math.min(t,r.kp/9*t)),c=t-a,d=l*n,p=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${c.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${u(be(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${i}</svg>`}function We(e,s,t,o,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let i=200,r=Math.min(...e),l=Math.max(...e),a=l-r||1,c=g=>o-2-(g-r)/a*(o-4),d=e.map((g,h)=>`${(h/(e.length-1)*i).toFixed(1)},${c(g).toFixed(1)}`).join(" "),p="";if(n&&r<0&&l>0){let g=c(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${i}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let m=e.map((g,h)=>`<rect x="${(h/(e.length-1)*i-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${u(s[h]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${i} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function At(e){var l,a;let s=(l=e.metrics.xray_class)!=null?l:"A",t=e.metrics.xray_flux_wm2,o=t!=null?t.toExponential(2)+" W/m\xB2":"\u2014",n=[{label:"A",color:"#888"},{label:"B",color:"#5cce8c"},{label:"C",color:"#aad47a"},{label:"M",color:"#e0a84a"},{label:"X",color:"#e05c5c"}],i=n.map(c=>{let d=c.label===s,p=d?'<div class="hw-xray-scale-marker"></div>':"";return`<div class="hw-xray-scale-band" style="background:${c.color}${d?"cc":"44"}">${p}</div>`}).join(""),r=n.map(c=>`<div class="hw-xray-scale-label" style="color:${c.label===s?"#c8d8dc":"#607880"}">${c.label}</div>`).join("");return`<div class="hw-kpi-popover">
    ${le("X-Ray \xB7 Current")}
    <div style="margin-bottom:8px">
      <div class="hw-xray-scale">${i}</div>
      <div class="hw-xray-scale-labels">${r}</div>
    </div>
    <div class="hw-kpi-hint">Class: <b style="color:${(a=ve[s])!=null?a:"#a0b4b8"}">${u(s)}-class</b> \xB7 ${u(o)}</div>
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function Ot(e){let a='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',c=[-10,-5,5,10].map(S=>{let k=100+S/20*100;return`<line x1="${k.toFixed(1)}" y1="16" x2="${k.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),d='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(e==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${a}${c}${d}</svg>`;let p=e<=-10?"#e05c5c":e<=-5?"#e0a84a":e<0?"#d4b84a":e>=5?"#5cce8c":"#7acca8",m=Math.max(-20,Math.min(20,e)),g=100+m/20*100,h=3,w=m<0?g-h:100-h,v=Math.max(2*h,Math.abs(g-100)+2*h),b=`<rect x="${w.toFixed(1)}" y="16" width="${v.toFixed(1)}" height="6" rx="${h}" fill="${p}" opacity="0.82"/>`,x=5,f=15,$=f-x*1.1,_=`<polygon points="${g.toFixed(1)},${f.toFixed(1)} ${(g-x).toFixed(1)},${$.toFixed(1)} ${(g+x).toFixed(1)},${$.toFixed(1)}" fill="${p}"/>`,M=`<line x1="${g.toFixed(1)}" y1="${f.toFixed(1)}" x2="${g.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${p}" stroke-width="1" opacity="0.6"/>`,H=`<text x="${g.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${p}" font-weight="600">${e>=0?"+":""}${e.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${a}${c}${d}${b}${_}${M}${H}
  </svg>`}function zt(e){var m;let s=e.metrics.imf_bz_nt,t=e.metrics.imf_bt_nt,o=e.metrics.solar_wind_kms,n=(m=e.metrics.pressure_npa)!=null?m:null,i=s!=null?s<=-10?"#e05c5c":s<=-5?"#e0a84a":s>=5?"#5cce8c":"#a0b4b8":"#607880",r=s!=null?(s>=0?"+":"")+s.toFixed(1)+" nT":"\u2014",l=t!=null?t.toFixed(1)+" nT":"\u2014",a=o!=null?`${Math.round(o)} km/s`:"\u2014",c=n!=null?`${n.toFixed(2)} nPa`:"\u2014",d=ye(e),p=s!=null&&s<-5?{msg:"Southward IMF \xB7 Aurora favorable",color:"#5cce8c"}:s!=null&&s<0?{msg:"Weakly southward \xB7 Conditions may improve",color:"#d4cc5c"}:{msg:"Northward IMF \xB7 Stable magnetosphere",color:"#96a8b8"};return`<div class="hw-kpi-popover">
    ...
    <div style="font-size:.65em;color:#607880">Coupling: <span style="color:${d.color};font-weight:600">${u(d.coupling)}</span> \xB7 Trend history: \u25B6 Details</div>
      ...
      ${H}
    </svg>`}else{let b=e.state==="storm"?16:e.state==="active"?26:38,x=155-b,f=e.state==="storm"?22:e.state==="active"?30:40,$=120-f,_=240,M=[`M ${x},60`,`C ${x-4},42 150,${f} 155,${f}`,`C 173,${f} ${_-5},${f+18} ${_},60`,`C ${_-5},${$-18} 173,${$} 155,${$}`,`C 150,${$} ${x-4},78 ${x},60`,"Z"].join(" "),y=`M ${x+2},60 C ${x+2},${60-b*.4} 152,54 150,60 C 152,66 ${x+2},${60+b*.4} ${x+2},60 Z`,H=r>700?"#e05c5c":r>500?"#e0a84a":r>350?"#d4c840":"#5cce8c",S=r>700?.4:r>500?.65:r>350?1.1:1.8,k=r>500?[10,24,40,57,74,90,106]:r>350?[14,34,57,82,104]:[20,50,82,108],L=16,T=22,Y=x-6,W=Math.ceil((Y-T)/L)+2,X=Array.from({length:W},(O,E)=>T-L+E*L),N=12,P=8,j=X.flatMap(O=>k.map(E=>`<path d="M ${O},${E} L ${O+N},${E} M ${O+P},${E-3} L ${O+N},${E} L ${O+P},${E+3}" stroke="${H}cc" stroke-width="1.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`)).join(""),C=`<g class="hw-wg-full" style="animation-duration:${S}s">${j}</g>`,R=s==null?"":s>0?'<path d="M 155,64 L 155,56 M 153,58 L 155,56 L 157,58" stroke="#5cce8c" stroke-width="1.3" fill="none" stroke-linecap="round"/>':'<path d="M 155,56 L 155,64 M 153,62 L 155,64 L 157,62" stroke="#e05c5c" stroke-width="1.3" fill="none" stroke-linecap="round"/>',Q=s==null?"":`<text x="163" y="62" font-size="6" fill="${s>0?"#5cce8c":"#e05c5c"}" font-family="monospace">Bz${s>0?"\u2191":"\u2193"}</text>`;return ke("magnetosphere",{magnetInfo:e,bz:s,windKms:t,uid:n})}}function ke(e,s={}){var C,R,Q,O,E,J;let i=(C=s.uid)!=null?C:"hse",r=0,l=160,a=800,c=17,d=a-c,p=s.magnetInfo,m=(R=p==null?void 0:p.state)!=null?R:"stable",g=(Q=p==null?void 0:p.color)!=null?Q:"#e0a84a",w=a-(m==="storm"?55:m==="active"?80:110),v=m==="storm"?58:m==="active"?76:95,b=1120,x=20,f=[`M ${w},130`,`C ${w-8},${130-v*.55} ${a-18},${130-v} ${a},${130-v}`,`C ${a+120},${130-v} ${b-180},${130-x} ${b},${130-x}`,`L ${b},${130+x}`,`C ${b-180},${130+x} ${a+120},${130+v} ${a},${130+v}`,`C ${a-18},${130+v} ${w-8},${130+v*.55} ${w},130`,"Z"].join(" "),$=e==="magnetosphere",_=$?m==="storm"?"0.12":"0.08":"0.04",M=$?"0.75":"0.28",y=`<g id="${i}-base-sun">
    ...
    <line x1="${l}" y1="130" x2="${w}" y2="130"
  ...
  </g>`,T="";if(e==="magnetosphere"){let z=(O=s.windKms)!=null?O:0,D=z>500,B=z<350,F=(D?.5:B?1.4:.9)*1.3,K=z>700?"#e05c5c":z>500?"#e0a84a":z>350?"#d4c840":"#5cce8c",te=l+8,ee=w-14,I=8,G=6,U=(ee-te)/(I-1),se=260/(G+1),V=38,ae=24,_e=2,Se=Array.from({length:I},(ne,de)=>{let oe=te+de*U,re=Array.from({length:G},(pe,he)=>{let Z=se*(he+1);return`<path d="M ${oe.toFixed(1)},${Z.toFixed(1)} L ${(oe+V).toFixed(1)},${Z.toFixed(1)} M ${(oe+ae).toFixed(1)},${(Z-6).toFixed(1)} L ${(oe+V).toFixed(1)},${Z.toFixed(1)} L ${(oe+ae).toFixed(1)},${(Z+6).toFixed(1)}"
          stroke="${K}" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`}).join("");return Array.from({length:_e},(pe,he)=>{let Z=-((I-de)/I*F)-he*F/_e;return`<g style="opacity:.12;animation:hw-arrow-chase ${F}s linear ${Z.toFixed(3)}s infinite">${re}</g>`}).join("")}).join(""),q=(E=s.bz)!=null?E:null,ce=q==null?"":(()=>{let ne=q>0?"#5cce8c":"#e05c5c";return`${q>0?`<path d="M ${a},137 L ${a},123 M ${a-3},126 L ${a},123 L ${a+3},126"
        ...
        fill="${ne}" font-family="monospace">Bz${q>0?"\u2191":"\u2193"}</text>`})();T=`${Se}${ce}`}let Y=`<g id="${i}-overlay-solar-wind">${T}</g>`,W="";if(e==="coronal_hole"&&s.hssState){let z=s.hssState,D=(J=z.speed)!=null?J:0,B=D>=420,F=z.color,K=B?"0.85":"0.25",te=D>=500?"0.18":B?"0.10":"0.04",ee=l+8,I=d-16,G=60,U=`${ee},130 ${I},${130-G} ${I},${130+G}`,se=I+6,V=`${se},120 ${se+18},130 ${se},140`;W=`
    ...
    <polygon points="${V}" fill="${F}" opacity="${B?"0.9":"0.25"}"/>`}let X=`<g id="${i}-overlay-coronal-hole">${W}</g>`,N="";if(e==="cme_cone"&&s.cmeState){let z=s.cmeState,D=l,B=d-10,F=B-D,K=U=>Math.tan(U*Math.PI/180),te=Math.round(K(9)*F),ee=Math.round(K(6)*F),I=Math.round(K(3)*F),G=U=>`${D},130 ${B},${130-U} ${B},${130+U}`;if(z.status!=="quiet"){let U=z.status==="impact"?"#e05c5c":"#d4cc5c";N=`
    ...
    <line x1="${D+14}" y1="130" x2="${d-14}" y2="130"
          stroke="#1e2c30" stroke-dasharray="7 5" stroke-width="2"/>`}let P=`<g id="${i}-overlay-cme-cone">${N}</g>`,j=`<g id="${i}-overlay-labels">
  ...
  </div>`}function Dt(e,s,t,o){switch(s){case"solar_wind":return It(e);case"xray":return At(e);case"imf_bz":return zt(e);case"aurora":return Pt(t,o);case"magnetosphere":return Nt(e);default:return""}}function Gt(e,s){if(e.length<2)return"\u2192";let t=e[e.length-1],o=Math.max(0,e.length-4),n=e[o];if(!isFinite(t)||!isFinite(n))return"\u2192";let i=t-n;return i>s?"\u2191":i<-s?"\u2193":"\u2192"}var Yt={quiet:"171",active:"195",elevated:"284",storm:"304"};function Wt(e,s){var o,n,i;if(s==null)return(n=(o=e.summary)==null?void 0:o.status)!=null?n:"quiet";let t=parseInt(((i=s.gScale)!=null?i:"G0").replace(/\D/g,""),10)||0;return t>=3?"storm":t>=1?"elevated":s.kp>=4?"active":"quiet"}function jt(e,s){var c,d;let t=(c=Yt[e])!=null?c:"171",o=`/assets/gifs/current_eit_${t}.gif`,n=cs(o,s),i=ps,r=(d=$e[e])!=null?d:$e.quiet,l=`${r.accent}44`,a=`0 0 10px ${r.accent}55,0 0 24px ${r.accent}22`;return`<div class="hw-solar-mini-wrap" style="cursor:default">
  ...
  </div>`}function Ut(e,s,t,o,n){var f,$,_;let{summary:i,metrics:r}=e,l=(f=$e[i.status])!=null?f:$e.quiet,a=t!=null?t.kp.toFixed(1):r.kp_latest!=null?r.kp_latest.toFixed(1):"\u2014",c=Ht(e,t),p=[{key:"G",text:c.g,title:"Geomagnetic storm level. Based on Kp index.",aria:"Geomagnetic storm level"},{key:"R",text:c.r,title:"Radio blackout level. Based on solar X-ray flux.",aria:"Radio blackout level"},{key:"S",text:c.s,title:"Solar radiation storm level. Based on energetic proton flux.",aria:"Solar radiation storm level"},{key:"X",text:`X:${c.x}`,title:"Current solar X-ray activity class.",aria:"Solar X-ray activity"}].map(M=>{let y;return M.key==="G"?y=He(Ce(c.g,"G")):M.key==="R"?y=He(Ce(c.r,"R")):M.key==="S"?y=He(Ce(c.s,"S")):y=Ct(c.x),`<button type="button" class="hw-scale-chip hw-hero-scale-chip"
      style="${`color:${y.color};background:${y.background};border-color:${y.borderColor}`}" data-hero-chip="${M.key}" title="${A(M.title)}" aria-label="${A(M.aria)}">${u(M.text)}</button>`}).join(""),m=Gt((($=r.kp_history_1h)!=null?$:[]).map(M=>M.kp),.5),g=(_=r.kp_latest)!=null?_:0,h=g>=5,w=h?`linear-gradient(160deg, #0d2a1a 0%, ${l.bg}22 75%)`:`${l.bg}18`,v=Wt(e,t),x=h?`
    ...
    <div class="hw-hero" style="background:${w}">
      ...
      ${x}
    </div>`}function Xt(e){var d,p,m,g;let{metrics:s}=e,t=(d=s.kp_history_1h)!=null?d:[],o=(p=s.wind_history_1h)!=null?p:[],n=(m=s.bz_history_1h)!=null?m:[],i=(g=s.xray_history_1h)!=null?g:[],r=Et(t),l=We(o.map(h=>{var w;return(w=h.kms)!=null?w:0}).filter(h=>h>0),o.map(h=>be(h.t_utc)),"#5cce8c",28,!1),a=We(n.map(h=>h.bz),n.map(h=>be(h.t_utc)),"#d4cc5c",28,!0),c=Rt(i);return`
    ...
    </div>`;let b=320,x=38,f=14,$=x+f,_=b/d,M=C=>x-Math.max(2,Math.min(x-2,C/9*(x-2))),y="",H=c.map(C=>C.kp),S=qt(H);c.forEach((C,R)=>{let Q=M(C.kp),O=x-Q,E=R*_,J=E+_/2,z=C.kp>=6?"#e05c5c":C.kp>=5?"#e0a84a":C.kp>=4?"#d4cc5c":"#5cce8c",D=`Kp ${C.kp.toFixed(1)} \xB7 ${xe(C.t_utc)}`;if(y+=`<rect x="${E.toFixed(1)}" y="${Q.toFixed(1)}" width="${(_-1.5).toFixed(1)}" height="${O.toFixed(1)}" fill="${z}" fill-opacity="0.85" rx="1.5"/>`,y+=`<rect x="${E.toFixed(1)}" y="0" width="${_.toFixed(1)}" height="${x}" fill="transparent"><title>${A(D)}</title></rect>`,d<=8||R%2===0){let F=new Date(C.t_utc).getHours();y+=`<text x="${J.toFixed(1)}" y="${($-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${F.toString().padStart(2,"0")}</text>`}});let L=`<polyline points="${c.map((C,R)=>{let Q=R*_+_/2,O=M(S[R]);return`${Q.toFixed(1)},${O.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,T="";if(s>0&&d>0){let C=Math.min(b-1,s/(d*3)*b);T=`
    ...
    <div class="hw-forecast">
      ...
      ${N}`:""}
    </div>`}function ts(e){let s=/([NS])(\d+)([EW])(\d+)/i.exec(e);return s?{lat:(s[1].toUpperCase()==="N"?1:-1)*parseInt(s[2],10),lon:(s[3].toUpperCase()==="E"?1:-1)*parseInt(s[4],10)}:null}var et=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function ss(e){return e.x_flare_probability>0?"X":e.m_flare_probability>0?"M":e.c_flare_probability>0?"C":"quiet"}function os(e,s,t){let o=s/2,n=o*.87,i=s*.03,r=s*.009,l=e.map(a=>{var b,x;let c=ts(a.location);if(!c||Math.abs(c.lon)>88||a.location.includes("*"))return"";let d=ss(a);if(!t.has(d))return"";let p=et.find(f=>f.id===d).color,m=c.lat*Math.PI/180,g=c.lon*Math.PI/180,h=(o+n*Math.cos(m)*Math.sin(g)).toFixed(1),w=(o-n*Math.sin(m)).toFixed(1),v=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}function bs(e,s){let t=Math.max(0,Math.min(5,Math.round(e))),o=70,n=76,i=48,l=i-9,a=Math.PI,c=f=>a*(1-f/5),d=c(t),p=(f,$)=>({x:o+$*Math.cos(f),y:n-$*Math.sin(f)}),m=(f,$,_)=>{let M=p(f,i),y=p($,i),H=p(f,l),S=p($,l);return`<path d="M ${H.x.toFixed(2)} ${H.y.toFixed(2)} L ${M.x.toFixed(2)} ${M.y.toFixed(2)} A ${i} ${i} 0 0 1 ${y.x.toFixed(2)} ${y.y.toFixed(2)} L ${S.x.toFixed(2)} ${S.y.toFixed(2)} A ${l} ${l} 0 0 0 ${H.x.toFixed(2)} ${H.y.toFixed(2)} Z" fill="${_}"/>`},g=f=>{let $=c(f),_=p($,i+1),M=p($,i-5);return`<line x1="${_.x.toFixed(2)}" y1="${_.y.toFixed(2)}" x2="${M.x.toFixed(2)}" y2="${M.y.toFixed(2)}" stroke="#2a3a40" stroke-width="1" stroke-linecap="round"/>`},h=p(d,i-2),w=`<line x1="${o}" y1="${n}" x2="${h.x.toFixed(2)}" y2="${h.y.toFixed(2)}" stroke="#c8d6dc" stroke-width="2" stroke-linecap="round"/>`,v=`<circle cx="${o}" cy="${n}" r="3.5" fill="#3a4c52" stroke="#1e2c30" stroke-width="1"/>`,b=m(c(0),c(2),"#3d8f62")+m(c(2),c(4),"#b8982a")+m(c(4),c(5),"#b04048"),x=[0,1,2,3,4,5].map(g).join("");return`<div class="hw-storm-risk-gauge" role="img" aria-label="${A(s)} G${t}">
    <svg viewBox="0 0 140 88" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <rect width="140" height="88" fill="none"/>
      <path d="M ${p(a,i).x.toFixed(2)} ${p(a,i).y.toFixed(2)} A ${i} ${i} 0 0 1 ${p(0,i).x.toFixed(2)} ${p(0,i).y.toFixed(2)}" fill="none" stroke="#1a2428" stroke-width="2" stroke-linecap="round"/>
      ${b}
      ${x}
      ${w}
      ${v}
  ...
  </div>`}function vs(e,s){let t=st(e),o=fs(e),n=t.forecast_24h,r=`
    <div class="hw-storm-risk-now">
      <div class="hw-storm-risk-now-num" style="color:${ot(t.now.g_level)}">G${t.now.g_level}</div>
      <div class="hw-storm-risk-now-lbl">${u(t.now.label)}</div>
    </div>`,l=Math.max(0,Math.min(5,Math.round(n.max_expected))),a=`
        <div class="hw-storm-risk-fc-gauge">${bs(l,"Max expected geomagnetic level in next 24 hours")}</div>
        <div class="hw-gstorm-footer" style="margin-top:5px;line-height:1.35">Max expected level in next 24h</div>
        <div class="hw-gstorm-footer" style="margin-top:2px">From Kp forecast \xB7 G${l}</div>`;return`<div class="hw-impact-tip${s?" hw-impact-tip-open":""}">
    ${$s(o)}
    <div class="hw-storm-risk-grid">
      ...
      <div class="hw-storm-risk-rail">
        ...
        <div class="hw-storm-risk-fc">${a}</div>
      </div>
    </div>
  </div>`}var fe={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},nt={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function ys(e){var S;let s=fe,t=(S=nt[s.phase])!=null?S:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,i=52,r=10,l=i-6,a=i-18,c=.5,d=.19,p=k=>Math.exp(-Math.pow((k-c)/d,2)/2),m=k=>r+k*(n-2*r),g=k=>l-p(k)*a,h=80,w=[];for(let k=0;k<=h;k++){let L=k/h;w.push(`${k===0?"M":"L"}${m(L).toFixed(1)},${g(L).toFixed(1)}`)}let v=Math.round(s.progress_0_1*h),b=[];for(let k=0;k<=v;k++){let L=k/h;b.push(`${k===0?"M":"L"}${m(L).toFixed(1)},${g(L).toFixed(1)}`)}let x=m(s.progress_0_1),f=[`M${r},${l}`,...b.slice(1),`L${x.toFixed(1)},${l} Z`],$=g(s.progress_0_1),_=5,M=`M${x.toFixed(1)},${$.toFixed(1)} L${(x-_).toFixed(1)},${($-_*1.8).toFixed(1)} L${(x+_).toFixed(1)},${($-_*1.8).toFixed(1)} Z`,y=l+11;return`<div class="hw-impact-tip${e?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function rt(e){var a,c,d,p,m;let s=(c=(a=e.coronal_hole)==null?void 0:a.estimated_speed_kms)!=null?c:e.metrics.solar_wind_kms,t=(d=e.coronal_hole)==null?void 0:d.status,o=s!=null?s:0,n=t!=null?t:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",i={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},r={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},l={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:n,color:i[n],label:r[n],desc:(m=(p=e.coronal_hole)==null?void 0:p.note)!=null?m:l[n],speed:s}}function ks(e,s){var c;let t=rt(e),o=(c=t.speed)!=null?c:0,n=t.speed!=null?`${Math.round(t.speed)} km/s`:"\u2014",i=s?" hw-impact-tip-open":"",r=o>=420,l=ke("coronal_hole",{hssState:t,uid:"hss"}),a=r?'<div class="hw-hss-meta" style="font-size:.72em">Elevated speed may indicate Earth-facing coronal hole stream</div>':'<div class="hw-hss-meta" style="font-size:.72em">Background solar wind \xB7 no HSS detected</div>';return`<div class="hw-impact-tip${i}">
    ${l}
    <div class="hw-hss-meta">Solar wind: <b style="color:${t.color}">${u(n)}</b> \xB7 ${u(t.desc)}</div>
    ${a}
  </div>`}function at(e){var n,i;let s=(n=e.scales.g_scale)!=null?n:"G0",t=parseInt(s.slice(1),10),o=e.metrics.kp_latest;if(o==null){let r=(i=e.metrics.kp_forecast_3h)!=null?i:[],l=Date.now(),a=r.filter(c=>new Date(c.t_utc).getTime()<=l+3*60*60*1e3).sort((c,d)=>new Date(d.t_utc).getTime()-new Date(c.t_utc).getTime());a.length>0&&(o=a[0].kp)}return t>=2||o!=null&&o>=6?{level:2,color:"#e05c5c",label:"High",kp:o,gScale:s}:t>=1||o!=null&&o>=4?{level:1,color:"#d4cc5c",label:"Moderate",kp:o,gScale:s}:{level:0,color:"#5cce8c",label:"Low",kp:o,gScale:s}}function _s(e,s){let t=at(e),o=s?" hw-impact-tip-open":"",i=[{l:0,label:"Low",color:"#5cce8c",width:33,desc:"Normal density"},{l:1,label:"Moderate",color:"#d4cc5c",width:64,desc:"Elevated density"},{l:2,label:"High",color:"#e05c5c",width:100,desc:"Strong expansion"}].map(a=>{let c=a.l===t.level,d=c?a.color:"#566068",p=c?"0.88":"0.16";return`<div class="hw-satdrag-rung">
    ...
    </div>`}function Hs(e,s){let t=e.find(i=>i.kind==="aurora");if(t)return t;let o=s.aurora_hint;return{kind:"aurora",level:o.aurora_label==="good"?"moderate":o.aurora_label==="possible"?"low":"none",label:"Aurora",summary:o.summary}}function Ls(e,s,t,o){var x,f;let n=(x=qe[e.level])!=null?x:"#666",i=e.level==="none"?"None":e.level.charAt(0).toUpperCase()+e.level.slice(1),r=(f=Ee[e.kind])!=null?f:tt,l=e.level==="none"?"#606870":n,a=s.has("aurora"),c=a?" hw-impact-open":"",d=a?" hw-aurora-tip-open":"",p=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,m=null;t&&o.lat!=null&&o.lon!=null&&(m=Ve(t.entries,o.lat,o.lon));let g=o.lat!=null&&o.lon!=null,h=m!=null?m>=30?"#5cce8c":m>=10?"#d4cc5c":"#9ab4bc":"#607880",w=m!=null?`${m}%`:t?"n/a":"\u2026",v=g?`
    <div class="hw-aurora-obs-panel">
      <span>\u{1F4CD}</span>
    ...
    ${b}
  </div>`}function Ue(e,s,t,o,n,i){var w,v;let r=(w=qe[e.level])!=null?w:"#666",l=e.level==="none"?"None":e.level.charAt(0).toUpperCase()+e.level.slice(1),a=(v=Ee[e.kind])!=null?v:tt,c=e.level==="none"?"#606870":r,d=e.kind==="solar_activity"?t:i,p=d?" hw-impact-open":"",m;if(e.kind==="solar_activity"){let b=t?" hw-solar-open":"",x=et.map(f=>{let $=o.has(f.id),_=$?f.color+"22":"transparent",M=$?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${f.id}" style="color:${f.color};border-color:${f.color};background:${_};opacity:${M}">${f.label}</button>`}).join("");m=`<div class="hw-solar-tip${b}">
    ...
    ${m}
  </div>`}function Fs(e,s,t,o,n,i,r,l,a){var Ie,Ae,Oe,ze;let c=(Ae=(Ie=s==null?void 0:s.impacts)!=null?Ie:e.observer_impacts)!=null?Ae:[],d=Hs(c,e),p=Ls(d,r,l,a),m=c.find(Me=>Me.kind==="radio"),g=c.find(Me=>Me.kind==="solar_activity"),h=m?Ue(m,t,n,i,e,r.has("radio")):"",w=g?Ue(g,t,n,i,e,!1):"",x=`
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,{metrics:f}=e,$=f.xray_class,_=$?(Oe=ve[$])!=null?Oe:"#a0b4b8":"#607880",M=$?`${$}-class`:"\u2014",y=f.imf_bz_nt,H=y!=null?y<=-10?"#e05c5c":y<=-5?"#e0a84a":y>=5?"#5cce8c":"#a0b4b8":"#607880",S=y!=null?(y>=0?"+":"")+y.toFixed(1)+" nT":"\u2014",k=f.solar_wind_kms,L=k!=null?`${Math.round(k)} km/s`:"\u2014",T=k!=null?k>700?"#e05c5c":k>500?"#e0a84a":k>400?"#d4cc5c":"#5cce8c":"#607880",Y=Le("xray","X-Ray",ns,M,_,r,e,a,l),W=Le("imf_bz","IMF Bz",rs,S,H,r,e,a,l),X=Le("solar_wind","Solar Wind",as,L,T,r,e,a,l),N='<div class="hw-indicators-sep" role="separator" aria-hidden="true"></div>',P=r.has("geomag_storm"),j=st(e),C=ot(j.now.g_level),R=`G${j.now.g_level}`,O=`<div class="hw-impact-row${P?" hw-impact-open":""}" id="storm_risk" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${vs(e,P)}
    </div>`,E=r.has("solar_cycle"),J=(ze=nt[fe.phase])!=null?ze:"#96a8b8",z=fe.phase.charAt(0).toUpperCase()+fe.phase.slice(1),B=`<div class="hw-impact-row${E?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${ks(e,K)}
    </div>`,I=at(e),G=r.has("sat_drag"),se=`<div class="hw-impact-row${G?" hw-impact-open":""}" id="satellite_drag" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${_s(e,G)}
    </div>`,V=it(e),ae=r.has("gnss"),Se=`<div class="hw-impact-row${ae?" hw-impact-open":""}" id="gnss" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
...
${t.join(" \xB7 ")}`}function zs(e,s,t){var a;let o=e.is_active?Is:(a=Rs[e.level])!=null?a:"#445c64",n=e.event_time===t,i=e.event_time.slice(11,16)+" UTC",r=e.source==="NASA_DONKI"?"DONKI":"SWPC",l=n?`<div class="hw-tl-detail">${u(e.description)}${u(Os(e))}</div>`:"";return`
    ...
    </div>`}function Ts(e,s,t,o){var b,x;let n=(b=e.timeline)!=null?b:[],i=Date.now(),r=new Date(i).toISOString().slice(0,10),l=new Date(i-864e5).toISOString().slice(0,10),a=new Date(i-1728e5).toISOString().slice(0,10),c=new Set([r,l,a]),d=n.filter(f=>{var $;return c.has((($=f.event_time)!=null?$:"").slice(0,10))}).slice().reverse(),p=d.length,m=t?"\u25BC":"\u25B6",g=p>0?`Solar Activity Timeline (${p})`:"Solar Activity Timeline",h=`
    ...
    </div>`;if(!t||p===0)return`<div class="hw-timeline">${h}</div>`;let w=new Map;for(let f of d){let $=((x=f.event_time)!=null?x:"").slice(0,10);w.has($)||w.set($,[]),w.get($).push(f)}let v=[...w.entries()].map(([f,$])=>{let M=new Date(f+"T12:00:00Z").toLocaleDateString("en-US",{month:"short",day:"numeric",timeZone:"UTC"}),y=o.has(f),H=y?"\u25B6":"\u25BC",S=y?`<span class="hw-tl-day-count">${$.length} events</span>`:"",k=`
    ...
    <div class="hw-alerts">
      ${l}
      ${a}
    </div>`}function Bs(e,s){var W,X,N;let t=e.cme_tracker;if(!t)return"";let o=(W=vt[t.impact_level])!=null?W:"#96a8b8",n=(X=yt[t.status])!=null?X:t.status,i=300,r=44,l=18,a=r/2,c=10,d=i-18,p=7,m=`<line x1="${l+c}" y1="${a}" x2="${d-p}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,g=`<circle cx="${l}" cy="${a}" r="${c}" fill="#f0c040" opacity="0.92"/>`,h=`
    <circle cx="${d}" cy="${a}" r="${p}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${d}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,w=`<text x="${l}" y="${a+c+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,v=`<text x="${d}" y="${a+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,b="";if(t.progress!=null){let P=l+c+4,j=d-p-4,C=P+t.progress*(j-P),R=5;t.status==="arrival_window"?b=`
        ...
        </g>`:b=`<circle cx="${C.toFixed(1)}" cy="${a}" r="${R}" fill="${o}" opacity="0.85"/>`}let x=`<svg class="hw-cme-svg" viewBox="0 0 ${i} ${r}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${m}
    ${g}${w}
    ${h}${v}
    ${b}
  </svg>`,f=Pe(t.arrival_time_utc),$=Pe(t.launch_time_utc),_=t.speed_kms!=null?`${Math.round(t.speed_kms)} km/s`:"\u2014",M=t.half_angle_deg!=null?`${t.half_angle_deg}\xB0`:"\u2014",y=(N=t.source_location)!=null?N:"\u2014",H=t.is_earth_direct?"Direct hit":"Glancing blow",S=t.progress!=null?`${Math.round(t.progress*100)}%`:"\u2014",k=`
    ...
    </div>`,L=s?"\u25BC":"\u25B6",T=t.impact_level==="unknown"?"Unrated":t.impact_level.charAt(0).toUpperCase()+t.impact_level.slice(1),Y=s?`${x}${k}`:"";return`
    <div class="hw-cme">
      ...
      ${Y}
    </div>`}function Ps(e,s,t,o,n,i,r,l,a,c,d,p,m,g,h,w,v,b){var H;let x=Jt(e,o),f=(H=e.metrics.kp_history_1h)!=null?H:[],$=f.length?xe(f[f.length-1].t_utc):null,_=$?`Recent history \xB7 Last step ${$}`:"Recent history",M=`<div style="padding:10px 14px;border-bottom:1px solid #1e2c30"><div class="hw-section-row" data-hero-toggle style="margin-bottom:0">
    <span class="hw-section-caret">${t?"\u25BC":"\u25B6"}</span>
    <span class="hw-section-label" style="margin-bottom:0">${A(_)}</span>
  </div></div>`,y=t?Xt(e):"";return`
    <div class="hw-root">
      ${Kt(e)}
      ${Ut(e,t,x,v,b)}
      ${Fs(e,x,m,c,g,h,w,b,v)}
      ${M}
      ${y}
      ${es(e,o,x,p)}
      ${Bs(e,d)}
