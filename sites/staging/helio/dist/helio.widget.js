...
`,Ye=!1;function Ht(){if(Ye)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Lt,document.head.appendChild(e),Ye=!0}function Et(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.length,n=s/o,r=e.map((i,l)=>{let a=Math.max(2,Math.min(t,i.kp/9*t)),c=t-a,d=l*n,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${c.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(ye(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${r}</svg>`}function je(e,s,t,o,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let r=200,i=Math.min(...e),l=Math.max(...e),a=l-i||1,c=g=>o-2-(g-i)/a*(o-4),d=e.map((g,u)=>`${(u/(e.length-1)*r).toFixed(1)},${c(g).toFixed(1)}`).join(" "),p="";if(n&&i<0&&l>0){let g=c(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${r}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=e.map((g,u)=>`<rect x="${(u/(e.length-1)*r-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[u]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${r} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ...
    </div>`}function Wt(e){let s=$t(e.updated_utc);return`
    <div class="hw-header">
    ...
    <div class="hw-forecast">
      ...
      ${O}`:""}
    </div>`}function Vt(e){let s=/([NS])(\d+)([EW])(\d+)/i.exec(e);return s?{lat:(s[1].toUpperCase()==="N"?1:-1)*parseInt(s[2],10),lon:(s[3].toUpperCase()==="E"?1:-1)*parseInt(s[4],10)}:null}var Qe=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function Zt(e){return e.x_flare_probability>0?"X":e.m_flare_probability>0?"M":e.c_flare_probability>0?"C":"quiet"}function Jt(e,s,t){let o=s/2,n=o*.87,r=s*.03,i=s*.009,l=e.map(a=>{var b,x;let c=Vt(a.location);if(!c||Math.abs(c.lon)>88||a.location.includes("*"))return"";let d=Zt(a);if(!t.has(d))return"";let p=Qe.find(v=>v.id===d).color,h=c.lat*Math.PI/180,g=c.lon*Math.PI/180,u=(o+n*Math.cos(h)*Math.sin(g)).toFixed(1),f=(o-n*Math.sin(h)).toFixed(1),w=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    <div class="hw-cme-footer">${i}</div>
        ...
        </div>`}else if(T.kind==="aurora"){let xe=ce?" hw-aurora-tip-open":"",Ce=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,K=null;l&&a.lat!=null&&a.lon!=null&&(K=Xe(l.entries,a.lat,a.lon));let fe=a.lat!=null&&a.lon!=null,Me=K!=null?K>=30?"#5cce8c":K>=10?"#d4cc5c":"#9ab4bc":"#607880",Le=K!=null?`${K}%`:l?"n/a":"\u2026",rt=fe?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
          ...
          ${rt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else T.kind==="radio"?se=ls(e,ce):se=`<div class="hw-impact-tip${ce?" hw-impact-tip-open":""}">${m(T.summary)}</div>`;let oe=T.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${P(T.kind)}"`,it=T.kind==="aurora"?' id="aurora"':T.kind==="radio"?' id="radio"':"";return`<div class="hw-impact-row${Se}"${it}${oe}>
    ...
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,w=i.has("geomag_storm"),b=Ze(e),x=b.g1,v=b.g1>=30?Ve.g1:b.g1>0?"#7a9298":"#607880",y=x>0?`G1 ${x}%`:"None",C=`<div class="hw-impact-row${w?" hw-impact-open":""}" id="storm_risk" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${hs(L)}
    </div>`,R=et(e),G=i.has("hss"),Q=`<div class="hw-impact-row${G?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${ms(e,G)}
    </div>`,O=tt(e),j=i.has("sat_drag"),M=`<div class="hw-impact-row${j?" hw-impact-open":""}" id="satellite_drag" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${us(e,j)}
    </div>`,z=st(e),X=i.has("gnss"),F=`<div class="hw-impact-row${X?" hw-impact-open":""}" id="gnss" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${gs(e,X)}
    </div>`,V=ot(e),I=i.has("sw_pressure"),D=V.pressure!=null?`${V.pressure.toFixed(2)} nPa`:"\u2014",N=`<div class="hw-impact-row${I?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${ws(e,I)}
    </div>`,U=nt(e),Z=i.has("cme_cone"),Y=`<div class="hw-impact-row${Z?" hw-impact-open":""}" data-impact-row="cme_cone">
      <span class="hw-impact-caret">\u25B6</span>
...
${t.join(" \xB7 ")}`}function _s(e,s,t){var a;let o=e.is_active?vs:(a=$s[e.level])!=null?a:"#445c64",n=e.event_time===t,r=e.event_time.slice(11,16)+" UTC",i=e.source==="NASA_DONKI"?"DONKI":"SWPC",l=n?`<div class="hw-tl-detail">${m(e.description)}${m(ks(e))}</div>`:"";return`
    ...
    <div class="hw-alerts">
      ${l}
      ${a}
    </div>`}function Ms(e,s){var q,Q,O;let t=e.cme_tracker;if(!t)return"";let o=(q=ft[t.impact_level])!=null?q:"#96a8b8",n=(Q=bt[t.status])!=null?Q:t.status,r=300,i=44,l=18,a=i/2,c=10,d=r-18,p=7,h=`<line x1="${l+c}" y1="${a}" x2="${d-p}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,g=`<circle cx="${l}" cy="${a}" r="${c}" fill="#f0c040" opacity="0.92"/>`,u=`
