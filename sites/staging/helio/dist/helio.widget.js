...
`,At=!1;function he(){if(At)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=pe,document.head.appendChild(t),At=!0}function me(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,a=s/o,c=t.map((i,l)=>{let n=Math.max(2,Math.min(e,i.kp/9*e)),r=e-n,d=l*a,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${r.toFixed(1)}" width="${(a-1).toFixed(1)}" height="${n.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(ft(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${c}</svg>`}function Ft(t,s,e,o,a){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,i=Math.min(...t),l=Math.max(...t),n=l-i||1,r=g=>o-2-(g-i)/n*(o-4),d=t.map((g,w)=>`${(w/(t.length-1)*c).toFixed(1)},${r(g).toFixed(1)}`).join(" "),p="";if(a&&i<0&&l>0){let g=r(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${c}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((g,w)=>`<rect x="${(w/(t.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[w]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${p}
    ...
    ${r}
  </svg>`}function ht(t){return`<div class="hw-kpi-popover-title">
    <span>${t}</span>
    ...
    ${n}${r}${d}${$}${_}${L}${C}
  </svg>`}function $e(t){var h;let s=t.metrics.imf_bz_nt,e=t.metrics.imf_bt_nt,o=t.metrics.solar_wind_kms,a=(h=t.metrics.pressure_npa)!=null?h:null,c=s!=null?s<=-10?"#e05c5c":s<=-5?"#e0a84a":s>=5?"#5cce8c":"#a0b4b8":"#607880",i=s!=null?(s>=0?"+":"")+s.toFixed(1)+" nT":"\u2014",l=e!=null?e.toFixed(1)+" nT":"\u2014",n=o!=null?`${Math.round(o)} km/s`:"\u2014",r=a!=null?`${a.toFixed(2)} nPa`:"\u2014",d=mt(t),p=s!=null&&s<-5?{msg:"Southward IMF \xB7 Aurora favorable",color:"#5cce8c"}:s!=null&&s<0?{msg:"Weakly southward \xB7 Conditions may improve",color:"#d4cc5c"}:{msg:"Northward IMF \xB7 Stable magnetosphere",color:"#96a8b8"};return`<div class="hw-kpi-popover">
    ${ht("IMF Bz \xB7 Coupling")}
  ...
  </g>`,E="";if(t==="magnetosphere"){let A=(O=s.windKms)!=null?O:0,B=A>500,j=A<350,N=B?.55:j?1.5:1,Y=A>700?"#e05c5c":A>500?"#e0a84a":A>350?"#d4c840":"#5cce8c",U=l+8,et=u-14,z=A>500?[22,50,80,110,150,180,210,238]:[30,65,100,130,160,195,230],F=30,D=Math.ceil((et-U)/F)+2,nt=Array.from({length:D},(V,tt)=>U-F+tt*F),ot=22,at=15,lt=nt.flatMap(V=>z.map(tt=>`<path d="M ${V},${tt} L ${V+ot},${tt} M ${V+at},${tt-5} L ${V+ot},${tt} L ${V+at},${tt+5}"
       stroke="${Y}cc" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`)).join(""),it=(I=s.bz)!=null?I:null,P=it==null?"":(()=>{let V=it>0?"#5cce8c":"#e05c5c";return`${it>0?`<path d="M ${n},137 L ${n},123 M ${n-3},126 L ${n},123 L ${n+3},126"
           stroke="${V}" stroke-width="2.2" fill="none" stroke-linecap="round"/>`:`<path d="M ${n},123 L ${n},137 M ${n-3},134 L ${n},137 L ${n+3},134"
           stroke="${V}" stroke-width="2.2" fill="none" stroke-linecap="round"/>`}<text x="${n+22}" y="135" font-size="13"
        fill="${V}" font-family="monospace">Bz${it>0?"\u2191":"\u2193"}</text>`})(),rt=3,vt=Array.from({length:rt},(V,tt)=>`<g class="hw-wg-full" style="animation-duration:${N}s;animation-delay:-${(N/rt*tt).toFixed(2)}s" clip-path="url(#${c}-wclip)">${lt}</g>`).join("");E=`
    <defs><clipPath id="${c}-wclip"><rect x="${U}" y="0" width="${et-U}" height="260"/></clipPath></defs>
    ${vt}
    ${P}`}let G=`<g id="${c}-overlay-solar-wind">${E}</g>`,J="";if(t==="coronal_hole"&&s.hssState){let A=s.hssState,B=(Q=A.speed)!=null?Q:0,j=B>=420,N=A.color,Y=j?"0.85":"0.25",U=B>=500?"0.18":j?"0.10":"0.04",et=l+8,z=d-16,F=60,D=`${et},130 ${z},${130-F} ${z},${130+F}`,nt=z+6,ot=`${nt},120 ${nt+18},130 ${nt},140`;J=`
    <polygon points="${D}" fill="${N}" opacity="${U}"/>
    <line x1="${et}" y1="130" x2="${z-5}" y2="130"
          stroke="${N}" stroke-width="3.5" stroke-dasharray="10 6"
          stroke-linecap="round" opacity="${Y}"/>
    <polygon points="${ot}" fill="${N}" opacity="${j?"0.9":"0.25"}"/>`}let K=`<g id="${c}-overlay-coronal-hole">${J}</g>`,R="";if(t==="cme_cone"&&s.cmeState){let A=s.cmeState,B=l,j=d-10,N=j-B,Y=D=>Math.tan(D*Math.PI/180),U=Math.round(Y(9)*N),et=Math.round(Y(6)*N),z=Math.round(Y(3)*N),F=D=>`${B},130 ${j},${130-D} ${j},${130+D}`;if(A.status!=="quiet"){let D=A.status==="impact"?"#e05c5c":"#d4cc5c";R=`
    <polygon points="${F(U)}" fill="#253238" opacity="0.85"/>
    <polygon points="${F(et)}"   fill="#d4cc5c" opacity="0.14"/>
    ...
    <line x1="${B+14}" y1="130" x2="${d-14}" y2="130"
          stroke="#1e2c30" stroke-dasharray="7 5" stroke-width="2"/>`}let X=`<g id="${c}-overlay-cme-cone">${R}</g>`,st=`<g id="${c}-overlay-labels">
  ...
  </div>`}function ve(t,s){if(!s)return"";let e=mt(t),o=t.metrics.imf_bz_nt,a=t.metrics.solar_wind_kms,c=t.metrics.density,i=t.metrics.pressure_npa,l=o!=null?(o>=0?"+":"")+o.toFixed(1)+" nT":"\u2014",n=a!=null?`${Math.round(a)} km/s`:"\u2014",r=c!=null?`${c.toFixed(1)} p/cm\xB3`:"\u2014",d=i!=null?`${i.toFixed(2)} nPa`:"\u2014",p=o!=null?o<=-10?"#e05c5c":o<=-5?"#e0a84a":o>=5?"#5cce8c":"#a0b4b8":"#607880",h=a!=null?a>700?"#e05c5c":a>500?"#e0a84a":a>350?"#d4c840":"#5cce8c":"#607880",g=o!=null&&o<-5?"Southward IMF Bz is strongly coupling energy into the magnetosphere. Geomagnetic storm conditions likely.":o!=null&&o<0?"Southward IMF Bz is partially opening the magnetosphere. Enhanced aurora activity possible.":"Northward IMF Bz keeps the magnetosphere closed. Solar wind energy transfer is minimal.";return`<div class="hw-impact-tip">
    ...
    ${ht("Aurora Oval \xB7 Northern Hemisphere")}
  ...
  </div>`}function Se(t,s,e,o,a,c,i,l=0){var Q,A,B,j,N,Y,U,et;let{summary:n,scales:r,metrics:d,aurora_hint:p}=t,h=(Q=zt[n.status])!=null?Q:zt.quiet,g=a!=null?a.kp.toFixed(1):d.kp_latest!=null?d.kp_latest.toFixed(1):"\u2014",u=[a?a.gScale:r.g_scale,r.r_scale,r.s_scale].map(z=>{let F=de(z),D=F?`color:${h.accent};border-color:${h.accent}33`:"";return`<span class="hw-scale-chip${F?" hw-scale-active":""}" style="${D}">${m(z)}</span>`}).join(""),b=a?a.auroraLabel:p.aurora_label,$=b==="good"?"#5cce8c":b==="possible"?"#d4cc5c":"#607880",x=b.charAt(0).toUpperCase()+b.slice(1),f="#b4c6cc",v=d.solar_wind_kms!=null?`${Math.round(d.solar_wind_kms)} km/s`:"\u2014",_=d.imf_bz_nt,L=_!=null?_<=-10?"#e05c5c":_<=-5?"#e0a84a":_>=5?"#5cce8c":"#a0b4b8":"#607880",M=_!=null?(_>=0?"+":"")+_.toFixed(1)+" nT":"\u2014",C=d.xray_class,y=C?(A=Ct[C])!=null?A:"#a0b4b8":"#607880",k=C?`${C}-class`:"\u2014",H=xt(((B=d.kp_history_1h)!=null?B:[]).map(z=>z.kp),.5),E=xt(((j=d.wind_history_1h)!=null?j:[]).map(z=>z.kms),20),G=xt(((N=d.bz_history_1h)!=null?N:[]).map(z=>z.bz),1.5),J=xt(((Y=d.xray_history_1h)!=null?Y:[]).map(z=>Math.log10(z.flux+1e-9)),.15),K=(U=d.kp_history_1h)!=null?U:[],R=K.length?dt(K[K.length-1].t_utc):null,X=R?`Recent history \xB7 Last step ${R}`:"Recent history",st=mt(t),S=(et=d.kp_latest)!=null?et:0,T=S>=5,q=T?`linear-gradient(160deg, #0d2a1a 0%, ${h.bg}22 75%)`:`${h.bg}18`,O=(z,F,D,nt,ot)=>{let at=ot?`<span class="hw-trend">${ot}</span>`:"";return`<div class="hw-kpi-item${o===z?" hw-kpi-active":""}" data-kpi="${z}">
    ...
    </div>`;let $=320,x=38,f=14,v=x+f,_=$/d,L=S=>x-Math.max(2,Math.min(x-2,S/9*(x-2))),M="",C=r.map(S=>S.kp),y=Le(C);r.forEach((S,T)=>{let q=L(S.kp),O=x-q,I=T*_,Q=I+_/2,A=S.kp>=6?"#e05c5c":S.kp>=5?"#e0a84a":S.kp>=4?"#d4cc5c":"#5cce8c",B=`Kp ${S.kp.toFixed(1)} \xB7 ${dt(S.t_utc)}`;if(M+=`<rect x="${I.toFixed(1)}" y="${q.toFixed(1)}" width="${(_-1.5).toFixed(1)}" height="${O.toFixed(1)}" fill="${A}" fill-opacity="0.85" rx="1.5"/>`,M+=`<rect x="${I.toFixed(1)}" y="0" width="${_.toFixed(1)}" height="${x}" fill="transparent"><title>${W(B)}</title></rect>`,d<=8||T%2===0){let N=new Date(S.t_utc).getHours();M+=`<text x="${Q.toFixed(1)}" y="${(v-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${N.toString().padStart(2,"0")}</text>`}});let H=`<polyline points="${r.map((S,T)=>{let q=T*_+_/2,O=L(y[T]);return`${q.toFixed(1)},${O.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,E="";if(s>0&&d>0){let S=Math.min($-1,s/(d*3)*$);E=`
...
C: ${n.c_flare_probability}%  M: ${n.m_flare_probability}%  X: ${n.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    <div class="hw-cme-footer">${i}</div>
  </div>`}function Je(t,s,e,o,a,c,i,l,n){var at,lt,it;let r=(lt=(at=s==null?void 0:s.impacts)!=null?at:t.observer_impacts)!=null?lt:[],d=r.map(P=>{var Lt,Ht;let rt=(Lt=ne[P.level])!=null?Lt:"#666",vt=P.level==="none"?"None":P.level.charAt(0).toUpperCase()+P.level.slice(1),V=(Ht=Ie[P.kind])!=null?Ht:Pe,tt=P.level==="none"?"#606870":rt,ut=P.kind==="solar_activity"?a:i.has(P.kind),qt=ut?" hw-impact-open":"",ct;if(P.kind==="solar_activity"){let gt=a?" hw-solar-open":"",yt=Nt.map(Z=>{let wt=c.has(Z.id),kt=wt?Z.color+"22":"transparent",_t=wt?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${Z.id}" style="color:${Z.color};border-color:${Z.color};background:${kt};opacity:${_t}">${Z.label}</button>`}).join("");ct=`<div class="hw-solar-tip${gt}">
          ...
          <span class="hw-solar-tip-text">${m(P.summary)}</span>
        </div>`}else if(P.kind==="aurora"){let gt=ut?" hw-aurora-tip-open":"",yt=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,Z=null;l&&n.lat!=null&&n.lon!=null&&(Z=Ot(l.entries,n.lat,n.lon));let wt=n.lat!=null&&n.lon!=null,kt=Z!=null?Z>=30?"#5cce8c":Z>=10?"#d4cc5c":"#9ab4bc":"#607880",_t=Z!=null?`${Z}%`:l?"n/a":"\u2026",Vt=wt?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
        ...
        </div>`:"";ct=`<div class="hw-aurora-tip${gt}">
          <div class="hw-aurora-map-wrap">
          ...
          ${Vt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else P.kind==="radio"?ct=Ye(t,ut):ct=`<div class="hw-impact-tip${ut?" hw-impact-tip-open":""}">${m(P.summary)}</div>`;let Qt=P.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${W(P.kind)}"`;return`<div class="hw-impact-row${qt}"${Qt}>
      <span class="hw-impact-caret">\u25B6</span>
      <span class="hw-impact-kind" style="color:${tt}">${V}<span style="color:#b4c6cc">${m(P.label)}</span></span>
      <span class="hw-impact-badge" style="background:${rt}22;color:${rt}">${m(vt)}</span>
      ${ct}
    ...
    <div class="hw-section-row" data-impacts-toggle>
      ...
      ${Xe(t,G)}
    </div>`,R=Wt(t),X=i.has("sat_drag"),S=`<div class="hw-impact-row${X?" hw-impact-open":""}" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Qe(t,q)}
    </div>`,Q=Kt(t),A=i.has("sw_pressure"),B=Q.pressure!=null?`${Q.pressure.toFixed(2)} nPa`:"\u2014",N=`<div class="hw-impact-row${A?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ze(t,U)}
    </div>`,F=mt(t),D=i.has("magnetosphere"),ot=`<div class="hw-impact-row${D?" hw-impact-open":""}" data-impact-row="magnetosphere">
      <span class="hw-impact-caret">\u25B6</span>
...
${e.join(" \xB7 ")}`}function as(t,s,e){var n;let o=t.is_active?ss:(n=es[t.level])!=null?n:"#445c64",a=t.event_time===e,c=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",l=a?`<div class="hw-tl-detail">${m(t.description)}${m(ns(t))}</div>`:"";return`
    ...
    <circle cx="${d}" cy="${n}" r="2.5" fill="#fff" opacity="0.7"/>`,u=`<text x="${l}" y="${n+r+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,b=`<text x="${d}" y="${n+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,$="";if(e.progress!=null){let X=l+r+4,st=d-p-4,S=X+e.progress*(st-X),T=5;e.status==="arrival_window"?$=`
    ...
    <div class="hw-cme">
      ...
      ${G}
    </div>`}function cs(t,s,e,o,a,c,i,l,n,r,d,p,h,g,w,u,b,$,x,f,v=0){var H;let _=Ae(t,a),L=(H=t.metrics.kp_history_1h)!=null?H:[],M=L.length?dt(L[L.length-1].t_utc):null,C=M?`Recent history \xB7 Last step ${M}`:"Recent history",y=`<div style="padding:10px 14px;border-bottom:1px solid #1e2c30"><div class="hw-section-row" data-hero-toggle style="margin-bottom:0">
    <span class="hw-section-caret">${e?"\u25BC":"\u25B6"}</span>
