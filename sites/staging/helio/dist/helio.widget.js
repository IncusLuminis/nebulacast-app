...
`,Ye=!1;function Ft(){if(Ye)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Lt,document.head.appendChild(e),Ye=!0}function Rt(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.length,n=s/o,l=e.map((r,i)=>{let a=Math.max(2,Math.min(t,r.kp/9*t)),c=t-a,d=i*n,p=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${c.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${g(be(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${l}</svg>`}function We(e,s,t,o,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let l=200,r=Math.min(...e),i=Math.max(...e),a=i-r||1,c=u=>o-2-(u-r)/a*(o-4),d=e.map((u,h)=>`${(h/(e.length-1)*l).toFixed(1)},${c(u).toFixed(1)}`).join(" "),p="";if(n&&r<0&&i>0){let u=c(0);p=`<line x1="0" y1="${u.toFixed(1)}" x2="${l}" y2="${u.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let m=e.map((u,h)=>`<rect x="${(h/(e.length-1)*l-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${g(s[h]||"")} \xB7 ${u.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${l} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${p}
    <polyline points="${d}" fill="none" stroke="${t}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
    ${m}
  </svg>`}function Et(e){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.map(d=>Math.max(-9,Math.min(-3,Math.log10(d.flux)))),n=Math.min(...o),r=Math.max(...o)-n||1,i=d=>t-2-(d-n)/r*(t-4),a=o.map((d,p)=>`${(p/(o.length-1)*s).toFixed(1)},${i(d).toFixed(1)}`).join(" "),c=e.map((d,p)=>{let m=p/(o.length-1)*s,u=d.flux>=1e-4?"X":d.flux>=1e-5?"M":d.flux>=1e-6?"C":d.flux>=1e-7?"B":"A";return`<rect x="${(m-4).toFixed(1)}" y="0" width="8" height="${t}" fill="transparent"><title>${g(be(d.t_utc))} \xB7 ${u}-class (${d.flux.toExponential(2)})</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function It(e){var c;let s=(c=e.metrics.wind_history_1h)!=null?c:[],t=s[s.length-1],o=e.metrics.solar_wind_kms,n=o!=null?`${Math.round(o)} km/s`:"\u2014",l=o!=null?o>=700?"#e05c5c":o>=500?"#e0a84a":o>=400?"#d4cc5c":"#5cce8c":"#607880",r=(t==null?void 0:t.density)!=null?`${t.density.toFixed(2)} cm\u207B\xB3`:"\u2014",i=(t==null?void 0:t.temp_kk)!=null?`${t.temp_kk.toFixed(0)} kK`:"\u2014",a=(t==null?void 0:t.pressure_npa)!=null?`${t.pressure_npa.toFixed(2)} nPa`:"\u2014";return`<div class="hw-kpi-popover">
    ...
    ${a}${c}${d}${$}${k}${S}${H}
  </svg>`}function zt(e){var m;let s=e.metrics.imf_bz_nt,t=e.metrics.imf_bt_nt,o=e.metrics.solar_wind_kms,n=(m=e.metrics.pressure_npa)!=null?m:null,l=s!=null?s<=-10?"#e05c5c":s<=-5?"#e0a84a":s>=5?"#5cce8c":"#a0b4b8":"#607880",r=s!=null?(s>=0?"+":"")+s.toFixed(1)+" nT":"\u2014",i=t!=null?t.toFixed(1)+" nT":"\u2014",a=o!=null?`${Math.round(o)} km/s`:"\u2014",c=n!=null?`${n.toFixed(2)} nPa`:"\u2014",d=ye(e),p=s!=null&&s<-5?{msg:"Southward IMF \xB7 Aurora favorable",color:"#5cce8c"}:s!=null&&s<0?{msg:"Weakly southward \xB7 Conditions may improve",color:"#d4cc5c"}:{msg:"Northward IMF \xB7 Stable magnetosphere",color:"#96a8b8"};return`<div class="hw-kpi-popover">
  ...
  </div>`}function ye(e){var a,c;let s=e.metrics.imf_bz_nt,t=(a=e.metrics.kp_latest)!=null?a:0,o=(c=e.metrics.solar_wind_kms)!=null?c:0,n,l,r;if(s!=null&&s<-5||t>=6)n="storm",l="#e05c5c",r="Storm conditions";else if(s!=null&&s<0||t>=4||o>=400){let d=s!=null&&s<0;n="active",l="#e0a84a",r=d?"Active coupling":"Elevated"}else n="stable",l="#5cce8c",r="Stable";let i;return s==null?i="Unknown":s>2?i="Closed":s>0?i="Minimal":s>-5?i="Moderate":s>-10?i="Strong":i="Very strong",{state:n,color:l,label:r,coupling:i}}function Tt(e,s,t,o){let n=o?"mc":"mf",l=e.color,r=t!=null?t:0,i=r>500,a=r<350,c=i?.9:a?1.8:1.3;if(o){let h=45-(e.state==="storm"?11:e.state==="active"?16:21),w=e.state==="storm"?12:e.state==="active"?10:8,v=50-w,$=76,x=[`M ${h},25`,`C ${h-2},15 41,${w} 45,${w}`,`C 53,${w} ${$-8},${w+4} ${$},20`,`C ${$+1},23 ${$+1},27 ${$},30`,`C ${$-8},${v-4} 53,${v} 45,${v}`,`C 41,${v} ${h-2},35 ${h},25`,"Z"].join(" "),f=i?3:2,b=[14,25,36],k=M=>`<path d="M 0,${M} L ${i?8:6},${M} M ${i?6:4},${M-2} L ${i?8:6},${M} L ${i?6:4},${M+2}" stroke="${l}bb" stroke-width="${i?1.4:1}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`,S=b.map(M=>k(M)).join(""),_=Array.from({length:f},(M,y)=>`<g class="hw-wg" style="animation-duration:${c}s;animation-delay:${(c/f*y).toFixed(2)}s">${S}</g>`).join(""),H=s==null?"":s>0?`<path d="M 45,27 L 45,23 M ${45-1.5},${25-.5} L 45,23 L ${45+1.5},${25-.5}" stroke="#5cce8c" stroke-width="1" fill="none" stroke-linecap="round"/>`:`<path d="M 45,23 L 45,27 M ${45-1.5},${25+.5} L 45,27 L ${45+1.5},${25+.5}" stroke="#e05c5c" stroke-width="1" fill="none" stroke-linecap="round"/>`;return`<svg viewBox="0 0 80 50" style="width:78px;height:49px;display:block" xmlns="http://www.w3.org/2000/svg">
    ...
    <line x1="${i}" y1="130" x2="${w}" y2="130"
  ...
  </g>`,T="";if(e==="magnetosphere"){let z=(A=s.windKms)!=null?A:0,D=z>500,B=z<350,F=(D?.5:B?1.4:.9)*1.3,K=z>700?"#e05c5c":z>500?"#e0a84a":z>350?"#d4c840":"#5cce8c",te=i+8,ee=w-14,I=8,G=6,U=(ee-te)/(I-1),se=260/(G+1),V=38,ae=24,_e=2,Se=Array.from({length:I},(ne,de)=>{let oe=te+de*U,re=Array.from({length:G},(pe,he)=>{let Z=se*(he+1);return`<path d="M ${oe.toFixed(1)},${Z.toFixed(1)} L ${(oe+V).toFixed(1)},${Z.toFixed(1)} M ${(oe+ae).toFixed(1)},${(Z-6).toFixed(1)} L ${(oe+V).toFixed(1)},${Z.toFixed(1)} L ${(oe+ae).toFixed(1)},${(Z+6).toFixed(1)}"
          stroke="${K}" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`}).join("");return Array.from({length:_e},(pe,he)=>{let Z=-((I-de)/I*F)-he*F/_e;return`<g style="opacity:.12;animation:hw-arrow-chase ${F}s linear ${Z.toFixed(3)}s infinite">${re}</g>`}).join("")}).join(""),q=(R=s.bz)!=null?R:null,ce=q==null?"":(()=>{let ne=q>0?"#5cce8c":"#e05c5c";return`${q>0?`<path d="M ${a},137 L ${a},123 M ${a-3},126 L ${a},123 L ${a+3},126"
        ...
        fill="${ne}" font-family="monospace">Bz${q>0?"\u2191":"\u2193"}</text>`})();T=`${Se}${ce}`}let Y=`<g id="${l}-overlay-solar-wind">${T}</g>`,W="";if(e==="coronal_hole"&&s.hssState){let z=s.hssState,D=(J=z.speed)!=null?J:0,B=D>=420,F=z.color,K=B?"0.85":"0.25",te=D>=500?"0.18":B?"0.10":"0.04",ee=i+8,I=d-16,G=60,U=`${ee},130 ${I},${130-G} ${I},${130+G}`,se=I+6,V=`${se},120 ${se+18},130 ${se},140`;W=`
    ...
    <polygon points="${V}" fill="${F}" opacity="${B?"0.9":"0.25"}"/>`}let X=`<g id="${l}-overlay-coronal-hole">${W}</g>`,N="";if(e==="cme_cone"&&s.cmeState){let z=s.cmeState,D=i,B=d-10,F=B-D,K=U=>Math.tan(U*Math.PI/180),te=Math.round(K(9)*F),ee=Math.round(K(6)*F),I=Math.round(K(3)*F),G=U=>`${D},130 ${B},${130-U} ${B},${130+U}`;if(z.status!=="quiet"){let U=z.status==="impact"?"#e05c5c":"#d4cc5c";N=`
    ...
    <line x1="${D+14}" y1="130" x2="${d-14}" y2="130"
          stroke="#1e2c30" stroke-dasharray="7 5" stroke-width="2"/>`}let P=`<g id="${l}-overlay-cme-cone">${N}</g>`,j=`<g id="${l}-overlay-labels">
    ...
    <div class="hw-kpi-stat-row">
      ...
      <div class="hw-kpi-stat">
        ...
        <span class="hw-kpi-stat-value" style="color:${p}">${g(i)}</span>
      </div>
      <div class="hw-kpi-stat">
    ...
    <div class="hw-kpi-hint" style="margin-bottom:0">${g(u)}</div>
  </div>`}function Ze(e,s,t){if(!e.length)return null;let o=(t%360+360)%360,n=-1,l=1/0,r=Math.cos(s*Math.PI/180);for(let i of e){let a=i.lat-s,c=(i.lon-o+180+360)%360-180,d=a*a+c*r*(c*r);d<l&&(l=d,n=i.prob)}return n>=0?n:null}function Je(e){return`<svg viewBox="0 0 100 100" width="100%" height="100%"
  ...
  </div>`}function Dt(e,s,t,o){switch(s){case"solar_wind":return It(e);case"xray":return Ot(e);case"imf_bz":return zt(e);case"aurora":return Pt(t,o);case"magnetosphere":return Nt(e);default:return""}}function Gt(e,s){if(e.length<2)return"\u2192";let t=e[e.length-1],o=Math.max(0,e.length-4),n=e[o];if(!isFinite(t)||!isFinite(n))return"\u2192";let l=t-n;return l>s?"\u2191":l<-s?"\u2193":"\u2192"}var Yt={quiet:"171",active:"195",elevated:"284",storm:"304"};function Wt(e,s){var o,n,l;if(s==null)return(n=(o=e.summary)==null?void 0:o.status)!=null?n:"quiet";let t=parseInt(((l=s.gScale)!=null?l:"G0").replace(/\D/g,""),10)||0;return t>=3?"storm":t>=1?"elevated":s.kp>=4?"active":"quiet"}function jt(e,s){var c,d;let t=(c=Yt[e])!=null?c:"171",o=`/assets/gifs/current_eit_${t}.gif`,n=cs(o,s),l=ps,r=(d=$e[e])!=null?d:$e.quiet,i=`${r.accent}44`,a=`0 0 10px ${r.accent}55,0 0 24px ${r.accent}22`;return`<div class="hw-solar-mini-wrap" style="cursor:default">
    ...
    <div class="hw-hero" style="background:${w}">
      <div class="hw-hero-main">
        <div class="hw-kp-col">
          ...
          <span class="hw-status-badge" style="background:${i.accent}22;color:${i.accent};display:block;text-align:center">${g(l.label)}</span>
      ...
      ${x}
    </div>`}function Xt(e){var d,p,m,u;let{metrics:s}=e,t=(d=s.kp_history_1h)!=null?d:[],o=(p=s.wind_history_1h)!=null?p:[],n=(m=s.bz_history_1h)!=null?m:[],l=(u=s.xray_history_1h)!=null?u:[],r=Rt(t),i=We(o.map(h=>{var w;return(w=h.kms)!=null?w:0}).filter(h=>h>0),o.map(h=>be(h.t_utc)),"#5cce8c",28,!1),a=We(n.map(h=>h.bz),n.map(h=>be(h.t_utc)),"#d4cc5c",28,!0),c=Et(l);return`
    ...
    </div>`;let $=320,x=38,f=14,b=x+f,k=$/d,S=C=>x-Math.max(2,Math.min(x-2,C/9*(x-2))),_="",H=c.map(C=>C.kp),M=qt(H);c.forEach((C,E)=>{let Q=S(C.kp),A=x-Q,R=E*k,J=R+k/2,z=C.kp>=6?"#e05c5c":C.kp>=5?"#e0a84a":C.kp>=4?"#d4cc5c":"#5cce8c",D=`Kp ${C.kp.toFixed(1)} \xB7 ${xe(C.t_utc)}`;if(_+=`<rect x="${R.toFixed(1)}" y="${Q.toFixed(1)}" width="${(k-1.5).toFixed(1)}" height="${A.toFixed(1)}" fill="${z}" fill-opacity="0.85" rx="1.5"/>`,_+=`<rect x="${R.toFixed(1)}" y="0" width="${k.toFixed(1)}" height="${x}" fill="transparent"><title>${O(D)}</title></rect>`,d<=8||E%2===0){let F=new Date(C.t_utc).getHours();_+=`<text x="${J.toFixed(1)}" y="${(b-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${F.toString().padStart(2,"0")}</text>`}});let L=`<polyline points="${c.map((C,E)=>{let Q=E*k+k/2,A=S(M[E]);return`${Q.toFixed(1)},${A.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,T="";if(s>0&&d>0){let C=Math.min($-1,s/(d*3)*$);T=`
    ...
    <div class="hw-forecast">
      ...
      ${N}`:""}
    </div>`}function ts(e){let s=/([NS])(\d+)([EW])(\d+)/i.exec(e);return s?{lat:(s[1].toUpperCase()==="N"?1:-1)*parseInt(s[2],10),lon:(s[3].toUpperCase()==="E"?1:-1)*parseInt(s[4],10)}:null}var tt=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function ss(e){return e.x_flare_probability>0?"X":e.m_flare_probability>0?"M":e.c_flare_probability>0?"C":"quiet"}function os(e,s,t){let o=s/2,n=o*.87,l=s*.03,r=s*.009,i=e.map(a=>{var $,x;let c=ts(a.location);if(!c||Math.abs(c.lon)>88||a.location.includes("*"))return"";let d=ss(a);if(!t.has(d))return"";let p=tt.find(f=>f.id===d).color,m=c.lat*Math.PI/180,u=c.lon*Math.PI/180,h=(o+n*Math.cos(m)*Math.sin(u)).toFixed(1),w=(o-n*Math.sin(m)).toFixed(1),v=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}var us={0:"Quiet",1:"Minor Storm",2:"Moderate Storm",3:"Strong Storm",4:"Severe Storm",5:"Extreme Storm"};function gs(e){return e>=9?5:e>=8?4:e>=7?3:e>=6?2:e>=5?1:0}function ie(e,s){if(e<s-.7)return 0;if(e>s+1)return .9;let t=(e-(s-.7))/1.7;return Math.round(Math.pow(Math.max(0,t),.7)*90)/100}function ws(e){var c,d,p;let s=parseInt(((c=e.scales.g_scale)!=null?c:"G0").replace(/\D/g,""),10),t=Math.max(0,Math.min(5,Number.isFinite(s)?s:0)),o=(d=e.metrics.kp_forecast_3h)!=null?d:[],n=Date.now(),l=n+24*60*60*1e3,r=o.filter(m=>{let u=new Date(m.t_utc).getTime();return u>=n-3*60*60*1e3&&u<=l}),i=r.length===0?0:Math.max(...r.map(m=>m.kp)),a={G1:ie(i,5),G2:ie(i,6),G3:ie(i,7),G4:ie(i,8),G5:ie(i,9),max_expected:gs(i)};return{now:{g_level:t,label:(p=us[t])!=null?p:"Quiet"},forecast_24h:a}}function xs(e){if(!e||typeof e!="object")return null;let s=e,t=s.now,o=s.forecast_24h;if(!t||!o)return null;let n=typeof t.g_level=="number"?t.g_level:parseInt(String(t.g_level),10),l=typeof t.label=="string"?t.label:"Quiet";if(!Number.isFinite(n))return null;let r=c=>{let d=typeof c=="number"?c:parseFloat(String(c));return Number.isFinite(d)?Math.max(0,Math.min(1,d)):0},i=o.max_expected,a=typeof i=="number"&&Number.isFinite(i)?Math.max(0,Math.min(5,Math.round(i))):0;return{now:{g_level:Math.max(0,Math.min(5,Math.round(n))),label:l},forecast_24h:{G1:r(o.G1),G2:r(o.G2),G3:r(o.G3),G4:r(o.G4),G5:r(o.G5),max_expected:a}}}function ot(e){let s=ws(e),t=xs(e.storm_risk);if(!t)return s;let o=e.storm_risk,n=o&&typeof o.forecast_24h=="object"&&o.forecast_24h!==null?o.forecast_24h:void 0,l=n==null?void 0:n.max_expected;return typeof l=="number"&&Number.isFinite(l)?t:we(ge({},t),{forecast_24h:we(ge({},t.forecast_24h),{max_expected:s.forecast_24h.max_expected})})}function fs(e){return e>=4?"#e05c5c":e>=3?"#e0a84a":e>=1?"#d4cc5c":"#607880"}var je={rising:"#e0884a",peak:"#e05c5c",decline:"#d4cc5c"};function $s(e){var c,d,p;let s=(c=e.metrics.kp_latest)!=null?c:0,t=e.metrics.imf_bz_nt,o=e.metrics.solar_wind_kms,n=parseInt(((d=e.scales.g_scale)!=null?d:"G0").replace("G",""),10)||0,l=s>=5||n>=1,r=(p=e.metrics.kp_history_1h)!=null?p:[],i=0;if(r.length>=2&&(i=r[r.length-1].kp-r[r.length-2].kp),!l)return{active:!1,phase:"quiet",kp_current:s,kp_trend:i,bz_nt:t,solar_wind_kms:o};let a;return i>.3&&(t==null||t<-5)?a="rising":i<-.5?a="decline":a="peak",{active:!0,phase:a,kp_current:s,kp_trend:i,bz_nt:t,solar_wind_kms:o}}function bs(e){if(!e.active)return"";let s=[{key:"rising",label:"Rising"},{key:"peak",label:"Peak"},{key:"decline",label:"Decline"}],t=s.findIndex(i=>i.key===e.phase),o=je[e.phase],n=s[t].label,l=s.map((i,a)=>{let c=a===t,d=a<t,p=je[i.key],m=c?`background:${p};border-color:${p};box-shadow:0 0 6px ${p}88`:d?`background:${p}44;border-color:${p}66`:"background:#111b1e;border-color:#1e2c30",u=c?" hw-spi-dot-active":"",h=c?`color:${p};font-weight:700`:d?`color:${p}66`:"color:#2e4248",w=a<s.length-1?`<div class="hw-spi-arr">${d?`<span style="color:${p}55">\u2192</span>`:"\u2192"}</div>`:"";return`<div class="hw-spi-node">
  ...
  </div>`}function Ue(e,s,t){let o=Math.max(0,Math.min(5,Math.round(e))),n=(t==null?void 0:t.showDialCode)!==!1,l=70,r=76,i=48,c=i-9,d=Math.PI,p=k=>d*(1-k/5),m=p(o),u=(k,S)=>({x:l+S*Math.cos(k),y:r-S*Math.sin(k)}),h=(k,S,_)=>{let H=u(k,i),M=u(S,i),y=u(k,c),L=u(S,c);return`<path d="M ${y.x.toFixed(2)} ${y.y.toFixed(2)} L ${H.x.toFixed(2)} ${H.y.toFixed(2)} A ${i} ${i} 0 0 1 ${M.x.toFixed(2)} ${M.y.toFixed(2)} L ${L.x.toFixed(2)} ${L.y.toFixed(2)} A ${c} ${c} 0 0 0 ${y.x.toFixed(2)} ${y.y.toFixed(2)} Z" fill="${_}"/>`},w=k=>{let S=p(k),_=u(S,i+1),H=u(S,i-5);return`<line x1="${_.x.toFixed(2)}" y1="${_.y.toFixed(2)}" x2="${H.x.toFixed(2)}" y2="${H.y.toFixed(2)}" stroke="#2a3a40" stroke-width="1" stroke-linecap="round"/>`},v=u(m,i-2),$=`<line x1="${l}" y1="${r}" x2="${v.x.toFixed(2)}" y2="${v.y.toFixed(2)}" stroke="#c8d6dc" stroke-width="2" stroke-linecap="round"/>`,x=`<circle cx="${l}" cy="${r}" r="3.5" fill="#3a4c52" stroke="#1e2c30" stroke-width="1"/>`,f=h(p(0),p(2),"#3d8f62")+h(p(2),p(4),"#b8982a")+h(p(4),p(5),"#b04048"),b=[0,1,2,3,4,5].map(w).join("");return`<div class="hw-storm-risk-gauge" role="img" aria-label="${O(s)} G${o}">
  ...
  </div>`}function vs(e,s){let t=ot(e),o=$s(e),n=t.forecast_24h,l=`
    <div class="hw-storm-risk-now">
      <div class="hw-storm-risk-now-gauge">${Ue(t.now.g_level,"Current observed geomagnetic storm level",{showDialCode:!0})}</div>
    ...
    <div class="hw-storm-risk-grid">
      ...
      <div class="hw-storm-risk-rail">
        ...
        <div class="hw-storm-risk-fc">${i}</div>
      </div>
    </div>
  </div>`}var fe={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},nt={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function ys(e){var M;let s=fe,t=(M=nt[s.phase])!=null?M:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,l=52,r=10,i=l-6,a=l-18,c=.5,d=.19,p=y=>Math.exp(-Math.pow((y-c)/d,2)/2),m=y=>r+y*(n-2*r),u=y=>i-p(y)*a,h=80,w=[];for(let y=0;y<=h;y++){let L=y/h;w.push(`${y===0?"M":"L"}${m(L).toFixed(1)},${u(L).toFixed(1)}`)}let v=Math.round(s.progress_0_1*h),$=[];for(let y=0;y<=v;y++){let L=y/h;$.push(`${y===0?"M":"L"}${m(L).toFixed(1)},${u(L).toFixed(1)}`)}let x=m(s.progress_0_1),f=[`M${r},${i}`,...$.slice(1),`L${x.toFixed(1)},${i} Z`],b=u(s.progress_0_1),k=5,S=`M${x.toFixed(1)},${b.toFixed(1)} L${(x-k).toFixed(1)},${(b-k*1.8).toFixed(1)} L${(x+k).toFixed(1)},${(b-k*1.8).toFixed(1)} Z`,_=i+11;return`<div class="hw-impact-tip${e?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function rt(e){var a,c,d,p,m;let s=(c=(a=e.coronal_hole)==null?void 0:a.estimated_speed_kms)!=null?c:e.metrics.solar_wind_kms,t=(d=e.coronal_hole)==null?void 0:d.status,o=s!=null?s:0,n=t!=null?t:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",l={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},r={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},i={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:n,color:l[n],label:r[n],desc:(m=(p=e.coronal_hole)==null?void 0:p.note)!=null?m:i[n],speed:s}}function ks(e,s){var c;let t=rt(e),o=(c=t.speed)!=null?c:0,n=t.speed!=null?`${Math.round(t.speed)} km/s`:"\u2014",l=s?" hw-impact-tip-open":"",r=o>=420,i=ke("coronal_hole",{hssState:t,uid:"hss"}),a=r?'<div class="hw-hss-meta" style="font-size:.72em">Elevated speed may indicate Earth-facing coronal hole stream</div>':'<div class="hw-hss-meta" style="font-size:.72em">Background solar wind \xB7 no HSS detected</div>';return`<div class="hw-impact-tip${l}">
    ${i}
    <div class="hw-hss-meta">Solar wind: <b style="color:${t.color}">${g(n)}</b> \xB7 ${g(t.desc)}</div>
    ${a}
  </div>`}function at(e){var n,l;let s=(n=e.scales.g_scale)!=null?n:"G0",t=parseInt(s.slice(1),10),o=e.metrics.kp_latest;if(o==null){let r=(l=e.metrics.kp_forecast_3h)!=null?l:[],i=Date.now(),a=r.filter(c=>new Date(c.t_utc).getTime()<=i+3*60*60*1e3).sort((c,d)=>new Date(d.t_utc).getTime()-new Date(c.t_utc).getTime());a.length>0&&(o=a[0].kp)}return t>=2||o!=null&&o>=6?{level:2,color:"#e05c5c",label:"High",kp:o,gScale:s}:t>=1||o!=null&&o>=4?{level:1,color:"#d4cc5c",label:"Moderate",kp:o,gScale:s}:{level:0,color:"#5cce8c",label:"Low",kp:o,gScale:s}}function _s(e,s){let t=at(e),o=s?" hw-impact-tip-open":"",l=[{l:0,label:"Low",color:"#5cce8c",width:33,desc:"Normal density"},{l:1,label:"Moderate",color:"#d4cc5c",width:64,desc:"Elevated density"},{l:2,label:"High",color:"#e05c5c",width:100,desc:"Strong expansion"}].map(a=>{let c=a.l===t.level,d=c?a.color:"#566068",p=c?"0.88":"0.16";return`<div class="hw-satdrag-rung">
    ...
    <div class="hw-gnss-ladder">${l}</div>
    <div class="hw-gnss-meta">${r} \xB7 ${g(t.gScale)} \xB7 ${i[t.level]}</div>
    ${a}
  </div>`}function lt(e){var n;let s=e.metrics.pressure_npa,t=s!=null?s:null,o=(n=e.metrics.density)!=null?n:null;return t==null?{pressure:null,color:"#607880",label:"\u2014",density:o}:t>=6?{pressure:t,color:"#e05c5c",label:"Extreme",density:o}:t>=4?{pressure:t,color:"#e0a84a",label:"Strong",density:o}:t>=2?{pressure:t,color:"#d4cc5c",label:"Elevated",density:o}:t>=1?{pressure:t,color:"#5cce8c",label:"Typical",density:o}:{pressure:t,color:"#7a9298",label:"Weak",density:o}}function Ms(e,s){let t=lt(e),o=s?" hw-impact-tip-open":"",n=t.pressure,l=200,r=6,i=10,a=r+i,c=a+4,d=c+11,p=a+9,m=d+4,h=[{x:0,w:50,color:"#5cce8c"},{x:50,w:50,color:"#d4cc5c"},{x:100,w:50,color:"#e0a84a"},{x:150,w:50,color:"#e05c5c"}].map(M=>`<rect x="${M.x}" y="${r}" width="${M.w}" height="${i}" fill="${M.color}" opacity="0.55" rx="0"/>`).join(""),w=[{x:0,label:"0",anchor:"start"},{x:50,label:"2",anchor:"middle"},{x:100,label:"4",anchor:"middle"},{x:150,label:"6",anchor:"middle"},{x:200,label:"8+",anchor:"end"}],v=w.map(M=>`<line x1="${M.x}" y1="${a}" x2="${M.x}" y2="${c}" stroke="#3a5058" stroke-width="1"/>`).join(""),$=w.map(M=>`<text x="${M.x}" y="${d}" class="hw-swdp-axlabel" text-anchor="${M.anchor}">${M.label}</text>`).join(""),x="";if(n!=null){let y=Math.min(Math.max(n,0),8)/8*l;x=`<polygon points="${`${y-5},${p} ${y+5},${p} ${y},${a}`}" fill="${t.color}" opacity="0.95"/>
    <line x1="${y}" y1="${r}" x2="${y}" y2="${a}" stroke="${t.color}" stroke-width="1.5" opacity="0.7"/>`}let f=`<rect x="0" y="${r}" width="${l}" height="${i}" fill="none" stroke="#2a3c42" stroke-width="0.8" rx="0"/>`,b=`<svg class="hw-swdp-gauge" viewBox="0 0 ${l} ${m}" preserveAspectRatio="none" aria-hidden="true">
    ${h}${f}${x}${v}${$}
  ...
  </div>`}function ct(e){var c,d;let s=(c=e.alerts_all)!=null?c:[],t=s.find(p=>p.kind==="cme_impact"),o=s.find(p=>p.kind==="cme_watch"),n=t!=null?t:o;if(!n)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let l=((d=n.raw_body)!=null?d:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),r=l?parseInt(l[1],10):null,i=null;if(r&&n.t_utc){let p=1496e5/r*1e3;i=new Date(new Date(n.t_utc).getTime()+p).toISOString().replace(".000Z","Z")}let a=t?"impact":"watch";return{status:a,color:a==="impact"?"#e05c5c":"#d4cc5c",label:a==="impact"?"Active":"Watch",speed_kms:r,issued_utc:n.t_utc,arrival_utc:i}}function Cs(e,s){let t=ct(e),o=s?" hw-impact-tip-open":"",n="\u2014";if(t.arrival_utc){let a=new Date(t.arrival_utc),c=a.toLocaleString("en-US",{month:"short",timeZone:"UTC"}),d=a.getUTCDate(),p=String(a.getUTCHours()).padStart(2,"0"),m=String(a.getUTCMinutes()).padStart(2,"0");n=`~${c}\xA0${d}\xA0${p}:${m}\u202FUTC`}let l=t.speed_kms?`${t.speed_kms}\u202Fkm/s`:"\u2014",r=t.status!=="quiet"?`Velocity: <b style="color:#b4c6cc">${g(l)}</b>&ensp;Arrival: <b style="color:#b4c6cc">${g(n)}</b>`:"No Earth-directed CME in forecast window",i=ke("cme_cone",{cmeState:t,uid:"cme"});return`<div class="hw-impact-tip${o}">
    ${i}
    <div class="hw-cme-footer">${r}</div>
  </div>`}function Le(e,s,t,o,n,l,r,i,a){let c=l.has(e),d=c?" hw-impact-open":"",p=c?" hw-impact-tip-open":"";return`<div class="hw-impact-row${d}" id="${e}" data-impact-row="${e}">
    ...
    </div>`}function Hs(e,s){let t=e.find(l=>l.kind==="aurora");if(t)return t;let o=s.aurora_hint;return{kind:"aurora",level:o.aurora_label==="good"?"moderate":o.aurora_label==="possible"?"low":"none",label:"Aurora",summary:o.summary}}function Ls(e,s,t,o){var x,f;let n=(x=Qe[e.level])!=null?x:"#666",l=e.level==="none"?"None":e.level.charAt(0).toUpperCase()+e.level.slice(1),r=(f=Re[e.kind])!=null?f:st,i=e.level==="none"?"#606870":n,a=s.has("aurora"),c=a?" hw-impact-open":"",d=a?" hw-aurora-tip-open":"",p=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,m=null;t&&o.lat!=null&&o.lon!=null&&(m=Ze(t.entries,o.lat,o.lon));let u=o.lat!=null&&o.lon!=null,h=m!=null?m>=30?"#5cce8c":m>=10?"#d4cc5c":"#9ab4bc":"#607880",w=m!=null?`${m}%`:t?"n/a":"\u2026",v=u?`
    <div class="hw-aurora-obs-panel">
      <span>\u{1F4CD}</span>
    ...
    ${m}
  </div>`}function Fs(e,s,t,o,n,l,r,i,a){var Ie,Oe,Ae,ze;let c=(Oe=(Ie=s==null?void 0:s.impacts)!=null?Ie:e.observer_impacts)!=null?Oe:[],d=Hs(c,e),p=Ls(d,r,i,a),m=c.find(Me=>Me.kind==="radio"),u=c.find(Me=>Me.kind==="solar_activity"),h=m?Xe(m,t,n,l,e,r.has("radio")):"",w=u?Xe(u,t,n,l,e,!1):"",x=`
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,{metrics:f}=e,b=f.xray_class,k=b?(Ae=ve[b])!=null?Ae:"#a0b4b8":"#607880",S=b?`${b}-class`:"\u2014",_=f.imf_bz_nt,H=_!=null?_<=-10?"#e05c5c":_<=-5?"#e0a84a":_>=5?"#5cce8c":"#a0b4b8":"#607880",M=_!=null?(_>=0?"+":"")+_.toFixed(1)+" nT":"\u2014",y=f.solar_wind_kms,L=y!=null?`${Math.round(y)} km/s`:"\u2014",T=y!=null?y>700?"#e05c5c":y>500?"#e0a84a":y>400?"#d4cc5c":"#5cce8c":"#607880",Y=Le("xray","X-Ray",ns,S,k,r,e,a,i),W=Le("imf_bz","IMF Bz",rs,M,H,r,e,a,i),X=Le("solar_wind","Solar Wind",as,L,T,r,e,a,i),N='<div class="hw-indicators-sep" role="separator" aria-hidden="true"></div>',P=r.has("geomag_storm"),j=ot(e),C=fs(j.now.g_level),E=`G${j.now.g_level}`,A=`<div class="hw-impact-row${P?" hw-impact-open":""}" id="storm_risk" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${vs(e,P)}
    </div>`,R=r.has("solar_cycle"),J=(ze=nt[fe.phase])!=null?ze:"#96a8b8",z=fe.phase.charAt(0).toUpperCase()+fe.phase.slice(1),B=`<div class="hw-impact-row${R?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
...
${t.join(" \xB7 ")}`}function zs(e,s,t){var a;let o=e.is_active?Is:(a=Es[e.level])!=null?a:"#445c64",n=e.event_time===t,l=e.event_time.slice(11,16)+" UTC",r=e.source==="NASA_DONKI"?"DONKI":"SWPC",i=n?`<div class="hw-tl-detail">${g(e.description)}${g(As(e))}</div>`:"";return`
    ...
    </div>`}function Ts(e,s,t,o){var $,x;let n=($=e.timeline)!=null?$:[],l=Date.now(),r=new Date(l).toISOString().slice(0,10),i=new Date(l-864e5).toISOString().slice(0,10),a=new Date(l-1728e5).toISOString().slice(0,10),c=new Set([r,i,a]),d=n.filter(f=>{var b;return c.has(((b=f.event_time)!=null?b:"").slice(0,10))}).slice().reverse(),p=d.length,m=t?"\u25BC":"\u25B6",u=p>0?`Solar Activity Timeline (${p})`:"Solar Activity Timeline",h=`
    <div class="hw-section-row" data-tl-section>
      ...
      <div class="hw-tl-day-row" data-tl-day="${O(f)}">
        ...
        ${M}
      </div>`,L=_?"":b.map((T,Y)=>zs(T,Y<b.length-1,s)).join("");return`<div class="hw-tl-group">${y}${L}</div>`}).join("");return`
    <div class="hw-timeline">
      ${h}
      ${v}
    </div>`}function Ns(e,s,t){var c;let o=(c=e.alerts_all)!=null?c:[],n=o.length,l=s?"\u25BC":"\u25B6",r=n>0?`SWPC Alerts (${n})`:"SWPC Alerts",i=`
    ...
    <div class="hw-alerts">
      ${i}
      ${a}
    </div>`}function Bs(e,s){var W,X,N;let t=e.cme_tracker;if(!t)return"";let o=(W=vt[t.impact_level])!=null?W:"#96a8b8",n=(X=yt[t.status])!=null?X:t.status,l=300,r=44,i=18,a=r/2,c=10,d=l-18,p=7,m=`<line x1="${i+c}" y1="${a}" x2="${d-p}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,u=`<circle cx="${i}" cy="${a}" r="${c}" fill="#f0c040" opacity="0.92"/>`,h=`
    <circle cx="${d}" cy="${a}" r="${p}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${d}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,w=`<text x="${i}" y="${a+c+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,v=`<text x="${d}" y="${a+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,$="";if(t.progress!=null){let P=i+c+4,j=d-p-4,C=P+t.progress*(j-P),E=5;t.status==="arrival_window"?$=`
        ...
        </g>`:$=`<circle cx="${C.toFixed(1)}" cy="${a}" r="${E}" fill="${o}" opacity="0.85"/>`}let x=`<svg class="hw-cme-svg" viewBox="0 0 ${l} ${r}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${m}
    ${u}${w}
    ${h}${v}
    ${$}
