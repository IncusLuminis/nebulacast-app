...
`,Re=!1;function Ct(){if(Re)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Mt,document.head.appendChild(e),Re=!0}function Ht(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let o=200,t=32,s=e.length,n=o/s,c=e.map((r,i)=>{let a=Math.max(2,Math.min(t,r.kp/9*t)),l=t-a,d=i*n,p=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${l.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${w(we(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${o} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${c}</svg>`}function Oe(e,o,t,s,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,r=Math.min(...e),i=Math.max(...e),a=i-r||1,l=m=>s-2-(m-r)/a*(s-4),d=e.map((m,h)=>`${(h/(e.length-1)*c).toFixed(1)},${l(m).toFixed(1)}`).join(" "),p="";if(n&&r<0&&i>0){let m=l(0);p=`<line x1="0" y1="${m.toFixed(1)}" x2="${c}" y2="${m.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let g=e.map((m,h)=>`<rect x="${(h/(e.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${s}" fill="transparent"><title>${w(o[h]||"")} \xB7 ${m.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${s}" style="width:100%;height:${s}px;display:block" preserveAspectRatio="none">
    ...
    <line x1="${i}" y1="130" x2="${x}" y2="130"
  ...
  </g>`,A="";if(e==="magnetosphere"){let z=(T=o.windKms)!=null?T:0,j=z>500,D=z<350,E=(j?.5:D?1.4:.9)*1.3,K=z>700?"#e05c5c":z>500?"#e0a84a":z>350?"#d4c840":"#5cce8c",ee=i+8,J=x-14,O=8,W=6,U=(J-ee)/(O-1),te=260/(W+1),Q=38,ie=24,$e=2,be=Array.from({length:O},(se,re)=>{let V=ee+re*U,at=Array.from({length:W},(ct,ve)=>{let ne=te*(ve+1);return`<path d="M ${V.toFixed(1)},${ne.toFixed(1)} L ${(V+Q).toFixed(1)},${ne.toFixed(1)} M ${(V+ie).toFixed(1)},${(ne-6).toFixed(1)} L ${(V+Q).toFixed(1)},${ne.toFixed(1)} L ${(V+ie).toFixed(1)},${(ne+6).toFixed(1)}"
        ...
        fill="${se}" font-family="monospace">Bz${oe>0?"\u2191":"\u2193"}</text>`})();A=`${be}${le}`}let N=`<g id="${c}-overlay-solar-wind">${A}</g>`,P="";if(e==="coronal_hole"&&o.hssState){let z=o.hssState,j=(Z=z.speed)!=null?Z:0,D=j>=420,E=z.color,K=D?"0.85":"0.25",ee=j>=500?"0.18":D?"0.10":"0.04",J=i+8,O=d-16,W=60,U=`${J},130 ${O},${130-W} ${O},${130+W}`,te=O+6,Q=`${te},120 ${te+18},130 ${te},140`;P=`
    ...
    <polygon points="${Q}" fill="${E}" opacity="${D?"0.9":"0.25"}"/>`}let X=`<g id="${c}-overlay-coronal-hole">${P}</g>`,B="";if(e==="cme_cone"&&o.cmeState){let z=o.cmeState,j=i,D=d-10,E=D-j,K=U=>Math.tan(U*Math.PI/180),ee=Math.round(K(9)*E),J=Math.round(K(6)*E),O=Math.round(K(3)*E),W=U=>`${j},130 ${D},${130-U} ${D},${130+U}`;if(z.status!=="quiet"){let U=z.status==="impact"?"#e05c5c":"#d4cc5c";B=`
    <polygon points="${W(ee)}" fill="#253238" opacity="0.85"/>
    <polygon points="${W(J)}"   fill="#d4cc5c" opacity="0.14"/>
    <polygon points="${W(O)}" fill="#e0a84a" opacity="0.28"/>
    ...
    <line x1="${j+14}" y1="130" x2="${d-14}" y2="130"
          stroke="#1e2c30" stroke-dasharray="7 5" stroke-width="2"/>`}let G=`<g id="${c}-overlay-cme-cone">${B}</g>`,Y=`<g id="${c}-overlay-labels">
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    ${$}
  </div>`}function De(e,o,t,s,n,c){var x,v;let r=(x=We[e.level])!=null?x:"#666",i=e.level==="none"?"None":e.level.charAt(0).toUpperCase()+e.level.slice(1),a=(v=Ce[e.kind])!=null?v:Ve,l=e.level==="none"?"#606870":r,d=e.kind==="solar_activity"?t:c,p=d?" hw-impact-open":"",g;if(e.kind==="solar_activity"){let $=t?" hw-solar-open":"",u=Qe.map(f=>{let b=s.has(f.id),y=b?f.color+"22":"transparent",S=b?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${f.id}" style="color:${f.color};border-color:${f.color};background:${y};opacity:${S}">${f.label}</button>`}).join("");g=`<div class="hw-solar-tip${$}">
    ...
    <div class="hw-section-row" data-impacts-toggle>
      ...
      ${yo(e,K)}
    </div>`,O=rt(e),W=r.has("gnss"),te=`<div class="hw-impact-row${W?" hw-impact-open":""}" id="gnss" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
...
${t.join(" \xB7 ")}`}function Io(e,o,t){var a;let s=e.is_active?Lo:(a=Ho[e.level])!=null?a:"#445c64",n=e.event_time===t,c=e.event_time.slice(11,16)+" UTC",r=e.source==="NASA_DONKI"?"DONKI":"SWPC",i=n?`<div class="hw-tl-detail">${w(e.description)}${w(Fo(e))}</div>`:"";return`
    ...
    <circle cx="${d}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,x=`<text x="${i}" y="${a+l+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,v=`<text x="${d}" y="${a+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,$="";if(t.progress!=null){let G=i+l+4,Y=d-p-4,H=G+t.progress*(Y-G),R=5;t.status==="arrival_window"?$=`
