...
`,Re=!1;function _t(){if(Re)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=kt,document.head.appendChild(e),Re=!0}function St(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let o=200,t=32,s=e.length,n=o/s,c=e.map((r,i)=>{let a=Math.max(2,Math.min(t,r.kp/9*t)),l=t-a,d=i*n,p=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${l.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${w(we(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${o} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${c}</svg>`}function Oe(e,o,t,s,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,r=Math.min(...e),i=Math.max(...e),a=i-r||1,l=m=>s-2-(m-r)/a*(s-4),d=e.map((m,h)=>`${(h/(e.length-1)*c).toFixed(1)},${l(m).toFixed(1)}`).join(" "),p="";if(n&&r<0&&i>0){let m=l(0);p=`<line x1="0" y1="${m.toFixed(1)}" x2="${c}" y2="${m.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let g=e.map((m,h)=>`<rect x="${(h/(e.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${s}" fill="transparent"><title>${w(o[h]||"")} \xB7 ${m.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${s}" style="width:100%;height:${s}px;display:block" preserveAspectRatio="none">
    ...
    ${$o(e)}
    <div class="hw-kpi-hint" style="margin-top:6px;margin-bottom:0">${w(n)}</div>
  ...
  </div>`}function Ot(e,o,t,s){switch(o){case"solar_wind":return Ct(e);case"xray":return Ht(e);case"imf_bz":return Et(e);case"aurora":return Rt(t,s);case"magnetosphere":return It(e);default:return""}}function Tt(e,o){if(e.length<2)return"\u2192";let t=e[e.length-1],s=Math.max(0,e.length-4),n=e[s];if(!isFinite(t)||!isFinite(n))return"\u2192";let c=t-n;return c>o?"\u2191":c<-o?"\u2193":"\u2192"}var zt={quiet:"171",active:"195",elevated:"284",storm:"304"};function Nt(e,o){var s,n,c;if(o==null)return(n=(s=e.summary)==null?void 0:s.status)!=null?n:"quiet";let t=parseInt(((c=o.gScale)!=null?c:"G0").replace(/\D/g,""),10)||0;return t>=3?"storm":t>=1?"elevated":o.kp>=4?"active":"quiet"}function Pt(e,o){var l,d;let t=(l=zt[e])!=null?l:"171",s=`/assets/gifs/current_eit_${t}.gif`,n=so(s,o),c=ro,r=(d=ue[e])!=null?d:ue.quiet,i=`${r.accent}44`,a=`0 0 10px ${r.accent}55,0 0 24px ${r.accent}22`;return`<div class="hw-solar-mini-wrap" style="cursor:default">
    ...
    <div class="hw-hero" style="background:${x}">
      ...
      ${u}
    </div>`}function Dt(e){var d,p,g,m;let{metrics:o}=e,t=(d=o.kp_history_1h)!=null?d:[],s=(p=o.wind_history_1h)!=null?p:[],n=(g=o.bz_history_1h)!=null?g:[],c=(m=o.xray_history_1h)!=null?m:[],r=St(t),i=Oe(s.map(h=>{var x;return(x=h.kms)!=null?x:0}).filter(h=>h>0),s.map(h=>we(h.t_utc)),"#5cce8c",28,!1),a=Oe(n.map(h=>h.bz),n.map(h=>we(h.t_utc)),"#d4cc5c",28,!0),l=Mt(c);return`
    ...
    <div class="hw-forecast">
      ...
      ${B}`:""}
    </div>`}function qt(e){let o=/([NS])(\d+)([EW])(\d+)/i.exec(e);return o?{lat:(o[1].toUpperCase()==="N"?1:-1)*parseInt(o[2],10),lon:(o[3].toUpperCase()==="E"?1:-1)*parseInt(o[4],10)}:null}var Qe=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function Qt(e){return e.x_flare_probability>0?"X":e.m_flare_probability>0?"M":e.c_flare_probability>0?"C":"quiet"}function Vt(e,o,t){let s=o/2,n=s*.87,c=o*.03,r=o*.009,i=e.map(a=>{var $,u;let l=qt(a.location);if(!l||Math.abs(l.lon)>88||a.location.includes("*"))return"";let d=Qt(a);if(!t.has(d))return"";let p=Qe.find(f=>f.id===d).color,g=l.lat*Math.PI/180,m=l.lon*Math.PI/180,h=(s+n*Math.cos(g)*Math.sin(m)).toFixed(1),x=(s-n*Math.sin(g)).toFixed(1),v=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}var ge={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Je={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function xo(e){var C;let o=ge,t=(C=Je[o.phase])!=null?C:"#96a8b8",s=o.phase.charAt(0).toUpperCase()+o.phase.slice(1),n=280,c=52,r=10,i=c-6,a=c-18,l=.5,d=.19,p=_=>Math.exp(-Math.pow((_-l)/d,2)/2),g=_=>r+_*(n-2*r),m=_=>i-p(_)*a,h=80,x=[];for(let _=0;_<=h;_++){let L=_/h;x.push(`${_===0?"M":"L"}${g(L).toFixed(1)},${m(L).toFixed(1)}`)}let v=Math.round(o.progress_0_1*h),$=[];for(let _=0;_<=v;_++){let L=_/h;$.push(`${_===0?"M":"L"}${g(L).toFixed(1)},${m(L).toFixed(1)}`)}let u=g(o.progress_0_1),f=[`M${r},${i}`,...$.slice(1),`L${u.toFixed(1)},${i} Z`],b=m(o.progress_0_1),y=5,S=`M${u.toFixed(1)},${b.toFixed(1)} L${(u-y).toFixed(1)},${(b-y*1.8).toFixed(1)} L${(u+y).toFixed(1)},${(b-y*1.8).toFixed(1)} Z`,k=i+11;return`<div class="hw-impact-tip${e?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
    ...
    ${$}
  </div>`}function De(e,o,t,s,n,c){var x,v;let r=(x=Ye[e.level])!=null?x:"#666",i=e.level==="none"?"None":e.level.charAt(0).toUpperCase()+e.level.slice(1),a=(v=Ce[e.kind])!=null?v:Ve,l=e.level==="none"?"#606870":r,d=e.kind==="solar_activity"?t:c,p=d?" hw-impact-open":"",g;if(e.kind==="solar_activity"){let $=t?" hw-solar-open":"",u=Qe.map(f=>{let b=s.has(f.id),y=b?f.color+"22":"transparent",S=b?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${f.id}" style="color:${f.color};border-color:${f.color};background:${y};opacity:${S}">${f.label}</button>`}).join("");g=`<div class="hw-solar-tip${$}">
      ...
      </div>`}else g=ao(n,d);let m=e.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${F(e.kind)}"`,h=e.kind==="radio"?' id="radio"':"";return`<div class="hw-impact-row${p}"${h}${m}>
    <span class="hw-impact-caret">\u25B6</span>
    ...
    ${g}
  </div>`}function _o(e,o,t,s,n,c,r,i,a){var oe,le,se,re;let l=(le=(oe=o==null?void 0:o.impacts)!=null?oe:e.observer_impacts)!=null?le:[],d=yo(l,e),p=ko(d,r,i,a),g=l.find(V=>V.kind==="radio"),m=l.find(V=>V.kind==="solar_activity"),h=g?De(g,t,n,c,e,r.has("radio")):"",x=m?De(m,t,n,c,e,!1):"",u=`
    <div class="hw-section-row" data-impacts-toggle>
      ...
      ${wo(e,G)}
    </div>`,I=r.has("solar_cycle"),Z=(re=Je[ge.phase])!=null?re:"#96a8b8",z=ge.phase.charAt(0).toUpperCase()+ge.phase.slice(1),D=`<div class="hw-impact-row${I?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${xo(I)}
    </div>`,E=tt(e),K=r.has("sat_drag"),J=`<div class="hw-impact-row${K?" hw-impact-open":""}" id="satellite_drag" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${bo(e,K)}
    </div>`,O=ot(e),Y=r.has("gnss"),te=`<div class="hw-impact-row${Y?" hw-impact-open":""}" id="gnss" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
...
${t.join(" \xB7 ")}`}function Eo(e,o,t){var a;let s=e.is_active?Co:(a=Mo[e.level])!=null?a:"#445c64",n=e.event_time===t,c=e.event_time.slice(11,16)+" UTC",r=e.source==="NASA_DONKI"?"DONKI":"SWPC",i=n?`<div class="hw-tl-detail">${w(e.description)}${w(Lo(e))}</div>`:"";return`
    ...
    </div>`;if(!o||n===0)return`<div class="hw-alerts">${i}${o&&n===0?'<div class="hw-empty-alerts">No significant recent SWPC alerts</div>':""}</div>`;let a=s.map(d=>So(d,d.dedupe_key===t)).join("");return`
    <div class="hw-alerts">
      ${i}
      ${a}
    </div>`}function Ao(e,o){var P,X,B;let t=e.cme_tracker;if(!t)return"";let s=(P=ut[t.impact_level])!=null?P:"#96a8b8",n=(X=wt[t.status])!=null?X:t.status,c=300,r=44,i=18,a=r/2,l=10,d=c-18,p=7,g=`<line x1="${i+l}" y1="${a}" x2="${d-p}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,m=`<circle cx="${i}" cy="${a}" r="${l}" fill="#f0c040" opacity="0.92"/>`,h=`
    ...
    <div class="hw-cme">
      ...
      ${N}
    </div>`}function Ro(e,o,t,s,n,c,r,i,a,l,d,p,g,m,h,x,v,$){var M;let u=Xt(e,s),f=(M=e.metrics.kp_history_1h)!=null?M:[],b=f.length?me(f[f.length-1].t_utc):null,y=b?`Recent history \xB7 Last step ${b}`:"Recent history",S=`<div style="padding:10px 14px;border-bottom:1px solid #1e2c30"><div class="hw-section-row" data-hero-toggle style="margin-bottom:0">
