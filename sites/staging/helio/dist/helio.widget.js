...
`,kt=!1;function re(){if(kt)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=ie,document.head.appendChild(t),kt=!0}function le(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,n=s/o,c=t.map((i,r)=>{let a=Math.max(2,Math.min(e,i.kp/9*e)),l=e-a,p=r*n,d=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${p.toFixed(1)}" y="${l.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${d}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${u(it(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${c}</svg>`}function _t(t,s,e,o,n){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,i=Math.min(...t),r=Math.max(...t),a=r-i||1,l=w=>o-2-(w-i)/a*(o-4),p=t.map((w,g)=>`${(g/(t.length-1)*c).toFixed(1)},${l(w).toFixed(1)}`).join(" "),d="";if(n&&i<0&&r>0){let w=l(0);d=`<line x1="0" y1="${w.toFixed(1)}" x2="${c}" y2="${w.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((w,g)=>`<rect x="${(g/(t.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${u(s[g]||"")} \xB7 ${w.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ...
    <div class="hw-hero" style="background:${S}">
      ...
      <div class="hw-quick-details">
        ${F("aurora","Aurora",u(x),k)}
        ${F("solar_wind","Solar wind",u(b),$,T)}
        ${F("imf_bz","IMF Bz",u(H),L,C)}
        ${F("xray","X-ray",u(v),z,N)}
      </div>
      ${o?xe(t,o,c,i):""}
    ...
    <div class="hw-forecast">
      ...
      <polygon points="${S.toFixed(1)},${x} ${(S-4).toFixed(1)},${(x-7).toFixed(1)} ${(S+4).toFixed(1)},${(x-7).toFixed(1)}" fill="rgba(255,255,255,0.75)"/>`}let C=Math.round(d/4),N=Math.round(d/2),R=Math.round(d*3/4),j=`
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    <div class="hw-cme-footer">${I}</div>
        ...
        <div class="hw-aurora-obs-panel">
          ...
          ${Gt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else B.kind==="radio"?Z=Re(t,et):Z=`<div class="hw-impact-tip${et?" hw-impact-tip-open":""}">${u(B.summary)}</div>`;let Kt=B.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${G(B.kind)}"`;return`<div class="hw-impact-row${Ut}"${Kt}>
    ...
    <div class="hw-section-row" data-impacts-toggle>
      ...
      ${De(M)}
    </div>`,C=Ot(t),N=i.has("hss"),j=`<div class="hw-impact-row${N?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${We(t,A)}
    </div>`,D=Pt(t),K=i.has("sw_pressure"),Y=D.pressure!=null?`${D.pressure.toFixed(2)} nPa`:"\u2014",W=`<div class="hw-impact-row${K?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
...
${e.join(" \xB7 ")}`}function Qe(t,s,e){var a;let o=t.is_active?qe:(a=Ye[t.level])!=null?a:"#445c64",n=t.event_time===e,c=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",r=n?`<div class="hw-tl-detail">${u(t.description)}${u(Ze(t))}</div>`:"";return`
    <div class="hw-tl-item" data-timeline-key="${G(t.event_time)}">
      ...
      <div class="hw-tl-body">
        ...
        <div class="hw-tl-title${t.is_active?" hw-tl-active":""}">
          ${Ve(t.event_type)} ${u(t.event_title)}
        </div>
        ${r}
      </div>
    ...
    <div class="hw-alerts">
      ${r}
      ${a}
    </div>`}function es(t,s){var R,j,I;let e=t.cme_tracker;if(!e)return"";let o=(R=ee[e.impact_level])!=null?R:"#96a8b8",n=(j=se[e.status])!=null?j:e.status,c=300,i=44,r=18,a=i/2,l=10,p=c-18,d=7,h=`<line x1="${r+l}" y1="${a}" x2="${p-d}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,w=`<circle cx="${r}" cy="${a}" r="${l}" fill="#f0c040" opacity="0.92"/>`,g=`
    <circle cx="${p}" cy="${a}" r="${d}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${p}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,m=`<text x="${r}" y="${a+l+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,k=`<text x="${p}" y="${a+d+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,x="";if(e.progress!=null){let P=r+l+4,S=p-d-4,F=P+e.progress*(S-P),_=5;e.status==="arrival_window"?x=`
    ...
    <div class="hw-cme">
      ...
      ${N}
    </div>`}function ss(t,s,e,o,n,c,i,r,a,l,p,d,h,w,g,m,k,x,$,b){let f=Se(t,n);return`
    <div class="hw-root">
      ${be(t)}
      ${fe(t,e,w,o,f,$,b)}
      ${e?$e(t):""}
