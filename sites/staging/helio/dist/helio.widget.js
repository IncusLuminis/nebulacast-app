"use strict";var HelioWidgetModule=(()=>{var $e=Object.defineProperty,rt=Object.defineProperties,lt=Object.getOwnPropertyDescriptor,ct=Object.getOwnPropertyDescriptors,dt=Object.getOwnPropertyNames,Te=Object.getOwnPropertySymbols;var Fe=Object.prototype.hasOwnProperty,pt=Object.prototype.propertyIsEnumerable;var Oe=(e,s,t)=>s in e?$e(e,s,{enumerable:!0,configurable:!0,writable:!0,value:t}):e[s]=t,Re=(e,s)=>{for(var t in s||(s={}))Fe.call(s,t)&&Oe(e,t,s[t]);if(Te)for(var t of Te(s))pt.call(s,t)&&Oe(e,t,s[t]);return e},Ie=(e,s)=>rt(e,ct(s));var ht=(e,s)=>{for(var t in s)$e(e,t,{get:s[t],enumerable:!0})},mt=(e,s,t,o)=>{if(s&&typeof s=="object"||typeof s=="function")for(let n of dt(s))!Fe.call(e,n)&&n!==t&&$e(e,n,{get:()=>s[n],enumerable:!(o=lt(s,n))||o.enumerable});return e};var ut=e=>mt($e({},"__esModule",{value:!0}),e);var Cs={};ht(Cs,{HelioWidget:()=>nt});var Ne={quiet:{bg:"#1a2e22",accent:"#5cce8c"},active:{bg:"#2e2a1a",accent:"#d4cc5c"},elevated:{bg:"#2e1f10",accent:"#e0a84a"},storm:{bg:"#2e1212",accent:"#e05c5c"}},gt={none:"#666",low:"#5cce8c",moderate:"#e0a84a",high:"#e05c5c"},wt={info:"#666",watch:"#d4cc5c",warning:"#e05c5c"},Ee={A:"#888",B:"#5cce8c",C:"#aad47a",M:"#e0a84a",X:"#e05c5c"},xt={low:"#5cce8c",moderate:"#d4cc5c",high:"#e05c5c",unknown:"#96a8b8"},ft={detected:"Detected",inbound:"Inbound",arrival_window:"Arriving",arrived:"Arrived"};function $t(e){if(!e)return"Update time unavailable";try{let s=Math.round((Date.now()-new Date(e).getTime())/6e4);if(s<1)return"Updated just now";if(s<60)return`Updated ${s} min ago`;let t=Math.floor(s/60);return t<24?`Updated ${t}h ago`:`Updated ${Math.floor(t/24)}d ago`}catch(s){return"Updated recently"}}function bt(e){if(!e)return"\u2014";try{return new Date(e).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return e}}function me(e){if(!e)return"";try{return new Date(e).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return e}}function ye(e){try{return new Date(e).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return e.slice(11,16)}}function Pe(e){if(!e)return"\u2014";try{return new Date(e).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return e}}function N(e){return e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function m(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Le(e){return parseInt(e.slice(1),10)>0}var vt={G:"geomagnetic",R:"radio",S:"radiation",X:"solar"};function De(e){let s=e.trim();return s.toUpperCase().startsWith("X:")?s.slice(2).trim()||"\u2014":s||"\u2014"}function yt(e){return e==="C"||e==="M"||e==="X"}function kt(e,s){var r,i,l,a,c,d;let t=e.metrics.xray_class!=null?String(e.metrics.xray_class):"\u2014",o={g:(r=s==null?void 0:s.gScale)!=null?r:e.scales.g_scale,r:e.scales.r_scale,s:e.scales.s_scale,x:De(t)},n=(i=e.hero)==null?void 0:i.scales;return n?{g:(l=n.g)!=null?l:o.g,r:(a=n.r)!=null?a:o.r,s:(c=n.s)!=null?c:o.s,x:De((d=n.x)!=null?d:o.x)}:o}var _t=`
...
`,Be=!1;function St(){if(Be)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=_t,document.head.appendChild(e),Be=!0}function Mt(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.length,n=s/o,r=e.map((i,l)=>{let a=Math.max(2,Math.min(t,i.kp/9*t)),c=t-a,d=l*n,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${c.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(ye(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${r}</svg>`}function Ye(e,s,t,o,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let r=200,i=Math.min(...e),l=Math.max(...e),a=l-i||1,c=g=>o-2-(g-i)/a*(o-4),d=e.map((g,u)=>`${(u/(e.length-1)*r).toFixed(1)},${c(g).toFixed(1)}`).join(" "),p="";if(n&&i<0&&l>0){let g=c(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${r}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=e.map((g,u)=>`<rect x="${(u/(e.length-1)*r-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[u]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${r} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${p}
    <polyline points="${d}" fill="none" stroke="${t}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
    ${h}
  </svg>`}function Ct(e){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.map(d=>Math.max(-9,Math.min(-3,Math.log10(d.flux)))),n=Math.min(...o),i=Math.max(...o)-n||1,l=d=>t-2-(d-n)/i*(t-4),a=o.map((d,p)=>`${(p/(o.length-1)*s).toFixed(1)},${l(d).toFixed(1)}`).join(" "),c=e.map((d,p)=>{let h=p/(o.length-1)*s,g=d.flux>=1e-4?"X":d.flux>=1e-5?"M":d.flux>=1e-6?"C":d.flux>=1e-7?"B":"A";return`<rect x="${(h-4).toFixed(1)}" y="0" width="8" height="${t}" fill="transparent"><title>${m(ye(d.t_utc))} \xB7 ${g}-class (${d.flux.toExponential(2)})</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function Ht(e){var l,a;let s=(l=e.metrics.xray_class)!=null?l:"A",t=e.metrics.xray_flux_wm2,o=t!=null?t.toExponential(2)+" W/m\xB2":"\u2014",n=[{label:"A",color:"#888"},{label:"B",color:"#5cce8c"},{label:"C",color:"#aad47a"},{label:"M",color:"#e0a84a"},{label:"X",color:"#e05c5c"}],r=n.map(c=>{let d=c.label===s,p=d?'<div class="hw-xray-scale-marker"></div>':"";return`<div class="hw-xray-scale-band" style="background:${c.color}${d?"cc":"44"}">${p}</div>`}).join(""),i=n.map(c=>`<div class="hw-xray-scale-label" style="color:${c.label===s?"#c8d8dc":"#607880"}">${c.label}</div>`).join("");return`<div class="hw-kpi-popover">
    ${ge("X-Ray \xB7 Current")}
    <div style="margin-bottom:8px">
      <div class="hw-xray-scale">${r}</div>
      <div class="hw-xray-scale-labels">${i}</div>
    </div>
    <div class="hw-kpi-hint">Class: <b style="color:${(a=Ee[s])!=null?a:"#a0b4b8"}">${m(s)}-class</b> \xB7 ${m(o)}</div>
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function Et(e){let a='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',c=[-10,-5,5,10].map(b=>{let k=100+b/20*100;return`<line x1="${k.toFixed(1)}" y1="16" x2="${k.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),d='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(e==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${a}${c}${d}</svg>`;let p=e<=-10?"#e05c5c":e<=-5?"#e0a84a":e<0?"#d4b84a":e>=5?"#5cce8c":"#7acca8",h=Math.max(-20,Math.min(20,e)),g=100+h/20*100,u=3,f=h<0?g-u:100-u,w=Math.max(2*u,Math.abs(g-100)+2*u),$=`<rect x="${f.toFixed(1)}" y="16" width="${w.toFixed(1)}" height="6" rx="${u}" fill="${p}" opacity="0.82"/>`,x=5,v=15,y=v-x*1.1,_=`<polygon points="${g.toFixed(1)},${v.toFixed(1)} ${(g-x).toFixed(1)},${y.toFixed(1)} ${(g+x).toFixed(1)},${y.toFixed(1)}" fill="${p}"/>`,M=`<line x1="${g.toFixed(1)}" y1="${v.toFixed(1)}" x2="${g.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${p}" stroke-width="1" opacity="0.6"/>`,E=`<text x="${g.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${p}" font-weight="600">${e>=0?"+":""}${e.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${a}${c}${d}${$}${_}${M}${E}
  </svg>`}function At(e){var h;let s=e.metrics.imf_bz_nt,t=e.metrics.imf_bt_nt,o=e.metrics.solar_wind_kms,n=(h=e.metrics.pressure_npa)!=null?h:null,r=s!=null?s<=-10?"#e05c5c":s<=-5?"#e0a84a":s>=5?"#5cce8c":"#a0b4b8":"#607880",i=s!=null?(s>=0?"+":"")+s.toFixed(1)+" nT":"\u2014",l=t!=null?t.toFixed(1)+" nT":"\u2014",a=o!=null?`${Math.round(o)} km/s`:"\u2014",c=n!=null?`${n.toFixed(2)} nPa`:"\u2014",d=we(e),p=s!=null&&s<-5?{msg:"Southward IMF \xB7 Aurora favorable",color:"#5cce8c"}:s!=null&&s<0?{msg:"Weakly southward \xB7 Conditions may improve",color:"#d4cc5c"}:{msg:"Northward IMF \xB7 Stable magnetosphere",color:"#96a8b8"};return`<div class="hw-kpi-popover">
    ...
    <div style="font-size:.65em;color:#607880">Coupling: <span style="color:${d.color};font-weight:600">${m(d.coupling)}</span> \xB7 Trend history: \u25B6 Details</div>
      ...
      ${E}
    </svg>`}else{let $=e.state==="storm"?16:e.state==="active"?26:38,x=155-$,v=e.state==="storm"?22:e.state==="active"?30:40,y=120-v,_=240,M=[`M ${x},60`,`C ${x-4},42 150,${v} 155,${v}`,`C 173,${v} ${_-5},${v+18} ${_},60`,`C ${_-5},${y-18} 173,${y} 155,${y}`,`C 150,${y} ${x-4},78 ${x},60`,"Z"].join(" "),L=`M ${x+2},60 C ${x+2},${60-$*.4} 152,54 150,60 C 152,66 ${x+2},${60+$*.4} ${x+2},60 Z`,E=i>700?"#e05c5c":i>500?"#e0a84a":i>350?"#d4c840":"#5cce8c",b=i>700?.4:i>500?.65:i>350?1.1:1.8,k=i>500?[10,24,40,57,74,90,106]:i>350?[14,34,57,82,104]:[20,50,82,108],H=16,F=22,G=x-6,q=Math.ceil((G-F)/H)+2,Q=Array.from({length:q},(D,T)=>F-H+T*H),O=12,j=8,J=Q.flatMap(D=>k.map(T=>`<path d="M ${D},${T} L ${D+O},${T} M ${D+j},${T-3} L ${D+O},${T} L ${D+j},${T+3}" stroke="${E}cc" stroke-width="1.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`)).join(""),C=`<g class="hw-wg-full" style="animation-duration:${b}s">${J}</g>`,z=s==null?"":s>0?'<path d="M 155,64 L 155,56 M 153,58 L 155,56 L 157,58" stroke="#5cce8c" stroke-width="1.3" fill="none" stroke-linecap="round"/>':'<path d="M 155,56 L 155,64 M 153,62 L 155,64 L 157,62" stroke="#e05c5c" stroke-width="1.3" fill="none" stroke-linecap="round"/>',X=s==null?"":`<text x="163" y="62" font-size="6" fill="${s>0?"#5cce8c":"#e05c5c"}" font-family="monospace">Bz${s>0?"\u2191":"\u2193"}</text>`;return ke("magnetosphere",{magnetInfo:e,bz:s,windKms:t,uid:n})}}function ke(e,s={}){var C,z,X,D,T,V;let r=(C=s.uid)!=null?C:"hse",i=0,l=160,a=800,c=17,d=a-c,p=s.magnetInfo,h=(z=p==null?void 0:p.state)!=null?z:"stable",g=(X=p==null?void 0:p.color)!=null?X:"#e0a84a",f=a-(h==="storm"?55:h==="active"?80:110),w=h==="storm"?58:h==="active"?76:95,$=1120,x=20,v=[`M ${f},130`,`C ${f-8},${130-w*.55} ${a-18},${130-w} ${a},${130-w}`,`C ${a+120},${130-w} ${$-180},${130-x} ${$},${130-x}`,`L ${$},${130+x}`,`C ${$-180},${130+x} ${a+120},${130+w} ${a},${130+w}`,`C ${a-18},${130+w} ${f-8},${130+w*.55} ${f},130`,"Z"].join(" "),y=e==="magnetosphere",_=y?h==="storm"?"0.12":"0.08":"0.04",M=y?"0.75":"0.28",L=`<g id="${r}-base-sun">
    ...
    <line x1="${d}" y1="130" x2="${a+c}" y2="130"
          stroke="#4a8ab0" stroke-width="1.2" opacity="0.35"/>`,b=`<g id="${r}-base-earth">
    ...
    <line x1="${l}" y1="130" x2="${f}" y2="130"
  ...
  </g>`,F="";if(e==="magnetosphere"){let R=(D=s.windKms)!=null?D:0,B=R>500,W=R<350,I=(B?.5:W?1.4:.9)*1.3,U=R>700?"#e05c5c":R>500?"#e0a84a":R>350?"#d4c840":"#5cce8c",Z=l+8,ee=f-14,Y=8,S=6,P=(ee-Z)/(Y-1),te=260/(S+1),ne=38,ae=24,ie=2,de=Array.from({length:Y},(le,he)=>{let re=Z+he*P,ce=Array.from({length:S},(_e,se)=>{let oe=te*(se+1);return`<path d="M ${re.toFixed(1)},${oe.toFixed(1)} L ${(re+ne).toFixed(1)},${oe.toFixed(1)} M ${(re+ae).toFixed(1)},${(oe-6).toFixed(1)} L ${(re+ne).toFixed(1)},${oe.toFixed(1)} L ${(re+ae).toFixed(1)},${(oe+6).toFixed(1)}"
          stroke="${U}" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`}).join("");return Array.from({length:ie},(_e,se)=>{let oe=-((Y-he)/Y*I)-se*I/ie;return`<g style="opacity:.12;animation:hw-arrow-chase ${I}s linear ${oe.toFixed(3)}s infinite">${ce}</g>`}).join("")}).join(""),A=(T=s.bz)!=null?T:null,pe=A==null?"":(()=>{let le=A>0?"#5cce8c":"#e05c5c";return`${A>0?`<path d="M ${a},137 L ${a},123 M ${a-3},126 L ${a},123 L ${a+3},126"
        ...
        fill="${le}" font-family="monospace">Bz${A>0?"\u2191":"\u2193"}</text>`})();F=`${de}${pe}`}let G=`<g id="${r}-overlay-solar-wind">${F}</g>`,q="";if(e==="coronal_hole"&&s.hssState){let R=s.hssState,B=(V=R.speed)!=null?V:0,W=B>=420,I=R.color,U=W?"0.85":"0.25",Z=B>=500?"0.18":W?"0.10":"0.04",ee=l+8,Y=d-16,S=60,P=`${ee},130 ${Y},${130-S} ${Y},${130+S}`,te=Y+6,ne=`${te},120 ${te+18},130 ${te},140`;q=`
    <polygon points="${P}" fill="${I}" opacity="${Z}"/>
    <line x1="${ee}" y1="130" x2="${Y-5}" y2="130"
          stroke="${I}" stroke-width="3.5" stroke-dasharray="10 6"
          stroke-linecap="round" opacity="${U}"/>
    <polygon points="${ne}" fill="${I}" opacity="${W?"0.9":"0.25"}"/>`}let Q=`<g id="${r}-overlay-coronal-hole">${q}</g>`,O="";if(e==="cme_cone"&&s.cmeState){let R=s.cmeState,B=l,W=d-10,I=W-B,U=P=>Math.tan(P*Math.PI/180),Z=Math.round(U(9)*I),ee=Math.round(U(6)*I),Y=Math.round(U(3)*I),S=P=>`${B},130 ${W},${130-P} ${W},${130+P}`;if(R.status!=="quiet"){let P=R.status==="impact"?"#e05c5c":"#d4cc5c";O=`
    <polygon points="${S(Z)}" fill="#253238" opacity="0.85"/>
    <polygon points="${S(ee)}"   fill="#d4cc5c" opacity="0.14"/>
    <polygon points="${S(Y)}" fill="#e0a84a" opacity="0.28"/>
    <line x1="${B+14}" y1="130" x2="${W-5}" y2="130"
          stroke="#3a5058" stroke-dasharray="6 5" stroke-width="2"/>
    <circle cx="${a}" cy="130" r="${c+8}" fill="none"
            stroke="${P}" stroke-width="7" opacity="0.16"/>`}else O=`
    <line x1="${B+14}" y1="130" x2="${d-14}" y2="130"
          stroke="#1e2c30" stroke-dasharray="7 5" stroke-width="2"/>`}let j=`<g id="${r}-overlay-cme-cone">${O}</g>`,J=`<g id="${r}-overlay-labels">
    ...
    <div class="hw-kpi-hint" style="margin-bottom:0">${m(g)}</div>
  </div>`}function Ge(e,s,t){if(!e.length)return null;let o=(t%360+360)%360,n=-1,r=1/0,i=Math.cos(s*Math.PI/180);for(let l of e){let a=l.lat-s,c=(l.lon-o+180+360)%360-180,d=a*a+c*i*(c*i);d<r&&(r=d,n=l.prob)}return n>=0?n:null}function Xe(e){return`<svg viewBox="0 0 100 100" width="100%" height="100%"
    style="position:absolute;top:0;left:0;pointer-events:none">${['<text x="50" y="4"   font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">N</text>','<text x="96"  y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">W</text>','<text x="50" y="97"  font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">S</text>','<text x="4"   y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">E</text>'].join("")}</svg>`}function Ft(e,s){let t=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,o=null;s&&e.lat!=null&&e.lon!=null&&(o=Ge(s.entries,e.lat,e.lon));let n=e.lat!=null&&e.lon!=null,r=o!=null?o>=30?"#5cce8c":o>=10?"#d4cc5c":"#9ab4bc":"#607880",i=o!=null?`${o}%`:s?"n/a":"\u2026",l=n?`
  ...
  </div>`}function Rt(e,s,t,o){switch(s){case"solar_wind":return Lt(e);case"xray":return Ht(e);case"imf_bz":return At(e);case"aurora":return Ft(t,o);case"magnetosphere":return Tt(e);default:return""}}function be(e,s){if(e.length<2)return"\u2192";let t=e[e.length-1],o=Math.max(0,e.length-4),n=e[o];if(!isFinite(t)||!isFinite(n))return"\u2192";let r=t-n;return r>s?"\u2191":r<-s?"\u2193":"\u2192"}function It(e,s,t){let o=ue[s],n=ts,r=Jt(o.url,t);return`<div class="hw-solar-mini-wrap" style="cursor:default">
    <div class="hw-solar-mini-inner">
      ...
      <video class="hw-solar-mini-video" autoplay loop muted playsinline
        ...
        <source src="${N(ss)}" type="video/mp4">
      </video>
    </div>
  ...
  </div>`}function Nt(e,s,t,o,n,r,i,l=0){var R,B,W,I,U,Z,ee,Y;let{summary:a,scales:c,metrics:d,aurora_hint:p}=e,h=(R=Ne[a.status])!=null?R:Ne.quiet,g=n!=null?n.kp.toFixed(1):d.kp_latest!=null?d.kp_latest.toFixed(1):"\u2014",u=kt(e,n),w=[{key:"G",text:u.g,title:"Geomagnetic storm level. Based on Kp index.",aria:"Geomagnetic storm level",active:Le(u.g)},{key:"R",text:u.r,title:"Radio blackout level. Based on solar X-ray flux.",aria:"Radio blackout level",active:Le(u.r)},{key:"S",text:u.s,title:"Solar radiation storm level. Based on energetic proton flux.",aria:"Solar radiation storm level",active:Le(u.s)},{key:"X",text:`X:${u.x}`,title:"Current solar X-ray activity class.",aria:"Solar X-ray activity",active:yt(u.x)}].map(S=>{let P=vt[S.key],te=S.active?`color:${h.accent};border-color:${h.accent}33`:"";return`<button type="button" class="hw-scale-chip hw-hero-scale-chip${S.active?" hw-scale-active":""}"
      style="${te}" data-hero-scroll="${N(P)}" title="${N(S.title)}" aria-label="${N(S.aria)}">${m(S.text)}</button>`}).join(""),$=n?n.auroraLabel:p.aurora_label,x=$==="good"?"#5cce8c":$==="possible"?"#d4cc5c":"#607880",v=$.charAt(0).toUpperCase()+$.slice(1),y="#b4c6cc",_=d.solar_wind_kms!=null?`${Math.round(d.solar_wind_kms)} km/s`:"\u2014",M=d.imf_bz_nt,L=M!=null?M<=-10?"#e05c5c":M<=-5?"#e0a84a":M>=5?"#5cce8c":"#a0b4b8":"#607880",E=M!=null?(M>=0?"+":"")+M.toFixed(1)+" nT":"\u2014",b=d.xray_class,k=b?(B=Ee[b])!=null?B:"#a0b4b8":"#607880",H=b?`${b}-class`:"\u2014",F=be(((W=d.kp_history_1h)!=null?W:[]).map(S=>S.kp),.5),G=be(((I=d.wind_history_1h)!=null?I:[]).map(S=>S.kms),20),q=be(((U=d.bz_history_1h)!=null?U:[]).map(S=>S.bz),1.5),Q=be(((Z=d.xray_history_1h)!=null?Z:[]).map(S=>Math.log10(S.flux+1e-9)),.15),O=(ee=d.kp_history_1h)!=null?ee:[],j=O.length?me(O[O.length-1].t_utc):null,J=j?`Recent history \xB7 Last step ${j}`:"Recent history",C=we(e),z=(Y=d.kp_latest)!=null?Y:0,X=z>=5,D=X?`linear-gradient(160deg, #0d2a1a 0%, ${h.bg}22 75%)`:`${h.bg}18`,T=(S,P,te,ne,ae)=>{let ie=ae?`<span class="hw-trend">${ae}</span>`:"";return`<div class="hw-kpi-item${o===S?" hw-kpi-active":""}" data-kpi="${S}">
      <span class="hw-qd-label">${P}</span>
      <span class="hw-qd-value" style="color:${ne}">${te}${ie}</span>
    </div>`},V=X?`
    ...
    <div class="hw-hero" style="background:${D}">
      <div class="hw-hero-main">
        <div class="hw-kp-col">
          <div class="hw-kp-big" style="${n?"color:#9acf60":""}">Kp <b>${m(g)}</b>${n?"":`<span class="hw-trend">${F}</span>`}</div>
      ...
      <div class="hw-quick-details">
        ...
        ${T("solar_wind","Solar wind",m(_),y,G)}
        ${T("imf_bz","IMF Bz",m(E),L,q)}
        ${T("xray","X-ray",m(H),k,Q)}
      </div>
      ${o?Rt(e,o,r,i):""}`:""}
      ${V}
    </div>`}function Pt(e){var d,p,h,g;let{metrics:s}=e,t=(d=s.kp_history_1h)!=null?d:[],o=(p=s.wind_history_1h)!=null?p:[],n=(h=s.bz_history_1h)!=null?h:[],r=(g=s.xray_history_1h)!=null?g:[],i=Mt(t),l=Ye(o.map(u=>{var f;return(f=u.kms)!=null?f:0}).filter(u=>u>0),o.map(u=>ye(u.t_utc)),"#5cce8c",28,!1),a=Ye(n.map(u=>u.bz),n.map(u=>ye(u.t_utc)),"#d4cc5c",28,!0),c=Ct(r);return`
    ...
    </div>`}function Bt(e){return e.map((s,t)=>t===0?(s+e[1])/2:t===e.length-1?(e[t-1]+s)/2:(e[t-1]+s+e[t+1])/3)}function Yt(e){return e>=9?"G5":e>=8?"G4":e>=7?"G3":e>=6?"G2":e>=5?"G1":"G0"}function jt(e){return e>=5?"good":e>=3?"possible":"none"}function Ke(e){return e>=9?40:e>=8?45:e>=7?50:e>=6?55:e>=5?60:null}function Wt(e){let s=e>=7?"high":e>=5?"moderate":e>=3?"low":"none",t=Ke(e),o=s==="none"?"No aurora expected at mid-latitudes":t!=null?`Aurora possible equatorward of ~${t}\xB0 lat`:"Minor aurora possible at high latitudes",n=e>=7?"moderate":e>=5?"low":"none",r=n==="none"?"No significant HF degradation expected":n==="low"?"Minor HF degradation at high latitudes":"Moderate HF degradation, possible blackouts at high latitudes",i=e>=8?"high":e>=6?"moderate":e>=4?"low":"none";return[{kind:"aurora",level:s,label:"Aurora",summary:o},{kind:"radio",level:n,label:"HF Radio",summary:r},{kind:"solar_activity",level:i,label:"Solar Activity",summary:i==="none"?"Quiet geomagnetic conditions expected":i==="low"?"Active geomagnetic conditions possible":i==="moderate"?"Minor to moderate storm conditions":"Major geomagnetic storm conditions"}]}function Ut(e,s){var p;if(s<=0)return null;let t=(p=e.metrics.kp_forecast_3h)!=null?p:[];if(!t.length)return null;let o=Date.now()+s*36e5,n=t[0],r=1/0;for(let h of t){let g=Math.abs(new Date(h.t_utc).getTime()-o);g<r&&(r=g,n=h)}let i=n.kp,l=Yt(i),a=jt(i),c=Ke(i),d=Wt(i);return{offsetH:s,kp:i,gScale:l,auroraLabel:a,auroraMinLat:c,impacts:d}}function Gt(e,s,t,o){var J;let{forecast:n,metrics:r}=e,{kp_max_next_24h:i,kp_max_at_utc:l,trend:a}=n,c=((J=r.kp_forecast_3h)!=null?J:[]).slice(0,16),d=c.length,p=d*3,h=p>0?`${(s/p*100).toFixed(0)}%`:"0%",g=s>0?`\u23F1 +${Math.round(s)}h`:"Timeline",u="Kp forecast unavailable";if(i!=null){let C=me(l),z=a==="rising"?"rising":a==="falling"?"falling":"steady";u=`Peak Kp ${i.toFixed(1)} next 24h${C?` at ${C}`:""} \xB7 ${z}`}let f=c.length?me(c[0].t_utc):null,w=f?`Forecast \xB7 Next step ${f}`:"Forecast";if(!c.length)return`
    ...
    </div>`;let $=320,x=38,v=14,y=x+v,_=$/d,M=C=>x-Math.max(2,Math.min(x-2,C/9*(x-2))),L="",E=c.map(C=>C.kp),b=Bt(E);c.forEach((C,z)=>{let X=M(C.kp),D=x-X,T=z*_,V=T+_/2,R=C.kp>=6?"#e05c5c":C.kp>=5?"#e0a84a":C.kp>=4?"#d4cc5c":"#5cce8c",B=`Kp ${C.kp.toFixed(1)} \xB7 ${me(C.t_utc)}`;if(L+=`<rect x="${T.toFixed(1)}" y="${X.toFixed(1)}" width="${(_-1.5).toFixed(1)}" height="${D.toFixed(1)}" fill="${R}" fill-opacity="0.85" rx="1.5"/>`,L+=`<rect x="${T.toFixed(1)}" y="0" width="${_.toFixed(1)}" height="${x}" fill="transparent"><title>${N(B)}</title></rect>`,d<=8||z%2===0){let I=new Date(C.t_utc).getHours();L+=`<text x="${V.toFixed(1)}" y="${(y-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${I.toString().padStart(2,"0")}</text>`}});let H=`<polyline points="${c.map((C,z)=>{let X=z*_+_/2,D=M(b[z]);return`${X.toFixed(1)},${D.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,F="";if(s>0&&d>0){let C=Math.min($-1,s/(d*3)*$);F=`
      <line x1="${C.toFixed(1)}" y1="0" x2="${C.toFixed(1)}" y2="${x}" stroke="rgba(255,255,255,0.75)" stroke-width="1.5" stroke-dasharray="3,2"/>
      <polygon points="${C.toFixed(1)},${x} ${(C-4).toFixed(1)},${(x-7).toFixed(1)} ${(C+4).toFixed(1)},${(x-7).toFixed(1)}" fill="rgba(255,255,255,0.75)"/>`}let G=Math.round(p/4),q=Math.round(p/2),Q=Math.round(p*3/4),O=`
    ...
    <div class="hw-forecast">
      ...
      <svg viewBox="0 0 ${$} ${y}" style="width:100%;height:${y}px;display:block" preserveAspectRatio="none">
        ${L}
        ${H}
        ${F}
      </svg>
      ${O}`:""}
    </div>`}function Xt(e){let s=/([NS])(\d+)([EW])(\d+)/i.exec(e);return s?{lat:(s[1].toUpperCase()==="N"?1:-1)*parseInt(s[2],10),lon:(s[3].toUpperCase()==="E"?1:-1)*parseInt(s[4],10)}:null}var qe=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function Kt(e){return e.x_flare_probability>0?"X":e.m_flare_probability>0?"M":e.c_flare_probability>0?"C":"quiet"}function qt(e,s,t){let o=s/2,n=o*.87,r=s*.03,i=s*.009,l=e.map(a=>{var $,x;let c=Xt(a.location);if(!c||Math.abs(c.lon)>88||a.location.includes("*"))return"";let d=Kt(a);if(!t.has(d))return"";let p=qe.find(v=>v.id===d).color,h=c.lat*Math.PI/180,g=c.lon*Math.PI/180,u=(o+n*Math.cos(h)*Math.sin(g)).toFixed(1),f=(o-n*Math.sin(h)).toFixed(1),w=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}function rs(e,s){let t=Ve(e),o=as(e),n=(()=>{var g;let c=(g=e.metrics.kp_forecast_3h)!=null?g:[],d=Date.now(),p=d+24*60*60*1e3,h=c.filter(u=>new Date(u.t_utc).getTime()<=p);return h.length?Math.max(...h.map(u=>u.kp)):null})(),i=[{key:"g1",label:"G1"},{key:"g2",label:"G2"},{key:"g3",label:"G3"}].map(({key:c,label:d})=>{let p=t[c],h=Qe[c];return`<div class="hw-gstorm-row">
  ...
  </div>`}var ve={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Ze={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function ls(e){var b;let s=ve,t=(b=Ze[s.phase])!=null?b:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,r=52,i=10,l=r-6,a=r-18,c=.5,d=.19,p=k=>Math.exp(-Math.pow((k-c)/d,2)/2),h=k=>i+k*(n-2*i),g=k=>l-p(k)*a,u=80,f=[];for(let k=0;k<=u;k++){let H=k/u;f.push(`${k===0?"M":"L"}${h(H).toFixed(1)},${g(H).toFixed(1)}`)}let w=Math.round(s.progress_0_1*u),$=[];for(let k=0;k<=w;k++){let H=k/u;$.push(`${k===0?"M":"L"}${h(H).toFixed(1)},${g(H).toFixed(1)}`)}let x=h(s.progress_0_1),v=[`M${i},${l}`,...$.slice(1),`L${x.toFixed(1)},${l} Z`],y=g(s.progress_0_1),_=5,M=`M${x.toFixed(1)},${y.toFixed(1)} L${(x-_).toFixed(1)},${(y-_*1.8).toFixed(1)} L${(x+_).toFixed(1)},${(y-_*1.8).toFixed(1)} Z`,L=l+11;return`<div class="hw-impact-tip${e?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function Je(e){var a,c,d,p,h;let s=(c=(a=e.coronal_hole)==null?void 0:a.estimated_speed_kms)!=null?c:e.metrics.solar_wind_kms,t=(d=e.coronal_hole)==null?void 0:d.status,o=s!=null?s:0,n=t!=null?t:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",r={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},i={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},l={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:n,color:r[n],label:i[n],desc:(h=(p=e.coronal_hole)==null?void 0:p.note)!=null?h:l[n],speed:s}}function cs(e,s){var c;let t=Je(e),o=(c=t.speed)!=null?c:0,n=t.speed!=null?`${Math.round(t.speed)} km/s`:"\u2014",r=s?" hw-impact-tip-open":"",i=o>=420,l=ke("coronal_hole",{hssState:t,uid:"hss"}),a=i?'<div class="hw-hss-meta" style="font-size:.72em">Elevated speed may indicate Earth-facing coronal hole stream</div>':'<div class="hw-hss-meta" style="font-size:.72em">Background solar wind \xB7 no HSS detected</div>';return`<div class="hw-impact-tip${r}">
    ${l}
    <div class="hw-hss-meta">Solar wind: <b style="color:${t.color}">${m(n)}</b> \xB7 ${m(t.desc)}</div>
    ${a}
  </div>`}function et(e){var n,r;let s=(n=e.scales.g_scale)!=null?n:"G0",t=parseInt(s.slice(1),10),o=e.metrics.kp_latest;if(o==null){let i=(r=e.metrics.kp_forecast_3h)!=null?r:[],l=Date.now(),a=i.filter(c=>new Date(c.t_utc).getTime()<=l+3*60*60*1e3).sort((c,d)=>new Date(d.t_utc).getTime()-new Date(c.t_utc).getTime());a.length>0&&(o=a[0].kp)}return t>=2||o!=null&&o>=6?{level:2,color:"#e05c5c",label:"High",kp:o,gScale:s}:t>=1||o!=null&&o>=4?{level:1,color:"#d4cc5c",label:"Moderate",kp:o,gScale:s}:{level:0,color:"#5cce8c",label:"Low",kp:o,gScale:s}}function ds(e,s){let t=et(e),o=s?" hw-impact-tip-open":"",r=[{l:0,label:"Low",color:"#5cce8c",width:33,desc:"Normal density"},{l:1,label:"Moderate",color:"#d4cc5c",width:64,desc:"Elevated density"},{l:2,label:"High",color:"#e05c5c",width:100,desc:"Strong expansion"}].map(a=>{let c=a.l===t.level,d=c?a.color:"#566068",p=c?"0.88":"0.16";return`<div class="hw-satdrag-rung">
    ...
    ${u}${v}${x}${w}${$}
  </svg>`,_=n!=null?`${n.toFixed(2)} nPa`:"\u2014",M=t.density!=null?`${t.density.toFixed(2)} cm\u207B\xB3`:"\u2014",L=e.metrics.solar_wind_kms!=null?`${Math.round(e.metrics.solar_wind_kms)} km/s`:"\u2014",E=n==null?"":n>=4?" \xB7 Magnetosphere compressed":n>=2?" \xB7 Moderate compression":"";return`<div class="hw-impact-tip${o}">
    ${y}
    <div class="hw-swdp-meta"><b style="color:${t.color}">${m(_)}</b>${m(E)}</div>
    <div class="hw-swdp-meta" style="font-size:.72em">Speed ${m(L)} \xB7 Density ${m(M)}</div>
  </div>`}function ot(e){var c,d;let s=(c=e.alerts_all)!=null?c:[],t=s.find(p=>p.kind==="cme_impact"),o=s.find(p=>p.kind==="cme_watch"),n=t!=null?t:o;if(!n)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let r=((d=n.raw_body)!=null?d:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),i=r?parseInt(r[1],10):null,l=null;if(i&&n.t_utc){let p=1496e5/i*1e3;l=new Date(new Date(n.t_utc).getTime()+p).toISOString().replace(".000Z","Z")}let a=t?"impact":"watch";return{status:a,color:a==="impact"?"#e05c5c":"#d4cc5c",label:a==="impact"?"Active":"Watch",speed_kms:i,issued_utc:n.t_utc,arrival_utc:l}}function ms(e,s){let t=ot(e),o=s?" hw-impact-tip-open":"",n="\u2014";if(t.arrival_utc){let a=new Date(t.arrival_utc),c=a.toLocaleString("en-US",{month:"short",timeZone:"UTC"}),d=a.getUTCDate(),p=String(a.getUTCHours()).padStart(2,"0"),h=String(a.getUTCMinutes()).padStart(2,"0");n=`~${c}\xA0${d}\xA0${p}:${h}\u202FUTC`}let r=t.speed_kms?`${t.speed_kms}\u202Fkm/s`:"\u2014",i=t.status!=="quiet"?`Velocity: <b style="color:#b4c6cc">${m(r)}</b>&ensp;Arrival: <b style="color:#b4c6cc">${m(n)}</b>`:"No Earth-directed CME in forecast window",l=ke("cme_cone",{cmeState:t,uid:"cme"});return`<div class="hw-impact-tip${o}">
    ${l}
    <div class="hw-cme-footer">${i}</div>
  </div>`}function us(e,s,t,o,n,r,i,l,a){var ae,ie,de;let c=(ie=(ae=s==null?void 0:s.impacts)!=null?ae:e.observer_impacts)!=null?ie:[],d=c.map(A=>{var Ae,ze;let pe=(Ae=gt[A.level])!=null?Ae:"#666",le=A.level==="none"?"None":A.level.charAt(0).toUpperCase()+A.level.slice(1),he=(ze=Qt[A.kind])!=null?ze:Vt,re=A.level==="none"?"#606870":pe,ce=A.kind==="solar_activity"?n:i.has(A.kind),_e=ce?" hw-impact-open":"",se;if(A.kind==="solar_activity"){let xe=n?" hw-solar-open":"",Se=qe.map(K=>{let fe=r.has(K.id),Me=fe?K.color+"22":"transparent",Ce=fe?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${K.id}" style="color:${K.color};border-color:${K.color};background:${Me};opacity:${Ce}">${K.label}</button>`}).join("");se=`<div class="hw-solar-tip${xe}">
          <div class="hw-solar-disk-wrap" id="solar">
            <img class="hw-solar-disk-img" src="${es}" alt="Solar disk" loading="lazy" />
            ${t?qt(t,os,r):""}
          </div>
          <div class="hw-solar-layers">${Se}</div>
          <span class="hw-solar-tip-text">${m(A.summary)}</span>
        </div>`}else if(A.kind==="aurora"){let xe=ce?" hw-aurora-tip-open":"",Se=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,K=null;l&&a.lat!=null&&a.lon!=null&&(K=Ge(l.entries,a.lat,a.lon));let fe=a.lat!=null&&a.lon!=null,Me=K!=null?K>=30?"#5cce8c":K>=10?"#d4cc5c":"#9ab4bc":"#607880",Ce=K!=null?`${K}%`:l?"n/a":"\u2026",it=fe?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
          ...
          <div class="hw-aurora-map-wrap">
            <img class="hw-aurora-img" src="${N(Se)}" alt="NOAA Aurora Oval" loading="lazy" />
            ${Xe(a)}
          </div>
          ${it}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else A.kind==="radio"?se=ns(e,ce):se=`<div class="hw-impact-tip${ce?" hw-impact-tip-open":""}">${m(A.summary)}</div>`;let oe=A.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${N(A.kind)}"`,at=A.kind==="radio"?' id="radio"':A.kind==="solar_activity"?' id="radiation"':"";return`<div class="hw-impact-row${_e}"${at}${oe}>
    ...
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,w=i.has("geomag_storm"),$=Ve(e),x=$.g1,v=$.g1>=30?Qe.g1:$.g1>0?"#7a9298":"#607880",y=x>0?`G1 ${x}%`:"None",M=`<div class="hw-impact-row${w?" hw-impact-open":""}" id="geomagnetic" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${rs(e,w)}
    </div>`,L=i.has("solar_cycle"),E=(de=Ze[ve.phase])!=null?de:"#96a8b8",b=ve.phase.charAt(0).toUpperCase()+ve.phase.slice(1),H=`<div class="hw-impact-row${L?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      <span class="hw-impact-badge" style="background:${E}22;color:${E}">${b}</span>
      ${ls(L)}
    </div>`,F=Je(e),G=i.has("hss"),Q=`<div class="hw-impact-row${G?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${cs(e,G)}
    </div>`,O=et(e),j=i.has("sat_drag"),C=`<div class="hw-impact-row${j?" hw-impact-open":""}" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${ds(e,j)}
    </div>`,z=tt(e),X=i.has("gnss"),T=`<div class="hw-impact-row${X?" hw-impact-open":""}" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${ps(e,X)}
    </div>`,V=st(e),R=i.has("sw_pressure"),B=V.pressure!=null?`${V.pressure.toFixed(2)} nPa`:"\u2014",I=`<div class="hw-impact-row${R?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${hs(e,R)}
    </div>`,U=ot(e),Z=i.has("cme_cone"),Y=`<div class="hw-impact-row${Z?" hw-impact-open":""}" data-impact-row="cme_cone">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      <span class="hw-impact-badge" style="background:${U.color}22;color:${U.color}">${m(U.label)}</span>
      ${ms(e,Z)}
    </div>`,S=we(e),P=i.has("magnetosphere"),ne=`<div class="hw-impact-row${P?" hw-impact-open":""}" data-impact-row="magnetosphere">
      <span class="hw-impact-caret">\u25B6</span>
    ...
    ${i}
  </div>`}var ws={info:"#445c64",watch:"#e0a84a",warning:"#e05c5c"},xs="#4ae0a4";function fs(e){let s=(t,o="")=>`<svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" ${o}>${t}</svg>`;switch(e){case"solar_flare":return s(`<circle cx="6.5" cy="6.5" r="2.5"/>
...
${t.join(" \xB7 ")}`}function bs(e,s,t){var a;let o=e.is_active?xs:(a=ws[e.level])!=null?a:"#445c64",n=e.event_time===t,r=e.event_time.slice(11,16)+" UTC",i=e.source==="NASA_DONKI"?"DONKI":"SWPC",l=n?`<div class="hw-tl-detail">${m(e.description)}${m($s(e))}</div>`:"";return`
    <div class="hw-tl-item" data-timeline-key="${N(e.event_time)}">
      ...
      <div class="hw-tl-body">
        ...
        <div class="hw-tl-title${e.is_active?" hw-tl-active":""}">
          ${fs(e.event_type)} ${m(e.event_title)}
        </div>
        ${l}
      </div>
    </div>`}function vs(e,s,t,o){var $,x;let n=($=e.timeline)!=null?$:[],r=Date.now(),i=new Date(r).toISOString().slice(0,10),l=new Date(r-864e5).toISOString().slice(0,10),a=new Date(r-1728e5).toISOString().slice(0,10),c=new Set([i,l,a]),d=n.filter(v=>{var y;return c.has(((y=v.event_time)!=null?y:"").slice(0,10))}).slice().reverse(),p=d.length,h=t?"\u25BC":"\u25B6",g=p>0?`Solar Activity Timeline (${p})`:"Solar Activity Timeline",u=`
    ...
    </div>`;if(!t||p===0)return`<div class="hw-timeline">${u}</div>`;let f=new Map;for(let v of d){let y=((x=v.event_time)!=null?x:"").slice(0,10);f.has(y)||f.set(y,[]),f.get(y).push(v)}let w=[...f.entries()].map(([v,y])=>{let M=new Date(v+"T12:00:00Z").toLocaleDateString("en-US",{month:"short",day:"numeric",timeZone:"UTC"}),L=o.has(v),E=L?"\u25B6":"\u25BC",b=L?`<span class="hw-tl-day-count">${y.length} events</span>`:"",k=`
      <div class="hw-tl-day-row" data-tl-day="${N(v)}">
        <span class="hw-section-caret">${E}</span>
        <span class="hw-tl-date">${M}</span>
        ${b}
      </div>`,H=L?"":y.map((F,G)=>bs(F,G<y.length-1,s)).join("");return`<div class="hw-tl-group">${k}${H}</div>`}).join("");return`
    <div class="hw-timeline">
      ${u}
      ${w}
    </div>`}function ys(e,s,t){var c;let o=(c=e.alerts_all)!=null?c:[],n=o.length,r=s?"\u25BC":"\u25B6",i=n>0?`SWPC Alerts (${n})`:"SWPC Alerts",l=`
    ...
    </div>`;if(!s||n===0)return`<div class="hw-alerts">${l}${s&&n===0?'<div class="hw-empty-alerts">No significant recent SWPC alerts</div>':""}</div>`;let a=o.map(d=>gs(d,d.dedupe_key===t)).join("");return`
    <div class="hw-alerts">
      ${l}
      ${a}
    </div>`}function ks(e,s){var q,Q,O;let t=e.cme_tracker;if(!t)return"";let o=(q=xt[t.impact_level])!=null?q:"#96a8b8",n=(Q=ft[t.status])!=null?Q:t.status,r=300,i=44,l=18,a=i/2,c=10,d=r-18,p=7,h=`<line x1="${l+c}" y1="${a}" x2="${d-p}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,g=`<circle cx="${l}" cy="${a}" r="${c}" fill="#f0c040" opacity="0.92"/>`,u=`
    <circle cx="${d}" cy="${a}" r="${p}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${d}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,f=`<text x="${l}" y="${a+c+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,w=`<text x="${d}" y="${a+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,$="";if(t.progress!=null){let j=l+c+4,J=d-p-4,C=j+t.progress*(J-j),z=5;t.status==="arrival_window"?$=`
        ...
        </g>`:$=`<circle cx="${C.toFixed(1)}" cy="${a}" r="${z}" fill="${o}" opacity="0.85"/>`}let x=`<svg class="hw-cme-svg" viewBox="0 0 ${r} ${i}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${h}
    ${g}${f}
    ${u}${w}
    ${$}
  </svg>`,v=Pe(t.arrival_time_utc),y=Pe(t.launch_time_utc),_=t.speed_kms!=null?`${Math.round(t.speed_kms)} km/s`:"\u2014",M=t.half_angle_deg!=null?`${t.half_angle_deg}\xB0`:"\u2014",L=(O=t.source_location)!=null?O:"\u2014",E=t.is_earth_direct?"Direct hit":"Glancing blow",b=t.progress!=null?`${Math.round(t.progress*100)}%`:"\u2014",k=`
    <div class="hw-cme-detail">
      ...
      <div class="hw-cme-note">Half-angle: ${m(M)} \xB7 Source: ${m(L)} \xB7 ${m(E)} \xB7 Model: Enlil (NASA DONKI)</div>
    </div>`,H=s?"\u25BC":"\u25B6",F=t.impact_level==="unknown"?"Unrated":t.impact_level.charAt(0).toUpperCase()+t.impact_level.slice(1),G=s?`${x}${k}`:"";return`
    <div class="hw-cme">
      ...
      ${G}
    </div>`}function _s(e,s,t,o,n,r,i,l,a,c,d,p,h,g,u,f,w,$,x,v,y=0){var H;let _=Ut(e,n),M=(H=e.metrics.kp_history_1h)!=null?H:[],L=M.length?me(M[M.length-1].t_utc):null,E=L?`Recent history \xB7 Last step ${L}`:"Recent history",b=`<div style="padding:10px 14px;border-bottom:1px solid #1e2c30"><div class="hw-section-row" data-hero-toggle style="margin-bottom:0">
    <span class="hw-section-caret">${t?"\u25BC":"\u25B6"}</span>
    ...
    <div class="hw-root">
      ...
      ${vs(e,l,a,c)}
      ${ys(e,r,i)}
    </div>`}function Ss(e){return`<div class="hw-root">
