...
`,kt=!1;function re(){if(kt)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=ae,document.head.appendChild(t),kt=!0}function ie(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,n=s/o,c=t.map((a,l)=>{let r=Math.max(2,Math.min(e,a.kp/9*e)),i=e-r,d=l*n,p=a.kp>=6?"#e05c5c":a.kp>=5?"#e0a84a":a.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${i.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${r.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${a.kp.toFixed(1)} \xB7 ${h(rt(a.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${c}</svg>`}function _t(t,s,e,o,n){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,a=Math.min(...t),l=Math.max(...t),r=l-a||1,i=m=>o-2-(m-a)/r*(o-4),d=t.map((m,w)=>`${(w/(t.length-1)*c).toFixed(1)},${i(m).toFixed(1)}`).join(" "),p="";if(n&&a<0&&l>0){let m=i(0);p=`<line x1="0" y1="${m.toFixed(1)}" x2="${c}" y2="${m.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let u=t.map((m,w)=>`<rect x="${(w/(t.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${h(s[w]||"")} \xB7 ${m.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${p}
    <polyline points="${d}" fill="none" stroke="${e}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
    ${u}
  </svg>`}function le(t){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.map(d=>Math.max(-9,Math.min(-3,Math.log10(d.flux)))),n=Math.min(...o),a=Math.max(...o)-n||1,l=d=>e-2-(d-n)/a*(e-4),r=o.map((d,p)=>`${(p/(o.length-1)*s).toFixed(1)},${l(d).toFixed(1)}`).join(" "),i=t.map((d,p)=>{let u=p/(o.length-1)*s,m=d.flux>=1e-4?"X":d.flux>=1e-5?"M":d.flux>=1e-6?"C":d.flux>=1e-7?"B":"A";return`<rect x="${(u-4).toFixed(1)}" y="0" width="8" height="${e}" fill="transparent"><title>${h(rt(d.t_utc))} \xB7 ${m}-class (${d.flux.toExponential(2)})</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function ce(t){var i;let s=(i=t.metrics.wind_history_1h)!=null?i:[],e=s[s.length-1],o=t.metrics.solar_wind_kms,n=o!=null?`${Math.round(o)} km/s`:"\u2014",c=o!=null?o>=700?"#e05c5c":o>=500?"#e0a84a":o>=400?"#d4cc5c":"#5cce8c":"#607880",a=(e==null?void 0:e.density)!=null?`${e.density.toFixed(2)} cm\u207B\xB3`:"\u2014",l=(e==null?void 0:e.temp_kk)!=null?`${e.temp_kk.toFixed(0)} kK`:"\u2014",r=(e==null?void 0:e.pressure_npa)!=null?`${e.pressure_npa.toFixed(2)} nPa`:"\u2014";return`<div class="hw-kpi-popover">
    ...
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function pe(t){let r='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',i=[-10,-5,5,10].map(y=>{let k=100+y/20*100;return`<line x1="${k.toFixed(1)}" y1="16" x2="${k.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),d='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(t==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${r}${i}${d}</svg>`;let p=t<=-10?"#e05c5c":t<=-5?"#e0a84a":t<0?"#d4b84a":t>=5?"#5cce8c":"#7acca8",u=Math.max(-20,Math.min(20,t)),m=100+u/20*100,w=3,g=u<0?m-w:100-w,v=Math.max(2*w,Math.abs(m-100)+2*w),x=`<rect x="${g.toFixed(1)}" y="16" width="${v.toFixed(1)}" height="6" rx="${w}" fill="${p}" opacity="0.82"/>`,f=5,b=15,$=b-f*1.1,C=`<polygon points="${m.toFixed(1)},${b.toFixed(1)} ${(m-f).toFixed(1)},${$.toFixed(1)} ${(m+f).toFixed(1)},${$.toFixed(1)}" fill="${p}"/>`,L=`<line x1="${m.toFixed(1)}" y1="${b.toFixed(1)}" x2="${m.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${p}" stroke-width="1" opacity="0.6"/>`,H=`<text x="${m.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${p}" font-weight="600">${t>=0?"+":""}${t.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${r}${i}${d}${x}${C}${L}${H}
  </svg>`}function he(t){var u;let s=t.metrics.imf_bz_nt,e=t.metrics.imf_bt_nt,o=t.metrics.solar_wind_kms,n=(u=t.metrics.pressure_npa)!=null?u:null,c=s!=null?s<=-10?"#e05c5c":s<=-5?"#e0a84a":s>=5?"#5cce8c":"#a0b4b8":"#607880",a=s!=null?(s>=0?"+":"")+s.toFixed(1)+" nT":"\u2014",l=e!=null?e.toFixed(1)+" nT":"\u2014",r=o!=null?`${Math.round(o)} km/s`:"\u2014",i=n!=null?`${n.toFixed(2)} nPa`:"\u2014",d=mt(t),p=s!=null&&s<-5?{msg:"Southward IMF \xB7 Aurora favorable",color:"#5cce8c"}:s!=null&&s<0?{msg:"Weakly southward \xB7 Conditions may improve",color:"#d4cc5c"}:{msg:"Northward IMF \xB7 Stable magnetosphere",color:"#96a8b8"};return`<div class="hw-kpi-popover">
    ...
    <div class="hw-kpi-stat-row">
      <div class="hw-kpi-stat">
        ...
        <span class="hw-kpi-stat-value" style="color:${c}">${h(a)}</span>
      </div>
      <div class="hw-kpi-stat">
    ...
    <div style="font-size:.65em;color:#607880">Coupling: <span style="color:${d.color};font-weight:600">${h(d.coupling)}</span> \xB7 Trend history: \u25B6 Details</div>
  </div>`}function mt(t){var r,i;let s=t.metrics.imf_bz_nt,e=(r=t.metrics.kp_latest)!=null?r:0,o=(i=t.metrics.solar_wind_kms)!=null?i:0,n,c,a;if(s!=null&&s<-5||e>=6)n="storm",c="#e05c5c",a="Storm conditions";else if(s!=null&&s<0||e>=4||o>=400){let d=s!=null&&s<0;n="active",c="#e0a84a",a=d?"Active coupling":"Elevated"}else n="stable",c="#5cce8c",a="Stable";let l;return s==null?l="Unknown":s>2?l="Closed":s>0?l="Minimal":s>-5?l="Moderate":s>-10?l="Strong":l="Very strong",{state:n,color:c,label:a,coupling:l}}function Mt(t,s,e,o){let n=o?"mc":"mf",c=t.color,a=e!=null?e:0,l=a>500,r=a<350,i=l?.9:r?1.8:1.3;if(o){let w=45-(t.state==="storm"?11:t.state==="active"?16:21),g=t.state==="storm"?12:t.state==="active"?10:8,v=50-g,x=76,f=[`M ${w},25`,`C ${w-2},15 41,${g} 45,${g}`,`C 53,${g} ${x-8},${g+4} ${x},20`,`C ${x+1},23 ${x+1},27 ${x},30`,`C ${x-8},${v-4} 53,${v} 45,${v}`,`C 41,${v} ${w-2},35 ${w},25`,"Z"].join(" "),b=l?3:2,$=[14,25,36],C=y=>`<path d="M 0,${y} L ${l?8:6},${y} M ${l?6:4},${y-2} L ${l?8:6},${y} L ${l?6:4},${y+2}" stroke="${c}bb" stroke-width="${l?1.4:1}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`,L=$.map(y=>C(y)).join(""),M=Array.from({length:b},(y,k)=>`<g class="hw-wg" style="animation-duration:${i}s;animation-delay:${(i/b*k).toFixed(2)}s">${L}</g>`).join(""),H=s==null?"":s>0?`<path d="M 45,27 L 45,23 M ${45-1.5},${25-.5} L 45,23 L ${45+1.5},${25-.5}" stroke="#5cce8c" stroke-width="1" fill="none" stroke-linecap="round"/>`:`<path d="M 45,23 L 45,27 M ${45-1.5},${25+.5} L 45,27 L ${45+1.5},${25+.5}" stroke="#e05c5c" stroke-width="1" fill="none" stroke-linecap="round"/>`;return`<svg viewBox="0 0 80 50" style="width:78px;height:49px;display:block" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="${M}" fill="${c}08"/>
      <path d="${L}" fill="${c}12" stroke="${c}aa" stroke-width="1.2"/>
  ...
  </div>`}function Ct(t,s,e){if(!t.length)return null;let o=(e%360+360)%360,n=-1,c=1/0,a=Math.cos(s*Math.PI/180);for(let l of t){let r=l.lat-s,i=(l.lon-o+180+360)%360-180,d=r*r+i*a*(i*a);d<c&&(c=d,n=l.prob)}return n>=0?n:null}function Lt(t){return`<svg viewBox="0 0 100 100" width="100%" height="100%"
    ...
    <div class="hw-aurora-obs-panel">
      ...
      <span class="hw-aurora-prob" style="color:${c}">Aurora: ${a}</span>
    ...
    <div class="hw-hero" style="background:${S}">
      ...
      ${F}
    </div>`}function xe(t){var d,p,u,m;let{metrics:s}=t,e=(d=s.kp_history_1h)!=null?d:[],o=(p=s.wind_history_1h)!=null?p:[],n=(u=s.bz_history_1h)!=null?u:[],c=(m=s.xray_history_1h)!=null?m:[],a=ie(e),l=_t(o.map(w=>{var g;return(g=w.kms)!=null?g:0}).filter(w=>w>0),o.map(w=>rt(w.t_utc)),"#5cce8c",28,!1),r=_t(n.map(w=>w.bz),n.map(w=>rt(w.t_utc)),"#d4cc5c",28,!0),i=le(c);return`
    ...
    </div>`}function fe(t){return t.map((s,e)=>e===0?(s+t[1])/2:e===t.length-1?(t[e-1]+s)/2:(t[e-1]+s+t[e+1])/3)}function be(t){return t>=9?"G5":t>=8?"G4":t>=7?"G3":t>=6?"G2":t>=5?"G1":"G0"}function ve(t){return t>=5?"good":t>=3?"possible":"none"}function Ht(t){return t>=9?40:t>=8?45:t>=7?50:t>=6?55:t>=5?60:null}function ye(t){let s=t>=7?"high":t>=5?"moderate":t>=3?"low":"none",e=Ht(t),o=s==="none"?"No aurora expected at mid-latitudes":e!=null?`Aurora possible equatorward of ~${e}\xB0 lat`:"Minor aurora possible at high latitudes",n=t>=7?"moderate":t>=5?"low":"none",c=n==="none"?"No significant HF degradation expected":n==="low"?"Minor HF degradation at high latitudes":"Moderate HF degradation, possible blackouts at high latitudes",a=t>=8?"high":t>=6?"moderate":t>=4?"low":"none";return[{kind:"aurora",level:s,label:"Aurora",summary:o},{kind:"radio",level:n,label:"HF Radio",summary:c},{kind:"solar_activity",level:a,label:"Solar Activity",summary:a==="none"?"Quiet geomagnetic conditions expected":a==="low"?"Active geomagnetic conditions possible":a==="moderate"?"Minor to moderate storm conditions":"Major geomagnetic storm conditions"}]}function ke(t,s){var p;if(s<=0)return null;let e=(p=t.metrics.kp_forecast_3h)!=null?p:[];if(!e.length)return null;let o=Date.now()+s*36e5,n=e[0],c=1/0;for(let u of e){let m=Math.abs(new Date(u.t_utc).getTime()-o);m<c&&(c=m,n=u)}let a=n.kp,l=be(a),r=ve(a),i=Ht(a),d=ye(a);return{offsetH:s,kp:a,gScale:l,auroraLabel:r,auroraMinLat:i,impacts:d}}function _e(t,s,e,o){var I;let{forecast:n,metrics:c}=t,{kp_max_next_24h:a,kp_max_at_utc:l,trend:r}=n,i=((I=c.kp_forecast_3h)!=null?I:[]).slice(0,16),d=i.length,p=d*3,u=p>0?`${(s/p*100).toFixed(0)}%`:"0%",m=s>0?`\u23F1 +${Math.round(s)}h`:"Timeline",w="Kp forecast unavailable";if(a!=null){let S=vt(l),z=r==="rising"?"rising":r==="falling"?"falling":"steady";w=`Peak Kp ${a.toFixed(1)} next 24h${S?` at ${S}`:""} \xB7 ${z}`}let g=o?"\u25BC FORECAST":"\u25B6 FORECAST";if(!i.length)return`
    ...
    </div>`;let v=320,x=38,f=14,b=x+f,$=v/d,C=S=>x-Math.max(2,Math.min(x-2,S/9*(x-2))),L="",M=i.map(S=>S.kp),H=fe(M);i.forEach((S,z)=>{let F=C(S.kp),j=x-F,D=z*$,N=D+$/2,X=S.kp>=6?"#e05c5c":S.kp>=5?"#e0a84a":S.kp>=4?"#d4cc5c":"#5cce8c",K=`Kp ${S.kp.toFixed(1)} \xB7 ${vt(S.t_utc)}`;if(L+=`<rect x="${D.toFixed(1)}" y="${F.toFixed(1)}" width="${($-1.5).toFixed(1)}" height="${j.toFixed(1)}" fill="${X}" fill-opacity="0.85" rx="1.5"/>`,L+=`<rect x="${D.toFixed(1)}" y="0" width="${$.toFixed(1)}" height="${x}" fill="transparent"><title>${U(K)}</title></rect>`,d<=8||z%2===0){let V=new Date(S.t_utc).getHours();L+=`<text x="${N.toFixed(1)}" y="${(b-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${V.toString().padStart(2,"0")}</text>`}});let k=`<polyline points="${i.map((S,z)=>{let F=z*$+$/2,j=C(H[z]);return`${F.toFixed(1)},${j.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,T="";if(s>0&&d>0){let S=Math.min(v-1,s/(d*3)*v);T=`
...
C: ${r.c_flare_probability}%  M: ${r.m_flare_probability}%  X: ${r.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}var zt={g1:"#d4cc5c",g2:"#e0a84a",g3:"#e05c5c"};function Ft(t){var l;let s=(l=t.metrics.kp_forecast_3h)!=null?l:[],e=Date.now(),o=e+24*60*60*1e3,n=s.filter(r=>{let i=new Date(r.t_utc).getTime();return i>=e-3*60*60*1e3&&i<=o});if(n.length===0)return{g1:0,g2:0,g3:0};let c=Math.max(...n.map(r=>r.kp)),a=r=>{if(c<r-.7)return 0;if(c>r+1)return 90;let i=(c-(r-.7))/1.7;return Math.round(Math.pow(Math.max(0,i),.7)*90)};return{g1:a(5),g2:a(6),g3:a(7)}}function Ae(t,s){let e=Ft(t),o=(()=>{var u;let r=(u=t.metrics.kp_forecast_3h)!=null?u:[],i=Date.now(),d=i+24*60*60*1e3,p=r.filter(m=>new Date(m.t_utc).getTime()<=d);return p.length?Math.max(...p.map(m=>m.kp)):null})(),c=[{key:"g1",label:"G1"},{key:"g2",label:"G2"},{key:"g3",label:"G3"}].map(({key:r,label:i})=>{let d=e[r],p=zt[r];return`<div class="hw-gstorm-row">
  ...
  </div>`}var at={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},At={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function Ee(t){var y;let s=at,e=(y=At[s.phase])!=null?y:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,c=52,a=10,l=c-6,r=c-18,i=.5,d=.19,p=k=>Math.exp(-Math.pow((k-i)/d,2)/2),u=k=>a+k*(n-2*a),m=k=>l-p(k)*r,w=80,g=[];for(let k=0;k<=w;k++){let T=k/w;g.push(`${k===0?"M":"L"}${u(T).toFixed(1)},${m(T).toFixed(1)}`)}let v=Math.round(s.progress_0_1*w),x=[];for(let k=0;k<=v;k++){let T=k/w;x.push(`${k===0?"M":"L"}${u(T).toFixed(1)},${m(T).toFixed(1)}`)}let f=u(s.progress_0_1),b=[`M${a},${l}`,...x.slice(1),`L${f.toFixed(1)},${l} Z`],$=m(s.progress_0_1),C=5,L=`M${f.toFixed(1)},${$.toFixed(1)} L${(f-C).toFixed(1)},${($-C*1.8).toFixed(1)} L${(f+C).toFixed(1)},${($-C*1.8).toFixed(1)} Z`,M=l+11;return`<div class="hw-impact-tip${t?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function Et(t){var r,i,d,p,u;let s=(i=(r=t.coronal_hole)==null?void 0:r.estimated_speed_kms)!=null?i:t.metrics.solar_wind_kms,e=(d=t.coronal_hole)==null?void 0:d.status,o=s!=null?s:0,n=e!=null?e:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",c={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},a={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},l={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:n,color:c[n],label:a[n],desc:(u=(p=t.coronal_hole)==null?void 0:p.note)!=null?u:l[n],speed:s}}function Oe(t,s){var S;let e=Et(t),o=(S=e.speed)!=null?S:0,n=e.speed!=null?`${Math.round(e.speed)} km/s`:"\u2014",c=s?" hw-impact-tip-open":"",a=160,l=72,r=18,i=36,d=142,p=36,u=11,m=14,w=19,g=9,v=o>=500,x=o>=420,f=v?"#f5c540":x?"#c8a020":"#7a6010",b=v?"#f5c540":x?"#c8a020":"#3a3808",$=e.color,C=x?"0.9":"0.25",L=v?"0.18":x?"0.10":"0.04",M=[0,45,90,135,180,225,270,315].map(z=>{let F=z*Math.PI/180,j=(r+m*Math.cos(F)).toFixed(1),D=(i+m*Math.sin(F)).toFixed(1),N=(r+w*Math.cos(F)).toFixed(1),X=(i+w*Math.sin(F)).toFixed(1),K=z>300||z<60?"0.9":"0.5";return`<line x1="${j}" y1="${D}" x2="${N}" y2="${X}"
      stroke="${f}" stroke-width="1.6" stroke-linecap="round" opacity="${K}"/>`}).join(""),H=r+u+2,y=d-g-3,k=14,T=`${H},${i} ${y},${p-k} ${y},${p+k}`,_=y+1,O=`${_},${p-4} ${_+7},${p} ${_},${p+4}`,E=g,R=`
    <ellipse cx="${d}" cy="${p}" rx="${E}" ry="${(E*.42).toFixed(1)}"
             fill="none" stroke="#4a8ab0" stroke-width="0.8" opacity="0.6"/>
    <line x1="${d}" y1="${p-E}" x2="${d}" y2="${p+E}"
          stroke="#4a8ab0" stroke-width="0.8" opacity="0.6"/>
    <line x1="${d-E}" y1="${p}" x2="${d+E}" y2="${p}"
          stroke="#4a8ab0" stroke-width="0.8" opacity="0.35"/>`,A=`<svg class="hw-hss-diagram" width="100%" height="${l}"
      viewBox="0 0 ${a} ${l}" preserveAspectRatio="none" aria-hidden="true">
    <!-- stream fan -->
    ...
    ${M}
    <!-- Earth body -->
    <circle cx="${d}" cy="${p}" r="${E}" fill="#1a4a6e" opacity="0.92"/>
    ${R}
    <!-- labels -->
  ...
  </svg>`,I=x?'<div class="hw-hss-meta" style="font-size:.72em">Elevated speed may indicate Earth-facing coronal hole stream</div>':'<div class="hw-hss-meta" style="font-size:.72em">Background solar wind \xB7 no HSS detected</div>';return`<div class="hw-impact-tip${c}">
    ${A}
    <div class="hw-hss-meta">Solar wind: <b style="color:${e.color}">${h(n)}</b> \xB7 ${h(e.desc)}</div>
    ${I}
  </div>`}function Ot(t){var n,c;let s=(n=t.scales.g_scale)!=null?n:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let a=(c=t.metrics.kp_forecast_3h)!=null?c:[],l=Date.now(),r=a.filter(i=>new Date(i.t_utc).getTime()<=l+3*60*60*1e3).sort((i,d)=>new Date(d.t_utc).getTime()-new Date(i.t_utc).getTime());r.length>0&&(o=r[0].kp)}return e>=2||o!=null&&o>=6?{level:2,color:"#e05c5c",label:"High",kp:o,gScale:s}:e>=1||o!=null&&o>=4?{level:1,color:"#d4cc5c",label:"Moderate",kp:o,gScale:s}:{level:0,color:"#5cce8c",label:"Low",kp:o,gScale:s}}function Re(t,s){let e=Ot(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33,desc:"Normal density"},{l:1,label:"Moderate",color:"#d4cc5c",width:64,desc:"Elevated density"},{l:2,label:"High",color:"#e05c5c",width:100,desc:"Strong expansion"}].map(r=>{let i=r.l===e.level,d=i?r.color:"#566068",p=i?"0.88":"0.16";return`<div class="hw-satdrag-rung">
      <span class="hw-satdrag-label" style="color:${d}">${r.label}</span>
  ...
  </div>`}function Rt(t){var i,d,p;let s=(i=t.scales.g_scale)!=null?i:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let u=(d=t.metrics.kp_forecast_3h)!=null?d:[],m=Date.now(),w=u.filter(g=>new Date(g.t_utc).getTime()<=m+3*60*60*1e3).sort((g,v)=>new Date(v.t_utc).getTime()-new Date(g.t_utc).getTime());w.length>0&&(o=w[0].kp)}let n=0;e>=2||o!=null&&o>=6?n=2:(e>=1||o!=null&&o>=4)&&(n=1);let c=parseInt(((p=t.scales.r_scale)!=null?p:"R0").slice(1),10),a=c>=2&&n<2;c>=2&&(n=Math.min(2,n+1));let l={0:"#5cce8c",1:"#d4cc5c",2:"#e05c5c"},r={0:"Low",1:"Moderate",2:"High"};return{level:n,color:l[n],label:r[n],kp:o,gScale:s,boostedByFlare:a}}function Ie(t,s){var i;let e=Rt(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33},{l:1,label:"Moderate",color:"#d4cc5c",width:64},{l:2,label:"High",color:"#e05c5c",width:100}].map(d=>{let p=d.l===e.level,u=p?d.color:"#566068",m=p?"0.88":"0.16";return`<div class="hw-gnss-rung">
      <span class="hw-gnss-label" style="color:${u}">${d.label}</span>
    ...
    ${r}
  </div>`}function It(t){var n;let s=t.metrics.pressure_npa,e=s!=null?s:null,o=(n=t.metrics.density)!=null?n:null;return e==null?{pressure:null,color:"#607880",label:"\u2014",density:o}:e>=6?{pressure:e,color:"#e05c5c",label:"Extreme",density:o}:e>=4?{pressure:e,color:"#e0a84a",label:"Strong",density:o}:e>=2?{pressure:e,color:"#d4cc5c",label:"Elevated",density:o}:e>=1?{pressure:e,color:"#5cce8c",label:"Typical",density:o}:{pressure:e,color:"#7a9298",label:"Weak",density:o}}function De(t,s){let e=It(t),o=s?" hw-impact-tip-open":"",n=e.pressure,c=200,a=6,l=10,r=a+l,i=r+4,d=i+11,p=r+9,u=d+4,w=[{x:0,w:50,color:"#5cce8c"},{x:50,w:50,color:"#d4cc5c"},{x:100,w:50,color:"#e0a84a"},{x:150,w:50,color:"#e05c5c"}].map(y=>`<rect x="${y.x}" y="${a}" width="${y.w}" height="${l}" fill="${y.color}" opacity="0.55" rx="0"/>`).join(""),g=[{x:0,label:"0",anchor:"start"},{x:50,label:"2",anchor:"middle"},{x:100,label:"4",anchor:"middle"},{x:150,label:"6",anchor:"middle"},{x:200,label:"8+",anchor:"end"}],v=g.map(y=>`<line x1="${y.x}" y1="${r}" x2="${y.x}" y2="${i}" stroke="#3a5058" stroke-width="1"/>`).join(""),x=g.map(y=>`<text x="${y.x}" y="${d}" class="hw-swdp-axlabel" text-anchor="${y.anchor}">${y.label}</text>`).join(""),f="";if(n!=null){let k=Math.min(Math.max(n,0),8)/8*c;f=`<polygon points="${`${k-5},${p} ${k+5},${p} ${k},${r}`}" fill="${e.color}" opacity="0.95"/>
    <line x1="${k}" y1="${a}" x2="${k}" y2="${r}" stroke="${e.color}" stroke-width="1.5" opacity="0.7"/>`}let b=`<rect x="0" y="${a}" width="${c}" height="${l}" fill="none" stroke="#2a3c42" stroke-width="0.8" rx="0"/>`,$=`<svg class="hw-swdp-gauge" viewBox="0 0 ${c} ${u}" preserveAspectRatio="none" aria-hidden="true">
    ${w}${b}${f}${v}${x}
  ...
  </div>`}function Dt(t){var i,d;let s=(i=t.alerts_all)!=null?i:[],e=s.find(p=>p.kind==="cme_impact"),o=s.find(p=>p.kind==="cme_watch"),n=e!=null?e:o;if(!n)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let c=((d=n.raw_body)!=null?d:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),a=c?parseInt(c[1],10):null,l=null;if(a&&n.t_utc){let p=1496e5/a*1e3;l=new Date(new Date(n.t_utc).getTime()+p).toISOString().replace(".000Z","Z")}let r=e?"impact":"watch";return{status:r,color:r==="impact"?"#e05c5c":"#d4cc5c",label:r==="impact"?"Active":"Watch",speed_kms:a,issued_utc:n.t_utc,arrival_utc:l}}function Ne(t,s){let e=Dt(t),o=s?" hw-impact-tip-open":"",n=160,c=120,a=80,l=16,r=94,i=r-l,d=_=>Math.tan(_*Math.PI/180),p=Math.round(d(36)*i),u=Math.round(d(23)*i),m=Math.round(d(12)*i),w=_=>`${a},${l} ${a+_},${r} ${a-_},${r}`,g=e.status==="impact"?a:e.status==="watch"?a+u+10:a+p+20,v=Math.min(n-8,Math.max(8,g)),x=v>=a-m&&v<=a+m,f=v>=a-u&&v<=a+u,b=v>=a-p&&v<=a+p,$=x?"#e05c5c":f?"#d4cc5c":b?"#e0a84a":"#5cce8c",C=e.status!=="quiet"?`<polygon points="${w(p)}" fill="#253238" opacity="0.85"/>
...
${e.join(" \xB7 ")}`}function Ge(t,s,e){var r;let o=t.is_active?We:(r=je[t.level])!=null?r:"#445c64",n=t.event_time===e,c=t.event_time.slice(11,16)+" UTC",a=t.source==="NASA_DONKI"?"DONKI":"SWPC",l=n?`<div class="hw-tl-detail">${h(t.description)}${h(Ke(t))}</div>`:"";return`
    ...
    </div>`}function Ue(t,s,e,o){var x,f;let n=(x=t.timeline)!=null?x:[],c=Date.now(),a=new Date(c).toISOString().slice(0,10),l=new Date(c-864e5).toISOString().slice(0,10),r=new Date(c-1728e5).toISOString().slice(0,10),i=new Set([a,l,r]),d=n.filter(b=>{var $;return i.has((($=b.event_time)!=null?$:"").slice(0,10))}).slice().reverse(),p=d.length,u=e?"\u25BC":"\u25B6",m=p>0?`Solar Activity Timeline (${p})`:"Solar Activity Timeline",w=`
    ...
    <div class="hw-timeline">
      ${w}
      ${v}
    </div>`}function Ye(t,s,e){var i;let o=(i=t.alerts_all)!=null?i:[],n=o.length,c=s?"\u25BC":"\u25B6",a=n>0?`SWPC Alerts (${n})`:"SWPC Alerts",l=`
    ...
    </div>`;if(!s||n===0)return`<div class="hw-alerts">${l}${s&&n===0?'<div class="hw-empty-alerts">No significant recent SWPC alerts</div>':""}</div>`;let r=o.map(d=>Be(d,d.dedupe_key===e)).join("");return`
    <div class="hw-alerts">
      ${l}
      ${r}
    </div>`}function qe(t,s){var E,R,A;let e=t.cme_tracker;if(!e)return"";let o=(E=te[e.impact_level])!=null?E:"#96a8b8",n=(R=ee[e.status])!=null?R:e.status,c=300,a=44,l=18,r=a/2,i=10,d=c-18,p=7,u=`<line x1="${l+i}" y1="${r}" x2="${d-p}" y2="${r}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,m=`<circle cx="${l}" cy="${r}" r="${i}" fill="#f0c040" opacity="0.92"/>`,w=`
    <circle cx="${d}" cy="${r}" r="${p}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${d}" cy="${r}" r="2.5" fill="#fff" opacity="0.7"/>`,g=`<text x="${l}" y="${r+i+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,v=`<text x="${d}" y="${r+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,x="";if(e.progress!=null){let I=l+i+4,S=d-p-4,z=I+e.progress*(S-I),F=5;e.status==="arrival_window"?x=`
        ...
        </g>`:x=`<circle cx="${z.toFixed(1)}" cy="${r}" r="${F}" fill="${o}" opacity="0.85"/>`}let f=`<svg class="hw-cme-svg" viewBox="0 0 ${c} ${a}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${u}
    ${m}${g}
    ${w}${v}
    ${x}
    ...
    <div class="hw-cme">
      ...
      ${O}
    </div>`}function Ve(t,s,e,o,n,c,a,l,r,i,d,p,u,m,w,g,v,x,f,b){let $=ke(t,n);return`
    <div class="hw-root">
      ${$e(t)}
      ${we(t,e,m,o,$,f,b)}
      ${e?xe(t):""}
