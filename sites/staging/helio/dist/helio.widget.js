...
`,Ye=!1;function Ft(){if(Ye)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Lt,document.head.appendChild(e),Ye=!0}function Rt(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.length,n=s/o,l=e.map((r,i)=>{let a=Math.max(2,Math.min(t,r.kp/9*t)),c=t-a,d=i*n,p=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${c.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${g(be(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${l}</svg>`}function je(e,s,t,o,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let l=200,r=Math.min(...e),i=Math.max(...e),a=i-r||1,c=u=>o-2-(u-r)/a*(o-4),d=e.map((u,h)=>`${(h/(e.length-1)*l).toFixed(1)},${c(u).toFixed(1)}`).join(" "),p="";if(n&&r<0&&i>0){let u=c(0);p=`<line x1="0" y1="${u.toFixed(1)}" x2="${l}" y2="${u.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let m=e.map((u,h)=>`<rect x="${(h/(e.length-1)*l-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${g(s[h]||"")} \xB7 ${u.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${l} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${p}
    ...
    <line x1="${i}" y1="130" x2="${w}" y2="130"
        ...
        fill="${ne}" font-family="monospace">Bz${q>0?"\u2191":"\u2193"}</text>`})();T=`${Se}${ce}`}let Y=`<g id="${l}-overlay-solar-wind">${T}</g>`,j="";if(e==="coronal_hole"&&s.hssState){let z=s.hssState,D=(J=z.speed)!=null?J:0,B=D>=420,F=z.color,K=B?"0.85":"0.25",te=D>=500?"0.18":B?"0.10":"0.04",ee=i+8,I=d-16,G=60,U=`${ee},130 ${I},${130-G} ${I},${130+G}`,se=I+6,V=`${se},120 ${se+18},130 ${se},140`;j=`
    ...
    <polygon points="${V}" fill="${F}" opacity="${B?"0.9":"0.25"}"/>`}let X=`<g id="${l}-overlay-coronal-hole">${j}</g>`,N="";if(e==="cme_cone"&&s.cmeState){let z=s.cmeState,D=i,B=d-10,F=B-D,K=U=>Math.tan(U*Math.PI/180),te=Math.round(K(9)*F),ee=Math.round(K(6)*F),I=Math.round(K(3)*F),G=U=>`${D},130 ${B},${130-U} ${B},${130+U}`;if(z.status!=="quiet"){let U=z.status==="impact"?"#e05c5c":"#d4cc5c";N=`
    ...
    <div class="hw-hero" style="background:${w}">
      ...
      ${x}
    </div>`}function Xt(e){var d,p,m,u;let{metrics:s}=e,t=(d=s.kp_history_1h)!=null?d:[],o=(p=s.wind_history_1h)!=null?p:[],n=(m=s.bz_history_1h)!=null?m:[],l=(u=s.xray_history_1h)!=null?u:[],r=Rt(t),i=je(o.map(h=>{var w;return(w=h.kms)!=null?w:0}).filter(h=>h>0),o.map(h=>be(h.t_utc)),"#5cce8c",28,!1),a=je(n.map(h=>h.bz),n.map(h=>be(h.t_utc)),"#d4cc5c",28,!0),c=Et(l);return`
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}function vs(e,s){let t=ot(e),o=$s(e),n=t.forecast_24h,l=Math.max(0,Math.min(5,Math.round(n.max_expected))),r=`<div class="hw-storm-risk-now-gauge">${Ue(t.now.g_level,"Current observed geomagnetic storm level",{showDialCode:!0})}</div>`,i=`<div class="hw-storm-risk-fc-gauge">${Ue(l,"Max expected geomagnetic level in next 24 hours",{showDialCode:!1})}</div>`,a=`
...
${t.join(" \xB7 ")}`}function zs(e,s,t){var a;let o=e.is_active?Is:(a=Es[e.level])!=null?a:"#445c64",n=e.event_time===t,l=e.event_time.slice(11,16)+" UTC",r=e.source==="NASA_DONKI"?"DONKI":"SWPC",i=n?`<div class="hw-tl-detail">${g(e.description)}${g(As(e))}</div>`:"";return`
    ...
    <circle cx="${d}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,w=`<text x="${i}" y="${a+c+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,v=`<text x="${d}" y="${a+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,$="";if(t.progress!=null){let P=i+c+4,W=d-p-4,C=P+t.progress*(W-P),E=5;t.status==="arrival_window"?$=`
