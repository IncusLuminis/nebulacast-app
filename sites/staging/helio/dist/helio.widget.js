"use strict";var HelioWidgetModule=(()=>{var ht=Object.defineProperty;var Xt=Object.getOwnPropertyDescriptor;var Yt=Object.getOwnPropertyNames;var qt=Object.prototype.hasOwnProperty;var Vt=(t,s)=>{for(var e in s)ht(t,e,{get:s[e],enumerable:!0})},Zt=(t,s,e,o)=>{if(s&&typeof s=="object"||typeof s=="function")for(let n of Yt(s))!qt.call(t,n)&&n!==e&&ht(t,n,{get:()=>s[n],enumerable:!(o=Xt(s,n))||o.enumerable});return t};var Qt=t=>Zt(ht({},"__esModule",{value:!0}),t);var as={};Vt(as,{HelioWidget:()=>Dt});var bt={quiet:{bg:"#1a2e22",accent:"#5cce8c"},active:{bg:"#2e2a1a",accent:"#d4cc5c"},elevated:{bg:"#2e1f10",accent:"#e0a84a"},storm:{bg:"#2e1212",accent:"#e05c5c"}},Jt={none:"#666",low:"#5cce8c",moderate:"#e0a84a",high:"#e05c5c"},te={info:"#666",watch:"#d4cc5c",warning:"#e05c5c"},mt={A:"#888",B:"#5cce8c",C:"#aad47a",M:"#e0a84a",X:"#e05c5c"},ee={low:"#5cce8c",moderate:"#d4cc5c",high:"#e05c5c",unknown:"#96a8b8"},se={detected:"Detected",inbound:"Inbound",arrival_window:"Arriving",arrived:"Arrived"};function oe(t){if(!t)return"Update time unavailable";try{let s=Math.round((Date.now()-new Date(t).getTime())/6e4);if(s<1)return"Updated just now";if(s<60)return`Updated ${s} min ago`;let e=Math.floor(s/60);return e<24?`Updated ${e}h ago`:`Updated ${Math.floor(e/24)}d ago`}catch(s){return"Updated recently"}}function ne(t){if(!t)return"\u2014";try{return new Date(t).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return t}}function vt(t){if(!t)return"";try{return new Date(t).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return t}}function it(t){try{return new Date(t).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return t.slice(11,16)}}function yt(t){if(!t)return"\u2014";try{return new Date(t).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return t}}function G(t){return t.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function m(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ae(t){return parseInt(t.slice(1),10)>0}var ie=`
...
`,kt=!1;function re(){if(kt)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=ie,document.head.appendChild(t),kt=!0}function le(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,n=s/o,c=t.map((i,r)=>{let a=Math.max(2,Math.min(e,i.kp/9*e)),l=e-a,p=r*n,d=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${p.toFixed(1)}" y="${l.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${d}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(it(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${c}</svg>`}function _t(t,s,e,o,n){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,i=Math.min(...t),r=Math.max(...t),a=r-i||1,l=w=>o-2-(w-i)/a*(o-4),p=t.map((w,g)=>`${(g/(t.length-1)*c).toFixed(1)},${l(w).toFixed(1)}`).join(" "),d="";if(n&&i<0&&r>0){let w=l(0);d=`<line x1="0" y1="${w.toFixed(1)}" x2="${c}" y2="${w.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((w,g)=>`<rect x="${(g/(t.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[g]||"")} \xB7 ${w.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${d}
    <polyline points="${p}" fill="none" stroke="${e}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
    ${h}
  </svg>`}function ce(t){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.map(p=>Math.max(-9,Math.min(-3,Math.log10(p.flux)))),n=Math.min(...o),i=Math.max(...o)-n||1,r=p=>e-2-(p-n)/i*(e-4),a=o.map((p,d)=>`${(d/(o.length-1)*s).toFixed(1)},${r(p).toFixed(1)}`).join(" "),l=t.map((p,d)=>{let h=d/(o.length-1)*s,w=p.flux>=1e-4?"X":p.flux>=1e-5?"M":p.flux>=1e-6?"C":p.flux>=1e-7?"B":"A";return`<rect x="${(h-4).toFixed(1)}" y="0" width="8" height="${e}" fill="transparent"><title>${m(it(p.t_utc))} \xB7 ${w}-class (${p.flux.toExponential(2)})</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function de(t){var l;let s=(l=t.metrics.wind_history_1h)!=null?l:[],e=s[s.length-1],o=t.metrics.solar_wind_kms,n=o!=null?`${Math.round(o)} km/s`:"\u2014",c=o!=null?o>=700?"#e05c5c":o>=500?"#e0a84a":o>=400?"#d4cc5c":"#5cce8c":"#607880",i=(e==null?void 0:e.density)!=null?`${e.density.toFixed(2)} cm\u207B\xB3`:"\u2014",r=(e==null?void 0:e.temp_kk)!=null?`${e.temp_kk.toFixed(0)} kK`:"\u2014",a=(e==null?void 0:e.pressure_npa)!=null?`${e.pressure_npa.toFixed(2)} nPa`:"\u2014";return`<div class="hw-kpi-popover">
    ${Q("Solar Wind \xB7 Current")}
    <div class="hw-kpi-stat-row">
      <div class="hw-kpi-stat">
        <span class="hw-kpi-stat-label">Speed</span>
        <span class="hw-kpi-stat-value" style="color:${c}">${m(n)}</span>
      </div>
      <div class="hw-kpi-stat">
    ...
    <div class="hw-kpi-hint">Class: <b style="color:${(a=mt[s])!=null?a:"#a0b4b8"}">${m(s)}-class</b> \xB7 ${m(o)}</div>
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function he(t){let a='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',l=[-10,-5,5,10].map(v=>{let y=100+v/20*100;return`<line x1="${y.toFixed(1)}" y1="16" x2="${y.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),p='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(t==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${a}${l}${p}</svg>`;let d=t<=-10?"#e05c5c":t<=-5?"#e0a84a":t<0?"#d4b84a":t>=5?"#5cce8c":"#7acca8",h=Math.max(-20,Math.min(20,t)),w=100+h/20*100,g=3,u=h<0?w-g:100-g,k=Math.max(2*g,Math.abs(w-100)+2*g),x=`<rect x="${u.toFixed(1)}" y="16" width="${k.toFixed(1)}" height="6" rx="${g}" fill="${d}" opacity="0.82"/>`,$=5,b=15,f=b-$*1.1,L=`<polygon points="${w.toFixed(1)},${b.toFixed(1)} ${(w-$).toFixed(1)},${f.toFixed(1)} ${(w+$).toFixed(1)},${f.toFixed(1)}" fill="${d}"/>`,H=`<line x1="${w.toFixed(1)}" y1="${b.toFixed(1)}" x2="${w.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${d}" stroke-width="1" opacity="0.6"/>`,z=`<text x="${w.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${d}" font-weight="600">${t>=0?"+":""}${t.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${a}${l}${p}${x}${L}${H}${z}
    ...
    <div style="font-size:.65em;color:#607880">Coupling: <span style="color:${p.color};font-weight:600">${m(p.coupling)}</span> \xB7 Trend history: \u25B6 Details</div>
      ...
      <text x="${$+2}" y="${b-2}" font-size="7" fill="${c}" opacity=".8" font-family="sans-serif">${m(t.label)}</text>
    ...
    <div class="hw-kpi-stat-row">
      ...
      <div class="hw-kpi-stat">
        ...
        <span class="hw-kpi-stat-value" style="color:${d}">${m(r)}</span>
      </div>
      <div class="hw-kpi-stat">
    ...
    <div class="hw-kpi-stat-row" style="margin-top:4px">
      ...
      <div class="hw-kpi-stat">
        <span class="hw-kpi-stat-label">Pressure</span>
        <span class="hw-kpi-stat-value">${m(p)}</span>
      </div>
    </div>
    <div class="hw-kpi-hint" style="margin-bottom:0">${m(w)}</div>
  </div>`}function Lt(t,s,e){if(!t.length)return null;let o=(e%360+360)%360,n=-1,c=1/0,i=Math.cos(s*Math.PI/180);for(let r of t){let a=r.lat-s,l=(r.lon-o+180+360)%360-180,p=a*a+l*i*(l*i);p<c&&(c=p,n=r.prob)}return n>=0?n:null}function Ht(t){return`<svg viewBox="0 0 100 100" width="100%" height="100%"
  ...
  </div>`}function xe(t,s,e,o){switch(s){case"solar_wind":return de(t);case"xray":return pe(t);case"imf_bz":return ue(t);case"aurora":return we(e,o);case"magnetosphere":return ge(t);default:return""}}function nt(t,s){if(t.length<2)return"\u2192";let e=t[t.length-1],o=Math.max(0,t.length-4),n=t[o];if(!isFinite(e)||!isFinite(n))return"\u2192";let c=e-n;return c>s?"\u2191":c<-s?"\u2193":"\u2192"}function fe(t,s,e,o,n,c,i){var A,E,O,D,K,Y,V;let{summary:r,scales:a,metrics:l,aurora_hint:p}=t,d=(A=bt[r.status])!=null?A:bt.quiet,h=n!=null?n.kp.toFixed(1):l.kp_latest!=null?l.kp_latest.toFixed(1):"\u2014",g=[n?n.gScale:a.g_scale,a.r_scale,a.s_scale].map(j=>{let X=ae(j),q=X?`color:${d.accent};border-color:${d.accent}33`:"";return`<span class="hw-scale-chip${X?" hw-scale-active":""}" style="${q}">${m(j)}</span>`}).join(""),u=n?n.auroraLabel:p.aurora_label,k=u==="good"?"#5cce8c":u==="possible"?"#d4cc5c":"#607880",x=u.charAt(0).toUpperCase()+u.slice(1),$="#b4c6cc",b=l.solar_wind_kms!=null?`${Math.round(l.solar_wind_kms)} km/s`:"\u2014",f=l.imf_bz_nt,L=f!=null?f<=-10?"#e05c5c":f<=-5?"#e0a84a":f>=5?"#5cce8c":"#a0b4b8":"#607880",H=f!=null?(f>=0?"+":"")+f.toFixed(1)+" nT":"\u2014",M=l.xray_class,z=M?(E=mt[M])!=null?E:"#a0b4b8":"#607880",v=M?`${M}-class`:"\u2014",y=nt(((O=l.kp_history_1h)!=null?O:[]).map(j=>j.kp),.5),T=nt(((D=l.wind_history_1h)!=null?D:[]).map(j=>j.kms),20),C=nt(((K=l.bz_history_1h)!=null?K:[]).map(j=>j.bz),1.5),N=nt(((Y=l.xray_history_1h)!=null?Y:[]).map(j=>Math.log10(j.flux+1e-9)),.15),R=s?"\u25BC HISTORY":"\u25B6 HISTORY",W=gt(t),I=(V=l.kp_latest)!=null?V:0,P=I>=5,S=P?`linear-gradient(160deg, #0d2a1a 0%, ${d.bg}22 75%)`:`${d.bg}18`,F=(j,X,q,wt,J)=>{let tt=J?`<span class="hw-trend">${J}</span>`:"";return`<div class="hw-kpi-item${o===j?" hw-kpi-active":""}" data-kpi="${j}">
    ...
    <div class="hw-hero" style="background:${S}">
      <div class="hw-hero-main">
        <div class="hw-kp-col">
          <div class="hw-kp-big" style="${n?"color:#9acf60":""}">Kp <b>${m(h)}</b>${n?"":`<span class="hw-trend">${y}</span>`}</div>
          <span class="hw-status-badge" style="background:${d.accent}22;color:${d.accent};display:block;text-align:center">${m(r.label)}</span>
          <div class="hw-scales-row">${g}</div>
        </div>
        <div class="hw-info-col">
          <div class="hw-info-top-row">
            <div class="hw-summary-text" style="flex:1">${m(r.text)}</div>
            <div class="hw-solar-mini-wrap${o==="magnetosphere"?" hw-kpi-active":""}" data-kpi="magnetosphere" title="Magnetosphere status">
              <div class="hw-solar-mini-inner">
                <img class="hw-solar-mini-img" src="${G(Ae)}" alt="SOHO EIT 171"
                  onerror="if(this.src!=='${G(St)}')this.src='${G(St)}'" />
                <video class="hw-solar-mini-video" autoplay loop muted playsinline
                  oncanplay="this.style.opacity=1"
                  aria-label="Solar disk \xB7 SDO AIA 171 \xB7 last 24h">
                  <source src="${G(Ee)}" type="video/mp4">
                </video>
              </div>
      ...
      <div class="hw-quick-details">
        ${F("aurora","Aurora",m(x),k)}
        ${F("solar_wind","Solar wind",m(b),$,T)}
        ${F("imf_bz","IMF Bz",m(H),L,C)}
        ${F("xray","X-ray",m(v),z,N)}
      </div>
      ${o?xe(t,o,c,i):""}
    ...
    </div>`}function be(t){let s=oe(t.updated_utc);return`
    <div class="hw-header">
      <span class="hw-header-title">Space Weather</span>
      <span class="hw-freshness">${m(s)}</span>
    </div>`}function ve(t){return t.map((s,e)=>e===0?(s+t[1])/2:e===t.length-1?(t[e-1]+s)/2:(t[e-1]+s+t[e+1])/3)}function ye(t){return t>=9?"G5":t>=8?"G4":t>=7?"G3":t>=6?"G2":t>=5?"G1":"G0"}function ke(t){return t>=5?"good":t>=3?"possible":"none"}function zt(t){return t>=9?40:t>=8?45:t>=7?50:t>=6?55:t>=5?60:null}function _e(t){let s=t>=7?"high":t>=5?"moderate":t>=3?"low":"none",e=zt(t),o=s==="none"?"No aurora expected at mid-latitudes":e!=null?`Aurora possible equatorward of ~${e}\xB0 lat`:"Minor aurora possible at high latitudes",n=t>=7?"moderate":t>=5?"low":"none",c=n==="none"?"No significant HF degradation expected":n==="low"?"Minor HF degradation at high latitudes":"Moderate HF degradation, possible blackouts at high latitudes",i=t>=8?"high":t>=6?"moderate":t>=4?"low":"none";return[{kind:"aurora",level:s,label:"Aurora",summary:o},{kind:"radio",level:n,label:"HF Radio",summary:c},{kind:"solar_activity",level:i,label:"Solar Activity",summary:i==="none"?"Quiet geomagnetic conditions expected":i==="low"?"Active geomagnetic conditions possible":i==="moderate"?"Minor to moderate storm conditions":"Major geomagnetic storm conditions"}]}function Se(t,s){var d;if(s<=0)return null;let e=(d=t.metrics.kp_forecast_3h)!=null?d:[];if(!e.length)return null;let o=Date.now()+s*36e5,n=e[0],c=1/0;for(let h of e){let w=Math.abs(new Date(h.t_utc).getTime()-o);w<c&&(c=w,n=h)}let i=n.kp,r=ye(i),a=ke(i),l=zt(i),p=_e(i);return{offsetH:s,kp:i,gScale:r,auroraLabel:a,auroraMinLat:l,impacts:p}}function Me(t,s,e,o){var P;let{forecast:n,metrics:c}=t,{kp_max_next_24h:i,kp_max_at_utc:r,trend:a}=n,l=((P=c.kp_forecast_3h)!=null?P:[]).slice(0,16),p=l.length,d=p*3,h=d>0?`${(s/d*100).toFixed(0)}%`:"0%",w=s>0?`\u23F1 +${Math.round(s)}h`:"Timeline",g="Kp forecast unavailable";if(i!=null){let S=vt(r),F=a==="rising"?"rising":a==="falling"?"falling":"steady";g=`Peak Kp ${i.toFixed(1)} next 24h${S?` at ${S}`:""} \xB7 ${F}`}let u=o?"\u25BC FORECAST":"\u25B6 FORECAST";if(!l.length)return`
    <div class="hw-forecast">
      ...
      ${o?`<div class="hw-forecast-text">${m(g)}</div>`:""}
    </div>`;let k=320,x=38,$=14,b=x+$,f=k/p,L=S=>x-Math.max(2,Math.min(x-2,S/9*(x-2))),H="",M=l.map(S=>S.kp),z=ve(M);l.forEach((S,F)=>{let _=L(S.kp),A=x-_,E=F*f,O=E+f/2,D=S.kp>=6?"#e05c5c":S.kp>=5?"#e0a84a":S.kp>=4?"#d4cc5c":"#5cce8c",K=`Kp ${S.kp.toFixed(1)} \xB7 ${vt(S.t_utc)}`;if(H+=`<rect x="${E.toFixed(1)}" y="${_.toFixed(1)}" width="${(f-1.5).toFixed(1)}" height="${A.toFixed(1)}" fill="${D}" fill-opacity="0.85" rx="1.5"/>`,H+=`<rect x="${E.toFixed(1)}" y="0" width="${f.toFixed(1)}" height="${x}" fill="transparent"><title>${G(K)}</title></rect>`,p<=8||F%2===0){let V=new Date(S.t_utc).getHours();H+=`<text x="${O.toFixed(1)}" y="${(b-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${V.toString().padStart(2,"0")}</text>`}});let y=`<polyline points="${l.map((S,F)=>{let _=F*f+f/2,A=L(z[F]);return`${_.toFixed(1)},${A.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,T="";if(s>0&&p>0){let S=Math.min(k-1,s/(p*3)*k);T=`
      <line x1="${S.toFixed(1)}" y1="0" x2="${S.toFixed(1)}" y2="${x}" stroke="rgba(255,255,255,0.75)" stroke-width="1.5" stroke-dasharray="3,2"/>
      <polygon points="${S.toFixed(1)},${x} ${(S-4).toFixed(1)},${(x-7).toFixed(1)} ${(S+4).toFixed(1)},${(x-7).toFixed(1)}" fill="rgba(255,255,255,0.75)"/>`}let C=Math.round(d/4),N=Math.round(d/2),R=Math.round(d*3/4),W=`
    <div class="hw-scrub-wrap">
      <div class="hw-scrub-header">
        <span class="hw-scrub-title">${m(w)}</span>
    ...
    <div class="hw-forecast">
      ...
      ${o?`
      ${I}
      <div class="hw-forecast-text">${m(g)}</div>
      ...
      ${W}`:""}
    </div>`}function Ce(t){let s=/([NS])(\d+)([EW])(\d+)/i.exec(t);return s?{lat:(s[1].toUpperCase()==="N"?1:-1)*parseInt(s[2],10),lon:(s[3].toUpperCase()==="E"?1:-1)*parseInt(s[4],10)}:null}var Tt=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function Le(t){return t.x_flare_probability>0?"X":t.m_flare_probability>0?"M":t.c_flare_probability>0?"C":"quiet"}function He(t,s,e){let o=s/2,n=o*.87,c=s*.03,i=s*.009,r=t.map(a=>{var x,$;let l=Ce(a.location);if(!l||Math.abs(l.lon)>88||a.location.includes("*"))return"";let p=Le(a);if(!e.has(p))return"";let d=Tt.find(b=>b.id===p).color,h=l.lat*Math.PI/180,w=l.lon*Math.PI/180,g=(o+n*Math.cos(h)*Math.sin(w)).toFixed(1),u=(o-n*Math.sin(h)).toFixed(1),k=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    <div class="hw-radio-meta">X-ray: <b style="color:${(d=mt[o])!=null?d:"#a0b4b8"}">${m(o)}-class</b> \xB7 ${m(c)}</div>
  </div>`}var Ft={g1:"#d4cc5c",g2:"#e0a84a",g3:"#e05c5c"};function At(t){var r;let s=(r=t.metrics.kp_forecast_3h)!=null?r:[],e=Date.now(),o=e+24*60*60*1e3,n=s.filter(a=>{let l=new Date(a.t_utc).getTime();return l>=e-3*60*60*1e3&&l<=o});if(n.length===0)return{g1:0,g2:0,g3:0};let c=Math.max(...n.map(a=>a.kp)),i=a=>{if(c<a-.7)return 0;if(c>a+1)return 90;let l=(c-(a-.7))/1.7;return Math.round(Math.pow(Math.max(0,l),.7)*90)};return{g1:i(5),g2:i(6),g3:i(7)}}var Mt={rising:"#e0884a",peak:"#e05c5c",decline:"#d4cc5c"};function Ie(t){var l,p,d;let s=(l=t.metrics.kp_latest)!=null?l:0,e=t.metrics.imf_bz_nt,o=t.metrics.solar_wind_kms,n=parseInt(((p=t.scales.g_scale)!=null?p:"G0").replace("G",""),10)||0,c=s>=5||n>=1,i=(d=t.metrics.kp_history_1h)!=null?d:[],r=0;if(i.length>=2&&(r=i[i.length-1].kp-i[i.length-2].kp),!c)return{active:!1,phase:"quiet",kp_current:s,kp_trend:r,bz_nt:e,solar_wind_kms:o};let a;return r>.3&&(e==null||e<-5)?a="rising":r<-.5?a="decline":a="peak",{active:!0,phase:a,kp_current:s,kp_trend:r,bz_nt:e,solar_wind_kms:o}}function Pe(t){if(!t.active)return"";let s=[{key:"rising",label:"Rising"},{key:"peak",label:"Peak"},{key:"decline",label:"Decline"}],e=s.findIndex(r=>r.key===t.phase),o=Mt[t.phase],n=s[e].label,c=s.map((r,a)=>{let l=a===e,p=a<e,d=Mt[r.key],h=l?`background:${d};border-color:${d};box-shadow:0 0 6px ${d}88`:p?`background:${d}44;border-color:${d}66`:"background:#111b1e;border-color:#1e2c30",w=l?" hw-spi-dot-active":"",g=l?`color:${d};font-weight:700`:p?`color:${d}66`:"color:#2e4248",u=a<s.length-1?`<div class="hw-spi-arr">${p?`<span style="color:${d}55">\u2192</span>`:"\u2192"}</div>`:"";return`<div class="hw-spi-node">
  ...
  </div>`}var at={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Et={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function De(t){var v;let s=at,e=(v=Et[s.phase])!=null?v:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,c=52,i=10,r=c-6,a=c-18,l=.5,p=.19,d=y=>Math.exp(-Math.pow((y-l)/p,2)/2),h=y=>i+y*(n-2*i),w=y=>r-d(y)*a,g=80,u=[];for(let y=0;y<=g;y++){let T=y/g;u.push(`${y===0?"M":"L"}${h(T).toFixed(1)},${w(T).toFixed(1)}`)}let k=Math.round(s.progress_0_1*g),x=[];for(let y=0;y<=k;y++){let T=y/g;x.push(`${y===0?"M":"L"}${h(T).toFixed(1)},${w(T).toFixed(1)}`)}let $=h(s.progress_0_1),b=[`M${i},${r}`,...x.slice(1),`L${$.toFixed(1)},${r} Z`],f=w(s.progress_0_1),L=5,H=`M${$.toFixed(1)},${f.toFixed(1)} L${($-L).toFixed(1)},${(f-L*1.8).toFixed(1)} L${($+L).toFixed(1)},${(f-L*1.8).toFixed(1)} Z`,M=r+11;return`<div class="hw-impact-tip${t?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function Ot(t){var a,l,p,d,h;let s=(l=(a=t.coronal_hole)==null?void 0:a.estimated_speed_kms)!=null?l:t.metrics.solar_wind_kms,e=(p=t.coronal_hole)==null?void 0:p.status,o=s!=null?s:0,n=e!=null?e:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",c={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},i={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},r={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:n,color:c[n],label:i[n],desc:(h=(d=t.coronal_hole)==null?void 0:d.note)!=null?h:r[n],speed:s}}function Be(t,s){var S;let e=Ot(t),o=(S=e.speed)!=null?S:0,n=e.speed!=null?`${Math.round(e.speed)} km/s`:"\u2014",c=s?" hw-impact-tip-open":"",i=320,r=72,a=36,l=36,p=284,d=36,h=11,w=14,g=19,u=9,k=o>=500,x=o>=420,$=k?"#f5c540":x?"#c8a020":"#7a6010",b=k?"#f5c540":x?"#c8a020":"#3a3808",f=e.color,L=x?"0.9":"0.25",H=k?"0.18":x?"0.10":"0.04",M=[0,45,90,135,180,225,270,315].map(F=>{let _=F*Math.PI/180,A=(a+w*Math.cos(_)).toFixed(1),E=(l+w*Math.sin(_)).toFixed(1),O=(a+g*Math.cos(_)).toFixed(1),D=(l+g*Math.sin(_)).toFixed(1),K=F>300||F<60?"0.9":"0.5";return`<line x1="${A}" y1="${E}" x2="${O}" y2="${D}"
      stroke="${$}" stroke-width="1.6" stroke-linecap="round" opacity="${K}"/>`}).join(""),z=a+h+2,v=p-u-3,y=14,T=`${z},${l} ${v},${d-y} ${v},${d+y}`,C=v+1,N=`${C},${d-4} ${C+7},${d} ${C},${d+4}`,R=u,W=`
  ...
  </div>`}function It(t){var l,p,d;let s=(l=t.scales.g_scale)!=null?l:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let h=(p=t.metrics.kp_forecast_3h)!=null?p:[],w=Date.now(),g=h.filter(u=>new Date(u.t_utc).getTime()<=w+3*60*60*1e3).sort((u,k)=>new Date(k.t_utc).getTime()-new Date(u.t_utc).getTime());g.length>0&&(o=g[0].kp)}let n=0;e>=2||o!=null&&o>=6?n=2:(e>=1||o!=null&&o>=4)&&(n=1);let c=parseInt(((d=t.scales.r_scale)!=null?d:"R0").slice(1),10),i=c>=2&&n<2;c>=2&&(n=Math.min(2,n+1));let r={0:"#5cce8c",1:"#d4cc5c",2:"#e05c5c"},a={0:"Low",1:"Moderate",2:"High"};return{level:n,color:r[n],label:a[n],kp:o,gScale:s,boostedByFlare:i}}function We(t,s){var l;let e=It(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33},{l:1,label:"Moderate",color:"#d4cc5c",width:64},{l:2,label:"High",color:"#e05c5c",width:100}].map(p=>{let d=p.l===e.level,h=d?p.color:"#566068",w=d?"0.88":"0.16";return`<div class="hw-gnss-rung">
    ...
    ${a}
  </div>`}function Pt(t){var n;let s=t.metrics.pressure_npa,e=s!=null?s:null,o=(n=t.metrics.density)!=null?n:null;return e==null?{pressure:null,color:"#607880",label:"\u2014",density:o}:e>=6?{pressure:e,color:"#e05c5c",label:"Extreme",density:o}:e>=4?{pressure:e,color:"#e0a84a",label:"Strong",density:o}:e>=2?{pressure:e,color:"#d4cc5c",label:"Elevated",density:o}:e>=1?{pressure:e,color:"#5cce8c",label:"Typical",density:o}:{pressure:e,color:"#7a9298",label:"Weak",density:o}}function Ue(t,s){let e=Pt(t),o=s?" hw-impact-tip-open":"",n=e.pressure,c=200,i=6,r=10,a=i+r,l=a+4,p=l+11,d=a+9,h=p+4,g=[{x:0,w:50,color:"#5cce8c"},{x:50,w:50,color:"#d4cc5c"},{x:100,w:50,color:"#e0a84a"},{x:150,w:50,color:"#e05c5c"}].map(v=>`<rect x="${v.x}" y="${i}" width="${v.w}" height="${r}" fill="${v.color}" opacity="0.55" rx="0"/>`).join(""),u=[{x:0,label:"0",anchor:"start"},{x:50,label:"2",anchor:"middle"},{x:100,label:"4",anchor:"middle"},{x:150,label:"6",anchor:"middle"},{x:200,label:"8+",anchor:"end"}],k=u.map(v=>`<line x1="${v.x}" y1="${a}" x2="${v.x}" y2="${l}" stroke="#3a5058" stroke-width="1"/>`).join(""),x=u.map(v=>`<text x="${v.x}" y="${p}" class="hw-swdp-axlabel" text-anchor="${v.anchor}">${v.label}</text>`).join(""),$="";if(n!=null){let y=Math.min(Math.max(n,0),8)/8*c;$=`<polygon points="${`${y-5},${d} ${y+5},${d} ${y},${a}`}" fill="${e.color}" opacity="0.95"/>
    ...
    ${f}
    <div class="hw-swdp-meta"><b style="color:${e.color}">${m(L)}</b>${m(z)}</div>
    <div class="hw-swdp-meta" style="font-size:.72em">Speed ${m(M)} \xB7 Density ${m(H)}</div>
  </div>`}function Nt(t){var l,p;let s=(l=t.alerts_all)!=null?l:[],e=s.find(d=>d.kind==="cme_impact"),o=s.find(d=>d.kind==="cme_watch"),n=e!=null?e:o;if(!n)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let c=((p=n.raw_body)!=null?p:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),i=c?parseInt(c[1],10):null,r=null;if(i&&n.t_utc){let d=1496e5/i*1e3;r=new Date(new Date(n.t_utc).getTime()+d).toISOString().replace(".000Z","Z")}let a=e?"impact":"watch";return{status:a,color:a==="impact"?"#e05c5c":"#d4cc5c",label:a==="impact"?"Active":"Watch",speed_kms:i,issued_utc:n.t_utc,arrival_utc:r}}function Ke(t,s){let e=Nt(t),o=s?" hw-impact-tip-open":"",n=320,c=80,i=24,r=40,a=268,l=296,p=a-i,d=_=>Math.tan(_*Math.PI/180),h=Math.round(d(9)*p),w=Math.round(d(6)*p),g=Math.round(d(3)*p),u=_=>`${i},${r} ${a},${r-_} ${a},${r+_}`,k=e.status==="impact"?r:e.status==="watch"?r+w+8:r,x=Math.min(c-14,Math.max(14,k)),$=Math.abs(x-r)<=g,b=Math.abs(x-r)<=w,f=Math.abs(x-r)<=h,L=$?"#e05c5c":b?"#d4cc5c":f?"#e0a84a":"#5cce8c",H=e.status!=="quiet"?`<polygon points="${u(h)}" fill="#253238" opacity="0.85"/>
       <polygon points="${u(w)}"   fill="#d4cc5c" opacity="0.14"/>
       <polygon points="${u(g)}" fill="#e0a84a" opacity="0.28"/>
       <line x1="${i+9}" y1="${r}" x2="${a-2}" y2="${r}"
             stroke="#3a5058" stroke-dasharray="3 3" stroke-width="1"/>`:`<line x1="${i+9}" y1="${r}" x2="${l-9}" y2="${r}"
             stroke="#1e2c30" stroke-dasharray="4 3" stroke-width="1"/>`,M=8,z=11,v=15,y="#f5c540",T=[0,45,90,135,180,225,270,315].map(_=>{let A=_*Math.PI/180,E=(i+z*Math.cos(A)).toFixed(1),O=(r+z*Math.sin(A)).toFixed(1),D=(i+v*Math.cos(A)).toFixed(1),K=(r+v*Math.sin(A)).toFixed(1),Y=_<45||_>315?"0.9":"0.5";return`<line x1="${E}" y1="${O}" x2="${D}" y2="${K}"
      stroke="${y}" stroke-width="1.4" stroke-linecap="round" opacity="${Y}"/>`}).join(""),C=7,N=`
    <ellipse cx="${l}" cy="${x}" rx="${C}" ry="${(C*.42).toFixed(1)}"
             fill="none" stroke="#4a8ab0" stroke-width="0.8" opacity="0.6"/>
    <line x1="${l}" y1="${x-C}" x2="${l}" y2="${x+C}"
          stroke="#4a8ab0" stroke-width="0.8" opacity="0.6"/>`,R="\u2014";if(e.arrival_utc){let _=new Date(e.arrival_utc),A=_.toLocaleString("en-US",{month:"short",timeZone:"UTC"}),E=_.getUTCDate(),O=String(_.getUTCHours()).padStart(2,"0"),D=String(_.getUTCMinutes()).padStart(2,"0");R=`~${A}\xA0${E}\xA0${O}:${D}\u202FUTC`}let W=e.speed_kms?`${e.speed_kms}\u202Fkm/s`:"\u2014",I=e.status!=="quiet"?`Velocity: <b style="color:#b4c6cc">${m(W)}</b>&ensp;Arrival: <b style="color:#b4c6cc">${m(R)}</b>`:"No Earth-directed CME in forecast window",P=$?"Direct impact likely":b?"Glancing blow possible":f?"Near outer edge":"Impact unlikely",S=$?"#e05c5c":b?"#d4cc5c":f?"#e0a84a":"#5cce8c",F=`<svg class="hw-cme-cone-svg" width="100%" height="${c}"
    ...
    ${e.status!=="quiet"?`<text x="${Math.round((i+l)/2)}" y="${c-3}" font-size="7.5"
               fill="${S}" text-anchor="middle" font-family="inherit">${m(P)}</text>`:""}
  </svg>`;return`<div class="hw-impact-tip${o}">
    ${F}
    <div class="hw-cme-footer">${I}</div>
  </div>`}function Ge(t,s,e,o,n,c,i,r,a){var tt,rt,xt;let l=(rt=(tt=s==null?void 0:s.impacts)!=null?tt:t.observer_impacts)!=null?rt:[],p=l.map(B=>{var ft,$t;let lt=(ft=Jt[B.level])!=null?ft:"#666",Bt=B.level==="none"?"None":B.level.charAt(0).toUpperCase()+B.level.slice(1),jt=($t=ze[B.kind])!=null?$t:Te,Wt=B.level==="none"?"#606870":lt,et=B.kind==="solar_activity"?n:i.has(B.kind),Ut=et?" hw-impact-open":"",Z;if(B.kind==="solar_activity"){let st=n?" hw-solar-open":"",ct=Tt.map(U=>{let ot=c.has(U.id),dt=ot?U.color+"22":"transparent",pt=ot?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${U.id}" style="color:${U.color};border-color:${U.color};background:${dt};opacity:${pt}">${U.label}</button>`}).join("");Z=`<div class="hw-solar-tip${st}">
          ...
          <span class="hw-solar-tip-text">${m(B.summary)}</span>
        </div>`}else if(B.kind==="aurora"){let st=et?" hw-aurora-tip-open":"",ct=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,U=null;r&&a.lat!=null&&a.lon!=null&&(U=Lt(r.entries,a.lat,a.lon));let ot=a.lat!=null&&a.lon!=null,dt=U!=null?U>=30?"#5cce8c":U>=10?"#d4cc5c":"#9ab4bc":"#607880",pt=U!=null?`${U}%`:r?"n/a":"\u2026",Gt=ot?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
          ...
          <div class="hw-aurora-map-wrap">
            <img class="hw-aurora-img" src="${G(ct)}" alt="NOAA Aurora Oval" loading="lazy" />
            ${Ht(a)}
          </div>
          ${Gt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else B.kind==="radio"?Z=Re(t,et):Z=`<div class="hw-impact-tip${et?" hw-impact-tip-open":""}">${m(B.summary)}</div>`;let Kt=B.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${G(B.kind)}"`;return`<div class="hw-impact-row${Ut}"${Kt}>
      <span class="hw-impact-caret">\u25B6</span>
      <span class="hw-impact-kind" style="color:${Wt}">${jt}<span style="color:#b4c6cc">${m(B.label)}</span></span>
      <span class="hw-impact-badge" style="background:${lt}22;color:${lt}">${m(Bt)}</span>
      ${Z}
    </div>`}).join(""),d=s?'<span style="font-size:.65em;color:#7a9870;font-weight:normal;text-transform:none;letter-spacing:0"> \xB7 simulated</span>':"",h=l.length+7,w=o?"\u25BC":"\u25B6",g=h>0?`Observer Impacts (${h})`:"Observer Impacts",u=`
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,k=i.has("geomag_storm"),x=At(t),$=x.g1,b=x.g1>=30?Ft.g1:x.g1>0?"#7a9298":"#607880",f=$>0?`G1 ${$}%`:"None",H=`<div class="hw-impact-row${k?" hw-impact-open":""}" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ne(t,k)}
    </div>`,M=i.has("solar_cycle"),z=(xt=Et[at.phase])!=null?xt:"#96a8b8",v=at.phase.charAt(0).toUpperCase()+at.phase.slice(1),T=`<div class="hw-impact-row${M?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${De(M)}
    </div>`,C=Ot(t),N=i.has("hss"),W=`<div class="hw-impact-row${N?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Be(t,N)}
    </div>`,I=Rt(t),P=i.has("sat_drag"),F=`<div class="hw-impact-row${P?" hw-impact-open":""}" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${je(t,P)}
    </div>`,_=It(t),A=i.has("gnss"),O=`<div class="hw-impact-row${A?" hw-impact-open":""}" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${We(t,A)}
    </div>`,D=Pt(t),K=i.has("sw_pressure"),Y=D.pressure!=null?`${D.pressure.toFixed(2)} nPa`:"\u2014",j=`<div class="hw-impact-row${K?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      <span class="hw-impact-badge" style="background:${D.color}22;color:${D.color}">${Y}</span>
      ${Ue(t,K)}
    </div>`,X=Nt(t),q=i.has("cme_cone"),J=`<div class="hw-impact-row${q?" hw-impact-open":""}" data-impact-row="cme_cone">
      <span class="hw-impact-caret">\u25B6</span>
    ...
    ${i}
  </div>`}var Ye={info:"#445c64",watch:"#e0a84a",warning:"#e05c5c"},qe="#4ae0a4";function Ve(t){let s=(e,o="")=>`<svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" ${o}>${e}</svg>`;switch(t){case"solar_flare":return s(`<circle cx="6.5" cy="6.5" r="2.5"/>
        ...
        <circle cx="6.5" cy="3.8" r=".6" fill="currentColor" stroke="none"/>`)}}function Ze(t){var o;let s=(o=t.metadata)!=null?o:{},e=[];return s.source_code&&e.push(`Code: ${s.source_code}`),s.model&&e.push(`Model: ${String(s.model).toUpperCase()}`),e.length===0?"":`
${e.join(" \xB7 ")}`}function Qe(t,s,e){var a;let o=t.is_active?qe:(a=Ye[t.level])!=null?a:"#445c64",n=t.event_time===e,c=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",r=n?`<div class="hw-tl-detail">${m(t.description)}${m(Ze(t))}</div>`:"";return`
    <div class="hw-tl-item" data-timeline-key="${G(t.event_time)}">
      ...
      <div class="hw-tl-body">
        ...
        <div class="hw-tl-title${t.is_active?" hw-tl-active":""}">
          ${Ve(t.event_type)} ${m(t.event_title)}
        </div>
        ${r}
      </div>
    </div>`}function Je(t,s,e,o){var x,$;let n=(x=t.timeline)!=null?x:[],c=Date.now(),i=new Date(c).toISOString().slice(0,10),r=new Date(c-864e5).toISOString().slice(0,10),a=new Date(c-1728e5).toISOString().slice(0,10),l=new Set([i,r,a]),p=n.filter(b=>{var f;return l.has(((f=b.event_time)!=null?f:"").slice(0,10))}).slice().reverse(),d=p.length,h=e?"\u25BC":"\u25B6",w=d>0?`Solar Activity Timeline (${d})`:"Solar Activity Timeline",g=`
    ...
    </div>`;if(!e||d===0)return`<div class="hw-timeline">${g}</div>`;let u=new Map;for(let b of p){let f=(($=b.event_time)!=null?$:"").slice(0,10);u.has(f)||u.set(f,[]),u.get(f).push(b)}let k=[...u.entries()].map(([b,f])=>{let H=new Date(b+"T12:00:00Z").toLocaleDateString("en-US",{month:"short",day:"numeric",timeZone:"UTC"}),M=o.has(b),z=M?"\u25B6":"\u25BC",v=M?`<span class="hw-tl-day-count">${f.length} events</span>`:"",y=`
      <div class="hw-tl-day-row" data-tl-day="${G(b)}">
        <span class="hw-section-caret">${z}</span>
        <span class="hw-tl-date">${H}</span>
        ${v}
      </div>`,T=M?"":f.map((C,N)=>Qe(C,N<f.length-1,s)).join("");return`<div class="hw-tl-group">${y}${T}</div>`}).join("");return`
    <div class="hw-timeline">
      ${g}
      ${k}
    </div>`}function ts(t,s,e){var l;let o=(l=t.alerts_all)!=null?l:[],n=o.length,c=s?"\u25BC":"\u25B6",i=n>0?`SWPC Alerts (${n})`:"SWPC Alerts",r=`
    ...
    </div>`;if(!s||n===0)return`<div class="hw-alerts">${r}${s&&n===0?'<div class="hw-empty-alerts">No significant recent SWPC alerts</div>':""}</div>`;let a=o.map(p=>Xe(p,p.dedupe_key===e)).join("");return`
    <div class="hw-alerts">
      ${r}
      ${a}
    </div>`}function es(t,s){var R,W,I;let e=t.cme_tracker;if(!e)return"";let o=(R=ee[e.impact_level])!=null?R:"#96a8b8",n=(W=se[e.status])!=null?W:e.status,c=300,i=44,r=18,a=i/2,l=10,p=c-18,d=7,h=`<line x1="${r+l}" y1="${a}" x2="${p-d}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,w=`<circle cx="${r}" cy="${a}" r="${l}" fill="#f0c040" opacity="0.92"/>`,g=`
    <circle cx="${p}" cy="${a}" r="${d}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${p}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,u=`<text x="${r}" y="${a+l+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,k=`<text x="${p}" y="${a+d+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,x="";if(e.progress!=null){let P=r+l+4,S=p-d-4,F=P+e.progress*(S-P),_=5;e.status==="arrival_window"?x=`
        ...
        </g>`:x=`<circle cx="${F.toFixed(1)}" cy="${a}" r="${_}" fill="${o}" opacity="0.85"/>`}let $=`<svg class="hw-cme-svg" viewBox="0 0 ${c} ${i}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${h}
    ${w}${u}
    ${g}${k}
    ${x}
    ...
    <div class="hw-cme-detail">
      <div class="hw-cme-stat-grid">
        ...
        <div class="hw-cme-stat">
          ...
          <span class="hw-cme-stat-value" style="color:${o}">${m(L)}</span>
        </div>
        <div class="hw-cme-stat">
      ...
      <div class="hw-cme-note">Half-angle: ${m(H)} \xB7 Source: ${m(M)} \xB7 ${m(z)} \xB7 Model: Enlil (NASA DONKI)</div>
    </div>`,T=s?"\u25BC":"\u25B6",C=e.impact_level==="unknown"?"Unrated":e.impact_level.charAt(0).toUpperCase()+e.impact_level.slice(1),N=s?`${$}${y}`:"";return`
    <div class="hw-cme">
      <div class="hw-cme-row" data-cme-toggle>
        ...
        <span class="hw-cme-badge" style="background:${o}15;color:${o};margin-left:4px">${m(C)} impact</span>
      </div>
      ${N}
    </div>`}function ss(t,s,e,o,n,c,i,r,a,l,p,d,h,w,g,u,k,x,$,b){let f=Se(t,n);return`
    <div class="hw-root">
      ${be(t)}
      ${fe(t,e,w,o,f,$,b)}
      ${e?$e(t):""}
      ...
      ${Je(t,r,a,l)}
      ${ts(t,c,i)}
    </div>`}function os(t){return`<div class="hw-root">
