...
`,Pt=!1;function be(){if(Pt)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=fe,document.head.appendChild(t),Pt=!0}function ve(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,a=s/o,r=t.map((i,c)=>{let n=Math.max(2,Math.min(e,i.kp/9*e)),l=e-n,d=c*a,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${l.toFixed(1)}" width="${(a-1).toFixed(1)}" height="${n.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(yt(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${r}</svg>`}function Dt(t,s,e,o,a){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let r=200,i=Math.min(...t),c=Math.max(...t),n=c-i||1,l=g=>o-2-(g-i)/n*(o-4),d=t.map((g,w)=>`${(w/(t.length-1)*r).toFixed(1)},${l(g).toFixed(1)}`).join(" "),p="";if(a&&i<0&&c>0){let g=l(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${r}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((g,w)=>`<rect x="${(w/(t.length-1)*r-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[w]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${r} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ...
    <line x1="${c}" y1="130" x2="${u}" y2="130"
  ...
  </g>`,T="";if(t==="magnetosphere"){let I=(O=s.windKms)!=null?O:0,B=I>500,Y=I<350,P=(B?.5:Y?1.4:.9)*1.3,j=I>700?"#e05c5c":I>500?"#e0a84a":I>350?"#d4c840":"#5cce8c",Z=c+8,tt=u-14,L=8,R=6,D=(tt-Z)/(L-1),nt=260/(R+1),et=38,at=24,rt=2,wt=Array.from({length:L},(lt,pt)=>{let it=Z+pt*D,ct=Array.from({length:R},(_t,st)=>{let ot=nt*(st+1);return`<path d="M ${it.toFixed(1)},${ot.toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} M ${(it+at).toFixed(1)},${(ot-6).toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} L ${(it+at).toFixed(1)},${(ot+6).toFixed(1)}"
        ...
        fill="${lt}" font-family="monospace">Bz${z>0?"\u2191":"\u2193"}</text>`})();T=`${wt}${dt}`}let U=`<g id="${r}-overlay-solar-wind">${T}</g>`,V="";if(t==="coronal_hole"&&s.hssState){let I=s.hssState,B=(q=I.speed)!=null?q:0,Y=B>=420,P=I.color,j=Y?"0.85":"0.25",Z=B>=500?"0.18":Y?"0.10":"0.04",tt=c+8,L=d-16,R=60,D=`${tt},130 ${L},${130-R} ${L},${130+R}`,nt=L+6,et=`${nt},120 ${nt+18},130 ${nt},140`;V=`
    ...
    <polygon points="${et}" fill="${P}" opacity="${Y?"0.9":"0.25"}"/>`}let G=`<g id="${r}-overlay-coronal-hole">${V}</g>`,F="";if(t==="cme_cone"&&s.cmeState){let I=s.cmeState,B=c,Y=d-10,P=Y-B,j=D=>Math.tan(D*Math.PI/180),Z=Math.round(j(9)*P),tt=Math.round(j(6)*P),L=Math.round(j(3)*P),R=D=>`${B},130 ${Y},${130-D} ${Y},${130+D}`;if(I.status!=="quiet"){let D=I.status==="impact"?"#e05c5c":"#d4cc5c";F=`
  ...
  </div>`}function Te(t,s,e,o,a,r,i,c=0){var q,I,B,Y,P,j,Z,tt;let{summary:n,scales:l,metrics:d,aurora_hint:p}=t,h=(q=Rt[n.status])!=null?q:Rt.quiet,g=a!=null?a.kp.toFixed(1):d.kp_latest!=null?d.kp_latest.toFixed(1):"\u2014",u=[a?a.gScale:l.g_scale,l.r_scale,l.s_scale].map(L=>{let R=$e(L),D=R?`color:${h.accent};border-color:${h.accent}33`:"";return`<span class="hw-scale-chip${R?" hw-scale-active":""}" style="${D}">${m(L)}</span>`}).join(""),f=a?a.auroraLabel:p.aurora_label,$=f==="good"?"#5cce8c":f==="possible"?"#d4cc5c":"#607880",x=f.charAt(0).toUpperCase()+f.slice(1),b="#b4c6cc",v=d.solar_wind_kms!=null?`${Math.round(d.solar_wind_kms)} km/s`:"\u2014",_=d.imf_bz_nt,H=_!=null?_<=-10?"#e05c5c":_<=-5?"#e0a84a":_>=5?"#5cce8c":"#a0b4b8":"#607880",M=_!=null?(_>=0?"+":"")+_.toFixed(1)+" nT":"\u2014",C=d.xray_class,y=C?(I=Ht[C])!=null?I:"#a0b4b8":"#607880",k=C?`${C}-class`:"\u2014",E=bt(((B=d.kp_history_1h)!=null?B:[]).map(L=>L.kp),.5),T=bt(((Y=d.wind_history_1h)!=null?Y:[]).map(L=>L.kms),20),U=bt(((P=d.bz_history_1h)!=null?P:[]).map(L=>L.bz),1.5),V=bt(((j=d.xray_history_1h)!=null?j:[]).map(L=>Math.log10(L.flux+1e-9)),.15),G=(Z=d.kp_history_1h)!=null?Z:[],F=G.length?ht(G[G.length-1].t_utc):null,K=F?`Recent history \xB7 Last step ${F}`:"Recent history",J=gt(t),S=(tt=d.kp_latest)!=null?tt:0,A=S>=5,X=A?`linear-gradient(160deg, #0d2a1a 0%, ${h.bg}22 75%)`:`${h.bg}18`,O=(L,R,D,nt,et)=>{let at=et?`<span class="hw-trend">${et}</span>`:"";return`<div class="hw-kpi-item${o===L?" hw-kpi-active":""}" data-kpi="${L}">
    ...
    </div>`;let $=320,x=38,b=14,v=x+b,_=$/d,H=S=>x-Math.max(2,Math.min(x-2,S/9*(x-2))),M="",C=l.map(S=>S.kp),y=Ie(C);l.forEach((S,A)=>{let X=H(S.kp),O=x-X,N=A*_,q=N+_/2,I=S.kp>=6?"#e05c5c":S.kp>=5?"#e0a84a":S.kp>=4?"#d4cc5c":"#5cce8c",B=`Kp ${S.kp.toFixed(1)} \xB7 ${ht(S.t_utc)}`;if(M+=`<rect x="${N.toFixed(1)}" y="${X.toFixed(1)}" width="${(_-1.5).toFixed(1)}" height="${O.toFixed(1)}" fill="${I}" fill-opacity="0.85" rx="1.5"/>`,M+=`<rect x="${N.toFixed(1)}" y="0" width="${_.toFixed(1)}" height="${x}" fill="transparent"><title>${W(B)}</title></rect>`,d<=8||A%2===0){let P=new Date(S.t_utc).getHours();M+=`<text x="${q.toFixed(1)}" y="${(v-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${P.toString().padStart(2,"0")}</text>`}});let E=`<polyline points="${l.map((S,A)=>{let X=A*_+_/2,O=H(y[A]);return`${X.toFixed(1)},${O.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,T="";if(s>0&&d>0){let S=Math.min($-1,s/(d*3)*$);T=`
