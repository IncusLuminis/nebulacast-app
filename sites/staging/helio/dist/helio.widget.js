"use strict";var HelioWidgetModule=(()=>{var ht=Object.defineProperty;var Ut=Object.getOwnPropertyDescriptor;var Yt=Object.getOwnPropertyNames;var qt=Object.prototype.hasOwnProperty;var Vt=(t,s)=>{for(var e in s)ht(t,e,{get:s[e],enumerable:!0})},Zt=(t,s,e,o)=>{if(s&&typeof s=="object"||typeof s=="function")for(let n of Yt(s))!qt.call(t,n)&&n!==e&&ht(t,n,{get:()=>s[n],enumerable:!(o=Ut(s,n))||o.enumerable});return t};var Qt=t=>Zt(ht({},"__esModule",{value:!0}),t);var ss={};Vt(ss,{HelioWidget:()=>Dt});var bt={quiet:{bg:"#1a2e22",accent:"#5cce8c"},active:{bg:"#2e2a1a",accent:"#d4cc5c"},elevated:{bg:"#2e1f10",accent:"#e0a84a"},storm:{bg:"#2e1212",accent:"#e05c5c"}},Jt={none:"#666",low:"#5cce8c",moderate:"#e0a84a",high:"#e05c5c"},te={info:"#666",watch:"#d4cc5c",warning:"#e05c5c"},mt={A:"#888",B:"#5cce8c",C:"#aad47a",M:"#e0a84a",X:"#e05c5c"},ee={low:"#5cce8c",moderate:"#d4cc5c",high:"#e05c5c",unknown:"#96a8b8"},se={detected:"Detected",inbound:"Inbound",arrival_window:"Arriving",arrived:"Arrived"};function oe(t){if(!t)return"Update time unavailable";try{let s=Math.round((Date.now()-new Date(t).getTime())/6e4);if(s<1)return"Updated just now";if(s<60)return`Updated ${s} min ago`;let e=Math.floor(s/60);return e<24?`Updated ${e}h ago`:`Updated ${Math.floor(e/24)}d ago`}catch(s){return"Updated recently"}}function ne(t){if(!t)return"\u2014";try{return new Date(t).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return t}}function vt(t){if(!t)return"";try{return new Date(t).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return t}}function rt(t){try{return new Date(t).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return t.slice(11,16)}}function yt(t){if(!t)return"\u2014";try{return new Date(t).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return t}}function Y(t){return t.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function u(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ae(t){return parseInt(t.slice(1),10)>0}var re=`
...
`,kt=!1;function ie(){if(kt)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=re,document.head.appendChild(t),kt=!0}function le(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,n=s/o,c=t.map((r,i)=>{let a=Math.max(2,Math.min(e,r.kp/9*e)),l=e-a,p=i*n,d=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${p.toFixed(1)}" y="${l.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${d}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${u(rt(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${c}</svg>`}function _t(t,s,e,o,n){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,r=Math.min(...t),i=Math.max(...t),a=i-r||1,l=w=>o-2-(w-r)/a*(o-4),p=t.map((w,g)=>`${(g/(t.length-1)*c).toFixed(1)},${l(w).toFixed(1)}`).join(" "),d="";if(n&&r<0&&i>0){let w=l(0);d=`<line x1="0" y1="${w.toFixed(1)}" x2="${c}" y2="${w.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((w,g)=>`<rect x="${(g/(t.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${u(s[g]||"")} \xB7 ${w.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${d}
    <polyline points="${p}" fill="none" stroke="${e}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
    ${h}
  </svg>`}function ce(t){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.map(p=>Math.max(-9,Math.min(-3,Math.log10(p.flux)))),n=Math.min(...o),r=Math.max(...o)-n||1,i=p=>e-2-(p-n)/r*(e-4),a=o.map((p,d)=>`${(d/(o.length-1)*s).toFixed(1)},${i(p).toFixed(1)}`).join(" "),l=t.map((p,d)=>{let h=d/(o.length-1)*s,w=p.flux>=1e-4?"X":p.flux>=1e-5?"M":p.flux>=1e-6?"C":p.flux>=1e-7?"B":"A";return`<rect x="${(h-4).toFixed(1)}" y="0" width="8" height="${e}" fill="transparent"><title>${u(rt(p.t_utc))} \xB7 ${w}-class (${p.flux.toExponential(2)})</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function de(t){var l;let s=(l=t.metrics.wind_history_1h)!=null?l:[],e=s[s.length-1],o=t.metrics.solar_wind_kms,n=o!=null?`${Math.round(o)} km/s`:"\u2014",c=o!=null?o>=700?"#e05c5c":o>=500?"#e0a84a":o>=400?"#d4cc5c":"#5cce8c":"#607880",r=(e==null?void 0:e.density)!=null?`${e.density.toFixed(2)} cm\u207B\xB3`:"\u2014",i=(e==null?void 0:e.temp_kk)!=null?`${e.temp_kk.toFixed(0)} kK`:"\u2014",a=(e==null?void 0:e.pressure_npa)!=null?`${e.pressure_npa.toFixed(2)} nPa`:"\u2014";return`<div class="hw-kpi-popover">
    ...
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function he(t){let a='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',l=[-10,-5,5,10].map(v=>{let y=100+v/20*100;return`<line x1="${y.toFixed(1)}" y1="16" x2="${y.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),p='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(t==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${a}${l}${p}</svg>`;let d=t<=-10?"#e05c5c":t<=-5?"#e0a84a":t<0?"#d4b84a":t>=5?"#5cce8c":"#7acca8",h=Math.max(-20,Math.min(20,t)),w=100+h/20*100,g=3,m=h<0?w-g:100-g,k=Math.max(2*g,Math.abs(w-100)+2*g),x=`<rect x="${m.toFixed(1)}" y="16" width="${k.toFixed(1)}" height="6" rx="${g}" fill="${d}" opacity="0.82"/>`,f=5,b=15,$=b-f*1.1,L=`<polygon points="${w.toFixed(1)},${b.toFixed(1)} ${(w-f).toFixed(1)},${$.toFixed(1)} ${(w+f).toFixed(1)},${$.toFixed(1)}" fill="${d}"/>`,H=`<line x1="${w.toFixed(1)}" y1="${b.toFixed(1)}" x2="${w.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${d}" stroke-width="1" opacity="0.6"/>`,z=`<text x="${w.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${d}" font-weight="600">${t>=0?"+":""}${t.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${a}${l}${p}${x}${L}${H}${z}
  </svg>`}function ue(t){var h;let s=t.metrics.imf_bz_nt,e=t.metrics.imf_bt_nt,o=t.metrics.solar_wind_kms,n=(h=t.metrics.pressure_npa)!=null?h:null,c=s!=null?s<=-10?"#e05c5c":s<=-5?"#e0a84a":s>=5?"#5cce8c":"#a0b4b8":"#607880",r=s!=null?(s>=0?"+":"")+s.toFixed(1)+" nT":"\u2014",i=e!=null?e.toFixed(1)+" nT":"\u2014",a=o!=null?`${Math.round(o)} km/s`:"\u2014",l=n!=null?`${n.toFixed(2)} nPa`:"\u2014",p=gt(t),d=s!=null&&s<-5?{msg:"Southward IMF \xB7 Aurora favorable",color:"#5cce8c"}:s!=null&&s<0?{msg:"Weakly southward \xB7 Conditions may improve",color:"#d4cc5c"}:{msg:"Northward IMF \xB7 Stable magnetosphere",color:"#96a8b8"};return`<div class="hw-kpi-popover">
    ...
    <div style="font-size:.65em;color:#607880">Coupling: <span style="color:${p.color};font-weight:600">${u(p.coupling)}</span> \xB7 Trend history: \u25B6 Details</div>
  </div>`}function gt(t){var a,l;let s=t.metrics.imf_bz_nt,e=(a=t.metrics.kp_latest)!=null?a:0,o=(l=t.metrics.solar_wind_kms)!=null?l:0,n,c,r;if(s!=null&&s<-5||e>=6)n="storm",c="#e05c5c",r="Storm conditions";else if(s!=null&&s<0||e>=4||o>=400){let p=s!=null&&s<0;n="active",c="#e0a84a",r=p?"Active coupling":"Elevated"}else n="stable",c="#5cce8c",r="Stable";let i;return s==null?i="Unknown":s>2?i="Closed":s>0?i="Minimal":s>-5?i="Moderate":s>-10?i="Strong":i="Very strong",{state:n,color:c,label:r,coupling:i}}function Ct(t,s,e,o){let n=o?"mc":"mf",c=t.color,r=e!=null?e:0,i=r>500,a=r<350,l=i?.9:a?1.8:1.3;if(o){let g=45-(t.state==="storm"?11:t.state==="active"?16:21),m=t.state==="storm"?12:t.state==="active"?10:8,k=50-m,x=76,f=[`M ${g},25`,`C ${g-2},15 41,${m} 45,${m}`,`C 53,${m} ${x-8},${m+4} ${x},20`,`C ${x+1},23 ${x+1},27 ${x},30`,`C ${x-8},${k-4} 53,${k} 45,${k}`,`C 41,${k} ${g-2},35 ${g},25`,"Z"].join(" "),b=i?3:2,$=[14,25,36],L=v=>`<path d="M 0,${v} L ${i?8:6},${v} M ${i?6:4},${v-2} L ${i?8:6},${v} L ${i?6:4},${v+2}" stroke="${c}bb" stroke-width="${i?1.4:1}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`,H=$.map(v=>L(v)).join(""),M=Array.from({length:b},(v,y)=>`<g class="hw-wg" style="animation-duration:${l}s;animation-delay:${(l/b*y).toFixed(2)}s">${H}</g>`).join(""),z=s==null?"":s>0?`<path d="M 45,27 L 45,23 M ${45-1.5},${25-.5} L 45,23 L ${45+1.5},${25-.5}" stroke="#5cce8c" stroke-width="1" fill="none" stroke-linecap="round"/>`:`<path d="M 45,23 L 45,27 M ${45-1.5},${25+.5} L 45,27 L ${45+1.5},${25+.5}" stroke="#e05c5c" stroke-width="1" fill="none" stroke-linecap="round"/>`;return`<svg viewBox="0 0 80 50" style="width:78px;height:49px;display:block" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${f+2}" y="${b-2}" font-size="7" fill="${c}" opacity=".8" font-family="sans-serif">${u(t.label)}</text>
    ...
    <div class="hw-kpi-stat-row">
      ...
      <div class="hw-kpi-stat">
        ...
        <span class="hw-kpi-stat-value" style="color:${d}">${u(i)}</span>
      </div>
      <div class="hw-kpi-stat">
  ...
  </div>`}function Lt(t,s,e){if(!t.length)return null;let o=(e%360+360)%360,n=-1,c=1/0,r=Math.cos(s*Math.PI/180);for(let i of t){let a=i.lat-s,l=(i.lon-o+180+360)%360-180,p=a*a+l*r*(l*r);p<c&&(c=p,n=i.prob)}return n>=0?n:null}function Ht(t){return`<svg viewBox="0 0 100 100" width="100%" height="100%"
    style="position:absolute;top:0;left:0;pointer-events:none">${['<text x="50" y="4"   font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">N</text>','<text x="96"  y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">W</text>','<text x="50" y="97"  font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">S</text>','<text x="4"   y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">E</text>'].join("")}</svg>`}function ge(t,s){let e=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,o=null;s&&t.lat!=null&&t.lon!=null&&(o=Lt(s.entries,t.lat,t.lon));let n=t.lat!=null&&t.lon!=null,c=o!=null?o>=30?"#5cce8c":o>=10?"#d4cc5c":"#9ab4bc":"#607880",r=o!=null?`${o}%`:s?"n/a":"\u2026",i=n?`
  ...
  </div>`}function we(t,s,e,o){switch(s){case"solar_wind":return de(t);case"xray":return pe(t);case"imf_bz":return ue(t);case"aurora":return ge(e,o);case"magnetosphere":return me(t);default:return""}}function nt(t,s){if(t.length<2)return"\u2192";let e=t[t.length-1],o=Math.max(0,t.length-4),n=t[o];if(!isFinite(e)||!isFinite(n))return"\u2192";let c=e-n;return c>s?"\u2191":c<-s?"\u2193":"\u2192"}function xe(t,s,e,o,n,c,r){var A,E,O,B,G,U,V;let{summary:i,scales:a,metrics:l,aurora_hint:p}=t,d=(A=bt[i.status])!=null?A:bt.quiet,h=n!=null?n.kp.toFixed(1):l.kp_latest!=null?l.kp_latest.toFixed(1):"\u2014",g=[n?n.gScale:a.g_scale,a.r_scale,a.s_scale].map(W=>{let X=ae(W),q=X?`color:${d.accent};border-color:${d.accent}33`:"";return`<span class="hw-scale-chip${X?" hw-scale-active":""}" style="${q}">${u(W)}</span>`}).join(""),m=n?n.auroraLabel:p.aurora_label,k=m==="good"?"#5cce8c":m==="possible"?"#d4cc5c":"#607880",x=m.charAt(0).toUpperCase()+m.slice(1),f="#b4c6cc",b=l.solar_wind_kms!=null?`${Math.round(l.solar_wind_kms)} km/s`:"\u2014",$=l.imf_bz_nt,L=$!=null?$<=-10?"#e05c5c":$<=-5?"#e0a84a":$>=5?"#5cce8c":"#a0b4b8":"#607880",H=$!=null?($>=0?"+":"")+$.toFixed(1)+" nT":"\u2014",M=l.xray_class,z=M?(E=mt[M])!=null?E:"#a0b4b8":"#607880",v=M?`${M}-class`:"\u2014",y=nt(((O=l.kp_history_1h)!=null?O:[]).map(W=>W.kp),.5),F=nt(((B=l.wind_history_1h)!=null?B:[]).map(W=>W.kms),20),C=nt(((G=l.bz_history_1h)!=null?G:[]).map(W=>W.bz),1.5),N=nt(((U=l.xray_history_1h)!=null?U:[]).map(W=>Math.log10(W.flux+1e-9)),.15),R=s?"\u25BC HISTORY":"\u25B6 HISTORY",D=gt(t),I=(V=l.kp_latest)!=null?V:0,P=I>=5,S=P?`linear-gradient(160deg, #0d2a1a 0%, ${d.bg}22 75%)`:`${d.bg}18`,T=(W,X,q,wt,J)=>{let tt=J?`<span class="hw-trend">${J}</span>`:"";return`<div class="hw-kpi-item${o===W?" hw-kpi-active":""}" data-kpi="${W}">
    ...
    <div class="hw-hero" style="background:${S}">
      <div class="hw-hero-main">
        <div class="hw-kp-col">
          ...
          <span class="hw-status-badge" style="background:${d.accent}22;color:${d.accent};display:block;text-align:center">${u(i.label)}</span>
        ...
        <div class="hw-info-col">
          <div class="hw-info-top-row">
            ...
            <div class="hw-magnet-mini${o==="magnetosphere"?" hw-kpi-active":""}" data-kpi="magnetosphere" title="Magnetosphere status">
              ${Ct(D,$,l.solar_wind_kms,!0)}
              <div class="hw-magnet-state" style="color:${D.color}">${u(D.label)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="hw-quick-details">
        ...
        ${T("solar_wind","Solar wind",u(b),f,F)}
        ${T("imf_bz","IMF Bz",u(H),L,C)}
        ${T("xray","X-ray",u(v),z,N)}
      </div>
      ${o?we(t,o,c,r):""}
      ...
      ${_}
    </div>`}function $e(t){var p,d,h,w;let{metrics:s}=t,e=(p=s.kp_history_1h)!=null?p:[],o=(d=s.wind_history_1h)!=null?d:[],n=(h=s.bz_history_1h)!=null?h:[],c=(w=s.xray_history_1h)!=null?w:[],r=le(e),i=_t(o.map(g=>{var m;return(m=g.kms)!=null?m:0}).filter(g=>g>0),o.map(g=>rt(g.t_utc)),"#5cce8c",28,!1),a=_t(n.map(g=>g.bz),n.map(g=>rt(g.t_utc)),"#d4cc5c",28,!0),l=ce(c);return`
    ...
    </div>`;let k=320,x=38,f=14,b=x+f,$=k/p,L=S=>x-Math.max(2,Math.min(x-2,S/9*(x-2))),H="",M=l.map(S=>S.kp),z=be(M);l.forEach((S,T)=>{let _=L(S.kp),A=x-_,E=T*$,O=E+$/2,B=S.kp>=6?"#e05c5c":S.kp>=5?"#e0a84a":S.kp>=4?"#d4cc5c":"#5cce8c",G=`Kp ${S.kp.toFixed(1)} \xB7 ${vt(S.t_utc)}`;if(H+=`<rect x="${E.toFixed(1)}" y="${_.toFixed(1)}" width="${($-1.5).toFixed(1)}" height="${A.toFixed(1)}" fill="${B}" fill-opacity="0.85" rx="1.5"/>`,H+=`<rect x="${E.toFixed(1)}" y="0" width="${$.toFixed(1)}" height="${x}" fill="transparent"><title>${Y(G)}</title></rect>`,p<=8||T%2===0){let V=new Date(S.t_utc).getHours();H+=`<text x="${O.toFixed(1)}" y="${(b-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${V.toString().padStart(2,"0")}</text>`}});let y=`<polyline points="${l.map((S,T)=>{let _=T*$+$/2,A=L(z[T]);return`${_.toFixed(1)},${A.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,F="";if(s>0&&p>0){let S=Math.min(k-1,s/(p*3)*k);F=`
      <line x1="${S.toFixed(1)}" y1="0" x2="${S.toFixed(1)}" y2="${x}" stroke="rgba(255,255,255,0.75)" stroke-width="1.5" stroke-dasharray="3,2"/>
      <polygon points="${S.toFixed(1)},${x} ${(S-4).toFixed(1)},${(x-7).toFixed(1)} ${(S+4).toFixed(1)},${(x-7).toFixed(1)}" fill="rgba(255,255,255,0.75)"/>`}let C=Math.round(d/4),N=Math.round(d/2),R=Math.round(d*3/4),D=`
    ...
    <div class="hw-forecast">
      ...
      ${D}`:""}
    </div>`}function Me(t){let s=/([NS])(\d+)([EW])(\d+)/i.exec(t);return s?{lat:(s[1].toUpperCase()==="N"?1:-1)*parseInt(s[2],10),lon:(s[3].toUpperCase()==="E"?1:-1)*parseInt(s[4],10)}:null}var Ft=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function Ce(t){return t.x_flare_probability>0?"X":t.m_flare_probability>0?"M":t.c_flare_probability>0?"C":"quiet"}function Le(t,s,e){let o=s/2,n=o*.87,c=s*.03,r=s*.009,i=t.map(a=>{var x,f;let l=Me(a.location);if(!l||Math.abs(l.lon)>88||a.location.includes("*"))return"";let p=Ce(a);if(!e.has(p))return"";let d=Ft.find(b=>b.id===p).color,h=l.lat*Math.PI/180,w=l.lon*Math.PI/180,g=(o+n*Math.cos(h)*Math.sin(w)).toFixed(1),m=(o-n*Math.sin(h)).toFixed(1),k=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}var Tt={g1:"#d4cc5c",g2:"#e0a84a",g3:"#e05c5c"};function At(t){var i;let s=(i=t.metrics.kp_forecast_3h)!=null?i:[],e=Date.now(),o=e+24*60*60*1e3,n=s.filter(a=>{let l=new Date(a.t_utc).getTime();return l>=e-3*60*60*1e3&&l<=o});if(n.length===0)return{g1:0,g2:0,g3:0};let c=Math.max(...n.map(a=>a.kp)),r=a=>{if(c<a-.7)return 0;if(c>a+1)return 90;let l=(c-(a-.7))/1.7;return Math.round(Math.pow(Math.max(0,l),.7)*90)};return{g1:r(5),g2:r(6),g3:r(7)}}var St={rising:"#e0884a",peak:"#e05c5c",decline:"#d4cc5c"};function Ee(t){var l,p,d;let s=(l=t.metrics.kp_latest)!=null?l:0,e=t.metrics.imf_bz_nt,o=t.metrics.solar_wind_kms,n=parseInt(((p=t.scales.g_scale)!=null?p:"G0").replace("G",""),10)||0,c=s>=5||n>=1,r=(d=t.metrics.kp_history_1h)!=null?d:[],i=0;if(r.length>=2&&(i=r[r.length-1].kp-r[r.length-2].kp),!c)return{active:!1,phase:"quiet",kp_current:s,kp_trend:i,bz_nt:e,solar_wind_kms:o};let a;return i>.3&&(e==null||e<-5)?a="rising":i<-.5?a="decline":a="peak",{active:!0,phase:a,kp_current:s,kp_trend:i,bz_nt:e,solar_wind_kms:o}}function Oe(t){if(!t.active)return"";let s=[{key:"rising",label:"Rising"},{key:"peak",label:"Peak"},{key:"decline",label:"Decline"}],e=s.findIndex(i=>i.key===t.phase),o=St[t.phase],n=s[e].label,c=s.map((i,a)=>{let l=a===e,p=a<e,d=St[i.key],h=l?`background:${d};border-color:${d};box-shadow:0 0 6px ${d}88`:p?`background:${d}44;border-color:${d}66`:"background:#111b1e;border-color:#1e2c30",w=l?`color:${d};font-weight:700`:p?`color:${d}66`:"color:#2e4248",g=a<s.length-1?`<div class="hw-spi-arr">${p?`<span style="color:${d}55">\u2192</span>`:"\u2192"}</div>`:"";return`<div class="hw-spi-node">
        <div class="hw-spi-dot" style="${h}"></div>
        <div class="hw-spi-txt" style="${w}">${i.label}</div>
      </div>${g}`}).join(""),r=[`Kp ${t.kp_current.toFixed(1)}`];return t.bz_nt!=null&&r.push(`Bz ${t.bz_nt>0?"+":""}${t.bz_nt.toFixed(1)} nT`),t.solar_wind_kms!=null&&r.push(`Wind ${Math.round(t.solar_wind_kms)} km/s`),`<div class="hw-spi-wrap">
  ...
  </div>`}function Re(t,s){let e=At(t),o=Ee(t),n=(()=>{var w;let l=(w=t.metrics.kp_forecast_3h)!=null?w:[],p=Date.now(),d=p+24*60*60*1e3,h=l.filter(g=>new Date(g.t_utc).getTime()<=d);return h.length?Math.max(...h.map(g=>g.kp)):null})(),r=[{key:"g1",label:"G1"},{key:"g2",label:"G2"},{key:"g3",label:"G3"}].map(({key:l,label:p})=>{let d=e[l],h=Tt[l];return`<div class="hw-gstorm-row">
    ...
    </div>`}).join(""),i=n!=null?`Max Kp forecast 24h: <b style="color:#b4c6cc">${n.toFixed(1)}</b>`:"";return`<div class="hw-impact-tip${s?" hw-impact-tip-open":""}">
    ${Oe(o)}
    <div class="hw-gstorm-header">Storm probability \xB7 next 24h</div>
    <div class="hw-gstorm-rows">${r}</div>
    ${i?`<div class="hw-gstorm-footer">${i} \xB7 derived from Kp forecast</div>`:""}
  </div>`}var at={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Et={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function Ie(t){var v;let s=at,e=(v=Et[s.phase])!=null?v:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,c=52,r=10,i=c-6,a=c-18,l=.5,p=.19,d=y=>Math.exp(-Math.pow((y-l)/p,2)/2),h=y=>r+y*(n-2*r),w=y=>i-d(y)*a,g=80,m=[];for(let y=0;y<=g;y++){let F=y/g;m.push(`${y===0?"M":"L"}${h(F).toFixed(1)},${w(F).toFixed(1)}`)}let k=Math.round(s.progress_0_1*g),x=[];for(let y=0;y<=k;y++){let F=y/g;x.push(`${y===0?"M":"L"}${h(F).toFixed(1)},${w(F).toFixed(1)}`)}let f=h(s.progress_0_1),b=[`M${r},${i}`,...x.slice(1),`L${f.toFixed(1)},${i} Z`],$=w(s.progress_0_1),L=5,H=`M${f.toFixed(1)},${$.toFixed(1)} L${(f-L).toFixed(1)},${($-L*1.8).toFixed(1)} L${(f+L).toFixed(1)},${($-L*1.8).toFixed(1)} Z`,M=i+11;return`<div class="hw-impact-tip${t?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function Ot(t){var a,l,p,d,h;let s=(l=(a=t.coronal_hole)==null?void 0:a.estimated_speed_kms)!=null?l:t.metrics.solar_wind_kms,e=(p=t.coronal_hole)==null?void 0:p.status,o=s!=null?s:0,n=e!=null?e:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",c={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},r={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},i={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:n,color:c[n],label:r[n],desc:(h=(d=t.coronal_hole)==null?void 0:d.note)!=null?h:i[n],speed:s}}function Pe(t,s){var S;let e=Ot(t),o=(S=e.speed)!=null?S:0,n=e.speed!=null?`${Math.round(e.speed)} km/s`:"\u2014",c=s?" hw-impact-tip-open":"",r=320,i=72,a=36,l=36,p=284,d=36,h=11,w=14,g=19,m=9,k=o>=500,x=o>=420,f=k?"#f5c540":x?"#c8a020":"#7a6010",b=k?"#f5c540":x?"#c8a020":"#3a3808",$=e.color,L=x?"0.9":"0.25",H=k?"0.18":x?"0.10":"0.04",M=[0,45,90,135,180,225,270,315].map(T=>{let _=T*Math.PI/180,A=(a+w*Math.cos(_)).toFixed(1),E=(l+w*Math.sin(_)).toFixed(1),O=(a+g*Math.cos(_)).toFixed(1),B=(l+g*Math.sin(_)).toFixed(1),G=T>300||T<60?"0.9":"0.5";return`<line x1="${A}" y1="${E}" x2="${O}" y2="${B}"
      stroke="${f}" stroke-width="1.6" stroke-linecap="round" opacity="${G}"/>`}).join(""),z=a+h+2,v=p-m-3,y=14,F=`${z},${l} ${v},${d-y} ${v},${d+y}`,C=v+1,N=`${C},${d-4} ${C+7},${d} ${C},${d+4}`,R=m,D=`
    ...
    <line x1="${p-R}" y1="${d}" x2="${p+R}" y2="${d}"
          stroke="#4a8ab0" stroke-width="0.8" opacity="0.35"/>`,I=`<svg class="hw-hss-diagram" width="100%" height="${i}"
      viewBox="0 0 ${r} ${i}" preserveAspectRatio="none" aria-hidden="true">
    <!-- stream fan -->
    <polygon points="${F}" fill="${$}" opacity="${H}"/>
    <!-- dashed stream axis -->
    ...
    ${D}
    <!-- labels -->
    <text x="${a}" y="${i-4}" font-size="8" fill="#607880"
          text-anchor="middle" font-family="inherit">Sun</text>
    <text x="${p}" y="${i-4}" font-size="8" fill="#607880"
    ...
    ${P}
  </div>`}function Rt(t){var n,c;let s=(n=t.scales.g_scale)!=null?n:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let r=(c=t.metrics.kp_forecast_3h)!=null?c:[],i=Date.now(),a=r.filter(l=>new Date(l.t_utc).getTime()<=i+3*60*60*1e3).sort((l,p)=>new Date(p.t_utc).getTime()-new Date(l.t_utc).getTime());a.length>0&&(o=a[0].kp)}return e>=2||o!=null&&o>=6?{level:2,color:"#e05c5c",label:"High",kp:o,gScale:s}:e>=1||o!=null&&o>=4?{level:1,color:"#d4cc5c",label:"Moderate",kp:o,gScale:s}:{level:0,color:"#5cce8c",label:"Low",kp:o,gScale:s}}function Ne(t,s){let e=Rt(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33,desc:"Normal density"},{l:1,label:"Moderate",color:"#d4cc5c",width:64,desc:"Elevated density"},{l:2,label:"High",color:"#e05c5c",width:100,desc:"Strong expansion"}].map(a=>{let l=a.l===e.level,p=l?a.color:"#566068",d=l?"0.88":"0.16";return`<div class="hw-satdrag-rung">
    ...
    </div>`}).join(""),r=e.kp!=null?`Kp ${e.kp.toFixed(1)}`:"Kp \u2014",i={0:"Near-normal thermospheric density",1:"Elevated drag \u2014 minor orbit correction may be needed",2:"Strong thermospheric expansion \u2014 significant drag increase"};return`<div class="hw-impact-tip${o}">
    <div class="hw-satdrag-ladder">${c}</div>
    <div class="hw-satdrag-meta">${r} \xB7 ${u(e.gScale)} \xB7 ${i[e.level]}</div>
  </div>`}function It(t){var l,p,d;let s=(l=t.scales.g_scale)!=null?l:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let h=(p=t.metrics.kp_forecast_3h)!=null?p:[],w=Date.now(),g=h.filter(m=>new Date(m.t_utc).getTime()<=w+3*60*60*1e3).sort((m,k)=>new Date(k.t_utc).getTime()-new Date(m.t_utc).getTime());g.length>0&&(o=g[0].kp)}let n=0;e>=2||o!=null&&o>=6?n=2:(e>=1||o!=null&&o>=4)&&(n=1);let c=parseInt(((d=t.scales.r_scale)!=null?d:"R0").slice(1),10),r=c>=2&&n<2;c>=2&&(n=Math.min(2,n+1));let i={0:"#5cce8c",1:"#d4cc5c",2:"#e05c5c"},a={0:"Low",1:"Moderate",2:"High"};return{level:n,color:i[n],label:a[n],kp:o,gScale:s,boostedByFlare:r}}function De(t,s){var l;let e=It(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33},{l:1,label:"Moderate",color:"#d4cc5c",width:64},{l:2,label:"High",color:"#e05c5c",width:100}].map(p=>{let d=p.l===e.level,h=d?p.color:"#566068",w=d?"0.88":"0.16";return`<div class="hw-gnss-rung">
    ...
    </div>`}).join(""),r=e.kp!=null?`Kp ${e.kp.toFixed(1)}`:"Kp \u2014",i={0:"Stable ionosphere \xB7 normal positioning accuracy",1:"Possible signal delay or scintillation",2:"Significant positioning errors \xB7 possible signal loss"},a=e.boostedByFlare?`<div class="hw-gnss-meta" style="font-size:.72em">Risk elevated by solar flare activity (R${parseInt(((l=t.scales.r_scale)!=null?l:"R0").slice(1),10)})</div>`:"";return`<div class="hw-impact-tip${o}">
    <div class="hw-gnss-ladder">${c}</div>
    <div class="hw-gnss-meta">${r} \xB7 ${u(e.gScale)} \xB7 ${i[e.level]}</div>
    ${a}
  </div>`}function Pt(t){var n;let s=t.metrics.pressure_npa,e=s!=null?s:null,o=(n=t.metrics.density)!=null?n:null;return e==null?{pressure:null,color:"#607880",label:"\u2014",density:o}:e>=6?{pressure:e,color:"#e05c5c",label:"Extreme",density:o}:e>=4?{pressure:e,color:"#e0a84a",label:"Strong",density:o}:e>=2?{pressure:e,color:"#d4cc5c",label:"Elevated",density:o}:e>=1?{pressure:e,color:"#5cce8c",label:"Typical",density:o}:{pressure:e,color:"#7a9298",label:"Weak",density:o}}function Be(t,s){let e=Pt(t),o=s?" hw-impact-tip-open":"",n=e.pressure,c=200,r=6,i=10,a=r+i,l=a+4,p=l+11,d=a+9,h=p+4,g=[{x:0,w:50,color:"#5cce8c"},{x:50,w:50,color:"#d4cc5c"},{x:100,w:50,color:"#e0a84a"},{x:150,w:50,color:"#e05c5c"}].map(v=>`<rect x="${v.x}" y="${r}" width="${v.w}" height="${i}" fill="${v.color}" opacity="0.55" rx="0"/>`).join(""),m=[{x:0,label:"0",anchor:"start"},{x:50,label:"2",anchor:"middle"},{x:100,label:"4",anchor:"middle"},{x:150,label:"6",anchor:"middle"},{x:200,label:"8+",anchor:"end"}],k=m.map(v=>`<line x1="${v.x}" y1="${a}" x2="${v.x}" y2="${l}" stroke="#3a5058" stroke-width="1"/>`).join(""),x=m.map(v=>`<text x="${v.x}" y="${p}" class="hw-swdp-axlabel" text-anchor="${v.anchor}">${v.label}</text>`).join(""),f="";if(n!=null){let y=Math.min(Math.max(n,0),8)/8*c;f=`<polygon points="${`${y-5},${d} ${y+5},${d} ${y},${a}`}" fill="${e.color}" opacity="0.95"/>
    <line x1="${y}" y1="${r}" x2="${y}" y2="${a}" stroke="${e.color}" stroke-width="1.5" opacity="0.7"/>`}let b=`<rect x="0" y="${r}" width="${c}" height="${i}" fill="none" stroke="#2a3c42" stroke-width="0.8" rx="0"/>`,$=`<svg class="hw-swdp-gauge" viewBox="0 0 ${c} ${h}" preserveAspectRatio="none" aria-hidden="true">
    ${g}${b}${f}${k}${x}
  ...
  </div>`}function Nt(t){var l,p;let s=(l=t.alerts_all)!=null?l:[],e=s.find(d=>d.kind==="cme_impact"),o=s.find(d=>d.kind==="cme_watch"),n=e!=null?e:o;if(!n)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let c=((p=n.raw_body)!=null?p:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),r=c?parseInt(c[1],10):null,i=null;if(r&&n.t_utc){let d=1496e5/r*1e3;i=new Date(new Date(n.t_utc).getTime()+d).toISOString().replace(".000Z","Z")}let a=e?"impact":"watch";return{status:a,color:a==="impact"?"#e05c5c":"#d4cc5c",label:a==="impact"?"Active":"Watch",speed_kms:r,issued_utc:n.t_utc,arrival_utc:i}}function je(t,s){let e=Nt(t),o=s?" hw-impact-tip-open":"",n=320,c=80,r=24,i=40,a=268,l=296,p=a-r,d=_=>Math.tan(_*Math.PI/180),h=Math.round(d(9)*p),w=Math.round(d(6)*p),g=Math.round(d(3)*p),m=_=>`${r},${i} ${a},${i-_} ${a},${i+_}`,k=e.status==="impact"?i:e.status==="watch"?i+w+8:i,x=Math.min(c-14,Math.max(14,k)),f=Math.abs(x-i)<=g,b=Math.abs(x-i)<=w,$=Math.abs(x-i)<=h,L=f?"#e05c5c":b?"#d4cc5c":$?"#e0a84a":"#5cce8c",H=e.status!=="quiet"?`<polygon points="${m(h)}" fill="#253238" opacity="0.85"/>
       <polygon points="${m(w)}"   fill="#d4cc5c" opacity="0.14"/>
       <polygon points="${m(g)}" fill="#e0a84a" opacity="0.28"/>
       <line x1="${r+9}" y1="${i}" x2="${a-2}" y2="${i}"
             stroke="#3a5058" stroke-dasharray="3 3" stroke-width="1"/>`:`<line x1="${r+9}" y1="${i}" x2="${l-9}" y2="${i}"
             stroke="#1e2c30" stroke-dasharray="4 3" stroke-width="1"/>`,M=8,z=11,v=15,y="#f5c540",F=[0,45,90,135,180,225,270,315].map(_=>{let A=_*Math.PI/180,E=(r+z*Math.cos(A)).toFixed(1),O=(i+z*Math.sin(A)).toFixed(1),B=(r+v*Math.cos(A)).toFixed(1),G=(i+v*Math.sin(A)).toFixed(1),U=_<45||_>315?"0.9":"0.5";return`<line x1="${E}" y1="${O}" x2="${B}" y2="${G}"
    ...
    <line x1="${l}" y1="${x-C}" x2="${l}" y2="${x+C}"
          stroke="#4a8ab0" stroke-width="0.8" opacity="0.6"/>`,R="\u2014";if(e.arrival_utc){let _=new Date(e.arrival_utc),A=_.toLocaleString("en-US",{month:"short",timeZone:"UTC"}),E=_.getUTCDate(),O=String(_.getUTCHours()).padStart(2,"0"),B=String(_.getUTCMinutes()).padStart(2,"0");R=`~${A}\xA0${E}\xA0${O}:${B}\u202FUTC`}let D=e.speed_kms?`${e.speed_kms}\u202Fkm/s`:"\u2014",I=e.status!=="quiet"?`Velocity: <b style="color:#b4c6cc">${u(D)}</b>&ensp;Arrival: <b style="color:#b4c6cc">${u(R)}</b>`:"No Earth-directed CME in forecast window",P=f?"Direct impact likely":b?"Glancing blow possible":$?"Near outer edge":"Impact unlikely",S=f?"#e05c5c":b?"#d4cc5c":$?"#e0a84a":"#5cce8c",T=`<svg class="hw-cme-cone-svg" width="100%" height="${c}"
      viewBox="0 0 ${n} ${c}" preserveAspectRatio="none" aria-hidden="true">
    ${H}
    <!-- Sun glow -->
    <circle cx="${r}" cy="${i}" r="${M+5}" fill="none"
            stroke="${y}" stroke-width="1.2" opacity="0.25"/>
    <!-- Sun body -->
    <circle cx="${r}" cy="${i}" r="${M}" fill="${y}" opacity="0.92"/>
    ${F}
    <!-- Earth body -->
    ...
    <text x="${l}" y="${c-3}" font-size="8" fill="#607880"
          text-anchor="middle" font-family="inherit">Earth</text>
    ${e.status!=="quiet"?`<text x="${Math.round((r+l)/2)}" y="${c-3}" font-size="7.5"
               fill="${S}" text-anchor="middle" font-family="inherit">${u(P)}</text>`:""}
  </svg>`;return`<div class="hw-impact-tip${o}">
    ${T}
    <div class="hw-cme-footer">${I}</div>
  </div>`}function We(t,s,e,o,n,c,r,i,a){var tt,it,xt;let l=(it=(tt=s==null?void 0:s.impacts)!=null?tt:t.observer_impacts)!=null?it:[],p=l.map(j=>{var $t,ft;let lt=($t=Jt[j.level])!=null?$t:"#666",Bt=j.level==="none"?"None":j.level.charAt(0).toUpperCase()+j.level.slice(1),jt=(ft=He[j.kind])!=null?ft:ze,Wt=j.level==="none"?"#606870":lt,et=j.kind==="solar_activity"?n:r.has(j.kind),Kt=et?" hw-impact-open":"",Z;if(j.kind==="solar_activity"){let st=n?" hw-solar-open":"",ct=Ft.map(K=>{let ot=c.has(K.id),dt=ot?K.color+"22":"transparent",pt=ot?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${K.id}" style="color:${K.color};border-color:${K.color};background:${dt};opacity:${pt}">${K.label}</button>`}).join("");Z=`<div class="hw-solar-tip${st}">
          ...
          <span class="hw-solar-tip-text">${u(j.summary)}</span>
        </div>`}else if(j.kind==="aurora"){let st=et?" hw-aurora-tip-open":"",ct=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,K=null;i&&a.lat!=null&&a.lon!=null&&(K=Lt(i.entries,a.lat,a.lon));let ot=a.lat!=null&&a.lon!=null,dt=K!=null?K>=30?"#5cce8c":K>=10?"#d4cc5c":"#9ab4bc":"#607880",pt=K!=null?`${K}%`:i?"n/a":"\u2026",Xt=ot?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
          ...
          ${Xt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else j.kind==="radio"?Z=Ae(t,et):Z=`<div class="hw-impact-tip${et?" hw-impact-tip-open":""}">${u(j.summary)}</div>`;let Gt=j.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${Y(j.kind)}"`;return`<div class="hw-impact-row${Kt}"${Gt}>
      <span class="hw-impact-caret">\u25B6</span>
      <span class="hw-impact-kind" style="color:${Wt}">${jt}<span style="color:#b4c6cc">${u(j.label)}</span></span>
      <span class="hw-impact-badge" style="background:${lt}22;color:${lt}">${u(Bt)}</span>
      ${Z}
    ...
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,k=r.has("geomag_storm"),x=At(t),f=x.g1,b=x.g1>=30?Tt.g1:x.g1>0?"#7a9298":"#607880",$=f>0?`G1 ${f}%`:"None",H=`<div class="hw-impact-row${k?" hw-impact-open":""}" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Re(t,k)}
    </div>`,M=r.has("solar_cycle"),z=(xt=Et[at.phase])!=null?xt:"#96a8b8",v=at.phase.charAt(0).toUpperCase()+at.phase.slice(1),F=`<div class="hw-impact-row${M?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ie(M)}
    </div>`,C=Ot(t),N=r.has("hss"),D=`<div class="hw-impact-row${N?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Pe(t,N)}
    </div>`,I=Rt(t),P=r.has("sat_drag"),T=`<div class="hw-impact-row${P?" hw-impact-open":""}" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ne(t,P)}
    </div>`,_=It(t),A=r.has("gnss"),O=`<div class="hw-impact-row${A?" hw-impact-open":""}" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${De(t,A)}
    </div>`,B=Pt(t),G=r.has("sw_pressure"),U=B.pressure!=null?`${B.pressure.toFixed(2)} nPa`:"\u2014",W=`<div class="hw-impact-row${G?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
    ...
    ${r}
  </div>`}var Ge={info:"#445c64",watch:"#e0a84a",warning:"#e05c5c"},Xe="#4ae0a4";function Ue(t){let s=(e,o="")=>`<svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" ${o}>${e}</svg>`;switch(t){case"solar_flare":return s(`<circle cx="6.5" cy="6.5" r="2.5"/>
        ...
        <circle cx="6.5" cy="3.8" r=".6" fill="currentColor" stroke="none"/>`)}}function Ye(t){var o;let s=(o=t.metadata)!=null?o:{},e=[];return s.source_code&&e.push(`Code: ${s.source_code}`),s.model&&e.push(`Model: ${String(s.model).toUpperCase()}`),e.length===0?"":`
${e.join(" \xB7 ")}`}function qe(t,s,e){var a;let o=t.is_active?Xe:(a=Ge[t.level])!=null?a:"#445c64",n=t.event_time===e,c=t.event_time.slice(11,16)+" UTC",r=t.source==="NASA_DONKI"?"DONKI":"SWPC",i=n?`<div class="hw-tl-detail">${u(t.description)}${u(Ye(t))}</div>`:"";return`
    <div class="hw-tl-item" data-timeline-key="${Y(t.event_time)}">
      ...
      <div class="hw-tl-body">
        ...
        <div class="hw-tl-title${t.is_active?" hw-tl-active":""}">
          ${Ue(t.event_type)} ${u(t.event_title)}
        </div>
        ${i}
      </div>
    </div>`}function Ve(t,s,e,o){var x,f;let n=(x=t.timeline)!=null?x:[],c=Date.now(),r=new Date(c).toISOString().slice(0,10),i=new Date(c-864e5).toISOString().slice(0,10),a=new Date(c-1728e5).toISOString().slice(0,10),l=new Set([r,i,a]),p=n.filter(b=>{var $;return l.has((($=b.event_time)!=null?$:"").slice(0,10))}).slice().reverse(),d=p.length,h=e?"\u25BC":"\u25B6",w=d>0?`Solar Activity Timeline (${d})`:"Solar Activity Timeline",g=`
    ...
    </div>`;if(!e||d===0)return`<div class="hw-timeline">${g}</div>`;let m=new Map;for(let b of p){let $=((f=b.event_time)!=null?f:"").slice(0,10);m.has($)||m.set($,[]),m.get($).push(b)}let k=[...m.entries()].map(([b,$])=>{let H=new Date(b+"T12:00:00Z").toLocaleDateString("en-US",{month:"short",day:"numeric",timeZone:"UTC"}),M=o.has(b),z=M?"\u25B6":"\u25BC",v=M?`<span class="hw-tl-day-count">${$.length} events</span>`:"",y=`
      <div class="hw-tl-day-row" data-tl-day="${Y(b)}">
        <span class="hw-section-caret">${z}</span>
        <span class="hw-tl-date">${H}</span>
        ${v}
      </div>`,F=M?"":$.map((C,N)=>qe(C,N<$.length-1,s)).join("");return`<div class="hw-tl-group">${y}${F}</div>`}).join("");return`
    <div class="hw-timeline">
      ${g}
      ${k}
    </div>`}function Ze(t,s,e){var l;let o=(l=t.alerts_all)!=null?l:[],n=o.length,c=s?"\u25BC":"\u25B6",r=n>0?`SWPC Alerts (${n})`:"SWPC Alerts",i=`
    ...
    </div>`;if(!s||n===0)return`<div class="hw-alerts">${i}${s&&n===0?'<div class="hw-empty-alerts">No significant recent SWPC alerts</div>':""}</div>`;let a=o.map(p=>Ke(p,p.dedupe_key===e)).join("");return`
    <div class="hw-alerts">
      ${i}
      ${a}
    </div>`}function Qe(t,s){var R,D,I;let e=t.cme_tracker;if(!e)return"";let o=(R=ee[e.impact_level])!=null?R:"#96a8b8",n=(D=se[e.status])!=null?D:e.status,c=300,r=44,i=18,a=r/2,l=10,p=c-18,d=7,h=`<line x1="${i+l}" y1="${a}" x2="${p-d}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,w=`<circle cx="${i}" cy="${a}" r="${l}" fill="#f0c040" opacity="0.92"/>`,g=`
    <circle cx="${p}" cy="${a}" r="${d}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${p}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,m=`<text x="${i}" y="${a+l+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,k=`<text x="${p}" y="${a+d+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,x="";if(e.progress!=null){let P=i+l+4,S=p-d-4,T=P+e.progress*(S-P),_=5;e.status==="arrival_window"?x=`
        ...
        </g>`:x=`<circle cx="${T.toFixed(1)}" cy="${a}" r="${_}" fill="${o}" opacity="0.85"/>`}let f=`<svg class="hw-cme-svg" viewBox="0 0 ${c} ${r}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${h}
    ${w}${m}
    ${g}${k}
    ${x}
  </svg>`,b=yt(e.arrival_time_utc),$=yt(e.launch_time_utc),L=e.speed_kms!=null?`${Math.round(e.speed_kms)} km/s`:"\u2014",H=e.half_angle_deg!=null?`${e.half_angle_deg}\xB0`:"\u2014",M=(I=e.source_location)!=null?I:"\u2014",z=e.is_earth_direct?"Direct hit":"Glancing blow",v=e.progress!=null?`${Math.round(e.progress*100)}%`:"\u2014",y=`
    ...
    </div>`,F=s?"\u25BC":"\u25B6",C=e.impact_level==="unknown"?"Unrated":e.impact_level.charAt(0).toUpperCase()+e.impact_level.slice(1),N=s?`${f}${y}`:"";return`
    <div class="hw-cme">
      ...
      ${N}
    </div>`}function Je(t,s,e,o,n,c,r,i,a,l,p,d,h,w,g,m,k,x,f,b){let $=_e(t,n);return`
    <div class="hw-root">
      ${fe(t)}
      ${xe(t,e,w,o,$,f,b)}
      ${e?$e(t):""}
      ...
      ${Ve(t,i,a,l)}
      ${Ze(t,c,r)}
    </div>`}function ts(t){return`<div class="hw-root">
