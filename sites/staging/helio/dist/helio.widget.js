...
`,re=!1;function Le(){if(re)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Me,document.head.appendChild(e),re=!0}function Ce(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let t=200,o=32,r=e.length,n=t/r,i=e.map((s,a)=>{let l=Math.max(2,Math.min(o,s.kp/9*o)),d=o-l,c=a*n,p=s.kp>=6?"#e05c5c":s.kp>=5?"#e0a84a":s.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${c.toFixed(1)}" y="${d.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${l.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${s.kp.toFixed(1)} \xB7 ${u(R(s.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${t} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">${i}</svg>`}function U(e,t,o,r,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let i=200,s=Math.min(...e),a=Math.max(...e),l=a-s||1,d=g=>r-2-(g-s)/l*(r-4),c=e.map((g,h)=>`${(h/(e.length-1)*i).toFixed(1)},${d(g).toFixed(1)}`).join(" "),p="";if(n&&s<0&&a>0){let g=d(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${i}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let x=e.map((g,h)=>`<rect x="${(h/(e.length-1)*i-4).toFixed(1)}" y="0" width="8" height="${r}" fill="transparent"><title>${u(t[h]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${i} ${r}" style="width:100%;height:${r}px;display:block" preserveAspectRatio="none">
    ${p}
    <polyline points="${c}" fill="none" stroke="${o}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
    ${x}
  </svg>`}function Se(e){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let t=200,o=32,r=e.map(c=>Math.max(-9,Math.min(-3,Math.log10(c.flux)))),n=Math.min(...r),s=Math.max(...r)-n||1,a=c=>o-2-(c-n)/s*(o-4),l=r.map((c,p)=>`${(p/(r.length-1)*t).toFixed(1)},${a(c).toFixed(1)}`).join(" "),d=e.map((c,p)=>{let x=p/(r.length-1)*t,g=c.flux>=1e-4?"X":c.flux>=1e-5?"M":c.flux>=1e-6?"C":c.flux>=1e-7?"B":"A";return`<rect x="${(x-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${u(R(c.t_utc))} \xB7 ${g}-class (${c.flux.toExponential(2)})</title></rect>`}).join("");return`<svg viewBox="0 0 ${t} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function ze(e){var a;let t=(a=e.metrics.wind_history_1h)!=null?a:[],o=U(t.map(l=>{var d;return(d=l.kms)!=null?d:0}).filter(l=>l>0),t.map(l=>R(l.t_utc)),"#5cce8c",36,!1),r=t[t.length-1],n=(r==null?void 0:r.density)!=null?`${r.density.toFixed(2)} cm\u207B\xB3`:"\u2014",i=(r==null?void 0:r.temp_kk)!=null?`${r.temp_kk.toFixed(0)} kK`:"\u2014",s=(r==null?void 0:r.pressure_npa)!=null?`${r.pressure_npa.toFixed(2)} nPa`:"\u2014";return`<div class="hw-kpi-popover">
    ${B("Solar Wind \xB7 Last 24h")}
  ...
  </div>`}function Fe(e){var d,c,p;let t=(d=e.metrics.xray_history_1h)!=null?d:[],o=Se(t),r=(c=e.metrics.xray_class)!=null?c:"A",n=e.metrics.xray_flux_wm2,i=n!=null?n.toExponential(2)+" W/m\xB2":"\u2014",s=[{label:"A",color:"#888",start:1e-8,end:1e-7},{label:"B",color:"#5cce8c",start:1e-7,end:1e-6},{label:"C",color:"#aad47a",start:1e-6,end:1e-5},{label:"M",color:"#e0a84a",start:1e-5,end:1e-4},{label:"X",color:"#e05c5c",start:1e-4,end:.001}],a=s.map(x=>{let g=x.label===r,h=g?'<div class="hw-xray-scale-marker"></div>':"";return`<div class="hw-xray-scale-band" style="background:${x.color}${g?"cc":"44"}">${h}</div>`}).join(""),l=s.map(x=>`<div class="hw-xray-scale-label" style="color:${x.label===r?"#c8d8dc":"#607880"}">${x.label}</div>`).join("");return`<div class="hw-kpi-popover">
    ${B("X-Ray Flux \xB7 Last 24h")}
    <div class="hw-spark-wrap">${o}</div>
  ...
  </div>`}function He(e){var s;let t=(s=e.metrics.bz_history_5m)!=null?s:[],o=U(t.map(a=>a.bz),t.map(a=>R(a.t_utc)),"#d4cc5c",36,!0),r=e.metrics.imf_bz_nt,n=r!=null?r<=-10?"#e05c5c":r<=-5?"#e0a84a":r>=5?"#5cce8c":"#a0b4b8":"#607880",i=r!=null?(r>=0?"+":"")+r.toFixed(1)+" nT":"\u2014";return`<div class="hw-kpi-popover">
    ${B("IMF Bz \xB7 Last 6h")}
    <div class="hw-spark-wrap">${o}</div>
  ...
  </div>`}function se(e){var l,d;let t=e.metrics.imf_bz_nt,o=(l=e.metrics.kp_latest)!=null?l:0,r=(d=e.metrics.solar_wind_kms)!=null?d:0,n,i,s;t!=null&&t<-5||o>=6?(n="storm",i="#e05c5c",s="Storm conditions"):t!=null&&t<0||o>=4||r>=400?(n="active",i="#e0a84a",s="Active coupling"):(n="stable",i="#5cce8c",s="Stable");let a;return t==null?a="Unknown":t>2?a="Closed":t>0?a="Minimal":t>-5?a="Moderate":t>-10?a="Strong":a="Very strong",{state:n,color:i,label:s,coupling:a}}function le(e,t,o,r){let n=r?"mc":"mf",i=e.color,s=o!=null?o:0,a=s>500,l=s<350,d=a?.9:l?1.8:1.3;if(r){let h=45-(e.state==="storm"?11:e.state==="active"?16:21),m=e.state==="storm"?12:e.state==="active"?10:8,v=50-m,f=76,$=[`M ${h},25`,`C ${h-2},15 41,${m} 45,${m}`,`C 53,${m} ${f-8},${m+4} ${f},20`,`C ${f+1},23 ${f+1},27 ${f},30`,`C ${f-8},${v-4} 53,${v} 45,${v}`,`C 41,${v} ${h-2},35 ${h},25`,"Z"].join(" "),b=a?3:2,k=[14,25,36],z=y=>`<path d="M 0,${y} L ${a?8:6},${y} M ${a?6:4},${y-2} L ${a?8:6},${y} L ${a?6:4},${y+2}" stroke="${i}bb" stroke-width="${a?1.4:1}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`,M=k.map(y=>z(y)).join(""),A=Array.from({length:b},(y,H)=>`<g class="hw-wg" style="animation-duration:${d}s;animation-delay:${(d/b*H).toFixed(2)}s">${M}</g>`).join(""),F=t==null?"":t>0?`<path d="M 45,27 L 45,23 M ${45-1.5},${25-.5} L 45,23 L ${45+1.5},${25-.5}" stroke="#5cce8c" stroke-width="1" fill="none" stroke-linecap="round"/>`:`<path d="M 45,23 L 45,27 M ${45-1.5},${25+.5} L 45,27 L ${45+1.5},${25+.5}" stroke="#e05c5c" stroke-width="1" fill="none" stroke-linecap="round"/>`;return`<svg viewBox="0 0 80 50" style="width:78px;height:49px;display:block" xmlns="http://www.w3.org/2000/svg">
      <defs><clipPath id="${n}-wclip"><rect x="0" y="0" width="26" height="50"/></clipPath></defs>
      <circle cx="2" cy="25" r="5" fill="#f0c040" opacity=".7"/>
      <g clip-path="url(#${n}-wclip)">${A}</g>
      <path d="${$}" fill="${i}14" stroke="${i}b0" stroke-width="0.9"/>
      <circle cx="45" cy="25" r="${4.5}" fill="#2a4a6a" stroke="#4a7090" stroke-width="0.8"/>
      ${F}
    </svg>`}else{let g=e.state==="storm"?26:e.state==="active"?38:50,h=110-g,m=e.state==="storm"?28:e.state==="active"?22:18,v=120-m,f=188,$=[`M ${h},60`,`C ${h-5},38 100,${m} 110,${m}`,`C 128,${m} ${f-20},${m+10} ${f},48`,`C ${f+3},55 ${f+3},65 ${f},72`,`C ${f-20},${v-10} 128,${v} 110,${v}`,`C 100,${v} ${h-5},82 ${h},60`,"Z"].join(" "),b=`M ${h+2},60 C ${h+2},${60-g*.4} 106,52 100,60 C 106,68 ${h+2},${60+g*.4} ${h+2},60 Z`,k=a?4:3,z=[24,42,60,78,96],M=L=>`<path d="M 0,${L} L ${a?18:14},${L} M ${a?14:10},${L-4} L ${a?18:14},${L} L ${a?14:10},${L+4}" stroke="${i}bb" stroke-width="${a?2:1.5}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`,A=z.map(L=>M(L)).join(""),F=Array.from({length:k},(L,E)=>`<g class="hw-wg" style="animation-duration:${d}s;animation-delay:${(d/k*E).toFixed(2)}s">${A}</g>`).join(""),y=t==null?"":t>0?'<path d="M 110,65 L 110,55 M 107,58 L 110,55 L 113,58" stroke="#5cce8c" stroke-width="1.5" fill="none" stroke-linecap="round"/>':'<path d="M 110,55 L 110,65 M 107,62 L 110,65 L 113,62" stroke="#e05c5c" stroke-width="1.5" fill="none" stroke-linecap="round"/>',H=t==null?"":`<text x="123" y="62" font-size="7" fill="${t>0?"#5cce8c":"#e05c5c"}" font-family="monospace">Bz${t>0?"\u2191":"\u2193"}</text>`;return`<svg viewBox="0 0 200 120" style="width:100%;height:80px;display:block" xmlns="http://www.w3.org/2000/svg">
      <defs><clipPath id="${n}-wclip"><rect x="0" y="0" width="62" height="120"/></clipPath></defs>
      <rect width="200" height="120" fill="#0a1014" rx="3"/>
      <circle cx="8" cy="60" r="12" fill="#f0c040" opacity=".75"/>
      <g clip-path="url(#${n}-wclip)">${F}</g>
      <path d="${b}" fill="${i}08"/>
      <path d="${$}" fill="${i}12" stroke="${i}aa" stroke-width="1.2"/>
  ...
  </div>`}var ce=60;function Te(e,t,o,r,n){let i=n*(90-e)/ce,s=t*Math.PI/180;return{x:o+i*Math.sin(s),y:r-i*Math.cos(s)}}function Re(e,t,o){if(!e.length)return null;let r=(o%360+360)%360,n=-1,i=1/0,s=Math.cos(t*Math.PI/180);for(let a of e){let l=a.lat-t,d=(a.lon-r+180+360)%360-180,c=l*l+d*s*(d*s);c<i&&(i=c,n=a.prob)}return n>=0?n:null}function Ee(e){var l;let i=[40,50,60,70,80].map(d=>{let c=(46*(90-d)/ce).toFixed(1),p=(50+parseFloat(c)+1).toFixed(1);return`<circle cx="50" cy="50" r="${c}" fill="none" stroke="#fff" stroke-width=".3" stroke-dasharray="1.5 2" opacity=".3"/><text x="${p}" y="50" font-size="2.8" fill="#7c9ca8" font-family="monospace" dominant-baseline="middle" opacity=".7">${d}\xB0</text>`}).join(""),s=['<text x="50" y="4"   font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">N</text>','<text x="96"  y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">E</text>','<text x="50" y="97"  font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">S</text>','<text x="4"   y="51" font-size="3.2" fill="#6a8a98" text-anchor="middle" font-family="monospace" opacity=".6">W</text>'].join(""),a="";if(e.lat!=null&&e.lon!=null){let d=Te(e.lat,e.lon,50,50,46),c=d.x-50,p=d.y-50,x=Math.sqrt(c*c+p*p),g=x<=46?d.x:50+c/x*46,h=x<=46?d.y:50+p/x*46,m=g.toFixed(1),v=h.toFixed(1),f=(l=e.locationName)!=null?l:"",$=g<72,b=$?(g+2.8).toFixed(1):(g-2.8).toFixed(1),k=(h-2.2).toFixed(1),M=f?`<text x="${b}" y="${k}" font-size="3" fill="#e8f0f2" font-family="system-ui,sans-serif"
           text-anchor="${$?"start":"end"}" opacity=".82"
           style="text-shadow:0 0 2px #000">${u(f)}</text>`:"";a=`<circle cx="${m}" cy="${v}" r="2.6" fill="none" stroke="#fff" stroke-width=".5" opacity=".55" stroke-dasharray=".9 .6"/><circle cx="${m}" cy="${v}" r="1.4" fill="#fff" stroke="#000" stroke-width=".35" opacity=".92"/>`+M}return`<svg viewBox="0 0 100 100" width="100%" height="100%"
    style="position:absolute;top:0;left:0;pointer-events:none">${i}${s}${a}</svg>`}function Ie(e,t){let o=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,r=null;t&&e.lat!=null&&e.lon!=null&&(r=Re(t.entries,e.lat,e.lon));let n=e.lat!=null&&e.lon!=null,i=r!=null?r>=30?"#5cce8c":r>=10?"#d4cc5c":"#9ab4bc":"#607880",s=r!=null?`${r}%`:t?"n/a":"\u2026",a=n?`
    <div class="hw-aurora-obs-panel">
      ...
      <span class="hw-aurora-prob" style="color:${i}">Aurora: ${s}</span>
    </div>`:"";return`<div class="hw-kpi-popover">
    ${B("Aurora Oval \xB7 Northern Hemisphere")}
  ...
  </div>`}function Pe(e,t,o,r){switch(t){case"solar_wind":return ze(e);case"xray":return Fe(e);case"imf_bz":return He(e);case"aurora":return Ie(o,r);case"magnetosphere":return Ae(e);default:return""}}function X(e,t){if(e.length<2)return"\u2192";let o=e[e.length-1],r=Math.max(0,e.length-4),n=e[r];if(!isFinite(o)||!isFinite(n))return"\u2192";let i=o-n;return i>t?"\u2191":i<-t?"\u2193":"\u2192"}function Be(e,t,o,r,n,i){var T,N,O,W,q,D,J;let{summary:s,scales:a,metrics:l,aurora_hint:d}=e,c=(T=te[s.status])!=null?T:te.quiet,p=r!=null?r.kp.toFixed(1):l.kp_latest!=null?l.kp_latest.toFixed(1):"\u2014",g=[r?r.gScale:a.g_scale,a.r_scale,a.s_scale].map(_=>{let K=_e(_),Y=K?`color:${c.accent};border-color:${c.accent}33`:"";return`<span class="hw-scale-chip${K?" hw-scale-active":""}" style="${Y}">${u(_)}</span>`}).join(""),h=r?r.auroraLabel:d.aurora_label,m=h==="good"?"#5cce8c":h==="possible"?"#d4cc5c":"#607880",v=h.charAt(0).toUpperCase()+h.slice(1),f="#b4c6cc",$=l.solar_wind_kms!=null?`${Math.round(l.solar_wind_kms)} km/s`:"\u2014",b=l.imf_bz_nt,k=b!=null?b<=-10?"#e05c5c":b<=-5?"#e0a84a":b>=5?"#5cce8c":"#a0b4b8":"#607880",z=b!=null?(b>=0?"+":"")+b.toFixed(1)+" nT":"\u2014",M=l.xray_class,A=M?(N=ae[M])!=null?N:"#a0b4b8":"#607880",F=M?`${M}-class`:"\u2014",y=X(((O=l.kp_history_1h)!=null?O:[]).map(_=>_.kp),.5),H=X(((W=l.wind_history_1h)!=null?W:[]).map(_=>_.kms),20),L=X(((q=l.bz_history_1h)!=null?q:[]).map(_=>_.bz),1.5),E=X(((D=l.xray_history_1h)!=null?D:[]).map(_=>Math.log10(_.flux+1e-9)),.15),G=t?"\u25BC Details":"\u25B6 Details",j=se(e),I=(J=l.kp_latest)!=null?J:0,w=I>=5,C=w?`linear-gradient(160deg, #0d2a1a 0%, ${c.bg}22 75%)`:`${c.bg}18`,S=(_,K,Y,he,ee)=>{let ue=ee?`<span class="hw-trend">${ee}</span>`:"";return`<div class="hw-kpi-item${o===_?" hw-kpi-active":""}" data-kpi="${_}">
      <span class="hw-qd-label">${K}</span>
    ...
    <div class="hw-hero" style="background:${C}">
      <div class="hw-hero-main">
        <div class="hw-kp-col">
          <div class="hw-kp-big" style="${r?"color:#9acf60":""}">Kp <b>${u(p)}</b>${r?"":`<span class="hw-trend">${y}</span>`}</div>
          <span class="hw-status-badge" style="background:${c.accent}22;color:${c.accent};display:block;text-align:center">${u(s.label)}</span>
          <div class="hw-scales-row">${g}</div>
        </div>
        <div class="hw-info-col">
          <div class="hw-info-top-row">
            <div class="hw-summary-text" style="flex:1">${u(s.text)}</div>
            <div class="hw-magnet-mini${o==="magnetosphere"?" hw-kpi-active":""}" data-kpi="magnetosphere" title="Magnetosphere status">
              ${le(j,b,l.solar_wind_kms,!0)}
      ...
      <div class="hw-quick-details">
        ...
        ${S("solar_wind","Solar wind",u($),f,H)}
        ${S("imf_bz","IMF Bz",u(z),k,L)}
        ${S("xray","X-ray",u(F),A,E)}
      </div>
      <button class="hw-hero-toggle-btn hw-hero-click" aria-label="Toggle details">${G}</button>
      ${P}
      ${o?Pe(e,o,n,i):""}
    </div>`}function je(e){var l,d,c;let{metrics:t}=e,o=(l=t.kp_history_1h)!=null?l:[],r=(d=t.wind_history_1h)!=null?d:[],n=(c=t.bz_history_1h)!=null?c:[],i=Ce(o),s=U(r.map(p=>{var x;return(x=p.kms)!=null?x:0}).filter(p=>p>0),r.map(p=>R(p.t_utc)),"#5cce8c",28,!1),a=U(n.map(p=>p.bz),n.map(p=>R(p.t_utc)),"#d4cc5c",28,!0);return`
    ...
    </div>`}function Oe(e){return e.map((t,o)=>o===0?(t+e[1])/2:o===e.length-1?(e[o-1]+t)/2:(e[o-1]+t+e[o+1])/3)}function We(e){return e>=9?"G5":e>=8?"G4":e>=7?"G3":e>=6?"G2":e>=5?"G1":"G0"}function De(e){return e>=5?"good":e>=3?"possible":"none"}function de(e){return e>=9?40:e>=8?45:e>=7?50:e>=6?55:e>=5?60:null}function Ke(e){let t=e>=7?"high":e>=5?"moderate":e>=3?"low":"none",o=de(e),r=t==="none"?"No aurora expected at mid-latitudes":o!=null?`Aurora possible equatorward of ~${o}\xB0 lat`:"Minor aurora possible at high latitudes",n=e>=7?"moderate":e>=5?"low":"none",i=n==="none"?"No significant HF degradation expected":n==="low"?"Minor HF degradation at high latitudes":"Moderate HF degradation, possible blackouts at high latitudes",s=e>=8?"high":e>=6?"moderate":e>=4?"low":"none";return[{kind:"aurora",level:t,label:"Aurora",summary:r},{kind:"radio",level:n,label:"HF Radio",summary:i},{kind:"solar_activity",level:s,label:"Solar Activity",summary:s==="none"?"Quiet geomagnetic conditions expected":s==="low"?"Active geomagnetic conditions possible":s==="moderate"?"Minor to moderate storm conditions":"Major geomagnetic storm conditions"}]}function Xe(e,t){var p;if(t<=0)return null;let o=(p=e.metrics.kp_forecast_3h)!=null?p:[];if(!o.length)return null;let r=Date.now()+t*36e5,n=o[0],i=1/0;for(let x of o){let g=Math.abs(new Date(x.t_utc).getTime()-r);g<i&&(i=g,n=x)}let s=n.kp,a=We(s),l=De(s),d=de(s),c=Ke(s);return{offsetH:t,kp:s,gScale:a,auroraLabel:l,auroraMinLat:d,impacts:c}}function Ue(e,t,o){var I;let{forecast:r,metrics:n}=e,{kp_max_next_24h:i,kp_max_at_utc:s,trend:a}=r,l=((I=n.kp_forecast_3h)!=null?I:[]).slice(0,16),d=l.length,c=d*3,p=c>0?`${(t/c*100).toFixed(0)}%`:"0%",x=t>0?`\u23F1 +${Math.round(t)}h`:"Timeline",g="Kp forecast unavailable";if(i!=null){let w=oe(s),C=a==="rising"?"rising":a==="falling"?"falling":"steady";g=`Peak Kp ${i.toFixed(1)} next 24h${w?` at ${w}`:""} \xB7 ${C}`}if(!l.length)return`
    <div class="hw-forecast">
      <div class="hw-section-label">Kp Forecast \xB7 Next 24h</div>
      <div class="hw-forecast-text">${u(g)}</div>
    </div>`;let h=320,m=38,v=14,f=m+v,$=h/d,b=w=>m-Math.max(2,Math.min(m-2,w/9*(m-2))),k="",z=l.map(w=>w.kp),M=Oe(z);l.forEach((w,C)=>{let S=b(w.kp),P=m-S,T=C*$,N=T+$/2,O=w.kp>=6?"#e05c5c":w.kp>=5?"#e0a84a":w.kp>=4?"#d4cc5c":"#5cce8c",W=`Kp ${w.kp.toFixed(1)} \xB7 ${oe(w.t_utc)}`;if(k+=`<rect x="${T.toFixed(1)}" y="${S.toFixed(1)}" width="${($-1.5).toFixed(1)}" height="${P.toFixed(1)}" fill="${O}" fill-opacity="0.85" rx="1.5"/>`,k+=`<rect x="${T.toFixed(1)}" y="0" width="${$.toFixed(1)}" height="${m}" fill="transparent"><title>${Q(W)}</title></rect>`,d<=8||C%2===0){let D=new Date(w.t_utc).getHours();k+=`<text x="${N.toFixed(1)}" y="${(f-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${D.toString().padStart(2,"0")}</text>`}});let F=`<polyline points="${l.map((w,C)=>{let S=C*$+$/2,P=b(M[C]);return`${S.toFixed(1)},${P.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,y="";if(t>0&&d>0){let w=Math.min(h-1,t/(d*3)*h);y=`
      <line x1="${w.toFixed(1)}" y1="0" x2="${w.toFixed(1)}" y2="${m}" stroke="rgba(255,255,255,0.75)" stroke-width="1.5" stroke-dasharray="3,2"/>
      <polygon points="${w.toFixed(1)},${m} ${(w-4).toFixed(1)},${(m-7).toFixed(1)} ${(w+4).toFixed(1)},${(m-7).toFixed(1)}" fill="rgba(255,255,255,0.75)"/>`}let H=Math.round(c/4),L=Math.round(c/2),E=Math.round(c*3/4),G=`
    <div class="hw-scrub-wrap">
      ...
      <div class="hw-scrub-tick-row">
        ...
        <span class="hw-scrub-tick">+${L}h</span>
    ...
    <div class="hw-sim-banner">
      ...
      ${G}
    </div>`}function Ge(e){let t=/([NS])(\d+)([EW])(\d+)/i.exec(e);return t?{lat:(t[1].toUpperCase()==="N"?1:-1)*parseInt(t[2],10),lon:(t[3].toUpperCase()==="E"?1:-1)*parseInt(t[4],10)}:null}function qe(e,t){let o=t/2,r=o*.87,n=e.map(i=>{var g,h;let s=Ge(i.location);if(!s||Math.abs(s.lon)>88)return"";let a=s.lat*Math.PI/180,l=s.lon*Math.PI/180,d=(o+r*Math.cos(a)*Math.sin(l)).toFixed(1),c=(o-r*Math.sin(a)).toFixed(1),p=i.x_flare_probability>0?"#e05c5c":i.m_flare_probability>10?"#e0a84a":i.c_flare_probability>20?"#d4cc5c":"#c8d8e0",x=`AR ${i.region} \xB7 ${i.location}
...
C: ${i.c_flare_probability}%  M: ${i.m_flare_probability}%  X: ${i.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    </div>`;let i=t?r:r.slice(0,ie),a=n-ie>0?t?'<button class="hw-alerts-toggle hw-alerts-expand">\u25B2 Show less</button>':`<button class="hw-alerts-toggle hw-alerts-expand">\u25BC Show all ${n}</button>`:"",l=i.map(c=>et(c,c.dedupe_key===o)).join("");return`
    <div class="hw-alerts">
      <div class="hw-alerts-header">
      ...
      ${l}
    </div>`}function ot(e,t,o,r,n,i,s,a,l,d){let c=Xe(e,n);return`
    <div class="hw-root">
      ${Ne(e)}
      ${Be(e,o,r,c,l,d)}
      ${o?je(e):""}
