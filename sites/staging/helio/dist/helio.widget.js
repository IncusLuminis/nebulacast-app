...
`,We=!1;function Ft(){if(We)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Rt,document.head.appendChild(e),We=!0}function It(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.length,n=s/o,c=e.map((r,i)=>{let a=Math.max(2,Math.min(t,r.kp/9*t)),l=t-a,d=i*n,p=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${l.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${u(be(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${c}</svg>`}function je(e,s,t,o,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let c=200,r=Math.min(...e),i=Math.max(...e),a=i-r||1,l=g=>o-2-(g-r)/a*(o-4),d=e.map((g,h)=>`${(h/(e.length-1)*c).toFixed(1)},${l(g).toFixed(1)}`).join(" "),p="";if(n&&r<0&&i>0){let g=l(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${c}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let m=e.map((g,h)=>`<rect x="${(h/(e.length-1)*c-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${u(s[h]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${c} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
  ...
  </div>`}function zt(e){var l;let s=(l=e.metrics.wind_history_1h)!=null?l:[],t=s[s.length-1],o=e.metrics.solar_wind_kms,n=o!=null?`${Math.round(o)} km/s`:"\u2014",c=o!=null?o>=700?"#e05c5c":o>=500?"#e0a84a":o>=400?"#d4cc5c":"#5cce8c":"#607880",r=(t==null?void 0:t.density)!=null?`${t.density.toFixed(2)} cm\u207B\xB3`:"\u2014",i=(t==null?void 0:t.temp_kk)!=null?`${t.temp_kk.toFixed(0)} kK`:"\u2014",a=(t==null?void 0:t.pressure_npa)!=null?`${t.pressure_npa.toFixed(2)} nPa`:"\u2014";return`<div class="hw-kpi-popover">
    ...
    <line x1="${i}" y1="130" x2="${w}" y2="130"
  ...
  </g>`,T="";if(e==="magnetosphere"){let z=(O=s.windKms)!=null?O:0,G=z>500,P=z<350,E=(G?.5:P?1.4:.9)*1.3,K=z>700?"#e05c5c":z>500?"#e0a84a":z>350?"#d4c840":"#5cce8c",te=i+8,ee=w-14,I=8,D=6,U=(ee-te)/(I-1),se=260/(D+1),V=38,ae=24,ke=2,_e=Array.from({length:I},(ne,pe)=>{let oe=te+pe*U,re=Array.from({length:D},(he,me)=>{let Z=se*(me+1);return`<path d="M ${oe.toFixed(1)},${Z.toFixed(1)} L ${(oe+V).toFixed(1)},${Z.toFixed(1)} M ${(oe+ae).toFixed(1)},${(Z-6).toFixed(1)} L ${(oe+V).toFixed(1)},${Z.toFixed(1)} L ${(oe+ae).toFixed(1)},${(Z+6).toFixed(1)}"
        ...
        fill="${ne}" font-family="monospace">Bz${q>0?"\u2191":"\u2193"}</text>`})();T=`${_e}${de}`}let Y=`<g id="${c}-overlay-solar-wind">${T}</g>`,W="";if(e==="coronal_hole"&&s.hssState){let z=s.hssState,G=(J=z.speed)!=null?J:0,P=G>=420,E=z.color,K=P?"0.85":"0.25",te=G>=500?"0.18":P?"0.10":"0.04",ee=i+8,I=d-16,D=60,U=`${ee},130 ${I},${130-D} ${I},${130+D}`,se=I+6,V=`${se},120 ${se+18},130 ${se},140`;W=`
    ...
    <polygon points="${V}" fill="${E}" opacity="${P?"0.9":"0.25"}"/>`}let X=`<g id="${c}-overlay-coronal-hole">${W}</g>`,N="";if(e==="cme_cone"&&s.cmeState){let z=s.cmeState,G=i,P=d-10,E=P-G,K=U=>Math.tan(U*Math.PI/180),te=Math.round(K(9)*E),ee=Math.round(K(6)*E),I=Math.round(K(3)*E),D=U=>`${G},130 ${P},${130-U} ${P},${130+U}`;if(z.status!=="quiet"){let U=z.status==="impact"?"#e05c5c":"#d4cc5c";N=`
    ...
    <div class="hw-solar-mini-inner" style="border-color:${i};box-shadow:${a}">
      ...
      style="${`color:${k.color};background:${k.background};border-color:${k.borderColor}`}" data-hero-chip="${M.key}" title="${A(M.title)}" aria-label="${A(M.aria)}">${u(M.text)}</button>`}).join(""),m=Wt((($=r.kp_history_1h)!=null?$:[]).map(M=>M.kp),.5),g=(_=r.kp_latest)!=null?_:0,h=g>=5,w=h?`linear-gradient(160deg, #0d2a1a 0%, ${i.bg}22 75%)`:`${i.bg}18`,v=Ut(e,t),x=h?`
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    ${f}
      ...
      </div>`}else m=gs(n,d);let g=e.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${A(e.kind)}"`,h=e.kind==="radio"?' id="radio"':"";return`<div class="hw-impact-row${p}"${h}${g}>
    <span class="hw-impact-caret">\u25B6</span>
    ...
    ${m}
  </div>`}function Es(e,s,t,o,n,c,r,i,a){var Fe,Ie,Oe,ze;let l=(Ie=(Fe=s==null?void 0:s.impacts)!=null?Fe:e.observer_impacts)!=null?Ie:[],d=Hs(l,e),p=Ls(d,r,i,a),m=l.find(Se=>Se.kind==="radio"),g=l.find(Se=>Se.kind==="solar_activity"),h=m?Xe(m,t,n,c,e,r.has("radio")):"",w=g?Xe(g,t,n,c,e,!1):"",x=`
    <div class="hw-section-row" data-impacts-toggle>
      ...
      ${vs(e,B)}
    </div>`,R=r.has("solar_cycle"),J=(ze=at[xe.phase])!=null?ze:"#96a8b8",z=xe.phase.charAt(0).toUpperCase()+xe.phase.slice(1),P=`<div class="hw-impact-row${R?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
...
${t.join(" \xB7 ")}`}function As(e,s,t){var a;let o=e.is_active?Is:(a=Fs[e.level])!=null?a:"#445c64",n=e.event_time===t,c=e.event_time.slice(11,16)+" UTC",r=e.source==="NASA_DONKI"?"DONKI":"SWPC",i=n?`<div class="hw-tl-detail">${u(e.description)}${u(zs(e))}</div>`:"";return`
    ...
    <div class="hw-section-row" data-tl-section>
      ...
      <div class="hw-tl-day-row" data-tl-day="${A(b)}">
        <span class="hw-section-caret">${H}</span>
        <span class="hw-tl-date">${M}</span>
        ${S}
      </div>`,L=k?"":$.map((T,Y)=>As(T,Y<$.length-1,s)).join("");return`<div class="hw-tl-group">${y}${L}</div>`}).join("");return`
