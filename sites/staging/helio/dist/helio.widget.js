...
`,St=!1;function re(){if(St)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=ie,document.head.appendChild(t),St=!0}function le(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,n=s/o,d=t.map((i,r)=>{let a=Math.max(2,Math.min(e,i.kp/9*e)),l=e-a,c=r*n,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${c.toFixed(1)}" y="${l.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(pt(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${d}</svg>`}function Mt(t,s,e,o,n){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let d=200,i=Math.min(...t),r=Math.max(...t),a=r-i||1,l=g=>o-2-(g-i)/a*(o-4),c=t.map((g,w)=>`${(w/(t.length-1)*d).toFixed(1)},${l(g).toFixed(1)}`).join(" "),p="";if(n&&i<0&&r>0){let g=l(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${d}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((g,w)=>`<rect x="${(w/(t.length-1)*d-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[w]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${d} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ...
    ${l}
  </svg>`}function st(t){return`<div class="hw-kpi-popover-title">
    <span>${t}</span>
    ...
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function he(t){let a='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',l=[-10,-5,5,10].map(v=>{let y=100+v/20*100;return`<line x1="${y.toFixed(1)}" y1="16" x2="${y.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),c='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(t==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${a}${l}${c}</svg>`;let p=t<=-10?"#e05c5c":t<=-5?"#e0a84a":t<0?"#d4b84a":t>=5?"#5cce8c":"#7acca8",h=Math.max(-20,Math.min(20,t)),g=100+h/20*100,w=3,u=h<0?g-w:100-w,_=Math.max(2*w,Math.abs(g-100)+2*w),f=`<rect x="${u.toFixed(1)}" y="16" width="${_.toFixed(1)}" height="6" rx="${w}" fill="${p}" opacity="0.82"/>`,x=5,$=15,b=$-x*1.1,k=`<polygon points="${g.toFixed(1)},${$.toFixed(1)} ${(g-x).toFixed(1)},${b.toFixed(1)} ${(g+x).toFixed(1)},${b.toFixed(1)}" fill="${p}"/>`,z=`<line x1="${g.toFixed(1)}" y1="${$.toFixed(1)}" x2="${g.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${p}" stroke-width="1" opacity="0.6"/>`,L=`<text x="${g.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${p}" font-weight="600">${t>=0?"+":""}${t.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${a}${l}${c}${f}${k}${z}${L}
    ...
    <div style="font-size:.65em;color:#607880">Coupling: <span style="color:${c.color};font-weight:600">${m(c.coupling)}</span> \xB7 Trend history: \u25B6 Details</div>
      ...
      ${L}
    </svg>`}else{let f=t.state==="storm"?16:t.state==="active"?26:38,x=155-f,$=t.state==="storm"?22:t.state==="active"?30:40,b=120-$,k=240,z=[`M ${x},60`,`C ${x-4},42 150,${$} 155,${$}`,`C 173,${$} ${k-5},${$+18} ${k},60`,`C ${k-5},${b-18} 173,${b} 155,${b}`,`C 150,${b} ${x-4},78 ${x},60`,"Z"].join(" "),C=`M ${x+2},60 C ${x+2},${60-f*.4} 152,54 150,60 C 152,66 ${x+2},${60+f*.4} ${x+2},60 Z`,L=i>700?"#e05c5c":i>500?"#e0a84a":i>350?"#d4c840":"#5cce8c",v=i>700?.4:i>500?.65:i>350?1.1:1.8,y=i>500?[10,24,40,57,74,90,106]:i>350?[14,34,57,82,104]:[20,50,82,108],F=16,H=22,P=x-6,A=Math.ceil((P-H)/F)+2,N=Array.from({length:A},(T,E)=>H-F+E*F),R=12,D=8,K=N.flatMap(T=>y.map(E=>`<path d="M ${T},${E} L ${T+R},${E} M ${T+D},${E-3} L ${T+R},${E} L ${T+D},${E+3}" stroke="${L}cc" stroke-width="1.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`)).join(""),M=`<g class="hw-wg-full" style="animation-duration:${v}s">${K}</g>`,S=s==null?"":s>0?'<path d="M 155,64 L 155,56 M 153,58 L 155,56 L 157,58" stroke="#5cce8c" stroke-width="1.3" fill="none" stroke-linecap="round"/>':'<path d="M 155,56 L 155,64 M 153,62 L 155,64 L 157,62" stroke="#e05c5c" stroke-width="1.3" fill="none" stroke-linecap="round"/>',O=s==null?"":`<text x="163" y="62" font-size="6" fill="${s>0?"#5cce8c":"#e05c5c"}" font-family="monospace">Bz${s>0?"\u2191":"\u2193"}</text>`;return`<svg viewBox="-60 0 280 120" style="width:100%;height:80px;display:block" xmlns="http://www.w3.org/2000/svg">
    ...
    ${st("Aurora Oval \xB7 Northern Hemisphere")}
  ...
  </div>`}function $e(t,s,e,o,n,d,i,r=0){var B,G,X,ot,V,Y,Q,ht;let{summary:a,scales:l,metrics:c,aurora_hint:p}=t,h=(B=kt[a.status])!=null?B:kt.quiet,g=n!=null?n.kp.toFixed(1):c.kp_latest!=null?c.kp_latest.toFixed(1):"\u2014",u=[n?n.gScale:l.g_scale,l.r_scale,l.s_scale].map(j=>{let q=ae(j),Z=q?`color:${h.accent};border-color:${h.accent}33`:"";return`<span class="hw-scale-chip${q?" hw-scale-active":""}" style="${Z}">${m(j)}</span>`}).join(""),_=n?n.auroraLabel:p.aurora_label,f=_==="good"?"#5cce8c":_==="possible"?"#d4cc5c":"#607880",x=_.charAt(0).toUpperCase()+_.slice(1),$="#b4c6cc",b=c.solar_wind_kms!=null?`${Math.round(c.solar_wind_kms)} km/s`:"\u2014",k=c.imf_bz_nt,z=k!=null?k<=-10?"#e05c5c":k<=-5?"#e0a84a":k>=5?"#5cce8c":"#a0b4b8":"#607880",C=k!=null?(k>=0?"+":"")+k.toFixed(1)+" nT":"\u2014",L=c.xray_class,v=L?(G=ft[L])!=null?G:"#a0b4b8":"#607880",y=L?`${L}-class`:"\u2014",F=lt(((X=c.kp_history_1h)!=null?X:[]).map(j=>j.kp),.5),H=lt(((ot=c.wind_history_1h)!=null?ot:[]).map(j=>j.kms),20),P=lt(((V=c.bz_history_1h)!=null?V:[]).map(j=>j.bz),1.5),A=lt(((Y=c.xray_history_1h)!=null?Y:[]).map(j=>Math.log10(j.flux+1e-9)),.15),N=(Q=c.kp_history_1h)!=null?Q:[],R=N.length?ct(N[N.length-1].t_utc):null,D=R?`Recent history \xB7 Last step ${R}`:"Recent history",K=$t(t),M=(ht=c.kp_latest)!=null?ht:0,S=M>=5,O=S?`linear-gradient(160deg, #0d2a1a 0%, ${h.bg}22 75%)`:`${h.bg}18`,T=(j,q,Z,nt,I)=>{let J=I?`<span class="hw-trend">${I}</span>`:"";return`<div class="hw-kpi-item${o===j?" hw-kpi-active":""}" data-kpi="${j}">
      <span class="hw-qd-label">${q}</span>
      <span class="hw-qd-value" style="color:${nt}">${Z}${J}</span>
    </div>`},E=S?`
    ...
    <div class="hw-hero" style="background:${O}">
      ...
      <div class="hw-quick-details">
        ${T("aurora","Aurora",m(x),f)}
        ${T("solar_wind","Solar wind",m(b),$,H)}
        ${T("imf_bz","IMF Bz",m(C),z,P)}
        ${T("xray","X-ray",m(y),v,A)}
      </div>
      ${o?xe(t,o,d,i):""}
      <div class="hw-section-row" data-hero-toggle style="margin-top:6px">
        <span class="hw-section-caret">${s?"\u25BC":"\u25B6"}</span>
        <span class="hw-section-label" style="margin-bottom:0">${m(D)}</span>
      </div>`:""}
      ${E}
    ...
    </div>`;let f=320,x=38,$=14,b=x+$,k=f/c,z=M=>x-Math.max(2,Math.min(x-2,M/9*(x-2))),C="",L=l.map(M=>M.kp),v=ye(L);l.forEach((M,S)=>{let O=z(M.kp),T=x-O,E=S*k,B=E+k/2,G=M.kp>=6?"#e05c5c":M.kp>=5?"#e0a84a":M.kp>=4?"#d4cc5c":"#5cce8c",X=`Kp ${M.kp.toFixed(1)} \xB7 ${ct(M.t_utc)}`;if(C+=`<rect x="${E.toFixed(1)}" y="${O.toFixed(1)}" width="${(k-1.5).toFixed(1)}" height="${T.toFixed(1)}" fill="${G}" fill-opacity="0.85" rx="1.5"/>`,C+=`<rect x="${E.toFixed(1)}" y="0" width="${k.toFixed(1)}" height="${x}" fill="transparent"><title>${W(X)}</title></rect>`,c<=8||S%2===0){let V=new Date(M.t_utc).getHours();C+=`<text x="${B.toFixed(1)}" y="${(b-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${V.toString().padStart(2,"0")}</text>`}});let F=`<polyline points="${l.map((M,S)=>{let O=S*k+k/2,T=z(v[S]);return`${O.toFixed(1)},${T.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,H="";if(s>0&&c>0){let M=Math.min(f-1,s/(c*3)*f);H=`
      <line x1="${M.toFixed(1)}" y1="0" x2="${M.toFixed(1)}" y2="${x}" stroke="rgba(255,255,255,0.75)" stroke-width="1.5" stroke-dasharray="3,2"/>
      <polygon points="${M.toFixed(1)},${x} ${(M-4).toFixed(1)},${(x-7).toFixed(1)} ${(M+4).toFixed(1)},${(x-7).toFixed(1)}" fill="rgba(255,255,255,0.75)"/>`}let P=Math.round(p/4),A=Math.round(p/2),N=Math.round(p*3/4),R=`
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}var dt={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Ot={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function Be(t){var v;let s=dt,e=(v=Ot[s.phase])!=null?v:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,d=52,i=10,r=d-6,a=d-18,l=.5,c=.19,p=y=>Math.exp(-Math.pow((y-l)/c,2)/2),h=y=>i+y*(n-2*i),g=y=>r-p(y)*a,w=80,u=[];for(let y=0;y<=w;y++){let F=y/w;u.push(`${y===0?"M":"L"}${h(F).toFixed(1)},${g(F).toFixed(1)}`)}let _=Math.round(s.progress_0_1*w),f=[];for(let y=0;y<=_;y++){let F=y/w;f.push(`${y===0?"M":"L"}${h(F).toFixed(1)},${g(F).toFixed(1)}`)}let x=h(s.progress_0_1),$=[`M${i},${r}`,...f.slice(1),`L${x.toFixed(1)},${r} Z`],b=g(s.progress_0_1),k=5,z=`M${x.toFixed(1)},${b.toFixed(1)} L${(x-k).toFixed(1)},${(b-k*1.8).toFixed(1)} L${(x+k).toFixed(1)},${(b-k*1.8).toFixed(1)} Z`,C=r+11;return`<div class="hw-impact-tip${t?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function Rt(t){var a,l,c,p,h;let s=(l=(a=t.coronal_hole)==null?void 0:a.estimated_speed_kms)!=null?l:t.metrics.solar_wind_kms,e=(c=t.coronal_hole)==null?void 0:c.status,o=s!=null?s:0,n=e!=null?e:o>=600?"strong":o>=500?"active":o>=420?"watch":"quiet",d={strong:"#e05c5c",active:"#e0a84a",watch:"#d4cc5c",quiet:"#5cce8c"},i={strong:"Strong",active:"Active",watch:"Watch",quiet:"None"},r={strong:"Strong high-speed stream",active:"High-speed stream active",watch:"Elevated solar wind",quiet:"Background solar wind"};return{status:n,color:d[n],label:i[n],desc:(h=(p=t.coronal_hole)==null?void 0:p.note)!=null?h:r[n],speed:s}}function je(t,s){var K;let e=Rt(t),o=(K=e.speed)!=null?K:0,n=e.speed!=null?`${Math.round(e.speed)} km/s`:"\u2014",d=s?" hw-impact-tip-open":"",i=320,r=72,a=36,l=36,c=284,p=36,h=11,g=14,w=19,u=9,_=o>=500,f=o>=420,x=_?"#f5c540":f?"#c8a020":"#7a6010",$=_?"#f5c540":f?"#c8a020":"#3a3808",b=e.color,k=f?"0.9":"0.25",z=_?"0.18":f?"0.10":"0.04",C=[0,45,90,135,180,225,270,315].map(M=>{let S=M*Math.PI/180,O=(a+g*Math.cos(S)).toFixed(1),T=(l+g*Math.sin(S)).toFixed(1),E=(a+w*Math.cos(S)).toFixed(1),B=(l+w*Math.sin(S)).toFixed(1),G=M>300||M<60?"0.9":"0.5";return`<line x1="${O}" y1="${T}" x2="${E}" y2="${B}"
      stroke="${x}" stroke-width="1.6" stroke-linecap="round" opacity="${G}"/>`}).join(""),L=a+h+2,v=c-u-3,y=14,F=`${L},${l} ${v},${p-y} ${v},${p+y}`,H=v+1,P=`${H},${p-4} ${H+7},${p} ${H},${p+4}`,A=u,N=`
    ...
    <line x1="${c-A}" y1="${p}" x2="${c+A}" y2="${p}"
          stroke="#4a8ab0" stroke-width="0.8" opacity="0.35"/>`,R=`<svg class="hw-hss-diagram" width="100%" height="${r}"
      viewBox="0 0 ${i} ${r}" preserveAspectRatio="none" aria-hidden="true">
    <!-- stream fan -->
    <polygon points="${F}" fill="${b}" opacity="${z}"/>
    <!-- dashed stream axis -->
    <line x1="${L}" y1="${l}" x2="${v-2}" y2="${p}"
          stroke="${b}" stroke-width="2" stroke-dasharray="5 3.5"
          stroke-linecap="round" opacity="${k}"/>
    <!-- arrow -->
    <polygon points="${P}" fill="${b}" opacity="${f?"0.9":"0.25"}"/>
    <!-- Sun glow ring -->
  ...
  </svg>`,D=f?'<div class="hw-hss-meta" style="font-size:.72em">Elevated speed may indicate Earth-facing coronal hole stream</div>':'<div class="hw-hss-meta" style="font-size:.72em">Background solar wind \xB7 no HSS detected</div>';return`<div class="hw-impact-tip${d}">
    ${R}
    ...
    <div class="hw-swdp-meta" style="font-size:.72em">Speed ${m(C)} \xB7 Density ${m(z)}</div>
       ...
       <line x1="${i+9}" y1="${r}" x2="${a-2}" y2="${r}"
             ...
             stroke="#1e2c30" stroke-dasharray="4 3" stroke-width="1"/>`,C=8,L=11,v=15,y="#f5c540",F=[0,45,90,135,180,225,270,315].map(S=>{let O=S*Math.PI/180,T=(i+L*Math.cos(O)).toFixed(1),E=(r+L*Math.sin(O)).toFixed(1),B=(i+v*Math.cos(O)).toFixed(1),G=(r+v*Math.sin(O)).toFixed(1),X=S<45||S>315?"0.9":"0.5";return`<line x1="${T}" y1="${E}" x2="${B}" y2="${G}"
    ...
    <line x1="${l}" y1="${f-H}" x2="${l}" y2="${f+H}"
          stroke="#4a8ab0" stroke-width="0.8" opacity="0.6"/>`,A="\u2014";if(e.arrival_utc){let S=new Date(e.arrival_utc),O=S.toLocaleString("en-US",{month:"short",timeZone:"UTC"}),T=S.getUTCDate(),E=String(S.getUTCHours()).padStart(2,"0"),B=String(S.getUTCMinutes()).padStart(2,"0");A=`~${O}\xA0${T}\xA0${E}:${B}\u202FUTC`}let N=e.speed_kms?`${e.speed_kms}\u202Fkm/s`:"\u2014",R=e.status!=="quiet"?`Velocity: <b style="color:#b4c6cc">${m(N)}</b>&ensp;Arrival: <b style="color:#b4c6cc">${m(A)}</b>`:"No Earth-directed CME in forecast window",D=x?"Direct impact likely":$?"Glancing blow possible":b?"Near outer edge":"Impact unlikely",K=x?"#e05c5c":$?"#d4cc5c":b?"#e0a84a":"#5cce8c",M=`<svg class="hw-cme-cone-svg" width="100%" height="${d}"
    ...
    ${e.status!=="quiet"?`<text x="${Math.round((i+l)/2)}" y="${d-3}" font-size="7.5"
               fill="${K}" text-anchor="middle" font-family="inherit">${m(D)}</text>`:""}
  </svg>`;return`<div class="hw-impact-tip${o}">
    ${M}
    <div class="hw-cme-footer">${R}</div>
  </div>`}function Xe(t,s,e,o,n,d,i,r,a){var q,Z,nt;let l=(Z=(q=s==null?void 0:s.impacts)!=null?q:t.observer_impacts)!=null?Z:[],c=l.map(I=>{var vt,yt;let J=(vt=Jt[I.level])!=null?vt:"#666",bt=I.level==="none"?"None":I.level.charAt(0).toUpperCase()+I.level.slice(1),jt=(yt=Fe[I.kind])!=null?yt:Te,Wt=I.level==="none"?"#606870":J,at=I.kind==="solar_activity"?n:i.has(I.kind),Kt=at?" hw-impact-open":"",tt;if(I.kind==="solar_activity"){let it=n?" hw-solar-open":"",mt=Tt.map(U=>{let rt=d.has(U.id),ut=rt?U.color+"22":"transparent",gt=rt?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${U.id}" style="color:${U.color};border-color:${U.color};background:${ut};opacity:${gt}">${U.label}</button>`}).join("");tt=`<div class="hw-solar-tip${it}">
        ...
        </div>`}else if(I.kind==="aurora"){let it=at?" hw-aurora-tip-open":"",mt=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,U=null;r&&a.lat!=null&&a.lon!=null&&(U=Ht(r.entries,a.lat,a.lon));let rt=a.lat!=null&&a.lon!=null,ut=U!=null?U>=30?"#5cce8c":U>=10?"#d4cc5c":"#9ab4bc":"#607880",gt=U!=null?`${U}%`:r?"n/a":"\u2026",Gt=rt?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
        ...
        </div>`:"";tt=`<div class="hw-aurora-tip${it}">
          <div class="hw-aurora-map-wrap">
          ...
          ${Gt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else I.kind==="radio"?tt=Ie(t,at):tt=`<div class="hw-impact-tip${at?" hw-impact-tip-open":""}">${m(I.summary)}</div>`;let Ut=I.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${W(I.kind)}"`;return`<div class="hw-impact-row${Kt}"${Ut}>
    ...
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,_=i.has("geomag_storm"),f=At(t),x=f.g1,$=f.g1>=30?Et.g1:f.g1>0?"#7a9298":"#607880",b=x>0?`G1 ${x}%`:"None",z=`<div class="hw-impact-row${_?" hw-impact-open":""}" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${De(t,_)}
    </div>`,C=i.has("solar_cycle"),L=(nt=Ot[dt.phase])!=null?nt:"#96a8b8",v=dt.phase.charAt(0).toUpperCase()+dt.phase.slice(1),F=`<div class="hw-impact-row${C?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      <span class="hw-impact-badge" style="background:${L}22;color:${L}">${v}</span>
      ${Be(C)}
    </div>`,H=Rt(t),P=i.has("hss"),N=`<div class="hw-impact-row${P?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${je(t,P)}
    </div>`,R=It(t),D=i.has("sat_drag"),M=`<div class="hw-impact-row${D?" hw-impact-open":""}" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${We(t,D)}
    </div>`,S=Pt(t),O=i.has("gnss"),E=`<div class="hw-impact-row${O?" hw-impact-open":""}" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ke(t,O)}
    </div>`,B=Nt(t),G=i.has("sw_pressure"),X=B.pressure!=null?`${B.pressure.toFixed(2)} nPa`:"\u2014",V=`<div class="hw-impact-row${G?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
...
${e.join(" \xB7 ")}`}function Je(t,s,e){var a;let o=t.is_active?Ve:(a=qe[t.level])!=null?a:"#445c64",n=t.event_time===e,d=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",r=n?`<div class="hw-tl-detail">${m(t.description)}${m(Qe(t))}</div>`:"";return`
    ...
    </div>`;if(!e||p===0)return`<div class="hw-timeline">${w}</div>`;let u=new Map;for(let $ of c){let b=((x=$.event_time)!=null?x:"").slice(0,10);u.has(b)||u.set(b,[]),u.get(b).push($)}let _=[...u.entries()].map(([$,b])=>{let z=new Date($+"T12:00:00Z").toLocaleDateString("en-US",{month:"short",day:"numeric",timeZone:"UTC"}),C=o.has($),L=C?"\u25B6":"\u25BC",v=C?`<span class="hw-tl-day-count">${b.length} events</span>`:"",y=`
      <div class="hw-tl-day-row" data-tl-day="${W($)}">
        <span class="hw-section-caret">${L}</span>
        <span class="hw-tl-date">${z}</span>
        ${v}
      </div>`,F=C?"":b.map((H,P)=>Je(H,P<b.length-1,s)).join("");return`<div class="hw-tl-group">${y}${F}</div>`}).join("");return`
    ...
    <div class="hw-alerts">
      ${r}
      ${a}
    </div>`}function ss(t,s){var A,N,R;let e=t.cme_tracker;if(!e)return"";let o=(A=ee[e.impact_level])!=null?A:"#96a8b8",n=(N=se[e.status])!=null?N:e.status,d=300,i=44,r=18,a=i/2,l=10,c=d-18,p=7,h=`<line x1="${r+l}" y1="${a}" x2="${c-p}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,g=`<circle cx="${r}" cy="${a}" r="${l}" fill="#f0c040" opacity="0.92"/>`,w=`
    <circle cx="${c}" cy="${a}" r="${p}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${c}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,u=`<text x="${r}" y="${a+l+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,_=`<text x="${c}" y="${a+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,f="";if(e.progress!=null){let D=r+l+4,K=c-p-4,M=D+e.progress*(K-D),S=5;e.status==="arrival_window"?f=`
    ...
    ${w}${_}
    ${f}
  </svg>`,$=_t(e.arrival_time_utc),b=_t(e.launch_time_utc),k=e.speed_kms!=null?`${Math.round(e.speed_kms)} km/s`:"\u2014",z=e.half_angle_deg!=null?`${e.half_angle_deg}\xB0`:"\u2014",C=(R=e.source_location)!=null?R:"\u2014",L=e.is_earth_direct?"Direct hit":"Glancing blow",v=e.progress!=null?`${Math.round(e.progress*100)}%`:"\u2014",y=`
    ...
    </div>`,F=s?"\u25BC":"\u25B6",H=e.impact_level==="unknown"?"Unrated":e.impact_level.charAt(0).toUpperCase()+e.impact_level.slice(1),P=s?`${x}${y}`:"";return`
    <div class="hw-cme">
      ...
      ${P}
    </div>`}function os(t,s,e,o,n,d,i,r,a,l,c,p,h,g,w,u,_,f,x,$,b=0){let k=Me(t,n);return`
    <div class="hw-root">
      ${ve(t)}
      ${$e(t,e,g,o,k,x,$,b)}
      ${e?be(t):""}
