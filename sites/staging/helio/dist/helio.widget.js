...
`,je=!1;function It(){if(je)return;let e=document.createElement("style");e.id="helio-widget-css",e.textContent=Et,document.head.appendChild(e),je=!0}function At(e){if(!e.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,t=32,o=e.length,n=s/o,i=e.map((r,l)=>{let a=Math.max(2,Math.min(t,r.kp/9*t)),c=t-a,d=l*n,p=r.kp>=6?"#e05c5c":r.kp>=5?"#e0a84a":r.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${d.toFixed(1)}" y="${c.toFixed(1)}" width="${(n-1).toFixed(1)}" height="${a.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${r.kp.toFixed(1)} \xB7 ${u(xe(r.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${t}" style="width:100%;height:${t}px;display:block" preserveAspectRatio="none">${i}</svg>`}function We(e,s,t,o,n){if(e.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let i=200,r=Math.min(...e),l=Math.max(...e),a=l-r||1,c=g=>o-2-(g-r)/a*(o-4),d=e.map((g,h)=>`${(h/(e.length-1)*i).toFixed(1)},${c(g).toFixed(1)}`).join(" "),p="";if(n&&r<0&&l>0){let g=c(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${i}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let m=e.map((g,h)=>`<rect x="${(h/(e.length-1)*i-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${u(s[h]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${i} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ...
    <line x1="${l}" y1="130" x2="${w}" y2="130"
  ...
  </g>`,R="";if(e==="magnetosphere"){let A=(F=s.windKms)!=null?F:0,B=A>500,W=A<350,O=(B?.5:W?1.4:.9)*1.3,U=A>700?"#e05c5c":A>500?"#e0a84a":A>350?"#d4c840":"#5cce8c",Q=l+8,J=w-14,P=8,z=6,D=(J-Q)/(P-1),oe=260/(z+1),ne=38,ee=24,ce=2,Le=Array.from({length:P},(se,de)=>{let re=Q+de*D,ve=Array.from({length:z},(ie,le)=>{let V=oe*(le+1);return`<path d="M ${re.toFixed(1)},${V.toFixed(1)} L ${(re+ne).toFixed(1)},${V.toFixed(1)} M ${(re+ee).toFixed(1)},${(V-6).toFixed(1)} L ${(re+ne).toFixed(1)},${V.toFixed(1)} L ${(re+ee).toFixed(1)},${(V+6).toFixed(1)}"
          stroke="${U}" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`}).join("");return Array.from({length:ce},(ie,le)=>{let V=-((P-de)/P*O)-le*O/ce;return`<g style="opacity:.12;animation:hw-arrow-chase ${O}s linear ${V.toFixed(3)}s infinite">${ve}</g>`}).join("")}).join(""),ae=(T=s.bz)!=null?T:null,te=ae==null?"":(()=>{let se=ae>0?"#5cce8c":"#e05c5c";return`${ae>0?`<path d="M ${a},137 L ${a},123 M ${a-3},126 L ${a},123 L ${a+3},126"
        ...
        fill="${se}" font-family="monospace">Bz${ae>0?"\u2191":"\u2193"}</text>`})();R=`${Le}${te}`}let G=`<g id="${i}-overlay-solar-wind">${R}</g>`,K="";if(e==="coronal_hole"&&s.hssState){let A=s.hssState,B=(Z=A.speed)!=null?Z:0,W=B>=420,O=A.color,U=W?"0.85":"0.25",Q=B>=500?"0.18":W?"0.10":"0.04",J=l+8,P=d-16,z=60,D=`${J},130 ${P},${130-z} ${P},${130+z}`,oe=P+6,ne=`${oe},120 ${oe+18},130 ${oe},140`;K=`
    <polygon points="${D}" fill="${O}" opacity="${Q}"/>
    <line x1="${J}" y1="130" x2="${P-5}" y2="130"
          stroke="${O}" stroke-width="3.5" stroke-dasharray="10 6"
          stroke-linecap="round" opacity="${U}"/>
    <polygon points="${ne}" fill="${O}" opacity="${W?"0.9":"0.25"}"/>`}let X=`<g id="${i}-overlay-coronal-hole">${K}</g>`,N="";if(e==="cme_cone"&&s.cmeState){let A=s.cmeState,B=l,W=d-10,O=W-B,U=D=>Math.tan(D*Math.PI/180),Q=Math.round(U(9)*O),J=Math.round(U(6)*O),P=Math.round(U(3)*O),z=D=>`${B},130 ${W},${130-D} ${W},${130+D}`;if(A.status!=="quiet"){let D=A.status==="impact"?"#e05c5c":"#d4cc5c";N=`
    <polygon points="${z(Q)}" fill="#253238" opacity="0.85"/>
    <polygon points="${z(J)}"   fill="#d4cc5c" opacity="0.14"/>
    <polygon points="${z(P)}" fill="#e0a84a" opacity="0.28"/>
  ...
  </div>`}function Yt(e,s,t,o){switch(s){case"solar_wind":return Ot(e);case"xray":return Rt(e);case"imf_bz":return zt(e);case"aurora":return Dt(t,o);case"magnetosphere":return Bt(e);default:return""}}function jt(e,s){if(e.length<2)return"\u2192";let t=e[e.length-1],o=Math.max(0,e.length-4),n=e[o];if(!isFinite(t)||!isFinite(n))return"\u2192";let i=t-n;return i>s?"\u2191":i<-s?"\u2193":"\u2192"}function Wt(e){var o;return"https://sdo.gsfc.nasa.gov/assets/img/latest/mpeg/"+((o={eit171:"latest_512_0171.mp4",eit195:"latest_512_0193.mp4",eit284:"latest_512_0211.mp4",eit304:"latest_512_0304.mp4",cont:"latest_512_HMID.mp4",mag:"latest_512_HMIB.mp4"}[e])!=null?o:"latest_512_0171.mp4")}function Ut(e,s){let t=ge[e],o=ps,n=cs(t.url,s),i=Wt(t.id);return`<div class="hw-solar-mini-wrap" style="cursor:default">
    ...
    </div>`;let $=320,x=38,f=14,b=x+f,S=$/d,C=M=>x-Math.max(2,Math.min(x-2,M/9*(x-2))),k="",L=c.map(M=>M.kp),_=qt(L);c.forEach((M,E)=>{let q=C(M.kp),F=x-q,T=E*S,Z=T+S/2,A=M.kp>=6?"#e05c5c":M.kp>=5?"#e0a84a":M.kp>=4?"#d4cc5c":"#5cce8c",B=`Kp ${M.kp.toFixed(1)} \xB7 ${ue(M.t_utc)}`;if(k+=`<rect x="${T.toFixed(1)}" y="${q.toFixed(1)}" width="${(S-1.5).toFixed(1)}" height="${F.toFixed(1)}" fill="${A}" fill-opacity="0.85" rx="1.5"/>`,k+=`<rect x="${T.toFixed(1)}" y="0" width="${S.toFixed(1)}" height="${x}" fill="transparent"><title>${I(B)}</title></rect>`,d<=8||E%2===0){let O=new Date(M.t_utc).getHours();k+=`<text x="${Z.toFixed(1)}" y="${(b-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${O.toString().padStart(2,"0")}</text>`}});let H=`<polyline points="${c.map((M,E)=>{let q=E*S+S/2,F=C(_[E]);return`${q.toFixed(1)},${F.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,R="";if(s>0&&d>0){let M=Math.min($-1,s/(d*3)*$);R=`
    ...
    <div class="hw-forecast">
      ...
      ${N}`:""}
    </div>`}function ts(e){let s=/([NS])(\d+)([EW])(\d+)/i.exec(e);return s?{lat:(s[1].toUpperCase()==="N"?1:-1)*parseInt(s[2],10),lon:(s[3].toUpperCase()==="E"?1:-1)*parseInt(s[4],10)}:null}var et=[{id:"X",label:"X-risk",color:"#e05c5c"},{id:"M",label:"M-risk",color:"#e0a84a"},{id:"C",label:"C-risk",color:"#d4cc5c"},{id:"quiet",label:"Quiet",color:"#5cce8c"}];function ss(e){return e.x_flare_probability>0?"X":e.m_flare_probability>0?"M":e.c_flare_probability>0?"C":"quiet"}function os(e,s,t){let o=s/2,n=o*.87,i=s*.03,r=s*.009,l=e.map(a=>{var $,x;let c=ts(a.location);if(!c||Math.abs(c.lon)>88||a.location.includes("*"))return"";let d=ss(a);if(!t.has(d))return"";let p=et.find(f=>f.id===d).color,m=c.lat*Math.PI/180,g=c.lon*Math.PI/180,h=(o+n*Math.cos(m)*Math.sin(g)).toFixed(1),w=(o-n*Math.sin(m)).toFixed(1),v=`AR ${a.region} \xB7 ${a.location}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    ${m}
  </div>`}function Ss(e,s,t,o,n,i,r,l,a){var Ee,Ie,Ae,Te;let c=(Ie=(Ee=s==null?void 0:s.impacts)!=null?Ee:e.observer_impacts)!=null?Ie:[],d=ks(c,e),p=_s(d,r,l,a),m=c.find(ye=>ye.kind==="radio"),g=c.find(ye=>ye.kind==="solar_activity"),h=m?Ge(m,t,n,i,e,r.has("radio")):"",w=g?Ge(g,t,n,i,e,!1):"",x=`
    <div class="hw-section-row" data-impacts-toggle>
      ...
      ${ws(e,Y)}
    </div>`,Z=r.has("solar_cycle"),A=(Te=nt[we.phase])!=null?Te:"#96a8b8",B=we.phase.charAt(0).toUpperCase()+we.phase.slice(1),O=`<div class="hw-impact-row${Z?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      <span class="hw-impact-badge" style="background:${A}22;color:${A}">${B}</span>
      ${xs(Z)}
    </div>`,U=at(e),Q=r.has("hss"),P=`<div class="hw-impact-row${Q?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${ys(e,le)}
    </div>`,he=be(e),He=r.has("magnetosphere"),ht=`<div class="hw-impact-row${He?" hw-impact-open":""}" data-impact-row="magnetosphere">
      <span class="hw-impact-caret">\u25B6</span>
    ...
    <div class="hw-impacts">
      ${x}
      ${o?[p,T,h,ne,ae,G,w,N,K,ht,X,P,ve,pt,O].join(""):""}
...
${t.join(" \xB7 ")}`}function Is(e,s,t){var a;let o=e.is_active?Ls:(a=Ms[e.level])!=null?a:"#445c64",n=e.event_time===t,i=e.event_time.slice(11,16)+" UTC",r=e.source==="NASA_DONKI"?"DONKI":"SWPC",l=n?`<div class="hw-tl-detail">${u(e.description)}${u(Es(e))}</div>`:"";return`
    ...
    <div class="hw-timeline">
      ${h}
      ${v}
    </div>`}function Ts(e,s,t){var c;let o=(c=e.alerts_all)!=null?c:[],n=o.length,i=s?"\u25BC":"\u25B6",r=n>0?`SWPC Alerts (${n})`:"SWPC Alerts",l=`
    ...
    <div class="hw-alerts">
      ${l}
      ${a}
    </div>`}function Os(e,s){var K,X,N;let t=e.cme_tracker;if(!t)return"";let o=(K=yt[t.impact_level])!=null?K:"#96a8b8",n=(X=kt[t.status])!=null?X:t.status,i=300,r=44,l=18,a=r/2,c=10,d=i-18,p=7,m=`<line x1="${l+c}" y1="${a}" x2="${d-p}" y2="${a}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,g=`<circle cx="${l}" cy="${a}" r="${c}" fill="#f0c040" opacity="0.92"/>`,h=`
    ...
    ${h}${v}
    ${$}
  </svg>`,f=Pe(t.arrival_time_utc),b=Pe(t.launch_time_utc),S=t.speed_kms!=null?`${Math.round(t.speed_kms)} km/s`:"\u2014",C=t.half_angle_deg!=null?`${t.half_angle_deg}\xB0`:"\u2014",k=(N=t.source_location)!=null?N:"\u2014",L=t.is_earth_direct?"Direct hit":"Glancing blow",_=t.progress!=null?`${Math.round(t.progress*100)}%`:"\u2014",y=`
    ...
    <div class="hw-cme">
      ...
      ${G}
    </div>`}function Rs(e,s,t,o,n,i,r,l,a,c,d,p,m,g,h,w,v,$,x=0){var _;let f=Jt(e,o),b=(_=e.metrics.kp_history_1h)!=null?_:[],S=b.length?ue(b[b.length-1].t_utc):null,C=S?`Recent history \xB7 Last step ${S}`:"Recent history",k=`<div style="padding:10px 14px;border-bottom:1px solid #1e2c30"><div class="hw-section-row" data-hero-toggle style="margin-bottom:0">
    <span class="hw-section-caret">${t?"\u25BC":"\u25B6"}</span>
    <span class="hw-section-label" style="margin-bottom:0">${I(C)}</span>
  </div></div>`,L=t?Kt(e):"";return`
    <div class="hw-root">
      ${Xt(e)}
      ${Gt(e,t,f,v,$,x)}
      ${Ss(e,f,m,c,g,h,w,$,v)}
