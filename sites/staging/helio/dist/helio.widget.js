...
`,Et=!1;function he(){if(Et)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=pe,document.head.appendChild(t),Et=!0}function me(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,a=s/o,d=t.map((i,l)=>{let n=Math.max(2,Math.min(e,i.kp/9*e)),r=e-n,c=l*a,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${c.toFixed(1)}" y="${r.toFixed(1)}" width="${(a-1).toFixed(1)}" height="${n.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(vt(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${d}</svg>`}function Rt(t,s,e,o,a){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let d=200,i=Math.min(...t),l=Math.max(...t),n=l-i||1,r=g=>o-2-(g-i)/n*(o-4),c=t.map((g,w)=>`${(w/(t.length-1)*d).toFixed(1)},${r(g).toFixed(1)}`).join(" "),p="";if(a&&i<0&&l>0){let g=r(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${d}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((g,w)=>`<rect x="${(w/(t.length-1)*d-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[w]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${d} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ...
    <line x1="${l}" y1="130" x2="${u}" y2="130"
  ...
  </g>`,A="";if(t==="magnetosphere"){let O=(R=s.windKms)!=null?R:0,D=O>500,Y=O<350,P=(D?.5:Y?1.4:.9)*1.3,j=O>700?"#e05c5c":O>500?"#e0a84a":O>350?"#d4c840":"#5cce8c",Z=l+8,tt=u-14,L=8,I=6,B=(tt-Z)/(L-1),nt=260/(I+1),et=38,at=24,rt=2,wt=Array.from({length:L},(lt,pt)=>{let it=Z+pt*B,ct=Array.from({length:I},(kt,st)=>{let ot=nt*(st+1);return`<path d="M ${it.toFixed(1)},${ot.toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} M ${(it+at).toFixed(1)},${(ot-6).toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} L ${(it+at).toFixed(1)},${(ot+6).toFixed(1)}"
        ...
        fill="${lt}" font-family="monospace">Bz${F>0?"\u2191":"\u2193"}</text>`})();A=`${wt}${dt}`}let G=`<g id="${d}-overlay-solar-wind">${A}</g>`,V="";if(t==="coronal_hole"&&s.hssState){let O=s.hssState,D=(q=O.speed)!=null?q:0,Y=D>=420,P=O.color,j=Y?"0.85":"0.25",Z=D>=500?"0.18":Y?"0.10":"0.04",tt=l+8,L=c-16,I=60,B=`${tt},130 ${L},${130-I} ${L},${130+I}`,nt=L+6,et=`${nt},120 ${nt+18},130 ${nt},140`;V=`
    ...
    <polygon points="${et}" fill="${P}" opacity="${Y?"0.9":"0.25"}"/>`}let K=`<g id="${d}-overlay-coronal-hole">${V}</g>`,E="";if(t==="cme_cone"&&s.cmeState){let O=s.cmeState,D=l,Y=c-10,P=Y-D,j=B=>Math.tan(B*Math.PI/180),Z=Math.round(j(9)*P),tt=Math.round(j(6)*P),L=Math.round(j(3)*P),I=B=>`${D},130 ${Y},${130-B} ${Y},${130+B}`;if(O.status!=="quiet"){let B=O.status==="impact"?"#e05c5c":"#d4cc5c";E=`
    ...
    <line x1="${D+14}" y1="130" x2="${Y-5}" y2="130"
  ...
  </div>`}function ke(t,s,e,o){switch(s){case"solar_wind":return ge(t);case"xray":return we(t);case"imf_bz":return $e(t);case"aurora":return ye(e,o);case"magnetosphere":return be(t);default:return""}}function ft(t,s){if(t.length<2)return"\u2192";let e=t[t.length-1],o=Math.max(0,t.length-4),a=t[o];if(!isFinite(e)||!isFinite(a))return"\u2192";let d=e-a;return d>s?"\u2191":d<-s?"\u2193":"\u2192"}function _e(t,s){let e=mt[s],o=Be;return`<div class="hw-solar-mini-wrap" style="cursor:default">
  ...
  </div>`}function Se(t,s,e,o,a,d,i,l=0){var q,O,D,Y,P,j,Z,tt;let{summary:n,scales:r,metrics:c,aurora_hint:p}=t,h=(q=Ft[n.status])!=null?q:Ft.quiet,g=a!=null?a.kp.toFixed(1):c.kp_latest!=null?c.kp_latest.toFixed(1):"\u2014",u=[a?a.gScale:r.g_scale,r.r_scale,r.s_scale].map(L=>{let I=de(L),B=I?`color:${h.accent};border-color:${h.accent}33`:"";return`<span class="hw-scale-chip${I?" hw-scale-active":""}" style="${B}">${m(L)}</span>`}).join(""),f=a?a.auroraLabel:p.aurora_label,$=f==="good"?"#5cce8c":f==="possible"?"#d4cc5c":"#607880",x=f.charAt(0).toUpperCase()+f.slice(1),b="#b4c6cc",v=c.solar_wind_kms!=null?`${Math.round(c.solar_wind_kms)} km/s`:"\u2014",_=c.imf_bz_nt,H=_!=null?_<=-10?"#e05c5c":_<=-5?"#e0a84a":_>=5?"#5cce8c":"#a0b4b8":"#607880",M=_!=null?(_>=0?"+":"")+_.toFixed(1)+" nT":"\u2014",C=c.xray_class,y=C?(O=Ht[C])!=null?O:"#a0b4b8":"#607880",k=C?`${C}-class`:"\u2014",z=ft(((D=c.kp_history_1h)!=null?D:[]).map(L=>L.kp),.5),A=ft(((Y=c.wind_history_1h)!=null?Y:[]).map(L=>L.kms),20),G=ft(((P=c.bz_history_1h)!=null?P:[]).map(L=>L.bz),1.5),V=ft(((j=c.xray_history_1h)!=null?j:[]).map(L=>Math.log10(L.flux+1e-9)),.15),K=(Z=c.kp_history_1h)!=null?Z:[],E=K.length?ht(K[K.length-1].t_utc):null,U=E?`Recent history \xB7 Last step ${E}`:"Recent history",J=gt(t),S=(tt=c.kp_latest)!=null?tt:0,T=S>=5,X=T?`linear-gradient(160deg, #0d2a1a 0%, ${h.bg}22 75%)`:`${h.bg}18`,R=(L,I,B,nt,et)=>{let at=et?`<span class="hw-trend">${et}</span>`:"";return`<div class="hw-kpi-item${o===L?" hw-kpi-active":""}" data-kpi="${L}">
    ...
    <div class="hw-hero" style="background:${X}">
      ...
      <div class="hw-quick-details">
        ${R("aurora","Aurora",m(x),$)}
        ${R("solar_wind","Solar wind",m(v),b,A)}
...
C: ${n.c_flare_probability}%  M: ${n.m_flare_probability}%  X: ${n.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}function Ke(t,s){let e=jt(t),o=We(t),a=(()=>{var g;let r=(g=t.metrics.kp_forecast_3h)!=null?g:[],c=Date.now(),p=c+24*60*60*1e3,h=r.filter(w=>new Date(w.t_utc).getTime()<=p);return h.length?Math.max(...h.map(w=>w.kp)):null})(),i=[{key:"g1",label:"G1"},{key:"g2",label:"G2"},{key:"g3",label:"G3"}].map(({key:r,label:c})=>{let p=e[r],h=Yt[r];return`<div class="hw-gstorm-row">
    ...
    <div class="hw-cme-footer">${i}</div>
        ...
        <div class="hw-aurora-obs-panel">
          ...
          ${Vt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else F.kind==="radio"?st=je(t,ct):st=`<div class="hw-impact-tip${ct?" hw-impact-tip-open":""}">${m(F.summary)}</div>`;let ot=F.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${W(F.kind)}"`;return`<div class="hw-impact-row${kt}"${ot}>
    ...
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,f=i.has("geomag_storm"),$=jt(t),x=$.g1,b=$.g1>=30?Yt.g1:$.g1>0?"#7a9298":"#607880",v=x>0?`G1 ${x}%`:"None",H=`<div class="hw-impact-row${f?" hw-impact-open":""}" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ke(t,f)}
    </div>`,M=i.has("solar_cycle"),C=(wt=Wt[bt.phase])!=null?wt:"#96a8b8",y=bt.phase.charAt(0).toUpperCase()+bt.phase.slice(1),z=`<div class="hw-impact-row${M?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ve(t,O)}
    </div>`,j=qt(t),Z=i.has("cme_cone"),L=`<div class="hw-impact-row${Z?" hw-impact-open":""}" data-impact-row="cme_cone">
      <span class="hw-impact-caret">\u25B6</span>
...
${e.join(" \xB7 ")}`}function as(t,s,e){var n;let o=t.is_active?ss:(n=es[t.level])!=null?n:"#445c64",a=t.event_time===e,d=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",l=a?`<div class="hw-tl-detail">${m(t.description)}${m(ns(t))}</div>`:"";return`
    ...
    ${w}${f}
    ${$}
  </svg>`,b=At(e.arrival_time_utc),v=At(e.launch_time_utc),_=e.speed_kms!=null?`${Math.round(e.speed_kms)} km/s`:"\u2014",H=e.half_angle_deg!=null?`${e.half_angle_deg}\xB0`:"\u2014",M=(E=e.source_location)!=null?E:"\u2014",C=e.is_earth_direct?"Direct hit":"Glancing blow",y=e.progress!=null?`${Math.round(e.progress*100)}%`:"\u2014",k=`
