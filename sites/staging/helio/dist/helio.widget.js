...
.hw-spi-params{font-size:.70em;color:#607880;margin-top:7px;padding-top:6px;border-top:1px solid #1e2c30;text-align:center}
...
C: ${a.c_flare_probability}%  M: ${a.m_flare_probability}%  X: ${a.x_flare_probability}%`;return`<g style="pointer-events:all">
  ...
  </div>`}var at={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Et={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function Ie(t){var v;let s=at,e=(v=Et[s.phase])!=null?v:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),n=280,c=52,i=10,r=c-6,a=c-18,l=.5,p=.19,d=y=>Math.exp(-Math.pow((y-l)/p,2)/2),h=y=>i+y*(n-2*i),w=y=>r-d(y)*a,g=80,m=[];for(let y=0;y<=g;y++){let F=y/g;m.push(`${y===0?"M":"L"}${h(F).toFixed(1)},${w(F).toFixed(1)}`)}let k=Math.round(s.progress_0_1*g),x=[];for(let y=0;y<=k;y++){let F=y/g;x.push(`${y===0?"M":"L"}${h(F).toFixed(1)},${w(F).toFixed(1)}`)}let f=h(s.progress_0_1),b=[`M${i},${r}`,...x.slice(1),`L${f.toFixed(1)},${r} Z`],$=w(s.progress_0_1),L=5,H=`M${f.toFixed(1)},${$.toFixed(1)} L${(f-L).toFixed(1)},${($-L*1.8).toFixed(1)} L${(f+L).toFixed(1)},${($-L*1.8).toFixed(1)} Z`,M=r+11;return`<div class="hw-impact-tip${t?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function It(t){var l,p,d;let s=(l=t.scales.g_scale)!=null?l:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let h=(p=t.metrics.kp_forecast_3h)!=null?p:[],w=Date.now(),g=h.filter(m=>new Date(m.t_utc).getTime()<=w+3*60*60*1e3).sort((m,k)=>new Date(k.t_utc).getTime()-new Date(m.t_utc).getTime());g.length>0&&(o=g[0].kp)}let n=0;e>=2||o!=null&&o>=6?n=2:(e>=1||o!=null&&o>=4)&&(n=1);let c=parseInt(((d=t.scales.r_scale)!=null?d:"R0").slice(1),10),i=c>=2&&n<2;c>=2&&(n=Math.min(2,n+1));let r={0:"#5cce8c",1:"#d4cc5c",2:"#e05c5c"},a={0:"Low",1:"Moderate",2:"High"};return{level:n,color:r[n],label:a[n],kp:o,gScale:s,boostedByFlare:i}}function De(t,s){var l;let e=It(t),o=s?" hw-impact-tip-open":"",c=[{l:0,label:"Low",color:"#5cce8c",width:33},{l:1,label:"Moderate",color:"#d4cc5c",width:64},{l:2,label:"High",color:"#e05c5c",width:100}].map(p=>{let d=p.l===e.level,h=d?p.color:"#566068",w=d?"0.88":"0.16";return`<div class="hw-gnss-rung">
    ...
    <div class="hw-swdp-meta" style="font-size:.72em">Speed ${u(M)} \xB7 Density ${u(H)}</div>
  </div>`}function Nt(t){var l,p;let s=(l=t.alerts_all)!=null?l:[],e=s.find(d=>d.kind==="cme_impact"),o=s.find(d=>d.kind==="cme_watch"),n=e!=null?e:o;if(!n)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let c=((p=n.raw_body)!=null?p:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),i=c?parseInt(c[1],10):null,r=null;if(i&&n.t_utc){let d=1496e5/i*1e3;r=new Date(new Date(n.t_utc).getTime()+d).toISOString().replace(".000Z","Z")}let a=e?"impact":"watch";return{status:a,color:a==="impact"?"#e05c5c":"#d4cc5c",label:a==="impact"?"Active":"Watch",speed_kms:i,issued_utc:n.t_utc,arrival_utc:r}}function je(t,s){let e=Nt(t),o=s?" hw-impact-tip-open":"",n=320,c=80,i=24,r=40,a=268,l=296,p=a-i,d=_=>Math.tan(_*Math.PI/180),h=Math.round(d(9)*p),w=Math.round(d(6)*p),g=Math.round(d(3)*p),m=_=>`${i},${r} ${a},${r-_} ${a},${r+_}`,k=e.status==="impact"?r:e.status==="watch"?r+w+8:r,x=Math.min(c-14,Math.max(14,k)),f=Math.abs(x-r)<=g,b=Math.abs(x-r)<=w,$=Math.abs(x-r)<=h,L=f?"#e05c5c":b?"#d4cc5c":$?"#e0a84a":"#5cce8c",H=e.status!=="quiet"?`<polygon points="${m(h)}" fill="#253238" opacity="0.85"/>
       ...
       <line x1="${i+9}" y1="${r}" x2="${a-2}" y2="${r}"
             ...
             stroke="#1e2c30" stroke-dasharray="4 3" stroke-width="1"/>`,M=8,z=11,v=15,y="#f5c540",F=[0,45,90,135,180,225,270,315].map(_=>{let A=_*Math.PI/180,E=(i+z*Math.cos(A)).toFixed(1),O=(r+z*Math.sin(A)).toFixed(1),B=(i+v*Math.cos(A)).toFixed(1),G=(r+v*Math.sin(A)).toFixed(1),U=_<45||_>315?"0.9":"0.5";return`<line x1="${E}" y1="${O}" x2="${B}" y2="${G}"
    ...
    <div class="hw-cme-footer">${I}</div>
  </div>`}function We(t,s,e,o,n,c,i,r,a){var tt,rt,xt;let l=(rt=(tt=s==null?void 0:s.impacts)!=null?tt:t.observer_impacts)!=null?rt:[],p=l.map(j=>{var $t,ft;let lt=($t=Jt[j.level])!=null?$t:"#666",Bt=j.level==="none"?"None":j.level.charAt(0).toUpperCase()+j.level.slice(1),jt=(ft=He[j.kind])!=null?ft:ze,Wt=j.level==="none"?"#606870":lt,et=j.kind==="solar_activity"?n:i.has(j.kind),Kt=et?" hw-impact-open":"",Z;if(j.kind==="solar_activity"){let st=n?" hw-solar-open":"",ct=Ft.map(K=>{let ot=c.has(K.id),dt=ot?K.color+"22":"transparent",pt=ot?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${K.id}" style="color:${K.color};border-color:${K.color};background:${dt};opacity:${pt}">${K.label}</button>`}).join("");Z=`<div class="hw-solar-tip${st}">
        ...
        </div>`}else if(j.kind==="aurora"){let st=et?" hw-aurora-tip-open":"",ct=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,K=null;r&&a.lat!=null&&a.lon!=null&&(K=Lt(r.entries,a.lat,a.lon));let ot=a.lat!=null&&a.lon!=null,dt=K!=null?K>=30?"#5cce8c":K>=10?"#d4cc5c":"#9ab4bc":"#607880",pt=K!=null?`${K}%`:r?"n/a":"\u2026",Xt=ot?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
    ...
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,k=i.has("geomag_storm"),x=At(t),f=x.g1,b=x.g1>=30?Tt.g1:x.g1>0?"#7a9298":"#607880",$=f>0?`G1 ${f}%`:"None",H=`<div class="hw-impact-row${k?" hw-impact-open":""}" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Re(t,k)}
    </div>`,M=i.has("solar_cycle"),z=(xt=Et[at.phase])!=null?xt:"#96a8b8",v=at.phase.charAt(0).toUpperCase()+at.phase.slice(1),F=`<div class="hw-impact-row${M?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ie(M)}
    </div>`,C=Ot(t),N=i.has("hss"),D=`<div class="hw-impact-row${N?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Pe(t,N)}
    </div>`,I=Rt(t),P=i.has("sat_drag"),T=`<div class="hw-impact-row${P?" hw-impact-open":""}" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${De(t,A)}
    </div>`,B=Pt(t),G=i.has("sw_pressure"),U=B.pressure!=null?`${B.pressure.toFixed(2)} nPa`:"\u2014",W=`<div class="hw-impact-row${G?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
...
${e.join(" \xB7 ")}`}function qe(t,s,e){var a;let o=t.is_active?Xe:(a=Ge[t.level])!=null?a:"#445c64",n=t.event_time===e,c=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",r=n?`<div class="hw-tl-detail">${u(t.description)}${u(Ye(t))}</div>`:"";return`
    ...
    </div>`}function Ve(t,s,e,o){var x,f;let n=(x=t.timeline)!=null?x:[],c=Date.now(),i=new Date(c).toISOString().slice(0,10),r=new Date(c-864e5).toISOString().slice(0,10),a=new Date(c-1728e5).toISOString().slice(0,10),l=new Set([i,r,a]),p=n.filter(b=>{var $;return l.has((($=b.event_time)!=null?$:"").slice(0,10))}).slice().reverse(),d=p.length,h=e?"\u25BC":"\u25B6",w=d>0?`Solar Activity Timeline (${d})`:"Solar Activity Timeline",g=`
    ...
    <div class="hw-timeline">
      ${g}
      ${k}
    </div>`}function Ze(t,s,e){var l;let o=(l=t.alerts_all)!=null?l:[],n=o.length,c=s?"\u25BC":"\u25B6",i=n>0?`SWPC Alerts (${n})`:"SWPC Alerts",r=`
    ...
    </div>`;if(!s||n===0)return`<div class="hw-alerts">${r}${s&&n===0?'<div class="hw-empty-alerts">No significant recent SWPC alerts</div>':""}</div>`;let a=o.map(p=>Ke(p,p.dedupe_key===e)).join("");return`
    <div class="hw-alerts">
      ${r}
      ${a}
    ...
    <circle cx="${p}" cy="${a}" r="2.5" fill="#fff" opacity="0.7"/>`,m=`<text x="${r}" y="${a+l+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,k=`<text x="${p}" y="${a+d+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,x="";if(e.progress!=null){let P=r+l+4,S=p-d-4,T=P+e.progress*(S-P),_=5;e.status==="arrival_window"?x=`
        ...
        </g>`:x=`<circle cx="${T.toFixed(1)}" cy="${a}" r="${_}" fill="${o}" opacity="0.85"/>`}let f=`<svg class="hw-cme-svg" viewBox="0 0 ${c} ${i}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${h}
    ${w}${m}
    ${g}${k}
    ${x}
    ...
    <div class="hw-cme">
      ...
      ${N}
    </div>`}function Je(t,s,e,o,n,c,i,r,a,l,p,d,h,w,g,m,k,x,f,b){let $=_e(t,n);return`
    <div class="hw-root">
      ${fe(t)}
      ${xe(t,e,w,o,$,f,b)}
      ${e?$e(t):""}
