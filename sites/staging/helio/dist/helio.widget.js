"use strict";var HelioWidgetModule=(()=>{var Ct=Object.defineProperty;var Zt=Object.getOwnPropertyDescriptor;var Jt=Object.getOwnPropertyNames;var te=Object.prototype.hasOwnProperty;var ee=(t,s)=>{for(var e in s)Ct(t,e,{get:s[e],enumerable:!0})},se=(t,s,e,o)=>{if(s&&typeof s=="object"||typeof s=="function")for(let a of Jt(s))!te.call(t,a)&&a!==e&&Ct(t,a,{get:()=>s[a],enumerable:!(o=Zt(s,a))||o.enumerable});return t};var oe=t=>se(Ct({},"__esModule",{value:!0}),t);var hs={};ee(hs,{HelioWidget:()=>Qt});var Ft={quiet:{bg:"#1a2e22",accent:"#5cce8c"},active:{bg:"#2e2a1a",accent:"#d4cc5c"},elevated:{bg:"#2e1f10",accent:"#e0a84a"},storm:{bg:"#2e1212",accent:"#e05c5c"}},ne={none:"#666",low:"#5cce8c",moderate:"#e0a84a",high:"#e05c5c"},ae={info:"#666",watch:"#d4cc5c",warning:"#e05c5c"},Ht={A:"#888",B:"#5cce8c",C:"#aad47a",M:"#e0a84a",X:"#e05c5c"},ie={low:"#5cce8c",moderate:"#d4cc5c",high:"#e05c5c",unknown:"#96a8b8"},re={detected:"Detected",inbound:"Inbound",arrival_window:"Arriving",arrived:"Arrived"};function le(t){if(!t)return"Update time unavailable";try{let s=Math.round((Date.now()-new Date(t).getTime())/6e4);if(s<1)return"Updated just now";if(s<60)return`Updated ${s} min ago`;let e=Math.floor(s/60);return e<24?`Updated ${e}h ago`:`Updated ${Math.floor(e/24)}d ago`}catch(s){return"Updated recently"}}function ce(t){if(!t)return"\u2014";try{return new Date(t).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return t}}function ht(t){if(!t)return"";try{return new Date(t).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return t}}function vt(t){try{return new Date(t).toLocaleString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(s){return t.slice(11,16)}}function At(t){if(!t)return"\u2014";try{return new Date(t).toLocaleString("en-GB",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1})+" UTC"}catch(s){return t}}function W(t){return t.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function m(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function de(t){return parseInt(t.slice(1),10)>0}var pe=`
...
`,Et=!1;function he(){if(Et)return;let t=document.createElement("style");t.id="helio-widget-css",t.textContent=pe,document.head.appendChild(t),Et=!0}function me(t){if(!t.length)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.length,a=s/o,d=t.map((i,l)=>{let n=Math.max(2,Math.min(e,i.kp/9*e)),r=e-n,c=l*a,p=i.kp>=6?"#e05c5c":i.kp>=5?"#e0a84a":i.kp>=4?"#d4cc5c":"#5cce8c";return`<rect x="${c.toFixed(1)}" y="${r.toFixed(1)}" width="${(a-1).toFixed(1)}" height="${n.toFixed(1)}" fill="${p}" rx="1"><title>Kp ${i.kp.toFixed(1)} \xB7 ${m(vt(i.t_utc))} UTC</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">${d}</svg>`}function Rt(t,s,e,o,a){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let d=200,i=Math.min(...t),l=Math.max(...t),n=l-i||1,r=g=>o-2-(g-i)/n*(o-4),c=t.map((g,w)=>`${(w/(t.length-1)*d).toFixed(1)},${r(g).toFixed(1)}`).join(" "),p="";if(a&&i<0&&l>0){let g=r(0);p=`<line x1="0" y1="${g.toFixed(1)}" x2="${d}" y2="${g.toFixed(1)}" stroke="#2a3438" stroke-width="0.8" stroke-dasharray="3,2"/>`}let h=t.map((g,w)=>`<rect x="${(w/(t.length-1)*d-4).toFixed(1)}" y="0" width="8" height="${o}" fill="transparent"><title>${m(s[w]||"")} \xB7 ${g.toFixed(1)}</title></rect>`).join("");return`<svg viewBox="0 0 ${d} ${o}" style="width:100%;height:${o}px;display:block" preserveAspectRatio="none">
    ${p}
    <polyline points="${c}" fill="none" stroke="${e}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
    ${h}
  </svg>`}function ue(t){if(t.length<2)return'<div style="color:#405058;font-size:.72em;padding:4px">No data</div>';let s=200,e=32,o=t.map(c=>Math.max(-9,Math.min(-3,Math.log10(c.flux)))),a=Math.min(...o),i=Math.max(...o)-a||1,l=c=>e-2-(c-a)/i*(e-4),n=o.map((c,p)=>`${(p/(o.length-1)*s).toFixed(1)},${l(c).toFixed(1)}`).join(" "),r=t.map((c,p)=>{let h=p/(o.length-1)*s,g=c.flux>=1e-4?"X":c.flux>=1e-5?"M":c.flux>=1e-6?"C":c.flux>=1e-7?"B":"A";return`<rect x="${(h-4).toFixed(1)}" y="0" width="8" height="${e}" fill="transparent"><title>${m(vt(c.t_utc))} \xB7 ${g}-class (${c.flux.toExponential(2)})</title></rect>`}).join("");return`<svg viewBox="0 0 ${s} ${e}" style="width:100%;height:${e}px;display:block" preserveAspectRatio="none">
    ...
    <div class="hw-kpi-hint" style="margin-top:4px">Trend history: \u25B6 HISTORY</div>
  </div>`}function xe(t){let n='<rect x="0" y="16" width="200" height="6" rx="3" fill="#1e2c30"/>',r=[-10,-5,5,10].map(y=>{let k=100+y/20*100;return`<line x1="${k.toFixed(1)}" y1="16" x2="${k.toFixed(1)}" y2="22" stroke="#2a3c42" stroke-width="1"/>`}).join(""),c='<line x1="100" y1="14" x2="100" y2="24" stroke="#3a4c52" stroke-width="1.5"/>';if(t==null)return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">${n}${r}${c}</svg>`;let p=t<=-10?"#e05c5c":t<=-5?"#e0a84a":t<0?"#d4b84a":t>=5?"#5cce8c":"#7acca8",h=Math.max(-20,Math.min(20,t)),g=100+h/20*100,w=3,u=h<0?g-w:100-w,b=Math.max(2*w,Math.abs(g-100)+2*w),$=`<rect x="${u.toFixed(1)}" y="16" width="${b.toFixed(1)}" height="6" rx="${w}" fill="${p}" opacity="0.82"/>`,x=5,f=15,v=f-x*1.1,_=`<polygon points="${g.toFixed(1)},${f.toFixed(1)} ${(g-x).toFixed(1)},${v.toFixed(1)} ${(g+x).toFixed(1)},${v.toFixed(1)}" fill="${p}"/>`,H=`<line x1="${g.toFixed(1)}" y1="${f.toFixed(1)}" x2="${g.toFixed(1)}" y2="${19 .toFixed(1)}" stroke="${p}" stroke-width="1" opacity="0.6"/>`,C=`<text x="${g.toFixed(1)}" y="31" text-anchor="middle" font-size="8" fill="${p}" font-weight="600">${t>=0?"+":""}${t.toFixed(1)}</text>`;return`<svg viewBox="0 0 200 36" style="width:100%;height:36px;display:block">
    ${n}${r}${c}${$}${_}${H}${C}
    ...
    <div style="font-size:.65em;color:#607880">Coupling: <span style="color:${c.color};font-weight:600">${m(c.coupling)}</span> \xB7 Trend history: \u25B6 Details</div>
      ...
      ${C}
    </svg>`}else{let $=t.state==="storm"?16:t.state==="active"?26:38,x=155-$,f=t.state==="storm"?22:t.state==="active"?30:40,v=120-f,_=240,H=[`M ${x},60`,`C ${x-4},42 150,${f} 155,${f}`,`C 173,${f} ${_-5},${f+18} ${_},60`,`C ${_-5},${v-18} 173,${v} 155,${v}`,`C 150,${v} ${x-4},78 ${x},60`,"Z"].join(" "),M=`M ${x+2},60 C ${x+2},${60-$*.4} 152,54 150,60 C 152,66 ${x+2},${60+$*.4} ${x+2},60 Z`,C=i>700?"#e05c5c":i>500?"#e0a84a":i>350?"#d4c840":"#5cce8c",y=i>700?.4:i>500?.65:i>350?1.1:1.8,k=i>500?[10,24,40,57,74,90,106]:i>350?[14,34,57,82,104]:[20,50,82,108],z=16,A=22,G=x-6,V=Math.ceil((G-A)/z)+2,K=Array.from({length:V},(R,N)=>A-z+N*z),E=12,U=8,J=K.flatMap(R=>k.map(N=>`<path d="M ${R},${N} L ${R+E},${N} M ${R+U},${N-3} L ${R+E},${N} L ${R+U},${N+3}" stroke="${C}cc" stroke-width="1.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`)).join(""),S=`<g class="hw-wg-full" style="animation-duration:${y}s">${J}</g>`,T=s==null?"":s>0?'<path d="M 155,64 L 155,56 M 153,58 L 155,56 L 157,58" stroke="#5cce8c" stroke-width="1.3" fill="none" stroke-linecap="round"/>':'<path d="M 155,56 L 155,64 M 153,62 L 155,64 L 157,62" stroke="#e05c5c" stroke-width="1.3" fill="none" stroke-linecap="round"/>',X=s==null?"":`<text x="163" y="62" font-size="6" fill="${s>0?"#5cce8c":"#e05c5c"}" font-family="monospace">Bz${s>0?"\u2191":"\u2193"}</text>`;return yt("magnetosphere",{magnetInfo:t,bz:s,windKms:e,uid:a})}}function yt(t,s={}){var S,T,X,R,N,q;let d=(S=s.uid)!=null?S:"hse",i=0,l=160,n=800,r=17,c=n-r,p=s.magnetInfo,h=(T=p==null?void 0:p.state)!=null?T:"stable",g=(X=p==null?void 0:p.color)!=null?X:"#e0a84a",u=n-(h==="storm"?55:h==="active"?80:110),b=h==="storm"?58:h==="active"?76:95,$=1120,x=20,f=[`M ${u},130`,`C ${u-8},${130-b*.55} ${n-18},${130-b} ${n},${130-b}`,`C ${n+120},${130-b} ${$-180},${130-x} ${$},${130-x}`,`L ${$},${130+x}`,`C ${$-180},${130+x} ${n+120},${130+b} ${n},${130+b}`,`C ${n-18},${130+b} ${u-8},${130+b*.55} ${u},130`,"Z"].join(" "),v=t==="magnetosphere",_=v?h==="storm"?"0.12":"0.08":"0.04",H=v?"0.75":"0.28",M=`<g id="${d}-base-sun">
    ...
    ${p?`<text x="${u+5}" y="${130-b-7}" font-size="12" fill="${g}"
    ...
    <line x1="${l}" y1="130" x2="${u}" y2="130"
  ...
  </g>`,A="";if(t==="magnetosphere"){let O=(R=s.windKms)!=null?R:0,D=O>500,j=O<350,P=(D?.5:j?1.4:.9)*1.3,Y=O>700?"#e05c5c":O>500?"#e0a84a":O>350?"#d4c840":"#5cce8c",Z=l+8,tt=u-14,L=8,I=6,B=(tt-Z)/(L-1),nt=260/(I+1),et=38,at=24,rt=2,wt=Array.from({length:L},(lt,pt)=>{let it=Z+pt*B,ct=Array.from({length:I},(kt,st)=>{let ot=nt*(st+1);return`<path d="M ${it.toFixed(1)},${ot.toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} M ${(it+at).toFixed(1)},${(ot-6).toFixed(1)} L ${(it+et).toFixed(1)},${ot.toFixed(1)} L ${(it+at).toFixed(1)},${(ot+6).toFixed(1)}"
          stroke="${Y}" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`}).join("");return Array.from({length:rt},(kt,st)=>{let ot=-((L-pt)/L*P)-st*P/rt;return`<g style="opacity:.12;animation:hw-arrow-chase ${P}s linear ${ot.toFixed(3)}s infinite">${ct}</g>`}).join("")}).join(""),F=(N=s.bz)!=null?N:null,dt=F==null?"":(()=>{let lt=F>0?"#5cce8c":"#e05c5c";return`${F>0?`<path d="M ${n},137 L ${n},123 M ${n-3},126 L ${n},123 L ${n+3},126"
        ...
        fill="${lt}" font-family="monospace">Bz${F>0?"\u2191":"\u2193"}</text>`})();A=`${wt}${dt}`}let G=`<g id="${d}-overlay-solar-wind">${A}</g>`,V="";if(t==="coronal_hole"&&s.hssState){let O=s.hssState,D=(q=O.speed)!=null?q:0,j=D>=420,P=O.color,Y=j?"0.85":"0.25",Z=D>=500?"0.18":j?"0.10":"0.04",tt=l+8,L=c-16,I=60,B=`${tt},130 ${L},${130-I} ${L},${130+I}`,nt=L+6,et=`${nt},120 ${nt+18},130 ${nt},140`;V=`
    ...
    <polygon points="${et}" fill="${P}" opacity="${j?"0.9":"0.25"}"/>`}let K=`<g id="${d}-overlay-coronal-hole">${V}</g>`,E="";if(t==="cme_cone"&&s.cmeState){let O=s.cmeState,D=l,j=c-10,P=j-D,Y=B=>Math.tan(B*Math.PI/180),Z=Math.round(Y(9)*P),tt=Math.round(Y(6)*P),L=Math.round(Y(3)*P),I=B=>`${D},130 ${j},${130-B} ${j},${130+B}`;if(O.status!=="quiet"){let B=O.status==="impact"?"#e05c5c":"#d4cc5c";E=`
    ...
    <circle cx="${n}" cy="130" r="${r+8}" fill="none"
            stroke="${B}" stroke-width="7" opacity="0.16"/>`}else E=`
    <line x1="${D+14}" y1="130" x2="${c-14}" y2="130"
          stroke="#1e2c30" stroke-dasharray="7 5" stroke-width="2"/>`}let U=`<g id="${d}-overlay-cme-cone">${E}</g>`,J=`<g id="${d}-overlay-labels">
    ...
    <div style="border-radius:3px;overflow:hidden;margin-bottom:6px">${yt("magnetosphere",{windKms:a!=null?a:void 0,bz:o!=null?o:void 0})}</div>
    <div class="hw-kpi-stat-row">
      <div class="hw-kpi-stat">
        <span class="hw-kpi-stat-label">Solar wind</span>
        <span class="hw-kpi-stat-value" style="color:${h}">${m(n)}</span>
  ...
  </div>`}function ke(t,s,e,o){switch(s){case"solar_wind":return ge(t);case"xray":return we(t);case"imf_bz":return $e(t);case"aurora":return ye(e,o);case"magnetosphere":return fe(t);default:return""}}function bt(t,s){if(t.length<2)return"\u2192";let e=t[t.length-1],o=Math.max(0,t.length-4),a=t[o];if(!isFinite(e)||!isFinite(a))return"\u2192";let d=e-a;return d>s?"\u2191":d<-s?"\u2193":"\u2192"}function _e(t,s){let e=mt[s],o=Be;return`<div class="hw-solar-mini-wrap" style="cursor:default">
  ...
  </div>`}function Se(t,s,e,o,a,d,i,l=0){var q,O,D,j,P,Y,Z,tt;let{summary:n,scales:r,metrics:c,aurora_hint:p}=t,h=(q=Ft[n.status])!=null?q:Ft.quiet,g=a!=null?a.kp.toFixed(1):c.kp_latest!=null?c.kp_latest.toFixed(1):"\u2014",u=[a?a.gScale:r.g_scale,r.r_scale,r.s_scale].map(L=>{let I=de(L),B=I?`color:${h.accent};border-color:${h.accent}33`:"";return`<span class="hw-scale-chip${I?" hw-scale-active":""}" style="${B}">${m(L)}</span>`}).join(""),b=a?a.auroraLabel:p.aurora_label,$=b==="good"?"#5cce8c":b==="possible"?"#d4cc5c":"#607880",x=b.charAt(0).toUpperCase()+b.slice(1),f="#b4c6cc",v=c.solar_wind_kms!=null?`${Math.round(c.solar_wind_kms)} km/s`:"\u2014",_=c.imf_bz_nt,H=_!=null?_<=-10?"#e05c5c":_<=-5?"#e0a84a":_>=5?"#5cce8c":"#a0b4b8":"#607880",M=_!=null?(_>=0?"+":"")+_.toFixed(1)+" nT":"\u2014",C=c.xray_class,y=C?(O=Ht[C])!=null?O:"#a0b4b8":"#607880",k=C?`${C}-class`:"\u2014",z=bt(((D=c.kp_history_1h)!=null?D:[]).map(L=>L.kp),.5),A=bt(((j=c.wind_history_1h)!=null?j:[]).map(L=>L.kms),20),G=bt(((P=c.bz_history_1h)!=null?P:[]).map(L=>L.bz),1.5),V=bt(((Y=c.xray_history_1h)!=null?Y:[]).map(L=>Math.log10(L.flux+1e-9)),.15),K=(Z=c.kp_history_1h)!=null?Z:[],E=K.length?ht(K[K.length-1].t_utc):null,U=E?`Recent history \xB7 Last step ${E}`:"Recent history",J=gt(t),S=(tt=c.kp_latest)!=null?tt:0,T=S>=5,X=T?`linear-gradient(160deg, #0d2a1a 0%, ${h.bg}22 75%)`:`${h.bg}18`,R=(L,I,B,nt,et)=>{let at=et?`<span class="hw-trend">${et}</span>`:"";return`<div class="hw-kpi-item${o===L?" hw-kpi-active":""}" data-kpi="${L}">
      <span class="hw-qd-label">${I}</span>
      <span class="hw-qd-value" style="color:${nt}">${B}${at}</span>
    </div>`},N=T?`
    ...
    </div>`;let $=320,x=38,f=14,v=x+f,_=$/c,H=S=>x-Math.max(2,Math.min(x-2,S/9*(x-2))),M="",C=r.map(S=>S.kp),y=Le(C);r.forEach((S,T)=>{let X=H(S.kp),R=x-X,N=T*_,q=N+_/2,O=S.kp>=6?"#e05c5c":S.kp>=5?"#e0a84a":S.kp>=4?"#d4cc5c":"#5cce8c",D=`Kp ${S.kp.toFixed(1)} \xB7 ${ht(S.t_utc)}`;if(M+=`<rect x="${N.toFixed(1)}" y="${X.toFixed(1)}" width="${(_-1.5).toFixed(1)}" height="${R.toFixed(1)}" fill="${O}" fill-opacity="0.85" rx="1.5"/>`,M+=`<rect x="${N.toFixed(1)}" y="0" width="${_.toFixed(1)}" height="${x}" fill="transparent"><title>${W(D)}</title></rect>`,c<=8||T%2===0){let P=new Date(S.t_utc).getHours();M+=`<text x="${q.toFixed(1)}" y="${(v-3).toFixed(1)}" text-anchor="middle" font-size="9" fill="#7a9098">${P.toString().padStart(2,"0")}</text>`}});let z=`<polyline points="${r.map((S,T)=>{let X=T*_+_/2,R=H(y[T]);return`${X.toFixed(1)},${R.toFixed(1)}`}).join(" ")}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`,A="";if(s>0&&c>0){let S=Math.min($-1,s/(c*3)*$);A=`
      <line x1="${S.toFixed(1)}" y1="0" x2="${S.toFixed(1)}" y2="${x}" stroke="rgba(255,255,255,0.75)" stroke-width="1.5" stroke-dasharray="3,2"/>
      <polygon points="${S.toFixed(1)},${x} ${(S-4).toFixed(1)},${(x-7).toFixed(1)} ${(S+4).toFixed(1)},${(x-7).toFixed(1)}" fill="rgba(255,255,255,0.75)"/>`}let G=Math.round(p/4),V=Math.round(p/2),K=Math.round(p*3/4),E=`
...
C: ${n.c_flare_probability}%  M: ${n.m_flare_probability}%  X: ${n.x_flare_probability}%`;return`<g style="pointer-events:all">
    ...
    <div class="hw-radio-meta">X-ray: <b style="color:${(p=Ht[o])!=null?p:"#a0b4b8"}">${m(o)}-class</b> \xB7 ${m(d)}</div>
  ...
  </div>`}var ft={cycle_name:"Solar Cycle 25",phase:"declining",progress_0_1:.57,cycle_start_year:2019,expected_peak_year:2025,expected_end_year:2030,subtitle:"Activity remains elevated"},Wt={minimum:"#607880",rising:"#d4cc5c",maximum:"#e0a84a",declining:"#96a8c8"};function Ue(t){var y;let s=ft,e=(y=Wt[s.phase])!=null?y:"#96a8b8",o=s.phase.charAt(0).toUpperCase()+s.phase.slice(1),a=280,d=52,i=10,l=d-6,n=d-18,r=.5,c=.19,p=k=>Math.exp(-Math.pow((k-r)/c,2)/2),h=k=>i+k*(a-2*i),g=k=>l-p(k)*n,w=80,u=[];for(let k=0;k<=w;k++){let z=k/w;u.push(`${k===0?"M":"L"}${h(z).toFixed(1)},${g(z).toFixed(1)}`)}let b=Math.round(s.progress_0_1*w),$=[];for(let k=0;k<=b;k++){let z=k/w;$.push(`${k===0?"M":"L"}${h(z).toFixed(1)},${g(z).toFixed(1)}`)}let x=h(s.progress_0_1),f=[`M${i},${l}`,...$.slice(1),`L${x.toFixed(1)},${l} Z`],v=g(s.progress_0_1),_=5,H=`M${x.toFixed(1)},${v.toFixed(1)} L${(x-_).toFixed(1)},${(v-_*1.8).toFixed(1)} L${(x+_).toFixed(1)},${(v-_*1.8).toFixed(1)} Z`,M=l+11;return`<div class="hw-impact-tip${t?" hw-impact-tip-open":""}" style="padding:8px 6px 6px">
  ...
  </div>`}function Ut(t){var r,c,p;let s=(r=t.scales.g_scale)!=null?r:"G0",e=parseInt(s.slice(1),10),o=t.metrics.kp_latest;if(o==null){let h=(c=t.metrics.kp_forecast_3h)!=null?c:[],g=Date.now(),w=h.filter(u=>new Date(u.t_utc).getTime()<=g+3*60*60*1e3).sort((u,b)=>new Date(b.t_utc).getTime()-new Date(u.t_utc).getTime());w.length>0&&(o=w[0].kp)}let a=0;e>=2||o!=null&&o>=6?a=2:(e>=1||o!=null&&o>=4)&&(a=1);let d=parseInt(((p=t.scales.r_scale)!=null?p:"R0").slice(1),10),i=d>=2&&a<2;d>=2&&(a=Math.min(2,a+1));let l={0:"#5cce8c",1:"#d4cc5c",2:"#e05c5c"},n={0:"Low",1:"Moderate",2:"High"};return{level:a,color:l[a],label:n[a],kp:o,gScale:s,boostedByFlare:i}}function Qe(t,s){var r;let e=Ut(t),o=s?" hw-impact-tip-open":"",d=[{l:0,label:"Low",color:"#5cce8c",width:33},{l:1,label:"Moderate",color:"#d4cc5c",width:64},{l:2,label:"High",color:"#e05c5c",width:100}].map(c=>{let p=c.l===e.level,h=p?c.color:"#566068",g=p?"0.88":"0.16";return`<div class="hw-gnss-rung">
  ...
  </div>`}function qt(t){var r,c;let s=(r=t.alerts_all)!=null?r:[],e=s.find(p=>p.kind==="cme_impact"),o=s.find(p=>p.kind==="cme_watch"),a=e!=null?e:o;if(!a)return{status:"quiet",color:"#5cce8c",label:"None",speed_kms:null,issued_utc:null,arrival_utc:null};let d=((c=a.raw_body)!=null?c:"").match(/Estimated Velocity[:\s]+(\d+)\s*km\/s/i),i=d?parseInt(d[1],10):null,l=null;if(i&&a.t_utc){let p=1496e5/i*1e3;l=new Date(new Date(a.t_utc).getTime()+p).toISOString().replace(".000Z","Z")}let n=e?"impact":"watch";return{status:n,color:n==="impact"?"#e05c5c":"#d4cc5c",label:n==="impact"?"Active":"Watch",speed_kms:i,issued_utc:a.t_utc,arrival_utc:l}}function Ze(t,s){let e=qt(t),o=s?" hw-impact-tip-open":"",a="\u2014";if(e.arrival_utc){let n=new Date(e.arrival_utc),r=n.toLocaleString("en-US",{month:"short",timeZone:"UTC"}),c=n.getUTCDate(),p=String(n.getUTCHours()).padStart(2,"0"),h=String(n.getUTCMinutes()).padStart(2,"0");a=`~${r}\xA0${c}\xA0${p}:${h}\u202FUTC`}let d=e.speed_kms?`${e.speed_kms}\u202Fkm/s`:"\u2014",i=e.status!=="quiet"?`Velocity: <b style="color:#b4c6cc">${m(d)}</b>&ensp;Arrival: <b style="color:#b4c6cc">${m(a)}</b>`:"No Earth-directed CME in forecast window",l=yt("cme_cone",{cmeState:e,uid:"cme"});return`<div class="hw-impact-tip${o}">
    ${l}
    <div class="hw-cme-footer">${i}</div>
  </div>`}function Je(t,s,e,o,a,d,i,l,n){var at,rt,wt;let r=(rt=(at=s==null?void 0:s.impacts)!=null?at:t.observer_impacts)!=null?rt:[],c=r.map(F=>{var zt,Tt;let dt=(zt=ne[F.level])!=null?zt:"#666",lt=F.level==="none"?"None":F.level.charAt(0).toUpperCase()+F.level.slice(1),pt=(Tt=Ie[F.kind])!=null?Tt:Ne,it=F.level==="none"?"#606870":dt,ct=F.kind==="solar_activity"?a:i.has(F.kind),kt=ct?" hw-impact-open":"",st;if(F.kind==="solar_activity"){let xt=a?" hw-solar-open":"",_t=Dt.map(Q=>{let $t=d.has(Q.id),St=$t?Q.color+"22":"transparent",Mt=$t?"1":"0.32";return`<button class="hw-sl-btn" data-solar-layer="${Q.id}" style="color:${Q.color};border-color:${Q.color};background:${St};opacity:${Mt}">${Q.label}</button>`}).join("");st=`<div class="hw-solar-tip${xt}">
          <div class="hw-solar-disk-wrap">
            <img class="hw-solar-disk-img" src="${Pe}" alt="Solar disk" loading="lazy" />
            ${e?Oe(e,je,d):""}
          </div>
          <div class="hw-solar-layers">${_t}</div>
          <span class="hw-solar-tip-text">${m(F.summary)}</span>
        </div>`}else if(F.kind==="aurora"){let xt=ct?" hw-aurora-tip-open":"",_t=`https://services.swpc.noaa.gov/images/animations/ovation/north/latest.jpg?_=${Date.now()}`,Q=null;l&&n.lat!=null&&n.lon!=null&&(Q=Nt(l.entries,n.lat,n.lon));let $t=n.lat!=null&&n.lon!=null,St=Q!=null?Q>=30?"#5cce8c":Q>=10?"#d4cc5c":"#9ab4bc":"#607880",Mt=Q!=null?`${Q}%`:l?"n/a":"\u2026",Vt=$t?`
        <div class="hw-aurora-obs-panel">
          <span>\u{1F4CD}</span>
          <span>${n.locationName?m(n.locationName)+" \xB7 ":""}${n.lat.toFixed(1)}\xB0${n.lat>=0?"N":"S"} ${Math.abs(n.lon).toFixed(1)}\xB0${n.lon>=0?"E":"W"}</span>
          <span class="hw-aurora-prob" style="color:${St}">Aurora: ${Mt}</span>
        </div>`:"";st=`<div class="hw-aurora-tip${xt}">
          <div class="hw-aurora-map-wrap">
            <img class="hw-aurora-img" src="${W(_t)}" alt="NOAA Aurora Oval" loading="lazy" />
            ${Pt(n)}
          </div>
          ${Vt}
          <div class="hw-aurora-caption">NOAA OVATION Prime model \xB7 updates every 5 min</div>
        </div>`}else F.kind==="radio"?st=Ye(t,ct):st=`<div class="hw-impact-tip${ct?" hw-impact-tip-open":""}">${m(F.summary)}</div>`;let ot=F.kind==="solar_activity"?" data-solar-toggle":` data-impact-row="${W(F.kind)}"`;return`<div class="hw-impact-row${kt}"${ot}>
      <span class="hw-impact-caret">\u25B6</span>
      <span class="hw-impact-kind" style="color:${it}">${pt}<span style="color:#b4c6cc">${m(F.label)}</span></span>
    ...
    <div class="hw-section-row" data-impacts-toggle>
    ...
    </div>`,b=i.has("geomag_storm"),$=Yt(t),x=$.g1,f=$.g1>=30?jt.g1:$.g1>0?"#7a9298":"#607880",v=x>0?`G1 ${x}%`:"None",H=`<div class="hw-impact-row${b?" hw-impact-open":""}" data-impact-row="geomag_storm">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ke(t,b)}
    </div>`,M=i.has("solar_cycle"),C=(wt=Wt[ft.phase])!=null?wt:"#96a8b8",y=ft.phase.charAt(0).toUpperCase()+ft.phase.slice(1),z=`<div class="hw-impact-row${M?" hw-impact-open":""}" data-impact-row="solar_cycle">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ue(M)}
    </div>`,A=Gt(t),G=i.has("hss"),K=`<div class="hw-impact-row${G?" hw-impact-open":""}" data-impact-row="hss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Xe(t,G)}
    </div>`,E=Kt(t),U=i.has("sat_drag"),S=`<div class="hw-impact-row${U?" hw-impact-open":""}" data-impact-row="sat_drag">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${qe(t,U)}
    </div>`,T=Ut(t),X=i.has("gnss"),N=`<div class="hw-impact-row${X?" hw-impact-open":""}" data-impact-row="gnss">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Qe(t,X)}
    </div>`,q=Xt(t),O=i.has("sw_pressure"),D=q.pressure!=null?`${q.pressure.toFixed(2)} nPa`:"\u2014",P=`<div class="hw-impact-row${O?" hw-impact-open":""}" data-impact-row="sw_pressure">
      <span class="hw-impact-caret">\u25B6</span>
      ...
      ${Ze(t,Z)}
    </div>`,I=gt(t),B=i.has("magnetosphere"),et=`<div class="hw-impact-row${B?" hw-impact-open":""}" data-impact-row="magnetosphere">
      <span class="hw-impact-caret">\u25B6</span>
...
${e.join(" \xB7 ")}`}function as(t,s,e){var n;let o=t.is_active?ss:(n=es[t.level])!=null?n:"#445c64",a=t.event_time===e,d=t.event_time.slice(11,16)+" UTC",i=t.source==="NASA_DONKI"?"DONKI":"SWPC",l=a?`<div class="hw-tl-detail">${m(t.description)}${m(ns(t))}</div>`:"";return`
    ...
    </div>`}function is(t,s,e,o){var $,x;let a=($=t.timeline)!=null?$:[],d=Date.now(),i=new Date(d).toISOString().slice(0,10),l=new Date(d-864e5).toISOString().slice(0,10),n=new Date(d-1728e5).toISOString().slice(0,10),r=new Set([i,l,n]),c=a.filter(f=>{var v;return r.has(((v=f.event_time)!=null?v:"").slice(0,10))}).slice().reverse(),p=c.length,h=e?"\u25BC":"\u25B6",g=p>0?`Solar Activity Timeline (${p})`:"Solar Activity Timeline",w=`
    ...
    </div>`;if(!e||p===0)return`<div class="hw-timeline">${w}</div>`;let u=new Map;for(let f of c){let v=((x=f.event_time)!=null?x:"").slice(0,10);u.has(v)||u.set(v,[]),u.get(v).push(f)}let b=[...u.entries()].map(([f,v])=>{let H=new Date(f+"T12:00:00Z").toLocaleDateString("en-US",{month:"short",day:"numeric",timeZone:"UTC"}),M=o.has(f),C=M?"\u25B6":"\u25BC",y=M?`<span class="hw-tl-day-count">${v.length} events</span>`:"",k=`
      <div class="hw-tl-day-row" data-tl-day="${W(f)}">
        <span class="hw-section-caret">${C}</span>
        <span class="hw-tl-date">${H}</span>
        ${y}
      </div>`,z=M?"":v.map((A,G)=>as(A,G<v.length-1,s)).join("");return`<div class="hw-tl-group">${k}${z}</div>`}).join("");return`
    ...
    <div class="hw-alerts">
      ${l}
      ${n}
    </div>`}function ls(t,s){var V,K,E;let e=t.cme_tracker;if(!e)return"";let o=(V=ie[e.impact_level])!=null?V:"#96a8b8",a=(K=re[e.status])!=null?K:e.status,d=300,i=44,l=18,n=i/2,r=10,c=d-18,p=7,h=`<line x1="${l+r}" y1="${n}" x2="${c-p}" y2="${n}" stroke="#2a3c42" stroke-width="1.5" stroke-dasharray="5,4"/>`,g=`<circle cx="${l}" cy="${n}" r="${r}" fill="#f0c040" opacity="0.92"/>`,w=`
    <circle cx="${c}" cy="${n}" r="${p}" fill="#4a90c4" opacity="0.88"/>
    <circle cx="${c}" cy="${n}" r="2.5" fill="#fff" opacity="0.7"/>`,u=`<text x="${l}" y="${n+r+9}" text-anchor="middle" font-size="9" fill="#c8aa60">Sun</text>`,b=`<text x="${c}" y="${n+p+9}" text-anchor="middle" font-size="9" fill="#7ab0d4">Earth</text>`,$="";if(e.progress!=null){let U=l+r+4,J=c-p-4,S=U+e.progress*(J-U),T=5;e.status==="arrival_window"?$=`
    ...
    ${w}${b}
    ${$}
  </svg>`,f=At(e.arrival_time_utc),v=At(e.launch_time_utc),_=e.speed_kms!=null?`${Math.round(e.speed_kms)} km/s`:"\u2014",H=e.half_angle_deg!=null?`${e.half_angle_deg}\xB0`:"\u2014",M=(E=e.source_location)!=null?E:"\u2014",C=e.is_earth_direct?"Direct hit":"Glancing blow",y=e.progress!=null?`${Math.round(e.progress*100)}%`:"\u2014",k=`
    ...
    </div>`,z=s?"\u25BC":"\u25B6",A=e.impact_level==="unknown"?"Unrated":e.impact_level.charAt(0).toUpperCase()+e.impact_level.slice(1),G=s?`${x}${k}`:"";return`
    <div class="hw-cme">
      ...
      ${G}
    </div>`}function cs(t,s,e,o,a,d,i,l,n,r,c,p,h,g,w,u,b,$,x,f,v=0){var z;let _=Fe(t,a),H=(z=t.metrics.kp_history_1h)!=null?z:[],M=H.length?ht(H[H.length-1].t_utc):null,C=M?`Recent history \xB7 Last step ${M}`:"Recent history",y=`<div style="padding:10px 14px;border-bottom:1px solid #1e2c30"><div class="hw-section-row" data-hero-toggle style="margin-bottom:0">
    <span class="hw-section-caret">${e?"\u25BC":"\u25B6"}</span>
