...
export function mountHero(root, context, suppliedConfig = {}, host) {
  ...
  function renderClock() {
    ...
    if (typeof frame === "function" && config.orientation !== "horizontal") frame(() => { const dateWidth = dateElement.getBoundingClientRect?.().width, timeWidth = timeElement.getBoundingClientRect?.().width; if (dateWidth > 0 && timeWidth > 0) timeElement.style.fontSize = `${(parseFloat((view?.getComputedStyle || globalThis.getComputedStyle)(timeElement).fontSize) * dateWidth / timeWidth).toFixed(1)}px`; });
  }
  function render() {
    if (!alive) return;
