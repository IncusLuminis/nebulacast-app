(function (global) {
  ...
  function mountCalendarWidget(rootRef, suppliedConfig) {
    var root = requireRoot(rootRef), doc = documentFor(root);
    var config = Object.assign({ jsonUrl: '/calendar/daily_signal.json', rssUrl: '/alerts/rss.xml', maxItems: 20, filters: EVENT_FILTERS, iconBase: '/assets/icons/alerts', fetchTimeout: 15000 }, suppliedConfig || {});
    config.jsonUrl = config.jsonUrl || '/calendar/daily_signal.json'; config.rssUrl = config.rssUrl || '/alerts/rss.xml'; config.maxItems = config.maxItems || 20; config.filters = filters(config.filters, EVENT_FILTERS); config.fetchTimeout = config.fetchTimeout || 15000;
