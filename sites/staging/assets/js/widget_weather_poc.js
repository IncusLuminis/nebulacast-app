...
// Helper: format visibility (m to km)
function formatVisibility(visM) {
  if (visM === null || visM === undefined) return null;
  const km = visM / 1000;
...
function syncProfileSegmentUI() {
  const profileSeg = weatherCard?.querySelector(".seg.seg-profile");
  if (!profileSeg) return;
  profileSeg.querySelectorAll("button").forEach(btn => {
    const p = btn.dataset.profile;
    btn.dataset.active = (p === activeProfile) ? "true" : "false";
  });
}
...
function renderMiniCharts(hours, mode, nowHour) {
  const now = Date.now();
  let selectedHours = [];
  let maxBars = 24;
...
function showError(msg) {
  const subEl = weatherCard.querySelector(".sub");
  if (subEl) {
    subEl.innerHTML = `<span style="color: var(--bad)">Error: ${escapeHtml(msg)}</span>`;
  }
}
...
function showLoading() {
  const subEl = weatherCard.querySelector(".sub");
  if (subEl) {
    subEl.textContent = "Loading weather data...";
  }
}

// Render best windows (TONIGHT = next 12 hours)
function renderBestWindows(bestWindows, hours) {
  ...
  if (!bestWindows || bestWindows.length === 0) {
    ...
    if (next12h.length >= 2) {
      ...
      if (start && end) {
        ...
        text = `Tonight best window: ${startStr}–${endStr} (score ${score})`;
      }
    }
...
async function loadWeather() {
  ...
  try {
    ...
    var h2El = weatherCard.querySelector("h2");
    if (h2El) h2El.textContent = "Weather • " + ((data.location && data.location.name) ? data.location.name : "Unknown");
    var subEl = weatherCard.querySelector(".sub");
    if (subEl) {
