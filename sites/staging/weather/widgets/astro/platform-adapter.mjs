/**
 * Platform adapter for the legacy Astro widget.
 *
 * The legacy module remains the source of rendering and event behavior. This
 * adapter only translates Platform Context into the store-shaped API that the
 * legacy export already accepts.
 */
...
/**
 * Mount the legacy Astro export through Platform Context.
 *
 * @param {Element} root
...
export async function mount(root, context, config = {}, host) {
  ...
  if (typeof module.mountAstro !== "function") {
    throw new TypeError("Astro legacy module does not export mountAstro");
  }

  const facade = createLegacyStoreFacade(context, config);
  const mounted = normalizeLegacyMount(await module.mountAstro(root, facade, config, host));
  let destroyed = false;
